...
/**
 * Tests for MOSFETStamper - verifies correct MNA matrix stamping and state management.
 */
public class MOSFETStamperTest {
    ...
    public void testImplementsIMatrixStamper() {
        assertTrue("MOSFETStamper should implement IMatrixStamper",
                stamper instanceof IMatrixStamper);
    }

    @Test
    public void testImplementsIStatefulStamper() {
        assertTrue("MOSFETStamper should implement IStatefulStamper",
                stamper instanceof IStatefulStamper);
    }

    @Test
    public void testDefaultConstructor_InitiallyOff() {
        assertFalse("MOSFET should be OFF initially", stamper.isOn());
    }

    @Test
    public void testDefaultConstructor_DefaultParameters() {
        assertEquals("default rOn", MOSFETStamper.DEFAULT_R_ON, stamper.getROn(), TOLERANCE);
        assertEquals("default rOff", MOSFETStamper.DEFAULT_R_OFF, stamper.getROff(), TOLERANCE);
    }

    @Test
    public void testParameterizedConstructor() {
        MOSFETStamper customStamper = new MOSFETStamper(1e-4, 1e8);
    ...
    public void testGetCurrentResistance_Off() {
        ...
        assertEquals("OFF resistance", MOSFETStamper.DEFAULT_R_OFF, stamper.getCurrentResistance(), TOLERANCE);
    }

    @Test
    public void testGetCurrentResistance_On() {
        stamper.setState(true);
        assertEquals("ON resistance", MOSFETStamper.DEFAULT_R_ON, stamper.getCurrentResistance(), TOLERANCE);
    }
    ...
    public void testStampMatrixA_Off() {
        ...
        double expectedAdmittance = 1.0 / MOSFETStamper.DEFAULT_R_OFF;
    ...
    public void testStampMatrixA_On() {
        ...
        double expectedAdmittance = 1.0 / MOSFETStamper.DEFAULT_R_ON;
        assertEquals("a[1][1]", expectedAdmittance, a[1][1], TOLERANCE);
        assertEquals("a[2][2]", expectedAdmittance, a[2][2], TOLERANCE);
    }
    ...
    public void testStampVectorB_NoContribution() {
        ...
        // MOSFET has no forward voltage, so no B vector contribution
    ...
    public void testSetGateSignal_TurnOn() {
        ...
        assertTrue("MOSFET should turn ON", stamper.isOn());
    ...
    public void testSetGateSignal_TurnOff() {
        ...
        assertFalse("MOSFET should turn OFF", stamper.isOn());
        assertTrue("state should have changed", stamper.isStateChanged());
    }
    ...
    public void testSetGateSignal_StayOn() {
        ...
        assertTrue("MOSFET should stay ON", stamper.isOn());
    ...
    public void testSetGateSignal_StayOff() {
        ...
        assertFalse("MOSFET should stay OFF", stamper.isOn());
        assertFalse("state should not have changed", stamper.isStateChanged());
    }
    ...
    public void testUpdateStateFromParameters() {
        ...
        assertTrue("MOSFET should turn ON from parameters", stamper.isOn());
        assertTrue("state should have changed", stamper.isStateChanged());
    }
    ...
    public void testGetAdmittanceWeight_On() {
        ...
        assertEquals("ON admittance", 1.0 / MOSFETStamper.DEFAULT_R_ON, admittance, TOLERANCE);
    }
    ...
    public void testGetAdmittanceWeight_Off() {
        stamper.setState(false);
        double admittance = stamper.getAdmittanceWeight(0.0, 1e-6);
        assertEquals("OFF admittance", 1.0 / MOSFETStamper.DEFAULT_R_OFF, admittance, TOLERANCE);
    }
    ...
    public void testCreateParameters() {
        double[] params = MOSFETStamper.createParameters(1e-3, 1e-4, 1e8, 1.0);

        assertEquals("rCurrent", 1e-3, params[MOSFETStamper.PARAM_R_CURRENT], TOLERANCE);
        assertEquals("rOn", 1e-4, params[MOSFETStamper.PARAM_R_ON], TOLERANCE);
    ...
    public void testParameterConstants() {
        assertEquals("PARAM_R_CURRENT", 0, MOSFETStamper.PARAM_R_CURRENT);
        assertEquals("PARAM_R_ON", 2, MOSFETStamper.PARAM_R_ON);
        assertEquals("PARAM_R_OFF", 3, MOSFETStamper.PARAM_R_OFF);
        assertEquals("PARAM_GATE", 8, MOSFETStamper.PARAM_GATE);
    }

    @Test
    public void testDefaultConstants() {
        assertEquals("DEFAULT_R_ON", 1e-3, MOSFETStamper.DEFAULT_R_ON, TOLERANCE);
        assertEquals("DEFAULT_R_OFF", 1e9, MOSFETStamper.DEFAULT_R_OFF, TOLERANCE);
    }
    ...
    @Test
    public void testMOSFETStateCycle() {
    ...
    public void testBidirectionalConduction() {
        // MOSFET conducts in both directions when ON (unlike IGBT)
        stamper.setState(true);
        double[] parameter = MOSFETStamper.createDefaultParameters(1);
    ...
    @Test
    public void testMOSFETvsIdealSwitch_SameBehavior() {
        // MOSFET and IdealSwitch should behave identically in GeckoCIRCUITS
        MOSFETStamper mosfet = new MOSFETStamper(1e-3, 1e9);
        IdealSwitchStamper idealSwitch = new IdealSwitchStamper(1e-3, 1e9);

        mosfet.setState(true);
        idealSwitch.setState(true);

        double[] mosfetParams = MOSFETStamper.createDefaultParameters(1);
        double[] switchParams = IdealSwitchStamper.createDefaultParameters(1);

        double mosfetCurrent = mosfet.calculateCurrent(1.0, 0.0, mosfetParams, 1e-6, 0.0);
        double switchCurrent = idealSwitch.calculateCurrent(1.0, 0.0, switchParams, 1e-6, 0.0);

        assertEquals("MOSFET and IdealSwitch should have same current",
                     mosfetCurrent, switchCurrent, LOOSE_TOLERANCE);
    }
}
