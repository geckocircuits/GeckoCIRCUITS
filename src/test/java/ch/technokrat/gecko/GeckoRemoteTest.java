...
/**
 * This testing class is a bit "different" from normal ones. First, we check if GeckoRemote contains
 * ALL methods of the interface GeckoRemoteInterface with the same method name, but a "static"
 * modifier. Then, we exercise the GeckoRemote-Interface. In fact, I just check if the correct
 * exception is thrown (UndefinedOperationException) form a dummy proxy wrapper.
 *
 * @author andy
 */
public class GeckoRemoteTest {
  ...
  private static final List<String> _staticMethodNamesToExclude =
      Arrays.asList(
          ...
          "simulateStep",
          "simulateSteps");
  private static final List<String> _methodNamesToExclude =
      Arrays.asList(
          "connect",
          "disconnect",
          "getSessionID",
  ...
  public void testCallAllGeckoRemoteStaticMethods() {
    ...
    GeckoRemote._proxy =
        (GeckoRemoteIntWithoutExc) Proxy.newProxyInstance(loader, interfaces, handler);
    ...
    for (Method meth : getAllRelevantStaticMethods()) {
      ...
      try {
        ...
        Logger.getLogger(GeckoRemoteTest.class.getName()).log(Level.SEVERE, null, ex);
        assertTrue(
            "Unexpected exception in calling method " + meth.getName() + " exception type is " + ex,
            false);
      }
    }
  }
  ...
  public void testMethodsAvailableNonStaticToStatic() {
    ...
    for (Method meth : GeckoRemoteInterface.class.getMethods()) {
      ...
      assertTrue(
          "Error: the method " + meth + " MUST throw a superclass of RemoteException!",
          exceptionCheck);
      ...
      try {
        ...
        assertTrue(
            "Error: method " + found + " should be static! ",
            Modifier.isStatic(found.getModifiers()));
        assertTrue(
            "Error: static method should be public:", Modifier.isPublic(found.getModifiers()));
        foundCounter++;
      } catch (Exception ex) {
