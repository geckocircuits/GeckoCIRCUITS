 ...
 *
 * <p>The LAPACK Interface for C/C++ is used for accessing to the LAPACK functions. See MKL
 * Reference Manual, Chapters 3,4,5.
 *
 * <p>Both Java and C parts of the wrapper LAPACK demonstrate the straightforward approach similar
 * to CBLAS wrapper.
 *
 * <p>To make interface more suitable some modification of original LAPACK subroutines has been
 * performed. This is mainly removing work arrays and replacing chars arguments with integers ones,
 * similar to CBLAS interface to BLAS subroutines. Also "info" is returned as function value. For
 * this purposes one more layer has been created, see files clapack.h and clapack.c.
 *
 * <p>These wrappers assume using 1-dimensional Java arrays of the type double[] or float[] to
 * process floating-point data. A multi-dimensional data series is stored into 1-dimensional array
 * as column-major 1-dimensional data series.
 *
 * <p>A complex number is stored as 2-elements array: real part of the number as the 1st element of
 * the array, and imaginary part as the 2nd.
 *
 * <p>A complex data series of the length N is stored as a real series of the length 2*N. The n-th
 * complex element is stored as the pair of 2*n-th and (2*n+1)-th real elements: the even indexed
 * for real part and the odd indexed for imaginary part.
 *
...
public final class LAPACK {
  ...
  public static final class JOB {
    ...
    private JOB() {}
    /** job ='N' */
    public static final int JobN = 201;
    /** job ='V' */
    public static final int JobV = 202;
    /** job ='A' */
    public static final int JobA = 203;
    /** job ='S' */
    public static final int JobS = 204;
    /** job ='O' */
    public static final int JobO = 205;
  }
   ...
   *
   * @param uplo Indicates whether the upper or lower triangular part of A is stored and how A is
   *     factored: If uplo = 'U', the array a stores the upper triangular part of the matrix A. If
   *     uplo = 'L', the array a stores the lower triangular part of the matrix A.
   * @param n The order of matrix A; n ≥ 0.
   * @param a The array a contains either the upper or the lower triangular part of the matrix A
   *     (see uplo).
   * @param lda The first dimension of a.
   * @return If info=0, the execution is successful. If info = -i, the i-th parameter had an illegal
   *     value. If info = i, the leading minor of order i (and therefore the matrix A itself) is not
   *     positive-definite, and the factorization could not becompleted. This may indicate an error
   ...
   *
   * @param uplo Must be 'U' or 'L'. Indicates whether A is upper or lower triangular: If uplo =
   *     'U', then A is upper triangular. If uplo = 'L', then A is lower triangular.
   ...
   * @return matrix a is overwritten. Be careful, only half of the matrix containes the information,
   *     i.e. the matrix a is not yet symmetric!
   */
  ...
  /**
   * @param uplo Must be 'U' or 'L'. Indicates whether the upper or lower triangular part of A is
   *     stored and how A is factored: If uplo = 'U', the array a stores the upper triangular part
   *     of the matrix A. If uplo = 'L', the array a stores the lower triangular part of the matrix
   *     A.
   * @param n The order of matrix A; n ≥ 0.
   * @param a Array, DIMENSION (lda,*). The array a contains either the upper or the lower
   *     triangular part of the matrix A (see uplo). The second dimension of a must be at least
   *     max(1, n).
   * @param lda The first dimension of a.
   * @return INTEGER. If info=0, the execution is successful. If info = -i, the i-th parameter had
   *     an illegal value. If info = i, the leading minor of order i (and therefore the matrix A
   *     itself) is not positive-definite, and the factorization could not be completed. This may
   *     indicate an error in forming the matrix A.
   */
  ...
  public static int zgetrs(
      int trans, int n, int nrhs, double[] a, int lda, int[] ipiv, double[] b, int ldb) {
    return LAPACKNative.zgetrs(trans, n, nrhs, a, lda, ipiv, b, ldb);
  }

  /** Wrapper for MKL function dgetrf(). */
  public static int zgetrs2(
      int trans, int n, int nrhs, DoubleBuffer a, int lda, int[] ipiv, double[] b, int ldb) {
    return LAPACKNative.zgetrs2(trans, n, nrhs, a, lda, ipiv, b, ldb);
  }

  /** Wrapper for MKL function dgetrf(). */
  public static int cgetrs(
      int trans, int n, int nrhs, float[] a, int lda, int[] ipiv, float[] b, int ldb) {
    return LAPACKNative.cgetrs(trans, n, nrhs, a, lda, ipiv, b, ldb);
  }

  /** Wrapper for MKL function dgetrf(). */
  public static int sgetrs(
      int trans, int n, int nrhs, float[] a, int lda, int[] ipiv, float[] b, int ldb) {
    return LAPACKNative.sgetrs(trans, n, nrhs, a, lda, ipiv, b, ldb);
  }

  /** Wrapper for MKL function dgetrf(). */
  public static int dgetrs(
      int trans, int n, int nrhs, double[] a, int lda, int[] ipiv, double[] b, int ldb) {
   ...
   * @param a Array, DIMENSION (lda,*). Contains the matrix A. The second dimension of a must be at
   *     least max(1, n).
   * @param lda The first dimension of array a.
   * @param ipiv Array, DIMENSION at least max(1,min(m, n)). The pivot indices; for 1 ≤ i ≤ min(m,
   *     n) , row i was interchanged with row ipiv(i).
   * @return INTEGER. If info=0, the execution is successful. If info = -i, the i-th parameter had
   *     an illegal value. If info = i, uii is 0. The factorization has been completed, but U is
   *     exactly singular. Division by 0 will occur if you use the factor U for solving a system of
   *     linear equations.
   */
  ...
  /** Wrapper for MKL function dgetrf(). */
  public static int zsytrf(
      char uplo, int n, double[] a, int lda, int[] ipiv, double[] work, int lwork) {
    return LAPACKNative.zsytrf(uplo, n, a, lda, ipiv, work, lwork);
  }

  /** Wrapper for MKL function dgetrf(). */
  public static int zsytrs(
      char uplo, int n, int nrhs, double[] a, int lda, int[] ipiv, double[] b, int ldb) {
  ...
  public static int zsptrs(
      char uplo, int n, int nrhs, double[] a, int[] ipiv, double[] b, int ldb) {
   ...
   * @param lwork The size of the work array; lwork ≥ n. If lwork = -1, then a workspace query is
   *     assumed; the routine only calculates the optimal size of the work array, returns this value
   *     as the first entry of the work array, and no error message related to lwork is issued by
   *     xerbla.
   * @return
   */
   ...
   *
   * <p>Unlike the original LAPACK subroutine argument jobz is integer type. Also original LAPACK
   * arguments work and lwork are skipped.
   */
  ...
  /**
   * @param uplo Must be 'U' or 'L'. If uplo = 'U', the upper triangle of A is stored. If uplo =
   ...
   * @param work work(*) is a workspace array. The dimension of arrays ap and afp must be at least
   *     max(1,n(n+1)/2); the second dimension of b and x must be at least max(1,nrhs); the
   *     dimension of work must be at least max(1, 3*n) for real flavors and max(1, 2*n) for complex
   *     flavors.
   * @param rwork Workspace array, DIMENSION at least max(1, n).
   * @return info INTEGER. If info = 0, the execution is successful. If info = -i, the i-th
   *     parameter had an illegal valu
   */
  public static int csprfs(
      char uplo,
      int n,
      int nrhs,
      float[] af,
      ...
      float[] work,
      float[] rwork) {
    return LAPACKNative.csprfs(
        uplo, n, nrhs, af, afp, ipiv, b, ldb, x, ldx, ferr, berr, work, rwork);
  }
   ...
   *
   * <p>Unlike the original LAPACK subroutine argument jobvl and jobvr are integer type. Also
   * original LAPACK arguments work and lwork are skipped.
   */
  public static int sgeev(
      int jobvl,
      int jobvr,
      int n,
      float[] a,
   ...
   *
   * <p>Unlike the original LAPACK subroutine argument jobu and jobvt are integer type. Also
   * original LAPACK arguments work and lwork are skipped. Additional argument sd contains the
   * unconverged superdiagonal elements of an upper bidiagonal matrix B whose diagonal is in s if
   * result (info) > 0 (i.e work(2:min(m,n)))
   */
  public static int sgesvd(
      int jobu,
      int jobvt,
      int m,
      int n,
   ...
   *
   * <p>Unlike the original LAPACK subroutine argument jobu and jobvt are integer type. Also
   * original LAPACK arguments work, rwork and lwork are skipped. Additional argument sd contains
   * the unconverged superdiagonal elements of an upper bidiagonal matrix B whose diagonal is in s
   * if result (info) > 0 (i.e rwork(1:min(m,n)-1))
   */
  public static int cgesvd(
      int jobu,
      int jobvt,
      int m,
      int n,
  ...
  public static int PARDISO(
      ...
      double[] x,
      int error) {
    return LAPACKNative.PARADISO(
        pt, maxfct, mnum, mtype, phase, n, values, ai, aj, idum, nrhs, iparm, msglvl, b, x, error);
  }
