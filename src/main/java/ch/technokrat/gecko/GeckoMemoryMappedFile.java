...
/**
 * This class encapsulates the memory-mapped file (MMF) used for the alternative GeckoRemote
 * interface. It contains the pointer to the file and the objects used to write and read to it. It
 * is the sole interface between the memory-mapping "plumbing" and the rest of GeckoCIRCUITS /
 * GeckoRemote.
 *
 * @author andrija s.
 */
public class GeckoMemoryMappedFile {
  ...
  private static final int PIPE_OBJECT_POS =
      32; // the position of the serialized pipe object in the buffer
  ...
  /**
   * Create a new memory mapped file of a given size. This constructor is used on the GeckoCIRCUITS
   * side, to open up GeckoCIRCUITS to remote access.
   *
   ...
   */
  public GeckoMemoryMappedFile(final String fileName, final long bufferSize)
  ...
  /**
   * Create a new memory mapped file of the default size. This constructor is used on the
   * GeckoCIRCUITS side, to open up GeckoCIRCUITS to remote access.
   *
   * @param fileName the name of the file to be mapped to memory
   * @throws FileNotFoundException if the file of the given name cannot be found / created
   * @throws IOException if something goes wrong in the process of memory-mapping
  ...
  /**
   * Private constructor used by the factory method to return a GeckoMemoryMappedFile object
   * pointing to an already existing memory mapped-file created by GeckoCIRCUITS to be used for
   * remote access.
   *
  ...
  /**
   * Factory method used by the CLIENT connecting to GeckoCIRCUITS to receive a mapped file object
   * for use for remote access to GeckoCIRCUITS.
   *
   ...
   * @throws IOException if something goes wrong in the process of memory-mapping
   */
  public static GeckoMemoryMappedFile getGeckoMemoryMappedFile(final String fileName)
      throws FileNotFoundException, IOException {
    final File file = new File(fileName);
    // given file must exist already!
    if (!file.exists()) {
      throw new FileNotFoundException(
          "The file for remote access: "
              + fileName
              + " does not exist. Please check if GeckoCIRCUITS is enabled for remote access.");
    }
   ...
   *
   * @return the connection ID: -1 - no connection 0 - connection attempt in progress > 0 -
  ...
  /**
   * Check if the file is free to accept a connection. Connection ID must be -1 and status must be
   * 'disconnected'.
   *
   * @return true if it is free, false otherwise
   */
  public boolean isFree() {
    return ((-1 == _mmb.getLong(CONNECTION_ID_POS))
        && (DISCONNECTED == _mmb.getLong(STATUS_ID_POS)));
  }

  /**
   * Used by CLIENT ONLY to connect to GeckoCIRCUITS via this file. Will block until a connection
   * request is answered or the timeout expires.
   *
   * @param timeout the timeout time for the connection in milliseconds.
   * @return -2 if timeout, no connection -1 if file is occupied by another connection 0 if
   *     connection is rejected an connection ID > 0 if connection is accepted
   */
  ...
  private void checkConnectionID(final long connectionID) {
    if (connectionID != getConnectionID()) {
      throw new RuntimeException(
          "Given connection ID "
              + connectionID
              + " does not match active connection ID "
              + getConnectionID());
    }
  }
  ...
  /**
   * Used by either side to confirm a disconnection request, or by GeckoCIRCUITS to forcibly
   * disconnect a client.
   */
  ...
  /**
   * This method serializes a GeckoRemotePipeObject into a byte array, which can then be written
   * into the file.
   *
   ...
   */
  private GeckoRemotePipeObject deserializePipeObject(final byte[] serializedPipe)
      throws IOException, ClassNotFoundException {
    final ObjectInputStream pipeStream =
        new ObjectInputStream(new ByteArrayInputStream(serializedPipe));
    return ((GeckoRemotePipeObject) pipeStream.readObject());
  }

  /**
   * This method calls a GeckoRemote method via the memory-mapped file. The description of the
   * method, along with the arguments, must be encapsulated in a GeckoRemotePipeObject. This object
   * is serialized and written into the memory-mapped file. This method then blocks until the
   * memory-mapped file contains a response to the GeckoRemote method call.
   *
   ...
   * @return a new GeckoRemotePipeObject which contains the response to the GeckoRemote function
   *     call
   * @throws IOException if something goes wrong reading or writing the memory-mapped file or
   *     (de)serializing the pipe object
   * @throws ClassNotFoundException if the pipe object cannot be reconstructed properly
   */
  public GeckoRemotePipeObject callMethod(
      final long connectionID, final GeckoRemotePipeObject methodObject)
      throws IOException, ClassNotFoundException {
    checkConnectionID(connectionID);
    if (methodObject.isMethodCall()) {
      ...
      while (true) {
        ...
        if (status == DISCONNECT_REQUEST) {
          ...
          throw new RuntimeException(
              "GeckoCIRCUITS did not respond to method call for unknown reason.");
   ...
   * @throws IOException if something goes wrong reading the memory-mapped file or de-serializing
   *     the pipe object
   * @throws ClassNotFoundException if the pipe object cannot be reconstructed properly
   */
  public GeckoRemotePipeObject getPipeObject(final long connectionID)
      throws IOException, ClassNotFoundException {
    checkConnectionID(connectionID);
  ...
  public boolean isConnectionAttempt() {
    return (_mmb.getLong(STATUS_ID_POS) == CONNECTION_ATTEMPT
        && _mmb.getLong(CONNECTION_ID_POS) == 0);
  }
  ...
  /**
   * Respond to a method call from the client via the memory-mapped file. The response (whatever it
   * might be) is encapsulated in a GeckoRemotePipeObject. This object is serialized, and written to
   * the file. The method returns immediately after this is done.
   *
   * @param connectionID the ID of the connection. Must match the active connection ID
   ...
   */
  public void respondToMethodCall(final long connectionID, final GeckoRemotePipeObject methodReturn)
      throws IOException {
    checkConnectionID(connectionID);
    if (methodReturn.isMethodCall()) {
      throw new RuntimeException(
          "Given GeckoRemotePipeObject does not represent a return from a method!");
    } else {
  ...
  /**
   * Delete the memory-mapped file. Used ONLY BY GeckoCIRCUITS (SERVER)! This object becomes useless
   * after this method is called. Used only for disabling remote access completely via this file.
   */
