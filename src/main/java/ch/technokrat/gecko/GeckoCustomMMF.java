...
/**
 * The alternative, memory-mapped interface counterpart to the class GeckoCustomRemote. Calls the
 * GeckoRemote functions on the server side via the memory-mapped file approach.
 *
 * @author andrija s.
 */
public class GeckoCustomMMF extends AbstractGeckoCustom {
  ...
  /**
   * Enable remote access of GeckoCIRCUITS via a memory-mapped file. The file will be reinitialized
   * to a blank disconnected state. Be careful therefore not to pass here files with potentially
   * active connections! This method will initiate a new thread which will poll regularly the
   * contents of the file, to see if a connection should be established. The file is initialized to
   * the default size.
   *
   ...
   * @throws IOException if there is an error writing to the file
   */
  public void enableAccess(final String fileName, final long fileSize)
  ...
  /**
   * For transmission, all method arguments are converted to Objects, which means doubles become
   * Doubles, etc. However all the methods take primitive types, therefore we must convert back.
