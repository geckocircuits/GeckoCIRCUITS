...
public class GeckoSim extends JApplet {
  ...
  /** this flag is used for automated testing (JUnit), please don't remove it! Andy. */
  ...
  public static void main(final String[] args) {
    ...
    if (operatingmode == OperatingMode.REMOTE) {
      ...
      if (JavaMemoryRestart.startNewGeckoCIRCUITSJVM(reqMem, args, javaCommand)) {
        System.out.println(
            "GeckoCIRCUITS should now be running outside of MATLAB at port " + args[1]);
        remoteLoaded = true;
        remoteLoading = false;
      ...
      if (JavaMemoryRestart.startNewGeckoCIRCUITSJVM(reqMem, args, javaCommand)) {
        System.out.println(
            "GeckoCIRCUITS should now be running outside of MATLAB and accessible via file "
                + args[1]
                + " of size "
                + args[2]);
        mmfLoaded = true;
    ...
    if (operatingmode != OperatingMode.SIMULINK
        && operatingmode != OperatingMode.EXTERNAL
        && !Fenster.IS_APPLET
        && !GeckoSim._isTestingMode) {
      if (JavaMemoryRestart.isMemoryRestartRequired(reqMem)
          && JavaMemoryRestart.startNewGeckoCIRCUITSJVM(
              reqMem, args, applicationProps.getProperty("JAVACOMMAND"))) {
        System.exit(12);
      }
    }
    ...
    if (testIfBrandedVersion()) {
      ...
      if (args.length > 0) {
        if (!(args[0].equals("-p")
            || args[0].equals(
                "-mm"))) { // first argument could for port number or file for remote access
        ...
        if (args.length > 0) {
          ...
          for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-p")) {
              ...
              try {
                // if args is -p, without anything following it, it is a mistake, but to catch the
                // error, set to default port number
              ...
              try {
                ...
                // there should also be a second one - the file size - but if this one is missing we
                // can set the default size
                if (i != (args.length - 1)) {
  ...
  private void initialisiere() {
    ...
    try {
      int anz =
          (new Integer(this.getParameter("nrExample")))
              .intValue(); // throws exception if not Applet
      Fenster.IS_APPLET = true;
    ...
    // System.setProperty("file.encoding", "shift_jis");          //This doesn't change anything
    // that I can tell
    ...
    // "+GlobalFilePathes.PFAD_PICS_URL+"\n--------------");
    loadApplicationProperties();
    ...
    if (!Fenster.IS_APPLET
        && Arrays.asList("gnome-shell", "mate", "other...")
            .contains(System.getenv("DESKTOP_SESSION"))) {
      try {
    ...
    //        if (zeitLizenzOK && macLizenzOK) win.setActivationOfSimulator(true); else
    // win.setActivationOfSimulator(false);
    // =================================
  ...
  public static void forceLoadApplicationProperties() {
    ...
    try { // create and load default properties
      ...
      Logger.getLogger(GeckoSim.class.getName())
          .log(Level.SEVERE, "Could not find file: " + e.getMessage());
  ...
  private void checkJavaVersion() {
    try {
      ...
      if (jV < 1.6) {
        ...
        errorMessage.append(
            DialogAbout.VERSION + " needs Java 1.6 (or higher) to be installed\non your computer.");
        errorMessage.append(
            "Currently you employ Java " + javaVersion + " Currently you employ Java ");

        JOptionPane.showMessageDialog(
            null, errorMessage.toString(), "Memory error!", JOptionPane.ERROR_MESSAGE);

        if (operatingmode == OperatingMode.STANDALONE) {
          System.exit(0);
        } else if (operatingmode == OperatingMode.SIMULINK
            || operatingmode == OperatingMode.EXTERNAL) {
          _win.dispose();
        }
      }
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          ...
          err.toString()
              ...
              + "compatible with your Java JVM. GeckoCIRCUITS is currently running with JVM "
  ...
  public static void saveProperties() {
    ...
    try {
      ...
      Logger.getLogger(GeckoSim.class.getName())
          .log(Level.CONFIG, "Saving system properties: " + file.getAbsolutePath());
      ...
      Logger.getLogger(GeckoSim.class.getName())
          .log(
              Level.WARNING,
              "Could not write property file. Perhaps we do not have file write permissions.",
              "");
    } catch (IOException ex) {
      ex.printStackTrace();
      Logger.getLogger(GeckoSim.class.getName()).log(Level.SEVERE, null, ex);
  ...
  /**
   * when loading the file from the main thread, the gui can throw exceptions, since swing is not
   * thread-safe! Therefore, do this in the event dispatch thread.
   */
