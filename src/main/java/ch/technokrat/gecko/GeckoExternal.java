...
/**
 * This is a simple wrapper, so that users can access GeckoCIRCUITS from within matlab. Some
 * specific things have been done, e.g. startGui which creates another thread that lives to support
 * Gui-Actions.
 *
 * <p>Careful (for later versions of GeckoCIRCUITS): here, we do a lot of code duplication. The
 * methods in this wrapper are similar existent in GeckoCustom, GeckoSimulink and SimulationAccess
 * Later, we should do this a bit nicer...!
 *
 * @author andy
 */
 ...
 */
/**
 * This wrapper is now deprecated for use by GeckoCIRCUITS users. They should now use GeckoRemote.
 * We live these here for 1) backwards compatibility 2) our own internal use (possibly), i.e. with
 * demo programs
 *
 * @author anstupar
 */
public class GeckoExternal {
  ...
  public static void startGui() {
    ...
    System.out.println(
        "Please switch to using GeckoRemote (as explained in the Appendix of the GeckoSCRIPT tutorial included with your GeckoCIRCUITS distribution).\n");
    System.out.println("Reasons to switch: ");
    System.out.println(
        "Using GeckoRemote runs GeckoCIRCUITS in its own JVM, not inside MATLAB, this means that");
    System.out.println("1) You can close GeckoCIRCUITS without having to close all of MATLAB;");
    System.out.println(
        "2) You don't have to worry about memory allocation for GeckoCIRCUITS in the MATLAB JVM;");
    System.out.println(
        "3) You will not have problems with compiling Java blocks in your model as was the case with some MATLAB installations.\n");
    System.out.println(
        "GeckoExternal continues to function as before for backwards compatibility. However it is no longer maintained.");
    System.out.println("This means that:");
    System.out.println(
        "1) Any new problems with using GeckoExternal in MATLAB will not be addressed;");
    System.out.println(
        "2) Any new GeckoSCRIPT functions (version 1.6 and later) will not be available through GeckoExternal.");
    System.out.println("***WARNING: GeckoExternal is a DEPRECATED API***");
  ...
  public static double[] getSignalCharacteristics(
      String scopeName, int scopePort, double startTime, double endTime) {
    checkExternal();
  ...
  public static double[] getSignalCharacteristics(
      String scopeName, double startTime, double endTime) {
