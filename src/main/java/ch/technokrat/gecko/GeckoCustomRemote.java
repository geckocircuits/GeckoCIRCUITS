...
/**
 * This is an implementation of GeckoCustom, that is to be used for remote method invocation with
 * GeckoRemote.
 *
 * @author anstupar
 */
public final class GeckoCustomRemote extends AbstractGeckoCustom
    implements GeckoRemoteInterface, CallbackServerInterface {

  private boolean _free =
      true; // denotes if this instance of GeckoCIRCUITS is free for a remote connection
  private static long _lastSessionIDActive = 0;
  public static Map<Long, CallbackClientInterface> clients;

  private boolean _acceptsExtraConnections =
      false; // denotes if this instance of GeckoCIRCUITS allows more than one client to connect
  private int _numberOfExtraConnectionsAccepted =
      0; // denotes how many additional clients (besides the first one) this instance of
  // GeckoCIRCUITS will accept
  ...
  public long connect() {
    ...
    // add session ID to clients map with null value - method registerClientForCallback will place
    // an object there
  ...
  public void disconnect(final long remoteSessionID) {
    ...
    if (clients != null && !clients.isEmpty() && clients.containsKey(remoteSessionID)) {
      ...
      _free =
          (clients.isEmpty()
              || (_acceptsExtraConnections
                  && clients.size() < (_numberOfExtraConnectionsAccepted + 1)));
    }
  }
  ...
  @Override
  public void registerForCallback(final CallbackClientInterface callbackClientObject)
  ...
  public static String getClientInfo() {
    if (clients == null || clients.isEmpty()) {
      ...
      if (pong == null) {
        returnValue.append(
            I18nKeys.ERROR_CLIENT_IS_REGISTERED_BUT_CANNOT_BE_CONTACTED.getTranslation());
        Set<Entry<Long, CallbackClientInterface>> clientSet = clients.entrySet();
        CallbackClientInterface oneClient;
        for (Entry<Long, CallbackClientInterface> clientEntry : clientSet) {
          oneClient = clientEntry.getValue();
