...
public class CompileScript {
  ...
  protected static AbstractCompileObject _compileObject = new CompileObjectNull();
  ...
  static void compile(final ScriptWindow sw) {
    ...
    if (GeckoSim.compiler_toolsjar_missing) {
      ...
      sw._compMessagesTextArea.setText(
          "Compilar library tools.jar is missing in the ./lib directory!");
      return;
    }
    ...
    try {
      ...
      if (sw._advancedOption) {
        sw.addSourceLine(
            "    public "
                + sw._className
                + "(SimulationAccess simaccess, JTextArea outputArea, HashMap element_map) {");
      } else {
        sw.addSourceLine(
            "    public " + sw._className + "(SimulationAccess simaccess, JTextArea outputArea) {");
      }
      ...
      sw.addSourceLine(
          "    } catch(Throwable ex) { writeOutputLn(\"An error occured during script execution:\");");
    ...
    try {
      _compileObject =
          new CompileObject(
              sw._compileSourceCode, sw._className, sw._circuit._additionalSourceFiles);
          ...
          sw._scriptObject =
              (AbstractGeckoCustom)
                  constructor.newInstance(
                      new Object[] {sw._circuit, sw.jTextAreaOutput, sw._advancedObjects});
        } else {
          sw._scriptObject =
              (AbstractGeckoCustom)
                  constructor.newInstance(new Object[] {sw._circuit, sw.jTextAreaOutput});
        }
