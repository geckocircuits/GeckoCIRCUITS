...
public abstract class AbstractGeckoCustom implements GeckoRemoteInterface {
  ...
  public final void simulateStep() {
    if (!_simInited) {
      initSimulation();
      writeOutputLn(
          "Warning: Simulation was not initialized prior to calling simulateStep(). "
  ...
  public final void simulateSteps(final int steps) {
    if (!_simInited) {
      initSimulation();
      writeOutputLn(
          "Warning: Simulation was not initialized prior to calling simulateSteps(). "
              + "Simulation has been initialized to values specified in GeckoCIRCUITS.");
    }
  ...
  public final void simulateTime(final double time) {
    if (!_simInited) {
      initSimulation();
      writeOutputLn(
          "Warning: Simulation was not initialized prior to calling simulateTime(). "
              + "Simulation has been initialized to values specified in GeckoCIRCUITS.");
    }
  ...
  public final void endScript() {
    if (_simInited) {
      endSimulation();
      writeOutputLn(
          "Warning: Did not call endSimulation() after using simulation control methods! "
  ...
  @Override
  @SuppressWarnings(
      "PMD") // CHECKSTYLE:OFF I cannot rename this method, since it is already used by Gecko-Users!
  ...
  public String[] getThermalElements() {
    final List<? extends AbstractCircuitBlockInterface> thermal_elems =
        _circuit.se.getElementTHERM();
  ...
  private <T extends AbstractCircuitBlockInterface> String[] createComponentsList(
    ...
    try {
      ...
      if (component instanceof Operationable) {
        Operationable.OperationInterface operationInterface =
            Operationable.OperationInterface.fabricFromString(
                operationName, (Operationable) component);
        return operationInterface.doOperation(parameterValue);
      } else {
        throw new IllegalAccessException(
            "Operation " + operationName + " cannot be performed on component!");
      }
    } catch (IllegalAccessException e) {
      writerOutputErrorLn(e.getMessage());
      throw new RuntimeException(e);
  ...
  public final void setParameters(
    ...
    if (parameterNames.length == values.length) {
      ...
      writerOutputErrorLn(
          "Error writing values for element "
              + elementName
              + ": array of names not equal in length to array of values.");
    }
  }
  ...
  @Override
  @Deprecated
  public final void setLossFile(final String elementName, final String lossFileName)
  ...
  /**
   * gets the value of field of an element, taking the name of that field as argument
   *
   * @param elementName
   * @param parameterName
   * @return
  ...
  /**
   * get the output of a control block by name, for control elements with a single output (no output
   * name specified)
   *
   * @param elementName
   * @return
  ...
  @Override
  public final double[] getSignalCharacteristics(
      final String scopeName, final int scopePort, final double startTime, final double endTime) {
    try {
  ...
  public final double getAvg(
      final String scopeName, final int scopePort, final double startTime, final double endTime) {
    try {
      final double[] characteristics =
          _circuit.getSignalCharacteristics(scopeName, scopePort, startTime, endTime);
  ...
  @Override
  public final double[] getSignalCharacteristics(
      final String scopeName, final double startTime, final double endTime) {
  ...
  @Override
  public final double getSignalAvg(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
            NetzlisteCONTROL.globalData, row, startTime, endTime);
    try {
  ...
  @Override
  public final double getSignalRMS(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double getSignalMin(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double getSignalMax(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double getSignalTHD(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double getSignalShape(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double getSignalKlirr(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double getSignalRipple(
      final String signalName, final double startTime, final double endTime) {
    int[] row = {getSignalRow(signalName)};
    CharacteristicsCalculator charCalc =
        CharacteristicsCalculator.calculateFabric(
  ...
  @Override
  public final double[][] getSignalFourier(
  ...
  private static final double DEFAULT_TARGET_CORRELATION = 0.998;
  private static final double DEFAULT_TARGET_MEAN_PCT_DIFF = 0.005;

  @Override
  public final void initSteadyStateDetection(
      final String[] stateVariables,
    ...
    try {
      ...
      for (int i = 0; i < stateVariables.length; i++) {
        ...
        if (!((elem instanceof ReglerVOLT)
            || (elem instanceof ReglerAmperemeter)
            || (elem instanceof ReglerTEMP))) {
          writerOutputErrorLn(
              "Supplied element "
                  + stateVariables[i]
                  + " is not a measuring element. Steady state monitoring not initialized!");
          return;
        }
      }
      ...
      if ((_periods.length == 0) || (stateVariables.length == 0)) {
        writerOutputErrorLn(
            "Steady state monitoring not initialized: no supplied frequencies or state variables!");
        return;
      }
  ...
  /**
   * evaluate difference vector, return true if steady-state condition is satisfied take as argument
   * allowed error (difference between 2 vectors), and also comparison option option 1 - take
   * average of all differences option 2 - make sure every difference is below threshold option 3 -
   * make sure most differences are below threshold, and those that are not are below threshold*2
   *
   * @param diff
   * @param allowed_error
   * @param option
   * @return
   */
  private boolean evaluateVector(
      final double[] diff, final double allowed_error, final int option) {
    boolean steadystate;
    ...
    if (option <= 1) {
      ...
      if ((noOfPointsBelowError >= requiredPrecision)
          && (noOfPointsBelowTwiceError <= (diff.length - requiredPrecision))
          && (noOfPointsAboveTwiceError == 0)) {
        steadystate = true;
      } else {
        steadystate = false;
      }
  ...
  public final double[] simulateUntilSteadyState(final boolean supressMessages) {
    if (!_steadyStateInitialized) {
      writerOutputErrorLn(
          "Steady state detection is not initialized! Please call initSteadyStateDetection() with appropriate arguments first!");
      return null;
    }
    ...
    while (!steadyStateReached && (time < _steadyStateSimTime)) {
      ...
      time += _steadyStateDt;
      for (int i = 0; i < _periods.length; i++) {
        if (Math.abs((time - potentialCycleStartTime) - _periods[i])
            <= (2.1 * _steadyStateDt) /*
                         * (dt/1000)
                         */) {
          /*
      ...
      if (steadyStateReached) {
        writeOutputLn(
            "Steady state reached at "
                + time
                + " seconds with period of oscillation: "
                + period
                + " seconds.");
      } else {
  ...
  public final void initSteadyStateDetection(
    ...
    initSimulation(
        _steadyStateDt, _steadyStateSimTime); // initialize simulation to find signal names
  ...
  public final double[] simulateToSteadyState(final boolean supressMessages) {
    return simulateToSteadyState(
        supressMessages, DEFAULT_TARGET_CORRELATION, DEFAULT_TARGET_MEAN_PCT_DIFF);
  }

  @Override
  public final double[] simulateToSteadyState(
    ...
    time += period;
    while (!steadyStateReached && time < _steadyStateSimTime) {
      // we always simulate 1 period, and then check for cross-correlation between it and the
      // previously simulated period
      simulateTime(period);
      time += period;
      stateVarList.clear();
   ...
   *
   * @param waveforms a list of all the waveforms to evaluate (each waveform must have 2 periods)
   * @return a list which contains three arrays - one with the cross-correlations, one with %
   *     difference of the means of the two periods, and one with the % difference of the RMS values
   *     of the two periods
   */
  private List<double[]> getCrossCorrelation(List<float[]> waveforms) {
    ...
    for (int i = 0; i < t1mean.length; i++) {
      // System.out.println("t1mean[" + i + "] = " + t1mean[i] + " t2mean[" + i + "] = " +
      // t2mean[i]);
    ...
    for (int i = 0; i <= endp1; i++) {
      for (int j = 0; j < numOfWaveforms; j++) {
        ...
        sumT2[j] +=
            (waveformsAsArray[j][i + offset] - t2mean[j])
                * (waveformsAsArray[j][i + offset] - t2mean[j]);
        sumT1T2[j] +=
            (waveformsAsArray[j][i] - t1mean[j]) * (waveformsAsArray[j][i + offset] - t2mean[j]);
      }
    }

    // calculate cross correlation for each vector element
    crossCorrelations =
        new double[numOfWaveforms]; // return cross correlations plus means of the two series
  ...
  /**
   * Evaluate the results of the cross-correlation of 2 periods of the simulation, i.e. check if the
   * circuit has reached steady-state.
   *
   * @param correlationResults the results of the evaluation of the cross-correlation between 2
   *     periods of the relevant simulation waveforms, given as a list which contains three arrays -
   *     one with the cross-correlations, one with % difference of the means of the two periods, and
   *     one with the % difference of the RMS values of the two periods
   * @param targetCorrelation the target correlation. Ideally it is 1, (a number higher than 1 is
   *     invalid!), but realistically this is never reached, in most cases this should be in the
   *     range 0.99 - 0.999
   * @param targetMeanPctDiff the target percentage difference (0 - 1) between the means or RMS of
   *     the two periods. Ideally it is 0 (a value less than zero is invalid!), but realistically it
   *     should be 0.001 - 0.005
   * @return true if the correlations and mean/rms percentage differences are equal to or greater
   *     (less for means/rms) than their targets, i.e. if the circuit has reached steady state,
   *     false otherwise
   */
  private boolean evaluateCorrelations(
      final List<double[]> correlationResults,
      final double targetCorrelation,
      final double targetMeanPctDiff) {
  ...
  public final double[][] getFourier(
  ...
  public final void setPosition(
      final String elementName, final int xPosition, final int yPosition) {
  ...
  public void deleteAllComponents(String subcircuitName) {
    if (subcircuitName == null) {
      ...
      if (component instanceof SubcircuitBlock) {
        ...
        _circuit.se.deleteAllComponents(
            new ArrayList<AbstractCircuitSheetComponent>(scb._myCircuitSheet.allElements));
      } else {
  ...
  public final void createComponent(
    ...
    try {
      ...
      AbstractBlockInterface parentElement =
  ...
  @Override
  public final void setOutputNodeName(
    ...
    if (parentElement != null) {
      ...
      if (nodeIndex < 0 || nodeIndex >= outTerms.size()) {
        ...
        try {
          _circuit.setElementNodeLabel(
              parentElement, StartOrStopNode.STOP_NODE, nodeIndex, nodeName);
        } catch (Exception e) {
          writerOutputErrorLn(
              "ERROR setting node label for " + elementName + ": " + e.getMessage());
          throw new RuntimeException(e);
        }
      }
    }
  ...
  @Override
  public final void setInputNodeName(
      final String elementName, final int nodeIndex, final String nodeName) {
    final AbstractBlockInterface parentElement = IDStringDialog.getComponentByName(elementName);
    ...
    if (nodeIndex < 0 || nodeIndex >= inputPorts.size()) {
      ...
      try {
        _circuit.setElementNodeLabel(
            parentElement, StartOrStopNode.START_NODE.START_NODE, nodeIndex, nodeName);
      } catch (Exception e) {
  ...
  public final String getInputNodeName(final String elementName, final int nodeIndex) {
    ...
    if (parentElement != null) {
      if (nodeIndex < 0 || nodeIndex >= inputPorts.size()) {
        ...
        try {
          return _circuit.getElementNodeLabel(
              parentElement, StartOrStopNode.START_NODE.START_NODE, nodeIndex);
        } catch (Exception e) {
          writerOutputErrorLn(
              "ERROR setting node label for " + elementName + ": " + e.getMessage());
          throw new RuntimeException(e);
  ...
  /**
   * the direction is based on the flow direction! - i.e. "down" means flow direction arrow is
   * pointing down, etc.
   *
   * @param elementName
   * @param direction DOWN - the default, Typ.NS LEFT - the next in the default rotation, Typ.OW UP
   *     - the 2nd in the default rotation, Typ.SN RIGHT - the 3rd in the default rotation, Typ.WO
   */
  @Override
  ...
  /**
   * Helper class for the following two methods (getTimeArray and getSignalData), used to avoid code
   * duplication.
   */
  ...
  @Override
  public final float[] getSignalData(
    ...
    for (int i = timeIntervalData._startIndex, counter = 0;
        i < timeIntervalData._indexLimitMaximum;
  ...
  @Override
  public final double[] getTimeArray(
      final String signalName, final double tStart, final double tEnd, final int skipPoints) {
    final AbstractDataContainer data = NetzlisteCONTROL.globalData;
    final TimeIntervalData timeIntervalData = new TimeIntervalData(tStart, tEnd, skipPoints, data);
