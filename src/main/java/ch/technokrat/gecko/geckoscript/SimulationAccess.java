...
public class SimulationAccess implements GeckoFileable {
  ...
  static final long DUMMY_BLOCK_ID = -1231231987;
  ...
  public <T extends AbstractCircuitBlockInterface> List<T> getComponentsOfType(
      Class<T> searchClass) {
  ...
  private static List<List<AbstractBlockInterface>> doListSort(
      final List<? extends AbstractBlockInterface> unsorted) {
    List<List<AbstractBlockInterface>> sortedListofLists =
        new ArrayList<List<AbstractBlockInterface>>();
    List<Class> types = new ArrayList<Class>();
    HashMap<Class, List<AbstractBlockInterface>> listsByType =
        new HashMap<Class, List<AbstractBlockInterface>>();
  ...
  public double[] getSignalCharacteristics(
      String scopename, int port, double start_time, double end_time) throws Exception {
    AbstractBlockInterface block = IDStringDialog.getComponentByName(scopename);

    if (!(block instanceof ReglerOSZI)) {
      throw new Exception(
          "Supplied element " + scopename + "to getSignalCharacteristics function is not a SCOPE");
    } else {
  ...
  public double[][] doFourierAnalysis(
      String scopename, int port, double start_time, double end_time, int harmonics)
      throws Exception {
    AbstractBlockInterface block = IDStringDialog.getComponentByName(scopename);
  ...
  private boolean isPositionValid(/*
    ...
    if (x >= worksheetSizeX || y >= worksheetSizeY) {
      throw new Exception(
          "Given position is outside defined drawing area! Sheet size is "
              + worksheetSizeX
              + "x"
              + worksheetSizeY
              + " and given new position is "
              + x
              + "x"
              + y
              + ".");
      ...
      // BUT - to update this, we first have to set the new coordinates and then re-paint the
      // component
      // this is too complicated for now, so just give a "buffer zone" around the component and
      // check if position is far enough away from the sides
      // of the sheet
      int bufferSpace = 3;
      if (y < bufferSpace
          || x < bufferSpace
          || (worksheetSizeY - y) < bufferSpace
          || (worksheetSizeX - x) < bufferSpace) {
        valid = true;
        // throw new Exception("Given position is too close to worksheet sides. Sheet size is " +
        // worksheetSizeX + "x" + worksheetSizeY + " and given new position is " + x + "x" + y +
        // ".");
      } else {
        valid = true;
      }
  ...
  void createNewConnector(
    ...
    if (startPositionOK && endPositionOK) {
      newElement =
          se.externalCreateAndPlaceNewConnector(
              elementName, xStart, yStart, xEnd, yEnd, startHorizontal);
      if (newElement == null) {
  ...
  public AbstractBlockInterface createNewElement(
      final AbstractTypeInfo elementCategory, final String elementName, final int x, final int y)
      throws Exception {
  ...
  public void renameElement(AbstractBlockInterface element, String newName)
  ...
  public void setElementNodeLabel(
      final AbstractBlockInterface element,
      final AbstractGeckoCustom.StartOrStopNode labelType,
      final int nodeIndex,
    ...
    final NetzlisteAllg netzlisteAllg1 =
  ...
  // set the label for a model element
  public String getElementNodeLabel(
      final AbstractBlockInterface element,
      final AbstractGeckoCustom.StartOrStopNode labelType,
      final int nodeIndex)
      throws Exception {
    ...
    if (_populateFileList) {
      ...
      if (fileMissing) {
        final String errorMessage =
            filesMissing + " additional source files missing in GeckoSCRIPT code";
