...
final class JavaMemoryRestart {
  ...
  /**
   * trys to restart the program with same command line options, but with more memory. Function will
   * only return true, when the following is fulfilled: - initial memory size was smaller that
   * default JVM Memory size - the invoked process returned a "Starting"-String in its output -
   *
   * @param memorySize value in MB for the new memory
   * @return true if restart was successful
   */
  public static boolean startNewGeckoCIRCUITSJVM(
      final int memorySize, final String[] args, final String javaCommand) {
    ...
    try {
      ...
      System.err.println(
          "Could not re-start GeckoCIRCUITS.jar with increased memory size,"
    ...
    if (new File(pathToJarFile).exists()) {
      ...
      final List<String> commands =
          createJVMCallCommands(javaCommand, memorySize, pathToJarFile, args);
  ...
  private static boolean searchForReadyString(
      final BufferedReader stdBufRead, final BufferedReader errBufRead) {
    ...
    Runnable errPrintRunnable =
        new Runnable() {
          @Override
          public void run() {
            try {
              for (String line = errBufRead.readLine();
                  line != null;
                  line = errBufRead.readLine()) {
  ...
  private static List<String> createJVMCallCommands(
      final String javaCommand,
      final int memorySize,
      final String pathToJarFile,
      final String[] args) {
