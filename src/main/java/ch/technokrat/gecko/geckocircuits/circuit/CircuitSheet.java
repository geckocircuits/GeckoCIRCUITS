...
public class CircuitSheet extends JPanel {
  ...
  public final MapList allElements =
      new MapList() {
        @Override
  ...
  void paintConnectorTestBackground(final Graphics2D graphics) {
    ...
    for (Point point : CircuitSheet._showNodes) {
      graphics.fillOval(
          (int) (point.x * dpix) - RAD_CTM,
          (int) (point.y * dpix) - RAD_CTM,
          2 * RAD_CTM,
          2 * RAD_CTM);
    }
  }
    ...
    try {
      ...
      for (AbstractCircuitSheetComponent comp : getLocalSheetComponents()) {
        if (comp instanceof TextFieldBlock) {
          comp.paintGeckoComponent(
              _se.findGraphicsForComponent(comp, g2d, g2dSelected, g2dDisabled));
        }
      }
      ...
      for (Point pt : _findNodes) {
        ...
        graphics.fillOval(
            (int) (pt.x * dpix) - 2 * RAD_CTM,
            (int) (pt.y * dpix) - 2 * RAD_CTM,
            4 * RAD_CTM,
            4 * RAD_CTM);
      }
      ...
      try {
        for (AbstractCircuitSheetComponent elem : getLocalSheetComponents()) {
          ...
          elem.paintGeckoComponent(
              _se.findGraphicsForComponent(elem, g2d, g2dSelected, g2dDisabled));
        }
        ...
        for (AbstractCircuitSheetComponent elem : getLocalSheetComponents()) {
          ...
          elem.paintComponentForeGround(
              _se.findGraphicsForComponent(elem, g2d, g2dSelected, g2dDisabled));
        }

      } catch (ConcurrentModificationException ex) {
        // this happens sometimes, when the painting thread tries to paint and a new component is
        // inserted
        ...
        if (_se.x1markRe < _se.x2markRe) {
          if (_se.y1markRe < _se.y2markRe) {
            g2d.drawRect(
                _se.x1markRe,
                _se.y1markRe,
                (_se.x2markRe - _se.x1markRe),
                (_se.y2markRe - _se.y1markRe));
          } else {
            g2d.drawRect(
                _se.x1markRe,
                _se.y2markRe,
  ...
  public Set<String> findString(
      final String searchString, final boolean ignoreCase, final boolean startsWith) {
    Set<String> foundStrings = new HashSet<String>();
    _findNodes.clear();
    foundStrings.addAll(
        findElementInterface(getLocalSheetComponents(), searchString, ignoreCase, startsWith));
    repaint();
    return foundStrings;
  }
    ...
    switch (connectorType) {
      case CONTROL:
        NetzlisteAllg.fabricNetzlistComplete(
            getConnection(ConnectorType.CONTROL),
            allElements.getClassFromContainer(RegelBlock.class));
        break;
      case THERMAL:
        NetListLK.fabricExcludingSubcircuits(
            getConnection(ConnectorType.THERMAL), getLocalComponents(ConnectorType.THERMAL));
        break;
      default:
        NetListLK.fabricExcludingSubcircuits(
            getConnection(ConnectorType.LK_AND_RELUCTANCE), getLocalComponents(ConnectorType.LK));
        break;
    }
  }

  private Set<String> findElementInterface(
      Collection<? extends AbstractCircuitSheetComponent> elements,
      ...
      if (elem != null) {
        if (elem instanceof AbstractBlockInterface) {
          ...
          if (findFromString(searchString, block.getStringID(), ignoreCase, startsWith)) {
            ...
            foundStrings.add(
                "Component: " + block.getStringID() + " x=" + sheetPos.x + " y=" + sheetPos.y);
          }
        }
          ...
          for (TerminalInterface endTerm : ((ComponentTerminable) elem).getAllTerminals()) {
            ...
            if (findFromString(searchString, labelString, ignoreCase, startsWith)) {
              ...
              if (elem instanceof AbstractBlockInterface) {
                foundStrings.add(
                    "Label of: "
                        + ((AbstractBlockInterface) elem).getStringID()
                        + " x="
                        + sheetPos.x
                        + " y="
                        + sheetPos.y);
  ...
  private boolean findFromString(
      final String searchString,
  ...
  private boolean selectPotentialNodesToShow(
      PotentialArea[] pot,
      Point clickPoint,
  ...
  public void maus_connectorTest(final Point clickPoint) {
    // damit man nicht (wie unten) beim 'return' vorzeitig aussteigt und eine Verbindung
    // versehentlich
    // im Bearbeitungs-Modus laesst, die folgende kleine Schleife:
    CircuitSheet._showNodes.clear();

    List<AbstractBlockInterface> localElementsLK =
        getLocalComponents(ConnectorType.LK_AND_RELUCTANCE);
    ...
    PotentialArea[] pot =
        (NetzlisteAllg.fabricNetzlistComplete(
                getConnection(ConnectorType.LK_AND_RELUCTANCE), localElementsLK))
            .getPotentiale();
    ...
    PotentialArea[] pot2 =
        (NetzlisteAllg.fabricNetzlistComplete(
                getConnection(ConnectorType.CONTROL), localElementsCONTROL))
            .getPotentiale();
    ...
    PotentialArea[] pot3 =
        (NetzlisteAllg.fabricNetzlistComplete(
                getConnection(ConnectorType.THERMAL), localElementsTHERM))
            .getPotentiale();
  ...
  public final Set<AbstractCircuitSheetComponent> getElementsInRectangle(
      final int x1markRe, final int y1markRe, final int x2markRe, final int y2markRe) {
    final Set<AbstractCircuitSheetComponent> returnValue =
  ...
  private static boolean componentIsIncludedInRectangle(
      int x1MR, int y1MR, int x2MR, int y2MR, AbstractCircuitSheetComponent elem) {
  ...
  /**
   * @param elementName this is the full path of a component, for which we search the circuit sheet.
   *     E.g. Sub.1#inductor.3 would return the circuit sheet of Sub.1
   * @return
   */
  ...
  public List<AbstractBlockInterface> getLocalComponents(final ConnectorType connectorType) {
    ...
    final Collection<AbstractBlockInterface> allComponents =
        allElements.getClassFromContainer(AbstractBlockInterface.class);
    returnValue.addAll(allComponents);
    for (AbstractBlockInterface remove : allComponents) {
      if (connectorType == connectorType.RELUCTANCE
          || connectorType == ConnectorType.LK
          || connectorType == ConnectorType.LK_AND_RELUCTANCE) {
        if (remove.getSimulationDomain() != ConnectorType.LK
            && remove.getSimulationDomain() != ConnectorType.LK_AND_RELUCTANCE
            && remove.getSimulationDomain() != ConnectorType.RELUCTANCE) {
  ...
  public List<String> getLocalControlLabels() {
    ...
    final List<AbstractBlockInterface> allControlComponents =
        getLocalComponents(ConnectorType.CONTROL);
  ...
  /**
   * when componentes / connectors are deleted, whe have to enshure that the link to e.g. voltage
   * measurement components is deleted!
   */
  ...
  /** return all components that are sub-components of this circuit sheet (recursively) */
  public Collection<AbstractCircuitSheetComponent> getLocalRecursiveSubComponents() {
    Set<AbstractCircuitSheetComponent> returnValue =
        new LinkedHashSet<AbstractCircuitSheetComponent>();
  ...
  public void setNewScaling(final int dpix) {
    setPreferredSize(
        new Dimension(_worksheetSize.getSizeX() * dpix, _worksheetSize.getSizeY() * dpix));
  }
