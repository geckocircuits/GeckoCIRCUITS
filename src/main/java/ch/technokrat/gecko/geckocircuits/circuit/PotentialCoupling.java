...
public class PotentialCoupling {
  ...
  public PotentialCoupling(
      AbstractBlockInterface parent, int[] stringIDIndices, ConnectorType potentialType) {
    _parent = parent;
  ...
  void tryFindChangedLabels(
      Collection<? extends AbstractCircuitSheetComponent> exchangeNew, int endIndex) {
    for (int i = 0; i < _stringIDIndices.length; i++) {
      try {
  ...
  class SetOperation extends Operationable.OperationInterface {
    ...
    SetOperation(final int index) {
      super(
          ((index == 0) ? "setCouplingLabel" : "setSecondCouplingLabel"),
          I18nKeys.SET_COUPLING_LABEL);
      _index = index;
    }
    ...
    public Object doOperation(final Object parameterValue) {
      ...
      if (_parent instanceof AbstractPotentialMeasurement) {
        ((AbstractPotentialMeasurement) _parent)
            .getComponentCoupling()
            .setNewCouplingElement(0, null);
      }
      return null;
    }
  }
