...
public abstract class AbstractInductor extends AbstractNonLinearCircuitComponent
  ...
  final UserParameter<Double> _inductance =
      UserParameter.Builder.<Double>start("inductance", 3.0E-4)
          .longName(I18nKeys.INDUCTANCE)
          .shortName("L")
          .unit("H")
          .showInTextInfo(TextInfoType.SHOW_NEVER)
          . // noninearity: custom display
          arrayIndex(this, 0)
          .build();
  final UserParameter<Double> _initialCurrent =
      UserParameter.Builder.<Double>start("initialCurrent", 0.0)
