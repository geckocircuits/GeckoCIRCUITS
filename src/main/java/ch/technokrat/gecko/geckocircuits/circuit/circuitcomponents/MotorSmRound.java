...
public final class MotorSmRound extends AbstractMotorSM {
  ...
  public static AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(
          MotorSmRound.class, "SM-RO", I18nKeys.SM_RO, I18nKeys.ROUND_ROTOR_SYNCHRONOUS_MACHINE);
  private double _damperResistanceQ2;

  final UserParameter<Double> _damperResistanceQ2_par =
      UserParameter.Builder.<Double>start("damperResistanceQ2", INIT_DAMPER_RES_Q2_VALUE)
  ...
  final UserParameter<Double> _damperLeakageIndQ2Par =
      UserParameter.Builder.<Double>start("damperLeakageInductanceQ2", INIT_LEAKAGE_INDUCTANCE_Q2)
  ...
  final UserParameter<Double> _initDampCurrentPar =
      UserParameter.Builder.<Double>start("initialDamperCurrent", 0.0)
  ...
  void calculateMotorEquations(final double deltaT, double time) {
    ...
    double inv_Lm =
        (Mf - Mi)
                / Math.PI
                * (((phim - psiT) * Math.atan(tauT * (phim - psiT)) - psiT * Math.atan(tauT * psiT))
                    + 0.5
                        / tauT
                        * (Math.log(1 + (tauT * psiT) * (tauT * psiT))
                            - Math.log(1 + tauT * tauT * (phim - psiT) * (phim - psiT))))
                / phim
            + 0.5 * (Mf + Mi);

    double ddt_psimd =
        (isd * (Rkd / Llkd - _statorResistance / Lls)
                + isq * _omegaElectric
                + ifd * (Rkd / Llkd - Rf / Llf)
                - Rkd * inv_Lm / Llkd * psimd
                + _omegaElectric / Lls * psimq
                + ud / Lls
                + uf / Llf)
            / (inv_Lm + 1 / Lls + 1 / Llf + 1 / Llkd);
    double ddt_psimq =
        (-_omegaElectric * isd
                + isq * (_damperResistanceQ2 / _damperLeakageIndQ2 - _statorResistance / Lls)
                + (_damperResistanceQ2 / _damperLeakageIndQ2 - Rkq / Llkq1) * _ikq1
                - _omegaElectric / Lls * psimd
                - _damperResistanceQ2 * inv_Lm / _damperLeakageIndQ2 * psimq
                + uq / Lls)
            / (inv_Lm + 1 / Lls + 1 / Llkq1 + 1 / _damperLeakageIndQ2);
