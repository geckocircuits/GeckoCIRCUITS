...
public final class SchematicTextInfo {
  ...
  private Point
      _txtKlickPoint; // zum Starten des relativen Text-Verschiebens wurd initial auf diesen Punkt
  // geklickt
  ...
  // for some blocks, the textfield just does not make sense, e.g. textField Block, save
  // date-Block...
  ...
  public boolean isAngeklickt(final int mouseX, final int mouseY) {
    return (_xTxtKlickMin <= mouseX)
        && (mouseX <= _xTxtKlickMax)
        && (_yTxtKlickMin <= mouseY)
        && (mouseY <= _yTxtKlickMax);
  }
  ...
  void absetzenText(final Point position) {
    final int dpix = AbstractCircuitSheetComponent.dpix;
    _dxTxt =
        (_txtKlickPoint.x - _element.getSheetPosition().x)
            + position.x * 1.0 / dpix
            - _txtKlickPoint.x;
    _dyTxt =
        (_txtKlickPoint.y - _element.getSheetPosition().y)
            + position.y * 1.0 / dpix
            - _txtKlickPoint.y;
    if (_dxTxt != _dxTxtBeforeMove && _dxTxt != _dxTxtBeforeMove) {
      final MoveTextFieldUndoAction undoAction =
          new MoveTextFieldUndoAction(_dxTxtBeforeMove, _dyTxtBeforeMove, _dxTxt, _dyTxt);
  ...
  public void setNewRelativePosition(final Point relPosition) {
    ...
    _dxTxt =
        (_txtKlickPoint.x - _element.getSheetPosition().x)
            + relPosition.x * 1.0 / dpix
            - _txtKlickPoint.x;
    _dyTxt =
        (_txtKlickPoint.y - _element.getSheetPosition().y)
            + relPosition.y * 1.0 / dpix
            - _txtKlickPoint.y;
  }
  ...
  public void paint(final Graphics2D graphics) {
    ...
    final int dpix = AbstractCircuitSheetComponent.dpix;
    if (SchematischeEingabe2._thermDisplayMode
        .showName) { // falls zusaetzlich auch der Name angezeigt werden soll
    ...
    for (PrintParameter par : _printParameter) {
      ...
      final int stringYPos =
          (int) (_yTxtKlickMin + textHeight + counter * graphics.getFont().getSize());
      par.drawString(graphics, toDraw, _xTxtKlickMin, stringYPos);
      final double thisLength =
          graphics.getFont().getStringBounds(toDraw, frc).getWidth() * 1.0 / dpix;
  ...
  public void addParameters(
      List<UserParameter<? extends Object>> parameters, ElementDisplayProperties properties) {
    ...
    for (UserParameter<? extends Object> par : parameters) {
      ...
      if (_element instanceof Nonlinearable) {
        UserParameter<Double> replacedParameter =
            ((Nonlinearable) _element).getNonlinearReplacedParameter();
        AbstractNonLinearCircuitComponent component = (AbstractNonLinearCircuitComponent) _element;
        if (par == replacedParameter
            && !par.getNameOpt().isEmpty()
            && component._isNonlinear.getValue()) {
  ...
  private void addUserParameter(final UserParameter<? extends Object> userParameter) {
    ...
    if (value instanceof Enum<?>) {
      ...
      uTxt +=
          userParameter.getShortName() + "= " + tcf.formatENG(userParameter.getDoubleValue(), 3);
    }
  ...
  public static class PrintParameter {
    ...
    void drawString(
        final Graphics2D graphics, final String toDraw, final int xPos, final int yPos) {
  ...
  private final class MoveTextFieldUndoAction implements UndoableEdit {
    ...
    private MoveTextFieldUndoAction(
        final double oldPosX, final double oldPosY, final double newPosX, final double newPosY) {
      _oldPosX = oldPosX;
      _oldPosY = oldPosY;
      _newPosX = newPosX;
      _newPosY = newPosY;
