...
public final class MOSFET extends AbstractSwitch implements HiddenSubCircuitable {
  public static final AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(
          MOSFET.class, "MOSFET", I18nKeys.MOSFET, I18nKeys.IDEALIZED_METAL_OXIDE_FIELD_EFFECT);
  ...
  UserParameter<Double> _adRon =
      UserParameter.Builder.<Double>start("antiParallelDiodeRon", AbstractSwitch.RD_ON_DEFAULT)
          .longName(I18nKeys.ON_RESISTANCE_ANTIPARALLEL)
          .shortName("ad_rON")
          .unit("Ohm")
          .arrayIndex(this, -1)
          .build();
  UserParameter<Double> _adRoff =
      UserParameter.Builder.<Double>start("antiParallelDiodeRoff", AbstractSwitch.RD_OFF_DEFAULT)
          .longName(I18nKeys.OFF_RESISTANCE_ANTIPARALLEL)
          .shortName("ad_rOFF")
          .unit("Ohm")
          .arrayIndex(this, -1)
          .build();
  UserParameter<Double> _adUf =
      UserParameter.Builder.<Double>start("antiParallelDiodeUF", AbstractSwitch.UF_DEFAULT)
  ...
  public MOSFET() {
    ...
    double[] diodeParameter =
        new double[] {
          AbstractSwitch.RD_OFF_DEFAULT,
          550e-3,
          3.9e-3,
          AbstractSwitch.RD_OFF_DEFAULT,
          0,
  ...
  protected void drawForeground(final Graphics2D graphics) {
    ...
    graphics.drawPolyline(
        new int[] {-dpix, -dpix, (int) (-dpix * (1 + HEIGHT))}, new int[] {-dpix, dpix, dpix}, 3);
    ...
    graphics.fillPolygon(
        new int[] {
          (int) (-dpix * xOffset), (int) (-dpix * xOffset), (int) (-dpix * (WIDTH - xOffset))
        },
        new int[] {(int) (-dpix * triangleWidth), (int) (dpix * triangleWidth), 0},
        3);

    // Gate Diode:
    graphics.drawPolyline(
        new int[] {0, (int) (dpix * WIDTH), (int) (dpix * WIDTH), 0},
        new int[] {-dpix, -dpix, dpix, dpix},
        4);
  ...
  private void drawAntiParallelDiode(final Graphics2D graphics) {
    ...
    graphics.drawLine(
        (int) (dpix * (WIDTH - ANTI_DIODE_SIZE)),
        (int) (-dpix * (ANTI_DIODE_SIZE)),
        (int) (dpix * (WIDTH + ANTI_DIODE_SIZE)),
        (int) (-dpix * (ANTI_DIODE_SIZE)));
    graphics.fillPolygon(
        new int[] {
  ...
  protected void drawConnectorLines(final Graphics2D graphics) {
    graphics.drawPolyline(
        new int[] {0, 0, (int) (-dpix * WIDTH)}, new int[] {-2 * dpix, -dpix, -dpix}, 3);

    graphics.drawPolyline(
        new int[] {(int) (-dpix * WIDTH), 0, 0}, new int[] {dpix, dpix, dpix * 2}, 3);
  }
  ...
  protected void importIndividual(final TokenMap tokenMap) {
    super.importIndividual(
        tokenMap); // To change body of generated methods, choose Tools | Templates.
    ...
    if (_show170_22bugfixWarning && !warning1AlreadyShown) {
      JOptionPane.showMessageDialog(
          GeckoSim._win,
          "Your model contains MOSEFET components. Beginning from GeckoCIRCUITS release 1.70 build number 23,"
              ...
              + "      measurement it might behave different, now.",
          "MOSFET Bugfix Information",
    ...
    if (_show172_50bugfixWarning && !warning2AlreadyShown) {
      JOptionPane.showMessageDialog(
          GeckoSim._win,
          "Your model contains MOSEFET components. Beginning from GeckoCIRCUITS release 1.72 build number 50,"
              ...
              + "current during freewheeling state, then the simulation could behave different.",
          "MOSFET Bugfix Information",
          JOptionPane.PLAIN_MESSAGE);
    }
  }
  ...
  protected void exportAsciiIndividual(final StringBuffer ascii) {
    super.exportAsciiIndividual(
        ascii); // To change body of generated methods, choose Tools | Templates.
    ascii.append("\nbugfix_170_22 false");
    ascii.append("\nbugfix_172_50 false");
  }
