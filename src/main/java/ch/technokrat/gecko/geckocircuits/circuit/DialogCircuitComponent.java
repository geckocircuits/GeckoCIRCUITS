...
public abstract class DialogCircuitComponent<T extends AbstractBlockInterface> extends GeckoDialog
    implements Schliessable, WindowListener {

  final List<UserParameter<? extends Number>> registeredParameters =
      new ArrayList<UserParameter<? extends Number>>();
  ...
  public final ActionListener okActionListener =
      new ActionListener() {
  ...
  public final void setNewElementName() throws NameAlreadyExistsException {
    ...
    if (tfNam.getText().isEmpty() && !(element instanceof SubCircuitTerminable)) {
      JOptionPane.showMessageDialog(
          this, "Empty object name is not allowed!", "Warning", JOptionPane.WARNING_MESSAGE);
      return;
    }
  ...
  public static void setValueFromTextField(
      final UserParameter<Double> userParameter, final FormatJTextField formatJTextField) {
  ...
  public static void setValueFromIntegerTextField(
      final UserParameter<Integer> userParameter, final FormatJTextField formatJTextField) {
    double number = formatJTextField.getNumberFromField();
