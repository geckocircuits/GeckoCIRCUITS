...
class ReluctanceInductorDialog extends DialogElementLK<ReluctanceInductor> {
  ...
  protected void baueGUIIndividual() {
    ...
    reverseInput.addActionListener(
        new ActionListener() {
          @Override
    ...
    reverseOutput.addActionListener(
        new ActionListener() {
          @Override
    ...
    windingsField.addKeyListener(
        new KeyListener() {
          ...
          public void keyReleased(final KeyEvent event) {
            ampTurnsField.setNumberToField(
                windingsField.getNumberFromField() * ampTurnsField.getNumberFromField());
          }
        });
    ...
    initCurrentField.addKeyListener(
        new KeyListener() {
          @Override
          public void keyTyped(final KeyEvent event) {
            // nothing todo
          ...
          public void keyReleased(final KeyEvent event) {
            try {
              ampTurnsField.setNumberToField(
                  initCurrentField.getNumberFromField() * windingsField.getNumberFromField());
            } catch (Exception ex) {
              ex.printStackTrace();
            }
          }
          ...
          public void keyReleased(final KeyEvent event) {
            try {
              if (windingsField.getNumberFromField() != 0) {
                initCurrentField.setNumberToField(
                    ampTurnsField.getNumberFromField() / windingsField.getNumberFromField());
              }
            } catch (Exception ex) {
              // not output here. During typing, there may be unreadable numbers, e.g. "1e-" for
              // 1e-10"
            }
          }
        });
