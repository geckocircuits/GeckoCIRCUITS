...
public class KnotenLabel implements Serializable {
  ...
  private int xAnker,
      yAnker; // ScematicEntry-Koord. vom Ankerpunkt, d.h. wohin die graue Linie zielt
  ...
  public void setKoordTxt(int xT, int yT) {
    this.xTxt = xT;
    this.yTxt = yT;
  }

  public void setKoordAnker(int xA, int yA) {
    this.xAnker = xA;
    this.yAnker = yA;
  }
  ...
  public boolean istAngeklickt(int xPix, int yPix) {
    if ((x1click < xPix) && (xPix < x2click) && (y1click < yPix) && (yPix < y2click)) return true;
    else return false;
  }
