...
public class SimulationsKern {
  ...
  //
  private double[] pLK_ALT,
      pTHERM_ALT; // zur Speicherung der Systemmatrizen fuer korrektes Weitermachen nach CONTINUE
  // -------------------------------
  // Kopplung [ Leistungskreis - Regelkreis - thermischer Kreis ]
  //
  private int[]
      interessanteKnotenLK; // zum Aufnehmen der ZV-Spannungs-Kurven zwischen LK-Knoten mittels VOLT
  private int[]
      interessanteKnotenTHERM; // zum Aufnehmen der ZV-Temperatur-Kurven zwischen THERM-Knoten
  // mittels TEMP
  ...
  private int mult = 20,
      add = 50; // Erweiterung der Felder in den Interaktions-Methoden, um Spezialfaelle abzudecken
  //
  private int[][]
      zuordnung_SchalterLK_SWITCH; // Welcher Schalter wird durch welchen SWITCH angesteuert?
  private int[][]
      zuordnung_QuelleLK_signalCONTROL; // Welcher CONTROL-Knoten steuert welche signalgesteuerte
  // LK-Quelle?
  private int[][]
      zuordnung_QuelleTHERM_signalCONTROL; // Welcher CONTROL-Knoten steuert welche signalgesteuerte
  // THERM-Quelle?
  private int[][]
      zuordnung_MaschineLK_LoadParameterInCONTROL; // Welcher CONTROL-Knoten definiert welche
  // mechanischen Signale (zB. externes Moment) des
  // Motors?
  //
  ...
  private void simulateOneTimeStep() {
    ...
    if (simuliereLeistungskreis) {
      ...
      nl.berechneSubCircuitAlsDifferentialgleichung(
          dt, t); // interne Berechungen in SubCircuits diverser LK-Elemente
    ...
    if (simuliereThermKreis) {
      ...
      lkmTHERM.berechneBauteilStroeme(
          -1, dt, t, false,
          0); // stoergroesse '-1' nur fuer die Dioden relevant, hier sind Werte egal
      thermNL.berechneSubCircuitAlsDifferentialgleichung(
          dt, t); // eventuelle analytische Berechungen in SubCircuits der THERM-Elemente
  ...
  private void doDiodeErrorsRecalculations() {
    int switchingErrorCounter =
        0; // 'stoergroesse<1.0' verhindert, dass der Algorithmus beim Dioden-Umschalten zwischen
    // Zustaenden haengen bleibt
    double stoergroesse = 1; // 0.9999999;
    boolean isNewIteration = false;

    while (diodenSchaltfehler =
        lkmLK.berechneBauteilStroeme(stoergroesse, dt, t, isNewIteration, switchingErrorCounter)) {
      isNewIteration = true;
      if (switchingErrorCounter > 10000) {
        // new DialogDiodenError(switchingErrorCounter, t);
        this.lastUpdateOfScope(); // die endgueltige Simulation wird sicherheitshalber noch einmal
        // aktualisiert
  ...
  private void setControlledSourcesFromControlValue() {
    ...
    for (int i1 = 0; i1 < zuordnung_QuelleLK_signalCONTROL.length; i1++) {
      ...
      if (blockOutput != null) { // weil beim ersten Zeitschritt yout noch nicht definiert wurde
        par[1] =
            blockOutput[outputIndex][
                0]; // bei signalgesteuerten Quellen ist parameter[1] das Signal
      }
    }
  }
  ...
  private void setThermalControlledSourcesFromControlValues() {
    ...
    for (int i1 = 0; i1 < zuordnung_QuelleTHERM_signalCONTROL.length; i1++) {
      ...
      AbstractCircuitBlockInterface e =
          thermNL.elements[zuordnung_QuelleTHERM_signalCONTROL[i1][0]];
      double[] par = e.parameter;
      double[][] blockOutput = unsortedCalculators[reglerIndex]._outputSignal;
      if (blockOutput != null) { // weil beim ersten Zeitschritt yout noch nicht definiert wurde
        par[1] =
  ...
  private void dataTransferTherm() {
    ...
    for (int i1 = 0; i1 < interessanteKnotenTHERM.length; i1 += 2) {
      ...
      if (interessanteKnotenTHERM[i1 + 1] == 0) {
        ...
        potentialdifferenz =
            lkmTHERM.p[interessanteKnotenTHERM[i1]] - lkmTHERM.p[interessanteKnotenTHERM[i1 + 1]];
      }
      // Signal bei TEMP setzen:
      try {
        sortedCalculators[zeigerAufControlElementTHERM[i1 / 2]]._outputSignal[0][0] =
            potentialdifferenz;
      } catch (Exception ex) {
        System.err.println(
            controlParameters[zeigerAufControlElementTHERM[i1 / 2]].length
                + " "
                + zeigerAufControlElementTHERM[i1 / 2]);
        ex.printStackTrace();
      }
    }
  ...
  private void dataTransferLK_Control() {
    ...
    for (int i1 = 0; i1 < zuordnung_MaschineLK_LoadParameterInCONTROL.length; i1++) {
      AbstractCircuitBlockInterface e =
          nl.elements[zuordnung_MaschineLK_LoadParameterInCONTROL[i1][0]];
      ...
      double[] par = e.parameter;
      double[][] blockOutput = unsortedCalculators[reglerIndex]._outputSignal;
      par[((AbstractMotor) e).getIndexForLoadTorque()] =
          blockOutput[outputIndex][0]; // bei signalgesteuerten Quellen ist parameter[1] das Signal
    }
    // -------------------------------------------------------------------
    ...
    for (int i1 = 0; i1 < interessanteKnotenLK.length; i1 += 2) {
      ...
      if (interessanteKnotenLK[i1 + 1] == 0) {
        ...
        potentialdifferenz =
            (lkmLK.p[1 + interessanteKnotenLK[i1] - 1]
                - lkmLK.p[1 + interessanteKnotenLK[i1 + 1] - 1]);
      }
    ...
    // ------------------
    // den in VIEWMOT gewaehlten internen Maschinenparameter an den CONTROL-Ausgang von C_VIEWMOT
    // legen:
    for (int i1 = 0; i1 < zeiger_VIEWMOT_MaschineLK.length; i1++) {
      ...
      sortedCalculators[zeiger_VIEWMOT_MaschineLK[i1][0]]._outputSignal[0][0] =
          interneMaschienenGroesse;
    }
  }
  ...
  private boolean checkForSwitchAction() {
    ...
    for (int i1 = 0; i1 < zuordnung_SchalterLK_SWITCH.length; i1++) {
      double schaltSignal =
          sortedCalculators[zuordnung_SchalterLK_SWITCH[i1][0]]._inputSignal[0][0];
      ...
      switch (e.getCircuitTyp()) {
        ...
        case LK_MOSFET:
          ...
          break;
        case LK_THYR:
          // THYR wird wie eine Diode behandelt (siehe weiter unten!!), hier wird der aktuelle
          // Gate-Zustand gespeichert:
          if (schaltSignal > 0.5) {
            par[8] = 1;
          } else {
  ...
  public void simulateTime(double time) throws Exception {
    ...
    if (overReach) {
      throw new Exception(
          "Specified simulation time goes beyond specified simulated end time; simulated only up to end time");
    }
  }
  ...
  public void initSimulation(
      ...
      NetListContainer nlContainer,
      boolean recalculateMatrixFromDifferentDt) {
    ...
    //
    definiereInteraktion_VOLT_AMP_LK(); // wie messe ich Stroeme und Spannungen im LK mit
    // CONTROL-AMP bzw. CONTROL-VOLT?
    definiereInteraktion_MaschineLK_VIEWMOT(); // zur Messung von Maschinen-internen Parametern
    //
    zuordnung_SchalterLK_SWITCH =
        definiereInteraktion_Schalter_Regler(); // welches CONTROL-GATE steuert welchen LK_SWITCH
    // (LK_S,LK_IGBT,LK_THYR) an?
    zuordnung_QuelleLK_signalCONTROL =
        definiereInteraktion_SignalgesteuerteQuelle_Regler(
            nl); // welches allg. CONTROL-Signal steuert welche LK_QUELLE (LK_U, LK_I) an?
    zuordnung_QuelleTHERM_signalCONTROL =
        definiereInteraktion_SignalgesteuerteQuelle_Regler(
            thermNL); // welches allg. CONTROL-Signal steuert welche THERM_QUELLE (LK_TEMP, LK_FLOW)
    // an?

    zuordnung_MaschineLK_LoadParameterInCONTROL =
        definiereInteraktion_MaschineLK_LoadParameterInCONTROL(); // Welcher CONTROL-Knoten
    // definiert welche mechanischen
    // Signale (zB. externes Moment)
    // des Motors?
    definiereInteraktion_TEMP_FLOW_THERM(); // wie messe ich thermischen Fluss und
    // Temperatur-Differenzen im THERM mit CONTROL-FLOW bzw.
    // CONTROL-TEMP?
    setControlledSourcesFromControlValue();
    setThermalControlledSourcesFromControlValues();
    ...
    if (getAnfangsbedVomDialogfenster) {
      ...
      lkmLK.initMatrizen(
          ...
          true,
          Fenster._solverSettings.SOLVER_TYPE
              .getValue()); // pALT= new double[..];   iALT= new double[..];
      lkmLK.schreibeMatrix_A(dt, tAktuell, false);

      //
      // thermischer Kreis:
      lkmTHERM = new LKMatrices(Fenster._solverSettings.SOLVER_TYPE.getValue());
      lkmTHERM.initMatrizen(
          (NetListLK) thermNL,
          getAnfangsbedVomDialogfenster,
          false,
          Fenster._solverSettings.SOLVER_TYPE.getValue());
      lkmTHERM.schreibeMatrix_A(dt, tAktuell, false);
    }
    // =============================
    if (lkmLK.matrixSize
        < 2) { // wenn der Leistungskreis nicht exisiert, wird er konsequent nicht simuliert:
    ...
    if (lkmTHERM.matrixSize
        < 2) { // wenn der thermische Kreis nicht exisiert, wird er konsequent nicht simuliert:
  ...
  private int[][] definiereInteraktion_SignalgesteuerteQuelle_Regler(final NetListLK netlist) {
    // ---------------------------------------
    // Welche Signalgesteuerte Quelle im LK (VOLT, AMP, ...) wird von welchem CONTROL-Signal
    // angesteuert? -->
    //
    int[][] zuordnung_QuelleLK_signalCONTROL = new int[mult * controlANZAHL + add][];
    int counter = 0;
    AbstractCircuitBlockInterface[] allElements =
        netlist.elements; // careful: we DON't use elements including subcircuits. In this case,
    ...
    for (int i = 0; i < allElements.length; i++) {
      ...
      if (element instanceof AbstractVoltageSource || element instanceof AbstractCurrentSource) {
        if (element.getParameter()[0] == SourceType.QUELLE_SIGNALGESTEUERT_NEW
            || element.getParameter()[0] == SourceType.QUELLE_SIGNALGESTEUERT) {
          NetzlisteCONTROL.IndexConnection li =
              controlNL.getIndexConnection(
                  element.getParentCircuitSheet(), element.getParameterString()[0]);
  ...
  private int[][] definiereInteraktion_Schalter_Regler() {
    ...
    for (int iC = 0; iC < controlANZAHL; iC++) {
      if (controlNL._orderedControlBlocks[iC] instanceof ReglerGate) {
        ...
        AbstractBlockInterface controlledSwitch =
            reglerGate.getComponentCoupling()._coupledElements[0];
  ...
  private int[][] definiereInteraktion_MaschineLK_LoadParameterInCONTROL() {
    ...
    for (int iLK = 0; iLK < nl.elements.length; iLK++) {
      ...
      if (block instanceof AbstractMotor) {
        NetzlisteCONTROL.IndexConnection li =
            controlNL.getIndexConnection(
                nl.elements[iLK].getParentCircuitSheet(), nl.elements[iLK].getParameterString()[0]);
        if (li != null) {
          zuordnung_MaschineLK_LoadParameterInCONTROL[
                  zuordnungANZAHL_MaschineLK_LoadParameterInCONTROL] =
              new int[] {iLK, li._elementIndex, li._inBlockIndex_outputIndex};
          zuordnungANZAHL_MaschineLK_LoadParameterInCONTROL++;
        }
      }
    }
  ...
  private void definiereInteraktion_MaschineLK_VIEWMOT() {
    ...
    for (int i1 = 0; i1 < controlANZAHL; i1++) {
      if (c[i1] instanceof ReglerVIEWMOT) {
        ...
        AbstractBlockInterface selectedMotor =
            reglerVIEWMOT.getComponentCoupling()._coupledElements[0];
          ...
          for (int i2 = 0; i2 < nl.elements.length; i2++) {
            ...
            if (selectedMotor.equals(el)) {
              ...
              for (int i3 = 0; i3 < parameterStringIntern.size(); i3++) {
                if (iA2.equals(
                    parameterStringIntern.get(i3))) { // --> internen Motor-Parameter gefunden
  ...
  private void definiereInteraktion_VOLT_AMP_LK() {
    ...
    for (int i1 = 0; i1 < controlANZAHL; i1++) {
      if (c[i1] instanceof ReglerVOLT || c[i1] instanceof ReglerMMF) {
        ...
        AbstractBlockInterface directComponent =
            reglerVOLT.getComponentCoupling()._coupledElements[0];
        ...
        if ((!uA.isEmpty()) && (!uB.isEmpty())) {
          ...
          if (reglerVOLT instanceof ReglerMMF && (directComponent instanceof ReluctanceInductor)) {
  ...
  private void definiereInteraktion_TEMP_FLOW_THERM() {
    ...
    for (int i1 = 0; i1 < controlANZAHL; i1++) {
      if (c[i1] instanceof ReglerTEMP) {
        ...
        AbstractBlockInterface directComponent =
            reglerTEMP.getComponentCoupling()._coupledElements[0];
  ...
  private void lastUpdateOfScope(long sleep) {
    ...
    } // damit es nicht zu einer 'RacingCondition' mit einer eventuell noch laufenden Aktualisierung
    // aus takteAuffrischungScope() kommt
    // ---------------------------
    for (int i1 = 0; i1 < c.length; i1++) {
      try {
        if (c[i1] instanceof ReglerOSZI) {
  ...
  ////                double x= input[i1];
  ////                if (x==x) signals_from_external[i1]= x; else signals_from_external[i1]=0;  //
  // NaN, Inf
