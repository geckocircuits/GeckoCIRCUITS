 ...
 * @author andreas
 */
public final class MutualInductance extends AbstractCircuitBlockInterface
    implements ComponentCoupable {
  static final AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(
          MutualInductance.class,
          "K",
          I18nKeys.MAGNETIC_COUPLING_K,
          I18nKeys.MAGNETIC_COUPLING_DESCRIPTION);
  ...
  final UserParameter<Double> _couplingCoefficient =
      UserParameter.Builder.<Double>start("couplingCoefficient", DEFAULT_COEFFICIENT)
  ...
  final UserParameter<Boolean> _showLines =
      UserParameter.Builder.<Boolean>start("showInductorLine", false)
  ...
  private final ComponentCoupling _componentCoupling =
      new ComponentCoupling(2, this, new int[] {0, 1});
    ...
    if (_showLines.getValue()) {
      ...
      if (_componentCoupling._coupledElements[0] != null) {
        ...
        graphics.drawLine(
            ...
            (int)
                (dpix
                    * (lkop.getSheetPosition().y
                        - getSheetPosition().y))); // Verbindungslinie zu L1
      }
  ...
  protected void addTextInfoParameters() {
    ...
    if (firstInductor == null && secondInductor != null) {
      final String parStr =
          I18nKeys.NOT_DEFINED.getTranslation() + " @ " + secondInductor.getStringID();
      _textInfo.addErrorValue(parStr);
    }

    if (firstInductor != null && secondInductor == null) {
      final String parStr =
          firstInductor.getStringID() + " @ " + I18nKeys.NOT_DEFINED.getTranslation();
      _textInfo.addErrorValue(parStr);
    }
