...
public class CoupledInductorsGroup implements AStampable, CurrentCalculatable {
  ...
  private int[]
      globalAdjIndicesPlus; // some kind of "sparse" storage, for faster matrix-vector-products
  private int[]
      globalAdjIndicesMinus; // some kind of "sparse" storage, for faster matrix-vector-products
  ...
  CoupledInductorsGroup(
      final List<InductorCouplingCalculator> _inductors,
      final List<MutualCouplingCalculator> _mutualCouplings,
      int N) {
  ...
  public void initInductanceMatrix(
      final List<InductorCouplingCalculator> allInductors,
      final List<MutualCouplingCalculator> couplings,
      int N) {
  ...
  public void calculateCurrent(double[] p, double dt, double t) {
    ...
    if (pOld == null) pOld = new double[p.length]; // ugly, should be removed later!
    ...
    if (solver == SolverType.SOLVER_TRZ)
      pOld =
          p; // we need the old potential for the trapezoidal solver - yes, this is UGLY and should
    // be replaced with a better implementation
  }
  ...
  public double getLPproductForTRZ(InductorCouplingCalculator inductor) {
    ...
    } else { // QUESTION: this should somehow perhaps be initialized to the initial value of the
      // voltages??? -> or do we never set a non-zero initial voltage for coupled inductors?
      for (int i = 0; i < n; i++) {
        oldAP2[i] += 0.0;
        oldAP2[i] -= 0.0;
      }
