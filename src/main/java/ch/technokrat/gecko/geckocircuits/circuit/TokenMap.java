...
public final class TokenMap {
  ...
  private final Map<String, Deque<BlockInfo>> _duplicateMap =
      new LinkedHashMap<String, Deque<BlockInfo>>();
  private final Map<String, Deque<BlockInfo>> _specialMap =
      new LinkedHashMap<String, Deque<BlockInfo>>();
  ...
  public TokenMap(final String[] ascii, final boolean makeSpecialPairs) {
    ...
    if (makeSpecialPairs) {
      ...
      _specialTokens.put(
          "GeckoFileManager", new SpecialPair("<GeckoFileManager>", "<\\GeckoFileManager>"));
    }
      ...
      if (spaceIndex > 0) {
        ...
        if (makeSpecialPairs) {
          if (_specialTokens.containsKey(lineToken)) {
            ...
            //                        System.out.println("--------------");
            //                        System.err.println("linetoken: " + lineToken + " " +
            // pair._startToken + " " + pair._stopToken);
            //                        System.err.println("reading: " + ascii[readLineNumber + 0]);
            //                        System.err.println("reading: xxx " + ascii[readLineNumber +
            // 1]);
  ...
  private static void readTokenLine(
      final String lineToken, final int lineNumber, Map<String, Integer> map) {
  ...
  public String[] readDataLine(final String identifier, final String[] targetObject) {
    try {
      ...
      final StringTokenizer stk =
          new StringTokenizer(asciiDaten, DatenSpeicher.SEPARATOR_ASCII_STRINGARRAY);
  ...
  public List<String> readDataLineStringArray(final String identifier) {
    try {
      ...
      final String ascii = asciiLines[lineNumber];
      final StringTokenizer stk =
          new StringTokenizer(ascii, DatenSpeicher.SEPARATOR_ASCII_STRINGARRAY);
      stk.nextToken(); // erster Wert wird uebersprungen
  ...
  /**
   * reads a textblock, including spaces. In contradiction, LeseAsciiString would return only the
  ...
  private static void logErrorString(final String identifier, final Exception exception) {
    ...
    Logger.getLogger(TokenMap.class.getName()).log(Level.WARNING, messageString);
