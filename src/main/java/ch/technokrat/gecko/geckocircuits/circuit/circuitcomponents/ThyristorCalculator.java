...
public class ThyristorCalculator extends AbstractSwitchCalculator implements CurrentCalculatable {
  ...
  @Override
  public final void calculateCurrent(
      final double[] pVector, final double deltaT, final double time) {
  ...
  /**
   * function is overwritten, since the thyristor behaves different: - switch off only after
   * _tReverse - switch off only when current is < 0
   *
   * @param value
   */
  @Override
