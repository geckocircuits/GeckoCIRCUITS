...
public abstract class AbstractVoltageSource extends AbstractCircuitSource {
  ...
  public static final String[] SHORT_NAMES_FOR_AMPLITUDE =
      new String[] {"uMAX", "MMF_MAX", "T_MAX"};
  ...
  final UserParameter<Double> _lowerLimit =
      UserParameter.Builder.<Double>start("lowerLimit", Double.MIN_VALUE)
          ...
          .build();
  final UserParameter<Double> _upperLimit =
      UserParameter.Builder.<Double>start("upperLimit", Double.MAX_VALUE)
          .mapDomains(getDomains())
  ...
  private void drawPlusSymbol(final Graphics2D graphics, final int length) {
    ...
    graphics.drawLine(
        (int) PLUS_MINUS_X_OFFSET - length / 2,
        yHeight,
        (int) PLUS_MINUS_X_OFFSET + length / 2,
        yHeight);
    graphics.drawLine(
        PLUS_MINUS_X_OFFSET,
        (int) yHeight + length / 2,
        PLUS_MINUS_X_OFFSET,
        (int) yHeight - length / 2);
  }
