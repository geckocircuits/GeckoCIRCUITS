...
final class ThermMODUL extends AbstractCircuitBlockInterface {
  ...
  private static final int X_CENTER = 430,
      Y_CENTER = 250; // Zentrum des Modul-Bildes in PixelPunkten
  ...
  protected void drawForeground(final Graphics2D graphics) {
    ...
    _yOUT = (int) (_height + 1);
    graphics.drawLine(
        (int) (dpix * _xOUT), (int) (dpix * (0 + 1)), (int) (dpix * _xOUT), (int) (dpix * _yOUT));
  ...
  private void drawInputs(final Graphics graphics, final Color color1) {
    ...
    graphics.fillRect(
        (int) (dpix * getSheetPosition().x) - D_E,
        (int) (dpix * (_yBef[0] - 1 / 2.0)),
        2 * D_E,
    ...
    graphics.drawLine(
        (int) (dpix * getSheetPosition().x),
        (int) (dpix * getSheetPosition().y),
        (int) (dpix * getSheetPosition().x),
        (int) (dpix * (_yBef[_noOfChips - 1] + 1 / 2.0)));
  }
