...
public abstract class AbstractBlockInterface extends AbstractCircuitSheetComponent
  ...
  public List<UserParameter<? extends Object>> registeredParameters =
      new ArrayList<UserParameter<? extends Object>>();
  private ComponentDirection orientationBeforeMove = ComponentDirection.NORTH_SOUTH;
  private ch.technokrat.gecko.geckocircuits.control.Point _sheetPosBeforeMove =
      new ch.technokrat.gecko.geckocircuits.control.Point(0, 0);
  /**
   * I use "lazy initialization" for the idstring, since the string is built from the "type number",
   * and the type is not available within the constructor.
   */
  ...
  private ModelMVC<ComponentDirection> _componentDirection =
      new ModelMVC<ComponentDirection>(
          ComponentDirection.NORTH_SOUTH, "Rotation/orientation of sheet component");
  ...
  public final Set<ComponentCoupling> _isReferencedBy =
      new HashSet<ComponentCoupling>() {
        @Override
  ...
  public final void setAccessibleParameter(final String paramname, final double paramValue)
    ...
    throw new IllegalAccessException(
        "Parameter with name " + paramname + " not found in " + getStringID() + ".");
  }

  public final double getAccessibleParameterValue(final String paramname)
      throws IllegalAccessException {
  ...
  public AbstractCircuitSheetComponent copyFabric(final long shiftValue) {
    ...
    if (this instanceof ComponentCoupable) {
      ...
      ((ComponentCoupable) returnValue)
          .getComponentCoupling()
          .shiftCopyCouplingIDsFrom(oldCoupling, shiftValue);
    }
  ...
  public abstract ElementDisplayProperties getDisplayProperties();
  ...
  public void importASCII(final TokenMap tokenMap) {
    if (this instanceof SemiconductorLossCalculatable) {
      ...
      if (subBlock != null) {
        LossProperties verluste =
            (LossProperties) ((SemiconductorLossCalculatable) this).getVerlustBerechnung();
    ...
    if (tokenMap.containsToken("orientierung")) {
      setComponentDirection(
          ComponentDirection.getFromCode(
  ...
  public List<Verbindung> getShortConnectors() {
    ...
    for (int i = 0; i < Math.min(XIN.size(), YOUT.size()); i++) {
      final Verbindung verb =
          new VerbindungShortConnector(ConnectorType.LK, this.getParentCircuitSheet());
  ...
  public TerminalInterface clickedTerminal(final Point clickPoint) {
    for (TerminalInterface testTerm : getAllTerminals()) {
      if (testTerm.getPosition().equals(clickPoint)
          && !(testTerm instanceof TerminalHiddenSubcircuit)) {
        return testTerm;
      }
    }
    return null;
  ...
  public void moveComponent(final Point moveToPoint) {
    ...
    setSheetPositionWithoutUndo(
        new ch.technokrat.gecko.geckocircuits.control.Point(
            moveToPoint.x + _sheetPosBeforeMove.x, moveToPoint.y + _sheetPosBeforeMove.y));
  }
  ...
  public void copyLKBlockPars(final AbstractBlockInterface copy) {
    ...
    if (this instanceof SemiconductorLossCalculatable) {
      LossProperties origLosses =
          (LossProperties) ((SemiconductorLossCalculatable) this).getVerlustBerechnung();
      ((LossProperties) ((SemiconductorLossCalculatable) copy).getVerlustBerechnung())
          .copyPropertiesFrom(origLosses);
    }
  ...
  public void paintGeckoComponent(final Graphics2D graphics) {
    ...
    if (getDisplayProperties().showName
        || (this instanceof SpecialNameVisible
            && ((SpecialNameVisible) this)
                .isNameVisible())) { // falls zusaetzlich auch der Name angezeigt werden soll
  ...
  public int elementAngeklickt(final Point clickPoint) {
    return istAngeklickt((int) (dpix * clickPoint.x), (int) (dpix * clickPoint.y));
  }
  ...
  public void setPositionVorVerschieben(
      ch.technokrat.gecko.geckocircuits.control.Point positionVorVerschieben) {
  ...
  /**
   * when components are short-circuited, we paint a bold line through the component. Usually, this
   * is an error, and the line helps the user to see where he has short-components. However, do this
   * only with two-port components, but not with op-amps, transformers, ...
   *
   * @param graphics
   */
  private void paintShortCircuitConnections(final Graphics2D graphics) {
    ...
    for (int i = 0; i < Math.min(XIN.size(), YOUT.size()); i++) {
      final boolean isLabelConnection =
          XIN.get(i)
                  .getLabelObject()
                  .getLabelString()
                  .equals(YOUT.get(i).getLabelObject().getLabelString())
  ...
  public void doDoubleClickAction(final Point clickedPoint) {
    ...
    if (clickedTerm != null
        && clickedTerm.getCircuitSheet() == SchematischeEingabe2.Singleton._visibleCircuitSheet) {
