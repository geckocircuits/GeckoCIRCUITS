...
public final class NetzlisteAllg {
  ...
  // Ergebnis (wird zum Erstellen der Netzliste, dh. Verknuepfung der Elemente ueber die Potentiale)
  // verwendet:
  ...
  private NetzlisteAllg(
      final Collection<Verbindung> connectors,
  ...
  /**
   * build the complete netlist, including connections via geometry and identical labels.
   *
   * @param connectors
   * @param components
   * @return
   */
  public static NetzlisteAllg fabricNetzlistComplete(
      final Collection<Verbindung> connectors,
      final List<? extends AbstractBlockInterface> components) {
  ...
  public static NetzlisteAllg fabricNetzlistDisabledParentSubsRemoved(
      final Collection<Verbindung> connectors,
      final List<? extends AbstractBlockInterface> components) {

    final List<AbstractBlockInterface> eFilteredParentsEnabled =
        new ArrayList<AbstractBlockInterface>();
  ...
  public static NetzlisteAllg fabricNetzlistComponentLabelUpdate(
      AbstractBlockInterface element, ConnectorType terminalType) {

    CircuitSheet elementParent = element.getParentCircuitSheet();

    final List<? extends AbstractBlockInterface> elementsList =
        elementParent._se.getElementInterface(terminalType);
  ...
  /**
   * build a netlist, but do not make connections via labels (same name of nodes or connectors).
   *
   * @param connectors
   * @param components
   * @return
   */
  public static NetzlisteAllg fabricNetzlistLabelUpdate(
      final Collection<Verbindung> connectors,
      final List<? extends AbstractBlockInterface> components) {
  ...
  private void mergePotentialAreasViaLabels() {
    ...
    while (continueLoop) {
      continueLoop = false;
      final PotentialArea[] array =
          _potentialAreas.toArray(new PotentialArea[_potentialAreas.size()]);
  ...
  private void connectGlobalTerminals() {
    ...
    while (continueLoop) {
      continueLoop = false;
      final PotentialArea[] compare1Array =
          _potentialAreas.toArray(new PotentialArea[_potentialAreas.size()]);
      final PotentialArea[] compare2Array =
          _potentialAreas.toArray(new PotentialArea[_potentialAreas.size()]);
        ...
        for (int j = i + 1; j < compare2Array.length; j++) {
          ...
          for (SubCircuitTerminable term1 : pot1.getPotentialTerminals()) {
            for (SubCircuitTerminable term2 : pot2.getPotentialTerminals()) {
              if (term1 == term2
                  && term1.getBlockTerminal().getCategory()
                      == term2.getBlockTerminal().getCategory()) {
                final PotentialArea merged = pot1.mergePotential(pot2, false);
                _potentialAreas.remove(pot1);
                _potentialAreas.remove(pot2);
                _potentialAreas.add(merged);
      ...
      for (int j = i + 1; j < array.length; j++) {
        ...
        if (!alreadyConnected.contains(pot2)) {
          if (pot1.geometricOnSamePotential(pot2)) {
            ...
            /**
             * something is weird here! merged is equal to pot1, but just removing pot2 from the
             * list, and not deleting pot1 and adding it again makes problems? this has probably to
             * do with the order of the potentials in the container. -> fix later!
             */
            _potentialAreas.remove(pot1);
            _potentialAreas.remove(pot2);
            _potentialAreas.add(merged);
  ...
  private class GraphEdge {
    ...
    private void traverse(
        Map<PotentialArea, ArrayList<GraphEdge>> graphMap,
        LinkedHashSet<GraphEdge> connectedEdges) {
      isVisited = true;
  ...
  void deSingularizeIsolatedPotentials() {
    ...
    Map<PotentialArea, ArrayList<GraphEdge>> graphMap =
        new LinkedHashMap<PotentialArea, ArrayList<GraphEdge>>();
    ...
    ArrayList<LinkedHashSet<PotentialArea>> potentialAreasGroups =
        new ArrayList<LinkedHashSet<PotentialArea>>();
