...
public abstract class AbstractCircuitBlockInterface extends AbstractBlockInterface {
  ...
  static final int ARROW_LENGTH = 11,
      ARROW_WIDTH = 3; // Symbol-Pfeil fuer Flussrichtung --> Pfeilspitzenabmessung
  ...
  /**
   * Careful: this is the default behavior of CurrentMeasurable interface. However, some components
   * don't implement this interface (E.g. Motors, Terminals, ...);
   *
   * @return
   */
  ...
  /**
   * the background of all components is drawn first, so that it does not overlap foreground
   * elements.
   *
   * @param graphics
   */
  ...
  /**
   * the foreground is painted after the background. E.g. when you like to draw text strings, they
   * should not be covered by the background of other elements.
   *
   * @param graphics
   */
  ...
  public final ElementDisplayProperties getDisplayProperties() {
    switch (getSimulationDomain()) {
      ...
      default:
        assert false
            : "ciruit domain: " + getSimulationDomain() + " " + this + " " + this.getClass();
        break;
    }
    return null;
  }
  ...
  static List<? extends CircuitComponent> getCalculatorsFromSubComponents(
      final HiddenSubCircuitable subCircuitable) {
    Collection<? extends AbstractBlockInterface> hiddenSubs =
        subCircuitable.getHiddenSubCircuitElements();
    ...
    for (AbstractBlockInterface block : hiddenSubs) {
      ...
      returnValue.addAll(
          ((AbstractCircuitBlockInterface) block).getCircuitCalculatorsForSimulationStart());
    }
    return returnValue;
  }
}
