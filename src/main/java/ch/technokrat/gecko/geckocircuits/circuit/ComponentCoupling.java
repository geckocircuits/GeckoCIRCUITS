...
public final class ComponentCoupling {
  ...
  public ComponentCoupling(
      final int maxNumberCouplings, AbstractBlockInterface parent, int[] parStringIndices) {
  ...
  public void setNewCouplingElement(int index, AbstractBlockInterface element, String is) {
    ...
    List<String> internalStrings =
        ((AbstractCircuitBlockInterface) element).getParameterStringIntern();
  ...
  public void setNewCouplingElementUndoable(final int index, final AbstractBlockInterface element) {
    ...
    if (element != null) {
      ...
      // already exist. Could be done in a nicer way, e.g. creating a sub-class of
      // ComponentCouling...!
    }

    final CouplingUndoableEdit edit =
        new CouplingUndoableEdit(_coupledElements[index], element, index, true);
    AbstractUndoGenericModel.undoManager.addEdit(edit);
    setNewCouplingElement(index, element);
  }

  public void setNewCouplingElementInvisibleUndoable(
      final int index, final AbstractBlockInterface element) {
    if (element == _coupledElements[index]) {
      return;
    }

    final CouplingUndoableEdit edit =
        new CouplingUndoableEdit(_coupledElements[index], element, index, false);
    AbstractUndoGenericModel.undoManager.addEdit(edit);
    setNewCouplingElement(index, element);
  }
  ...
  public void importASCII(final TokenMap tokenMap) {
    ...
    if (tokenMap.containsToken("coupledReferenceID[]")) {
      long[] coupledIdentifiers =
          tokenMap.readDataLine("coupledReferenceID[]", _coupledIdentifiers);
    ...
    if (tokenMap.containsToken("copyCoupledReferenceID[]")) {
      _coupledIdentifiersBeforeCopy =
          tokenMap.readDataLine("copyCoupledReferenceID[]", new long[0]);
    }
  ...
  public void exportASCII(StringBuffer ascii) {
    ...
    /** careful: this is used fore restoring connections, when copy export -> import is used! */
    DatenSpeicher.appendAsString(ascii.append("\ncopyCoupledReferenceID"), _coupledIdentifiers);
    DatenSpeicher.appendAsString(ascii.append("\ninternalIndex"), _internalStringIndex);
    DatenSpeicher.appendAsString(ascii.append("\ninternalString"), _internalString);
  }

  public void refreshCoupledReferences(
      final List<? extends AbstractCircuitSheetComponent> allSheetElements) {
    ...
    for (AbstractCircuitSheetComponent elem : allSheetElements) {
      if (elem instanceof HiddenSubCircuitable) {
        for (AbstractBlockInterface sub :
            ((HiddenSubCircuitable) elem).getHiddenSubCircuitElements()) {
          allElements.add(sub);
        }
      }
    }
    ...
    for (int i = 0; i < _coupledElements.length; i++) {
      ...
      if ((!parameterString[parStringIndex].isEmpty() && _coupledElements[i] == null)
        ...
        for (AbstractBlockInterface elem : allElements) {
          ...
          // System.out.println("searching: " + elem.getStringID() + " " +
          // elem.getUniqueObjectIdentifier());
          ...
          if (!internStringAdded && elem.getUniqueObjectIdentifier() == _coupledIdentifiers[i]
              || _coupledIdentifiers[i] == 0
                  && elem.getStringID().equals(parameterString[parStringIndex])) {
            _coupledIdentifiers[i] = elem.getUniqueObjectIdentifier();
            if (_coupledElements[i] != null) {
              _coupledElements[i]._isReferencedBy.remove(this);
            }
  ...
  /**
   * try to find the new coupling partner, when a group of elements was copied.
   *
   * @param origElement
  ...
  void setOldCopyIdentifiers(final ComponentCoupling oldCoupling) {
    ...
    System.arraycopy(
        coupledIdentifiers, 0, _coupledIdentifiersBeforeCopy, 0, coupledIdentifiers.length);
  }
  ...
  private void removeWithSingleReference(final AbstractBlockInterface partner, final int index) {
    for (ComponentCoupling otherCoup :
        partner._isReferencedBy.toArray(new ComponentCoupling[partner._isReferencedBy.size()])) {
      if (otherCoup._parentElement instanceof ReglerGate && _parentElement instanceof ReglerGate) {
  ...
  class SetOperation extends Operationable.OperationInterface {
    ...
    SetOperation(final int index) {
      super(
          ((index == 0) ? "setComponentCoupling" : "setSecondComponentCoupling"),
          I18nKeys.SET_COMPONENT_COUPLING);
      _index = index;
    }
    ...
    public Object doOperation(final Object parameterValue) {
      ...
      final AbstractBlockInterface toCouple =
          IDStringDialog.getComponentByName((String) parameterValue);
      ...
      if (toCouple instanceof HiddenSubCircuitable) {
        Collection<? extends AbstractBlockInterface> subs =
            ((HiddenSubCircuitable) toCouple).getHiddenSubCircuitElements();
      ...
      throw new RuntimeException(
          "Component "
              + _parentElement.getStringID()
              + " cannot be coupled to component "
              + parameterValue);
    }
  }

  public List<Operationable.OperationInterface> getOperationInterfaces() {
    List<Operationable.OperationInterface> returnValue =
        new ArrayList<Operationable.OperationInterface>();
  ...
  private class CouplingUndoableEdit implements UndoableEdit {
    ...
    private CouplingUndoableEdit(
        final AbstractBlockInterface oldReference,
        final AbstractBlockInterface newReference,
        final int index,
