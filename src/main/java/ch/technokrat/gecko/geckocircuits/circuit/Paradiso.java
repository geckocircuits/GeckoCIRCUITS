...
public class Paradiso {
  ...
  private static final int nrhs = 1;
  private static final int idum = 0; /* Integer dummy. */
  ...
  public static void factorize(
      double[] values, int[] ai, int[] aj, int n, int mtype, Paradiso paradiso) {
    /* Auxiliary variables. */
    ...
    paradiso.iparm[12] =
        0; /* Maximum weighted matching algorithm is switched-on (default for non-symmetric) */
    ...
    if (error != 0) {
      Logger.getLogger(Paradiso.class.getName())
    ...
    if (error != 0) {
      Logger.getLogger(Paradiso.class.getName())
          .log(Level.SEVERE, "ERROR during numerical factorization: " + error);
    }
  ...
  public static double[] solve(
    ...
    // int dummy = LAPACK.PARDISO(pt, maxfct, mnum, mtype, phase, n, ddum, ai, aj, idum, nrhs,
