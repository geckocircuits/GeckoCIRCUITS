...
class DetailedConductionLossPanel extends DetailledLossPanel<LeitverlusteMesskurve> {
  ...
  public double uMaxCOND,
      iMaxCOND; // maximale Bereichsgrenze aller 'messkurvePvCOND[]' bzw 'messkurvePvSWITCH[]' -->
  // korekte Darstellung im Grafer
  public double[] tjGrenzenCOND,
      b0COND,
      b1COND,
      c0COND,
      c1COND,
      d0COND,
      d1COND; // Koeffizienten der Naeherungspoloynome (siehe Publikation IPEC'05) --> Econd(i,tj)
  final JCheckBox useInSolver =
      new JCheckBox("<html>Use curve in electric<br>model characteristic</html>");
  ...
  public void useNonlinearInElectric(final Diode diode) {
    ...
    useInSolver.setToolTipText(
        "Option is only available if curve for only 1 temperature is given.");
    useInSolver.addActionListener(
        new ActionListener() {
