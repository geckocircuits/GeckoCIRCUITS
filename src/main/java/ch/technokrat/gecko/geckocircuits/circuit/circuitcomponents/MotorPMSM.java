...
public final class MotorPMSM extends AbstractThreePhaseMotor {
  ...
  public static final AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(
          MotorPMSM.class, "PMSM", I18nKeys.PMSM, I18nKeys.PERMANENT_MAGNET_SYNCHRONOUS_MACHINE);

  private double _fluxLinkage;
  final UserParameter<Double> _fluxLinkagePar =
      UserParameter.Builder.<Double>start("inducedbythepermanentMagnet", 0.15)
  ...
  final UserParameter<Double> _statorInductanceDPar =
      UserParameter.Builder.<Double>start("statorInductanceD", 0.0020)
  ...
  final UserParameter<Double> _statorInductanceQPar =
      UserParameter.Builder.<Double>start("statorInductanceQ", 0.0020)
  ...
  public void setzeParameterZustandswerteAufNULL() {
    ...
    isd0 =
        2.0
            / 3.0
            * (isa0 * cos(_thetaElectric)
                + isb0 * cos(_thetaElectric - 2 * PI / 3)
                - (isa0 + isb0) * cos(_thetaElectric + 2 * PI / 3));
    isq0 =
        -2.0
            / 3.0
            * (isa0 * sin(_thetaElectric)
                + isb0 * sin(_thetaElectric - 2 * PI / 3)
                - (isa0 + isb0) * sin(_thetaElectric + 2 * PI / 3));
  ...
  void calculateMotorEquations(final double deltaT, double time) {
    ...
    double ud =
        (2 * uab * cos(_thetaElectric)
                + ubc * (cos(_thetaElectric) + sqrt(3) * sin(_thetaElectric)))
            / 3;
    double uq =
        -(2 * uab * sin(_thetaElectric)
                + ubc * (sin(_thetaElectric) - sqrt(3) * cos(_thetaElectric)))
            / 3;
    ...
    isd =
        (ud + udi + _statorInductanceD * isd0 / deltaT)
            / (_statorResistance + _statorInductanceD / deltaT);
    isq =
        (uq - uqi + _statorInductanceQ * isq0 / deltaT)
            / (_statorResistance + _statorInductanceQ / deltaT);
  }
