...
abstract class DetailledLossPanel<T extends LossCurve> extends JPanel {
  ...
  public final List<T> _lossCurves =
      new ArrayList<T>() {
        @Override
          ...
          for (LossCurve oldCurve : this) {
            ...
            if (Math.abs(oldTemperature - newTemperature) < 0.1) {
              ...
              JOptionPane.showMessageDialog(
                  parent,
                  "A curve with the selected temperature "
                      + newCurve.tj.getValue()
                      + " is already\n"
                      + "defined. Please select a different curve temperature.",
                  "Error",
  ...
  final void baueGUI() {
    ...
    _jPanelCurvesSelection.setBorder(
    ...
    // jpSWed.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "",
    // TitledBorder.LEFT, TitledBorder.TOP, TxtI.ti_Font_border));
    ...
    _table.setBorder(
        BorderFactory.createTitledBorder(
            BorderFactory.createEtchedBorder(),
            "Data of Selected Curve",
            TitledBorder.LEFT,
            TitledBorder.TOP));

    _jPanelTemperatureInput.setBorder(
        BorderFactory.createTitledBorder(
            BorderFactory.createEtchedBorder(),
            "Curve temperature Parameter",
            TitledBorder.LEFT,
            TitledBorder.TOP));
  ...
  private DataTablePanel createTable() {
    ...
    returnValue.addTableModelListener(
   ...
   * @param appliedVoltage still ugly: for conduction losses, the applied voltage does not have any
   *     effect
   */
