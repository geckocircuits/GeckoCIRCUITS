...
final class OperationalAmplifierDialog extends DialogElementLK<OperationalAmplifier> {
  ...
  protected JTabbedPane createGUI_OPV1() {
    JPanel mainParameter =
        createParameterPanel(
            element.gainValue,
            element.inputResistance,
            element.outputResistance,
            element.frequencyDependency);
    ...
    JPanel limitPanel =
        createParameterPanel(element.voltageLimitationMax, element.voltageLimitationMin);
        ...
        new JComponent() {
          public void paint(final Graphics graphics) {
            try {
              ...
              Image img =
                  (new ImageIcon(new URL(GlobalFilePathes.PFAD_PICS_URL, "dialog_opv_model.png")))
                      .getImage();
