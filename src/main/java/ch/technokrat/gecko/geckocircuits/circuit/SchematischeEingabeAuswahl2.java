...
public class SchematischeEingabeAuswahl2 extends JTabbedPane {
  ...
  private final Map<AbstractComponentTyp, AbstractBlockInterface> _map =
      new HashMap<AbstractComponentTyp, AbstractBlockInterface>();
  ...
  private CircuitTyp[] _typLK =
      new CircuitTyp[] {
        CircuitTyp.LK_U, CircuitTyp.LK_I, CircuitTyp.LK_R, CircuitTyp.LK_C, CircuitTyp.LK_L,
        CircuitTyp.LK_LKOP2, CircuitTyp.LK_M, CircuitTyp.LK_S, CircuitTyp.LK_IGBT,
            CircuitTyp.LK_MOSFET,
        CircuitTyp.LK_D, CircuitTyp.LK_THYR, CircuitTyp.LK_BJT, CircuitTyp.LK_OPV1,
            CircuitTyp.LK_TRANS // Typ.LK_LKOP
      };
  private CircuitTyp[] _typMotor =
      new CircuitTyp[] {
        CircuitTyp.LK_MOTOR,
        CircuitTyp.LK_MOTOR_PERM,
        CircuitTyp.LK_MOTOR_PMSM,
        CircuitTyp.LK_MOTOR_SMSALIENT,
        CircuitTyp.LK_MOTOR_SMROUND,
        CircuitTyp.LK_MOTOR_IMC,
        CircuitTyp.LK_MOTOR_IMA,
        CircuitTyp.LK_MOTOR_IMSAT, /*
         * Typ.LK_MOTOR_IMB,
         */
        CircuitTyp.LK_LISN,
      };

  private AbstractComponentTyp[] _typSubcircuit =
      new AbstractComponentTyp[] {
        SpecialTyp.SUBCIRCUIT,
        CircuitTyp.LK_TERMINAL,
        CircuitTyp.TH_TERMINAL,
        ControlTyp.C_TERMINAL,
        CircuitTyp.REL_TERMINAL,
        CircuitTyp.LK_GLOBAL_TERMINAL,
        CircuitTyp.TH_GLOBAL_TERMINAL,
        ControlTyp.C_GLOBAL_TERMINAL,
        CircuitTyp.REL_GLOBAL_TERMINAL,
        ControlTyp.C_MUX,
        ControlTyp.C_DEMUX
      };

  private ControlTyp[] _typCONTROL =
      new ControlTyp[] {
        C_GAIN, C_PT1, C_PT2, C_INT, C_PI, C_PD, C_HYS, C_LIMIT, C_ADD, C_SUB, C_MUL, C_DIV, C_MIN,
        C_MAX, C_SIGN, C_TF
      };
  private ControlTyp[] _typMeasure =
      new ControlTyp[] {
  ...
  private ControlTyp[] _typDigital =
      new ControlTyp[] {
        C_NOT, C_AND, C_OR, C_XOR, C_DELAY, C_SAMPLEHOLD, C_COUNTER, C_GE, C_GT, C_EQ, C_NE
      };
  private ControlTyp[] _typMath =
      new ControlTyp[] {
        C_ADD, C_SUB, C_MUL, C_DIV, C_ABS, C_SIGN, C_ROUND, C_SQR, C_SQRT, C_POW, C_EXP, C_LN,
        C_SIN, C_COS, C_TAN, C_ASIN, C_ACOS, C_ATAN, C_SDFT
      };
  private ControlTyp[] _typSourceSink =
      new ControlTyp[] {
        C_SIGNALSOURCE,
        C_CONST,
        C_SWITCH,
        ...
        C_FROM_EXTERNAL,
        C_DATA_EXPORT,
        C_SOURCE_IMPORT_DATA,
        C_SOURCE_RANDOM
      };

  private AbstractComponentTyp[] _typSpecial =
      new AbstractComponentTyp[] {
        ControlTyp.C_JAVA_FUNCTION,
        ControlTyp.C_NATIVE_C_FUNCTION,
        ControlTyp.C_SMALL_SIG,
        ControlTyp.C_ABCDQ,
        ControlTyp.C_DQABC,
        ControlTyp.C_TIME,
        ControlTyp.C_SPARSEMATRIX,
        ControlTyp.C_PMSM_CONTROL,
        ControlTyp.C_PMSM_MODULATOR,
        ControlTyp.C_THYR_CTRL,
        /*ControlTyp.C_DEBUG,*/ SpecialTyp.TEXTFIELD
      };
  private AbstractComponentTyp[] _typTherm =
      new AbstractComponentTyp[] {
        CircuitTyp.TH_TEMP,
        CircuitTyp.TH_FLOW,
        CircuitTyp.TH_PvCHIP,
        CircuitTyp.TH_RTH,
        CircuitTyp.TH_CTH,
        CircuitTyp.TH_AMBIENT
      };
  private CircuitTyp[] _typReluctance =
      new CircuitTyp[] {
        CircuitTyp.REL_RELUCTANCE,
        CircuitTyp.NONLIN_REL,
        CircuitTyp.REL_MMF,
        CircuitTyp.REL_INDUCTOR
      };
  ...
  private void createButtonsForPanel(AbstractComponentTyp[] types, JPanel compCircuit) {
    for (int i = 0; i < types.length; i++) {
      ...
      JButton testButton =
          new SchematischeEingabeAuswahl2.ComponentSelectionButton(types[i], block);
  ...
  public void registerSearchField(final SuggestionField searchTestField) {
    ...
    searchTestField.addSelectionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            for (AbstractBlockInterface block : _showBlocks) {
              if (block.getTypeDescription().getTranslation().equals(searchTestField.getText())) {
                _lastComponentButton.setSelectedShowComponent(
                    block, _showBlocksType.get(_showBlocks.indexOf(block)));
              }
            }
          }
        });
  ...
  public void registerLastComponentButton(LastComponentButton lastComponentButton) {
    ...
    _lastComponentButton.addActionListener(
        new ActionListener() {
          @Override
    ...
    SchematischeEingabeAuswahl2.MouseDraggedOutsideListener listener =
        new SchematischeEingabeAuswahl2.MouseDraggedOutsideListener(_lastComponentButton);
  ...
  class ComponentSelectionButton extends JButton {
    ...
    public ComponentSelectionButton(
        final AbstractComponentTyp typ, final AbstractBlockInterface exampleBlock) {
      ...
      final String descriptionVerbose =
          LangInit.getTranslatedString(exampleBlock.getTypeDescriptionVerbose());
      ...
      SchematischeEingabeAuswahl2.MouseDraggedOutsideListener dragListener =
          new SchematischeEingabeAuswahl2.MouseDraggedOutsideListener(_typ);
