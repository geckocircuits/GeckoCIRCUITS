...
public final class MotorSmSalient extends AbstractMotorSM {
  ...
  public static final AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(
          MotorSmSalient.class,
          "SM-SAL",
          I18nKeys.SM_SAL,
          I18nKeys.SALIENT_POLE_SYNCHRONOUS_MACHINE);

  private double _Lmq0;
  final UserParameter<Double> _unsatMagnetizingInductQPar =
      UserParameter.Builder.<Double>start(
              "unsaturatedMagnetizingInductanceQ", INITIAL_UNSAT_INDUCTANCE)
  ...
  void calculateMotorEquations(final double deltaT, double time) {
    ...
    double inv_Lmd =
        (Mf - Mi)
                / Math.PI
                * (((phimu - psiT) * Math.atan(tauT * (phimu - psiT))
                        - psiT * Math.atan(tauT * psiT))
                    + 0.5
                        / tauT
                        * (Math.log(1 + (tauT * psiT) * (tauT * psiT))
                            - Math.log(1 + tauT * tauT * (phimu - psiT) * (phimu - psiT))))
                / phimu
            + 0.5 * (Mf + Mi);
    ...
    double ddt_psimd =
        (isd * (Rkd / Llkd - _statorResistance / Lls)
                + isq * _omegaElectric
                + ifd * (Rkd / Llkd - Rf / Llf)
                - Rkd * inv_Lmd / Llkd * psimd
                + _omegaElectric / Lls * psimq
                + ud / Lls
                + uf / Llf)
            / (inv_Lmd + 1 / Lls + 1 / Llf + 1 / Llkd);
    double ddt_psimq =
        (-_omegaElectric * isd
                + isq * (Rkq / Llkq1 - _statorResistance / Lls)
                - _omegaElectric / Lls * psimd
                - Rkq * inv_Lmq / Llkq1 * psimq
                + uq / Lls)
            / (inv_Lmq + 1 / Lls + 1 / Llkq1);
