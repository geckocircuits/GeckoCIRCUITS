...
public final class IDStringDialog {
  ...
  private static Map<String, ArrayList<AbstractBlockInterface>> _allIDStrings =
      new HashMap<String, ArrayList<AbstractBlockInterface>>();
  ...
  /**
   * use this fabric when you would like to construct an ID string, that is not considered in the
   * program as "name already used". Only know usage for now: for creating the initial components
   * for the SchematischeEingabe component display / search
   *
   * @return
   */
  ...
  /**
   * use this fabric, if the name is a suggestion. If it already exists, iterate a counter number
   * and give the modified value as return value.
   *
   * @param newName
   * @return
   */
  public static IDStringDialog fabricVariableName(
      final AbstractBlockInterface parent, final String newName) {
  ...
  public void setNewNameChecked(final String newName) throws NameAlreadyExistsException {
    ...
    if (_allIDStrings.containsKey(newName)) {
      ...
      for (AbstractBlockInterface candidate : possibleConflicts) {
        if (candidate.getParentCircuitSheet() == null
            && _parent.getParentCircuitSheet() == null
            && candidate.getParentSheetIdentifier() != 0
            && _parent.getParentSheetIdentifier() != 0
            && candidate.getParentSheetIdentifier() == _parent.getParentSheetIdentifier()) {
          throw new NameAlreadyExistsException(
              "The name: \"" + newName + "\" is already in use in the circuit sheet!");
        }
  ...
  public void deleteIDString() {
    if (_allIDStrings.containsKey(this.toString())) {
      ...
      for (AbstractBlockInterface deleteSearch :
          possibleComponents.toArray(new AbstractBlockInterface[0])) {
  ...
  public static AbstractBlockInterface getComponentByName(final String searchName) {
    ...
    if (_allIDStrings.containsKey(searchName)) {
      ...
      if (possibleComponents.size() == 1) {
        ...
        throw new NoSuchElementException(
            "A component with the name " + searchName + " could not be found!");
      } else if (possibleComponents.size() > 1) {
        throw new RuntimeException(
            "Name ambiguousness error! The model contains several components with the name: "
                + searchName
                + ".\n"
    ...
    String truncatedElementName =
        searchName.substring(searchName.lastIndexOf('#') + 1, searchName.length());

    for (AbstractBlockInterface toTest :
        parent.allElements.getClassFromContainer(AbstractBlockInterface.class)) {
