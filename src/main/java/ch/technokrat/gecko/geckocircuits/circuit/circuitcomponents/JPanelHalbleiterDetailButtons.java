...
final class JPanelHalbleiterDetailButtons extends JPanel {
  ...
  JPanelHalbleiterDetailButtons(final VerlustBerechnungDetailed detailedLosses) {
    ...
    jbS2load.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent actionEvent) {
            GeckoFileManagerWindow dialog =
                new GeckoFileManagerWindow(
                    _detailedLosses, ".scl", "Semicond. losses", _jlS2, _jlS2b, jbS2edit);
    ...
    final Window parentWindow =
        SwingUtilities.getWindowAncestor(JPanelHalbleiterDetailButtons.this);
    jbS2edit.setMaximumSize(new Dimension(BUTTON_WIDTH, 25));
    jbS2edit.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent actionEvent) {
            // focusIstFreigegeben= true;  // damit kann der Focus an das nun zu Oeffnende Fenster
            // weitergegeben werden
            final DialogVerlusteDetail dialog =
                DialogVerlusteDetail.fabricCreateExisiting(_detailedLosses, parentWindow);
            dialog.setVisible(true);
            checkFileLink();
          }
        });
    ...
    jbS2new.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent actionEvent) {
            final DialogVerlusteDetail dialog =
                DialogVerlusteDetail.fabricCreateNew(_detailedLosses, parentWindow);
            dialog.setVisible(true);
            checkFileLink();
          }
        });
  ...
  private void checkFileLink() {
    final boolean isFileLinkOK =
        _detailedLosses
            .pruefeLinkAufHalbleiterDatei(); // ist die Datei mit der Verlustbeschreibung ueberhaupt
    // vorhanden?
    //
    if (isFileLinkOK) { // Datei vorhanden und OK
      ...
      } else { // when using file from windows on linux computers, the upper approach generates a
        // problem.
    ...
    if (Fenster.IS_APPLET) {
      fS2 =
          Color.decode(
              "0x006400"); // weil im Applet-Modus keine Datei verwendet wird, ist die Farbe hier
      // nie 'rot'
    }
    int cls = 20;
