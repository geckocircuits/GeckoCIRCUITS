...
public final class SchematischeEingabe2 implements MouseListener, MouseMotionListener {
  ...
  public static boolean zustandGeaendert =
      false; // zeigt an, ob 'QuitWithoutSaving' aktiviert werden muss
  //
  ...
  public static int DY_ZEILENABSTAND_TXT =
      circuitFont.getSize()
          + 3; // Parameter, Namen und andere Beschriftungen --> vertikaler Zeilenabstand
  ...
  public void updateAllComponentReferences() {
    for (ComponentCoupable elem :
        _circuitSheet.getAllElements().getClassFromContainer(ComponentCoupable.class)) {
  ...
  public void updateSingleReference(final AbstractBlockInterface elementToUpdate) {
    if (elementToUpdate instanceof ComponentCoupable) {
      ((ComponentCoupable) elementToUpdate)
          .getComponentCoupling()
          .refreshCoupledReferences(getBlockInterfaceComponents());
    }
  }

  private boolean testRenameLabelWhereOldStillExists(
      final String originalLabelBeforeRename,
    ...
    for (AbstractBlockInterface elem : elements) {
      for (TerminalInterface term : elem.getAllTerminals()) {
        ...
        if ((!originalLabelBeforeRename.equals(""))
            && (existingCompareLabel.equals(originalLabelBeforeRename))) {
          /**
           * special case: control input terminal. This terminal does not generate "output" values,
           * therefore don't consider this for the link detection!
           */
  ...
  public void setNewVisibleCircuitSheet(final CircuitSheet newCircuitSheet) {
    ...
    for (AbstractCircuitSheetComponent searchTerminal :
        _selectedComponents.toArray(
            new AbstractCircuitSheetComponent[_selectedComponents.size()])) {
  ...
  public List<AbstractBlockInterface> getBlockInterfaceComponents() {
    return Collections.unmodifiableList(
        _circuitSheet.getAllElements().getClassFromContainer(AbstractBlockInterface.class));
  }

  void setNewScaling(final int dpix) {
    _circuitSheet.setNewScaling(dpix);
    for (AbstractBlockInterface block :
        _circuitSheet.getAllElements().getClassFromContainer(AbstractSpecialBlock.class)) {
  ...
  public void insertNewElement(AbstractCircuitSheetComponent newElement) {
    if (newElement != null) {
      ...
      if (_elementsJustInitialized
          && !(newElement instanceof RegelBlock)
          && newElement instanceof AbstractCircuitBlockInterface) {
  ...
  private boolean checkSheetComponentsValidForImport(CircuitSheet newCircuitSheet) {
    List<AbstractBlockInterface> allNamableComponents =
        newCircuitSheet.allElements.getClassFromContainer(AbstractBlockInterface.class);
    ...
    if (!problematicComponentCoupling.isEmpty()) {
      ...
      for (ComponentCoupling tmp : problematicComponentCoupling) {
        problematicLabelString +=
            "\n"
                + "Component: "
                + tmp.getParent().getStringID()
                + " reference to component: "
                + tmp._coupledElements[0].getStringID();
      }
      Object[] options = {"Delete component references", "Cancel"};
      int n =
    ...
    if (!problematicLabelReferenceComponent.isEmpty()) {
      ...
      Object[] options = {"Delete label references", "Cancel"};
      int n =
          JOptionPane.showOptionDialog(
              GeckoSim._win,
    ...
    if (!foundComponentsForRename.isEmpty()) {
      ...
      Object[] options = {"Rename components", "Cancel"};
      int n =
          JOptionPane.showOptionDialog(
              GeckoSim._win,
    ...
    if (!labelsToRename.isEmpty()) {
      ...
      Object[] options = {"Rename labels", "Merge without rename", "Cancel"};
      int n =
          JOptionPane.showOptionDialog(
              GeckoSim._win,
  ...
  /**
   * when moving components from one sheet to another, labels could "disapear" within a sheet, when
   * all components with a specific labels are shifted to another sheet. Therefore, check if this is
   * the case and give a warning that missing references could occur!
   *
   * @param newCircuitSheet
   */
  private Set<String> checkForMissingLabelReferencesAfterSheetMove() {
    Set<AbstractCircuitSheetComponent> remainingComponents =
        new HashSet<AbstractCircuitSheetComponent>();
  ...
  private Set<AbstractCircuitSheetComponent> _selectedComponents =
      new LinkedHashSet<AbstractCircuitSheetComponent>() {
        @Override
  ...
  private boolean
      _elementsJustInitialized; // damit man eine laufende Initialisierung abbrechen kann, zB. mit
  // der ESCAPE-Taste
  private Point _moveStartPoint =
      new Point(
          0, 0); // Ausgangspunkt (ScematicEntry-Koord.) fuer Verschieben/Kopieren von Elementen
  boolean connectorTestMode = false;
  public boolean wirePenVisible = false;
  public int wireModeVersteckt = WIRE_MODE_LK;
  public static final int WIRE_MODE_OFF = 0,
      WIRE_MODE_LK = 1,
      WIRE_MODE_CONTROL = 2,
      WIRE_MODE_THERM = 3,
      WIRE_MODE_RELUCTANCE = 4;
  //
  int[] xStift, yStift; // Symbol Zeichenstift
  // if connectorTestMode is set, the Potentailgebiebt can be drawn: ovals at Element-nodes and fat
  // (colored) Verbindungen
  ...
  public void resetCircuitSheetsForNewFile() {
    for (AbstractCircuitSheetComponent comp :
        _circuitSheet.getAllElements().toArray(new AbstractCircuitSheetComponent[0])) {
  ...
  public List<AbstractCircuitBlockInterface> getElementLK() {
    LinkedList<AbstractCircuitBlockInterface> returnValue =
        new LinkedList<AbstractCircuitBlockInterface>();
    List<AbstractCircuitBlockInterface> allBlocks =
        _circuitSheet.getAllElements().getClassFromContainer(AbstractCircuitBlockInterface.class);
  ...
  public List<RegelBlock> getElementCONTROL() {
    return Collections.unmodifiableList(
        _circuitSheet.getAllElements().getClassFromContainer(RegelBlock.class));
  }

  public List<AbstractCircuitBlockInterface> getElementTHERM() {
    LinkedList<AbstractCircuitBlockInterface> toFilter =
        new LinkedList<AbstractCircuitBlockInterface>();
    toFilter.addAll(
        _circuitSheet.getAllElements().getClassFromContainer(AbstractCircuitBlockInterface.class));
    for (AbstractCircuitBlockInterface remove :
        toFilter.toArray(new AbstractCircuitBlockInterface[0])) {
  ...
  public Collection<Verbindung> getConnection(final ConnectorType connectorType) {
    ...
    Set<Verbindung> returnValue = new LinkedHashSet<Verbindung>();
    for (AbstractCircuitSheetComponent verbCand :
        _circuitSheet.getAllElements().getClassFromContainer(Verbindung.class)) {
  ...
  public void ladeGespeicherteNetzlisteVonDatenSpeicher(
      final DatenSpeicher daten, final String insertIntoSubCircuitName) {
  ...
  public void toggleEnable(final boolean shortDisableCommand) {
    ...
    // Element-Gruppe wurden gewaehlt ('innerhalb des Markierungsrechtecks'), aber noch nicht
    // manipuliert (zB. verschoben) -->
    // System.out.println("deselect() --> markierungsRechteckAktiviert");
    // ======================
    _mouseMoveMode = MouseMoveMode.NONE;

    DoActionOnSelectedComponents action =
        new DoActionOnSelectedComponents() {
          @Override
  ...
  Graphics2D findGraphicsForComponent(
      final AbstractCircuitSheetComponent comp,
      final Graphics2D normal,
      final Graphics2D selected,
      Graphics2D disabled) {
  ...
  public void mousePressed(final MouseEvent me) {
    ...
    if (me.getClickCount() < 2
        && System.currentTimeMillis() - _lastMouseClickTime < 350
    ...
    if (!wirePenVisible
        && (_selectedComponents.size() > 0 && !isShiftClick)
        && (me.getClickCount() <= 1)) {

      if (_mouseMoveMode == MouseMoveMode.MOVE_COMPONENTS) {
        for (SubcircuitBlock testClickSub :
            _circuitSheet.getAllElements().getClassFromContainer(SubcircuitBlock.class)) {
    ...
    if (me.getClickCount() > 1) {
      ...
      for (AbstractCircuitSheetComponent elem : _visibleCircuitSheet.getLocalSheetComponents()) {
        if (elem.testDoDoubleClickAction(clickPoint)) {
          if (elem
              instanceof
              TextFieldBlock) { // other components should have click-priority in comparison to
            // textfield-block!
      ...
      if (clickedBlock != null) {
        ...
        if (clickedBlock instanceof AbstractBlockInterface) {
          ((AbstractBlockInterface) clickedBlock)
              .setParameterWithoutUnDo(((AbstractBlockInterface) clickedBlock).getParameter());
        }
    ...
    if (me.getClickCount() <= 1) {
      // Element zur Bearbeitung markieren: es kann hierbei nur genau ein Element zur Bearbeitung
      // markiert werden -->
  ...
  public void deselect() {
    switch (_mouseMoveMode) {
      ...
      case MOVE_COMPONENTS:
        if (_elementsJustInitialized) {
          // Gerade geschaffenes Element, aber noch nicht abgesetzt: Loeschen und
          // Verschwinden-lassen -->
          ...
          // Selektierte Element-Gruppe wurde bereits VERSCHOBEN, aber noch nicht abgesetzt -->
          // de-markieren und an der alten Position absetzen -->
  ...
  private void mausAbsetzenMarkierteGruppe() {
    final DoActionOnSelectedComponents action =
        new DoActionOnSelectedComponents() {
          @Override
  ...
  private abstract class DoActionOnSelectedComponents {
    ...
    public final void execute() {
      ...
      if (groupStart != null) {
        AbstractUndoGenericModel.undoManager.addEdit(
            new GroupableUndoManager.GroupUndoStop(groupStart));
      }
    }
    ...
    for (AbstractCircuitSheetComponent elem : _visibleCircuitSheet.getLocalSheetComponents()) {
      if (elem instanceof AbstractBlockInterface) {
        ...
        if (block.elementTEXTAngeklickt(x, y)) {
          ...
          _selectedTextFieldToMove._textInfo.setPositionTextClickPointInitial(
              x,
              y); // Verschiebung des Textfeldes relativ zum Element bezieht sich auf diesen initial
          // angeklicken Punkt
          _visibleCircuitSheet.repaint();
          return true;
        }
  ...
  private void maus_VerbindungSetzeEndknoten(final Point clickPoint) {
    ...
    NetListLK.fabricExcludingSubcircuits(
        getConnection(ConnectorType.LK_AND_RELUCTANCE), getElementLK());
  ...
  public void mouseMoved(MouseEvent me) {
    ...
    // Verbindung (LK bzw. CONTROL bzw. THERM) oder Element (LK bzw. CONTROL bzw. THERM) wird
    // herumbewegt  -->
    //
    switch (_mouseMoveMode) {
      case DRAW_CONNECTION:
      ...
      case MOVE_COMPONENTS:
        if (!_selectedComponents.isEmpty()) {
          Point movePosition =
              new Point(movePoint.x - _moveStartPoint.x, movePoint.y - _moveStartPoint.y);
  ...
  public Verbindung externalCreateAndPlaceNewConnector(
      String elementName, int xStart, int yStart, int xEnd, int yEnd, boolean startHorizontal) {
  ...
  public AbstractBlockInterface externalCreateAndPlaceNewElement(
      final String elementName,
      final AbstractTypeInfo elementCategory,
      final int posX,
      final int posY)
  ...
  /**
   * @param movePoint the position where to move the selection. Be careful: the start position of
   *     the movement is involved!
   */
  ...
  public void mouseDragged(final MouseEvent mouseEvent) {
    ...
    if (_mouseMoveMode == MouseMoveMode.MOVE_TEXT) {
      _selectedTextFieldToMove._textInfo.setNewRelativePosition(
  ...
  public void mouseReleased(final MouseEvent me) {
    ...
    // alle im Markierungsrechteck enthaltenen Elemente und Verbindungen muessen in den
    // BEARBEITUNGS-Modus gesetzt werden,
    // alles ausserhalb wird 'FERTIG' gesetzt (deaktiviert fuer Verarbeitung) -->
    //
    if (_mouseMoveMode == MouseMoveMode.SELECT_WINDOW
        && (me.getModifiers()
            != me.BUTTON3_MASK)) { // --> keine Reaktion beim Druecken der rechten Maus-Taste
      _selectedComponents.clear();
      _selectedComponents.addAll(
          _visibleCircuitSheet.getElementsInRectangle(x1markRe, y1markRe, x2markRe, y2markRe));
      _mouseMoveMode = MouseMoveMode.NONE;
      _singleComponentMouseDrag = false;
    }
    _visibleCircuitSheet.repaint();
  ...
  public void mouseEntered(final MouseEvent me) {
    _visibleCircuitSheet
        .requestFocusInWindow(); // damit koennen KeyEvents in Fenster() abgearbeitet werden
    try {
      Thread.sleep(10);
  ...
  public void deleteSelectedComponentsWithUndo() {
    ...
    GroupableUndoManager.GroupUndoStop groupStop =
        new GroupableUndoManager.GroupUndoStop(groupStart);
    if (_selectedComponents.size() == 1) {
  ...
  private void updateNewNetlists() {
    final NetListLK netzliste =
        NetListLK.fabricExcludingSubcircuits(
            getConnection(ConnectorType.LK_AND_RELUCTANCE), getElementLK());
    final NetListLK netzlisteTherm =
        NetListLK.fabricExcludingSubcircuits(
            getConnection(ConnectorType.THERMAL), getElementTHERM());
    final NetzlisteAllg nlCONTROL =
        NetzlisteAllg.fabricNetzlistComplete(
            getConnection(ConnectorType.CONTROL), getElementCONTROL());
  ...
  public void updateRenamedLabel(
      final String originalLabel, String neuerLabel, final ConnectorType renamedLabelType) {

    if (testRenameLabelWhereOldStillExists(
        originalLabel, _visibleCircuitSheet.getLocalComponents(renamedLabelType))) {
    ...
    for (AbstractCircuitSheetComponent search : _visibleCircuitSheet.allElements) {
      if (search instanceof PotentialCoupable) {
        ...
        if (potCoup.getLinkType() == renamedLabelType
            || renamedLabelType == ConnectorType.LK
                && potCoup.getLinkType() == ConnectorType.LK_AND_RELUCTANCE
            || renamedLabelType == ConnectorType.LK_AND_RELUCTANCE
                && potCoup.getLinkType() == ConnectorType.LK) {
  ...
  // (1) Ein LK-Schalter wird umbenannt --> der entsprechend verknuepfte GATE-Ansteuerblock muss
  // aktualisiert werden,
  // also mit dem neuen LK-Schalter-Namen korrekt verknuepft -->
  // (2) Ein LK-Schalter wird umbenannt --> der entsprechend verknuepfte PVCHIP-THERM-Block zur
  // Verlustmessung
  // muss aktualisiert werden, also mit dem neuen LK-Schalter-Namen korrekt verknuepft -->
  // (3) Der Name von LK_KOP2 wurde umbenannt --> eventuelle Verknuepfungen mit Kopplungen LK_M
  // aktualisieren -->
  // (4) Ein LK-Namen wurde umbenannt --> CURRENT/VIEWMOT - Messungen muessen angepasst werden -->
  // (5) Ein LK-Name wird umbenannt --> Voltage-Direct Control der LK-Quellen U und I muss angepasst
  // werden
  //
  ...
  public void kopiereAllesImBearbeitungsModus() {
    ...
    // -->
    final List<AbstractCircuitSheetComponent> exchangeNew =
        new ArrayList<AbstractCircuitSheetComponent>();
    final List<TerminalInterface> allTerminals = new ArrayList<TerminalInterface>();

    final Random shiftRandom = new Random(System.currentTimeMillis());
    final long shiftValue = shiftRandom.nextLong();

    for (AbstractCircuitSheetComponent elem :
  ...
  private static Point findMoveAnchorPoint(final List<TerminalInterface> allInvolvedTerminals) {
    ...
    for (TerminalInterface term : allInvolvedTerminals) {
      ...
      if (point.x < 0
          || point.y < 0) { // don't remove. Thermal ambient temperature has a node at negative
        // coordinates!
        continue;
      }
  ...
  private void renameConflictingLabels(
      final Collection<? extends AbstractCircuitSheetComponent> exchangeNew,
      final List<String> allNetlistLabels,
      final List<String> copiedLabels) {
    ...
    for (AbstractCircuitSheetComponent search : exchangeNew) {
      if (search instanceof PotentialCoupable) {
        ((PotentialCoupable) search)
            .getPotentialCoupling()
            .tryFindChangedLabels(exchangeNew, endIndex);
      }
    }
  }
  ...
  public void export_allesImBearbeitungsModus() {
    try {
      ...
      final StringSelection tr1 =
          new StringSelection(
              transferString); // die gewaehlten Elemente als String ins Clipboard schreiben
  ...
  private String writeSelectedElementsToASCIIString() {
    ...
    final Set<AbstractCircuitSheetComponent> allComponentsIncludingSubs =
        new LinkedHashSet<AbstractCircuitSheetComponent>();
    ...
    for (AbstractCircuitSheetComponent searchSub : _selectedComponents) {
      if (searchSub instanceof SubcircuitBlock) {
        ...
        allComponentsIncludingSubs.addAll(
            subBlock._myCircuitSheet.getLocalRecursiveSubComponents());
      }
    }
  ...
  public void readSelectedElementsFromASCIIString(final String[] ascii) {
    ...
    final List<AbstractCircuitSheetComponent> visibleComponents =
        new ArrayList<AbstractCircuitSheetComponent>();
  ...
  private void processElements(
      final Collection<? extends AbstractCircuitSheetComponent> dsc,
      List<String> copiedLabelNames) {
  ...
  private int findPossibleEndIndex(
      final List<String> vecCopiedLabelsCONTROL, final List<String> allOrigLabelsCONTROL) {
  ...
  private void renameComponentLabels(
      final Collection<? extends AbstractCircuitSheetComponent> exchangeNew, final int endIndex) {
  ...
  private final class CreateVerbindungUndoAction implements UndoableEdit {
    ...
    public void undo() {
      ...
      final List<AbstractCircuitSheetComponent> toDeleteList =
          new ArrayList<AbstractCircuitSheetComponent>();
  ...
  public final class DeleteComponentUndoAction implements UndoableEdit {
    ...
    private final List<AbstractCircuitSheetComponent> _toDeleteElements =
        new ArrayList<AbstractCircuitSheetComponent>();

    private DeleteComponentUndoAction(
        final Collection<? extends AbstractCircuitSheetComponent> toDeleteElements) {
  ...
  private final class CreateComponentUndoAction implements UndoableEdit {
    ...
    private CreateComponentUndoAction(
        final List<? extends AbstractCircuitSheetComponent> newElements) {
    ...
    for (AbstractBlockInterface block : getBlockInterfaceComponents()) {
      try {
        ...
        throw new RuntimeException(
            "Error in component " + block.getStringID() + ":\n" + ex.getMessage());
      }
    }
  }
}
