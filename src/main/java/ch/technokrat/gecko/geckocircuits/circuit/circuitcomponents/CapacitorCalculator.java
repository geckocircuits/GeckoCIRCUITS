...
public final class CapacitorCalculator extends CircuitComponent
    implements AStampable,
        BStampable,
        DirectCurrentCalculatable,
        CurrentCalculatable,
        HistoryUpdatable {
  ...
  public void stampMatrixA(double[][] matrix, double dt) {
    ...
    if (initCapacitor && _initialValue != 0) {
      initVoltageSource =
          new VoltageSourceCalculator(
              -_initialValue, matrixIndices[0], matrixIndices[1], _z, _componentNumber, _parent);
  ...
  public final void stampVectorB(double[] b, double t, double dt) {
    ...
    // double bW = (_lkCap.getParameter()[6] / dt) * (pALT[matrixIndices[0]] -
    // pALT[matrixIndices[1]])
    ...
    if (initCapacitor && _initialValue != 0) {
      ...
      if (_isNonLinear) {
        ...
        if (_solverType == SolverType.SOLVER_BE) {
          ...
          bW =
              2 * (_capacitance / dt) * (_potential1 - _potential2)
                  + _oldCurrent
                  + _newOldCapRatio * _oldCurrent;
        } else if (_solverType == SolverType.SOLVER_GS) {
          bW =
              (_capacitance / dt)
                      * (2 * (_potential1 - _potential2) - 0.5 * (_potential1 - _potential2))
                  + _newOldCapRatio * _oldCurrent;
        }
      } else {
  ...
  public final void calculateCurrent(final double[] p, final double dt, final double t) {
    ...
    //        if (capError) {
    //            i[_componentNumber] = _lkCap.getParameter()[6] / dt * ((p[matrixIndices[0]] -
    // p[matrixIndices[1]]) - (pALT[matrixIndices[0]] - pALT[matrixIndices[1]]));
    ...
    //            if (_lkCap.getParameter()[2] * (i[_componentNumber] + nonLinearCorrectionCurrent)
    // < 0) {
    //                capCorrection = true;
    //            }
    //            if (Math.abs((_lkCap.getParameter()[6] - _lkCap.getParameter()[7]) /
    // (_lkCap.getParameter()[6] + _lkCap.getParameter()[7])) > 0.1) {
    //                capCorrection = true;
    //            }
    //            //---------
    //            if (capCorrection) {
    ...
    //                bWOld = (_lkCap.getParameter()[6] / dt) * (pALT[matrixIndices[0]] -
    // pALT[matrixIndices[1]]) + facOld * _lkCap.getParameter()[10];
    ...
    //                bWNew = (_lkCap.getParameter()[6] / dt) * (pALT[matrixIndices[0]] -
    // pALT[matrixIndices[1]]) + facNew * _lkCap.getParameter()[10];
    //
    ...
    if (initCapacitor && _initialValue != 0) {
      ...
      if (_solverType == SolverType.SOLVER_BE) {
        _current =
            _capacitance
                / dt
                * ((p[matrixIndices[0]] - p[matrixIndices[1]]) - (_potential1 - _potential2));
      } else if (_solverType == SolverType.SOLVER_TRZ) {
        _current =
            2
                    * (_capacitance / dt)
        ...
        _current =
            _capacitance
                / dt
                * (1.5 * (p[matrixIndices[0]] - p[matrixIndices[1]])
                    - 2 * (_potential1 - _potential2)
                    + 0.5 * (_potOld1 - _potOld2));
      }

      if (_isNonLinear) {
        if (!DiodeCalculator.inSwitchErrorMode) {
          if (((_current + _nonLinearCorrectionCurrent) * (_oldCurrent))
              < 0) // wrong value of current calculated, capacitance must be changed!
          {
            capError = true;
            _capacitance =
                (1 - _newOldCapRatio)
                    * _capacitance; // set capacitance to new value (reconstructed from the ratio,
            // see updateNonLinearCapacitance method
        ...
        _capCorrected = false;
        // System.out.println("" + t + " " + _capacitance + " " + ((1 -
        // _newOldCapRatio)*_capacitance));
      }
    }
  }
  ...
  public void updateNonLinearCapacitance() {
    ...
    if (initCapacitor) {
      ...
      new_capacitance =
          _NonLinearCapacitance.getCapacitanceAtV(Math.abs(_potential1 - _potential2));
    }
    ...
    else if (Math.abs((_capacitance - new_capacitance) / (_capacitance + new_capacitance)) > 0.1
        && !DiodeCalculator
            .inSwitchErrorMode) // do not change capacitance unless this deviation is more than 10%,
    // to save effort in recalculating A matrix
    {
      _capacitance = new_capacitance;
      capError = true;
      _newOldCapRatio = 0;
