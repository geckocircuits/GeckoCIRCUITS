...
public class TerminalVerbindung implements TerminalInterface {
  ...
  public TerminalVerbindung(
      final Verbindung verb, final List<Point> connectorPoints, final Location loc) {
    _loc = loc;
    _verb = verb;
  ...
  public void paintComponent(final Graphics graphics) {
    ...
    graphics.fillOval(
        (int) (dpix * getPosition().x) - AbstractTerminal.POINT_DIAMETER / 2,
        (int) (dpix * getPosition().y) - AbstractTerminal.POINT_DIAMETER / 2,
        AbstractTerminal.POINT_DIAMETER,
        AbstractTerminal.POINT_DIAMETER);
  }
}
