...
public class DetailedLossLookupTable {
  ...
  static DetailedLossLookupTable fabric(
      final List<? extends LossCurve> messkurvePvSWITCH, final int dataIndex) {
  ...
  private DetailedLossLookupTable(
      final double[][] currentValues, final double[][] energyValues, final double[] temperatures) {
  ...
  public double getInterpolatedYValue(double temp, double current) {
    ...
    double upperTempEnergy =
        findLossValueOnCurve(signCorrectedCurrent, _currentValues[upperTempIndex], data);
    ...
    double lowerTempEnergy =
        findLossValueOnCurve(signCorrectedCurrent, _currentValues[lowerTempIndex], data);
  ...
  public double getInterpolatedXValue(double temp, double yValue) {
    ...
    double upperTempEnergy =
        findLossValueOnCurve(yValue, _energyValues[upperTempIndex], _currentValues[upperTempIndex]);
    if (upperTempIndex == lowerTempIndex) {
      return upperTempEnergy;
    }

    double lowerTempEnergy =
        findLossValueOnCurve(yValue, _energyValues[lowerTempIndex], _currentValues[lowerTempIndex]);
    ...
    assert wheigt1 + wheigt2 < 1.01 && wheigt1 + wheigt2 > 0.99;
    double returnValue = (lowerTempEnergy * wheigt1 + upperTempEnergy * wheigt2);
    // changed: negative values are possible here, since then both voltage and current could be
    // negative!
    return returnValue;
  }
