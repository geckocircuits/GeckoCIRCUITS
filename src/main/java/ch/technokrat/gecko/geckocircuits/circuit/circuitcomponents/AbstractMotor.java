...
public abstract class AbstractMotor extends AbstractCircuitBlockInterface
    implements PotentialCoupable, HiddenSubCircuitable, PostCalculatable {

  private static final double SIXTY_OVER_TWOPI = 60.0 / (2 * Math.PI);
  static final double RADIUS_MOTOR_SYMBOL = 1.0, BLOCK_SYMB_X = 1.3, BLOCK_SYMB_Y = 0.3;
  ...
  final UserParameter<Double> _polePairsParameter =
      UserParameter.Builder.<Double>start("polePairs", 1.0)
  ...
  final UserParameter<Double> _inertiaParameter =
      UserParameter.Builder.<Double>start("inertia", 1.0)
  ...
  final UserParameter<Double> _initialRotationalSpeed =
      UserParameter.Builder.<Double>start("initialRotationalSpeed", 0.0)
  ...
  final UserParameter<Double> _frictionParameter =
      UserParameter.Builder.<Double>start("frictionCoefficient", 0.1)
  ...
  final UserParameter<Double> _initialRotorPosition =
      UserParameter.Builder.<Double>start("initialRotorPosition", 0.0)
  ...
  protected final void drawBackground(final Graphics2D graphics) {
    graphics.fillOval(
        (int) (-dpix * RADIUS_MOTOR_SYMBOL),
        (int) (-dpix * RADIUS_MOTOR_SYMBOL),
        (int) (dpix * 2 * RADIUS_MOTOR_SYMBOL),
        (int) (dpix * 2 * RADIUS_MOTOR_SYMBOL));
  }
  ...
  public static AbstractCircuitBlockInterface fabricHiddenSub(
      final CircuitTyp typ, final AbstractCircuitSheetComponent parent) {
  ...
  final void calculateMechanicalParameters(final double deltaT) {
    ...
    _omegaMechanic =
        (_torqueEl - _torqueMech + _inertia * _omegaMechanicOld / deltaT)
            / (_inertia / deltaT + _frictionMech);
    _omegaElectric = _omegaMechanic * _polePairs;
