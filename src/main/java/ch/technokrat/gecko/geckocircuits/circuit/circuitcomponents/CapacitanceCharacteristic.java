...
public class CapacitanceCharacteristic {
  ...
  private int characteristicContainsVpoint(double V) {
    // checks if there is already a capacitance defined for a particular V point, returns index if
    // it does, otherwise returns negative value
  ...
  public double getCapacitanceAtV(double V) {
    // function that evaluates the non-linear characteristic to return a capacitance at a specific
    // voltage
    int i = 0;
