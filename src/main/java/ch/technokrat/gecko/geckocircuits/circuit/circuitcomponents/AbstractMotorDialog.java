...
abstract class AbstractMotorDialog<T extends AbstractMotor> extends DialogElementLK<T> {
  ...
  void createOrderedComponents() {
    ...
    for (int i = 0; i < orderedUserPars.size(); i++) {
      ...
      variableNames[i] =
          labelFabric(orderedUserPars.get(i).getShortName() + " [" + unitString + "] ");
  ...
  JPanel buildPanelParameters(
      int startIndex, int endIndex, int[] separatorIndices, boolean addTorqueCombo) {
