...
public final class PotentialArea {
  ...
  public final Set<SubCircuitTerminable> _potentialTerminals =
      new LinkedHashSet<SubCircuitTerminable>();
  private final Set<AbstractBlockInterface> _globalTerminals =
      new LinkedHashSet<AbstractBlockInterface>();
  ...
  public static PotentialArea fabricFromConnector(final Verbindung connector) {
    ...
    assert returnValue._potentialTyp != ConnectorType.LK_AND_RELUCTANCE
        : "mixed type not possible, here!";
    return returnValue;
  }

  public static PotentialArea fabricElementTerminal(
      final AbstractBlockInterface element, final TerminalInterface terminal) {
  ...
  public List<Point> getAllElementKnotenXY(
      final List<? extends AbstractBlockInterface> elements, final CircuitSheet circuitSheet) {
  ...
  public PotentialArea mergePotential(
      final PotentialArea pot2, final boolean doLabelConnectRename) {
    ...
    this._upperLeftCorner =
        new Point(
            Math.min(pot1._upperLeftCorner.x, pot2._upperLeftCorner.x),
            Math.min(pot1._upperLeftCorner.y, pot2._upperLeftCorner.y));

    this._lowerRightCorner =
        new Point(
            Math.max(pot1._lowerRightCorner.x, pot2._lowerRightCorner.x),
    ...
    // assert this._potentialTyp != ConnectorType.NONE && this._potentialTyp !=
    // ConnectorType.LK_AND_RELUCTANCE;
    return this;
  }
   ...
   */
  public static boolean hasComponentConnection(
      final PotentialArea potArea1, final PotentialArea potArea2) {
    for (ElementNodes elNodes : potArea1._elementNodeTerminals) {
      for (ElementNodes elNodes2 : potArea2._elementNodeTerminals) {
        if (elNodes._element == elNodes2._element) {
          ...
          if (element instanceof ReluctanceInductor || element instanceof IdealTransformer) {
            ...
            if (terminals1.contains(element.XIN.get(0))
                && terminals2.contains(element.XIN.get(1))) {
              return true;
            }
            if (terminals1.contains(element.YOUT.get(0))
                && terminals2.contains(element.YOUT.get(1))) {
              return true;
            }

            if (terminals1.contains(element.XIN.get(1))
                && terminals2.contains(element.XIN.get(0))) {
              return true;
            }
            if (terminals1.contains(element.YOUT.get(1))
                && terminals2.contains(element.YOUT.get(0))) {
              return true;
            }
  ...
  public void updateLabel(final String label, final Collection<AbstractBlockInterface> element) {
    ...
    // Labels auf Element-Knoten werden gesetzt (aber nur, wenn dort schon ein String eingetragen
    // wurde, dh. '... !="" ...')
    for (Verbindung verb : _potentialConnections) {
  ...
  public void aktualisiereLabel(
      final List<Verbindung> connector, final Collection<AbstractBlockInterface> element) {
  ...
  public boolean isEmptyPotential() {
    ...
    if (_elementNodeTerminals.size()
        == 2) { // special case: when a terminal is defined, but nothing else connected
      ...
      //                    System.out.println("removing empty potential! " + ((TerminalCircuit)
      // nodes._element).getStringID() + " " + ((TerminalCircuit)
      // nodes._element).getParentSheetIdentifier());
      //                    return true;
      //                }
      //            }
    }
  ...
  SubCircuitTerminable[] getPotentialTerminals() {
    if (_memoryOptimization == null || _memoryOptimization.length != _potentialTerminals.size()) {
      _memoryOptimization =
          _potentialTerminals.toArray(new SubCircuitTerminable[_potentialTerminals.size()]);
    }
  ...
  private static class ElementNodes {
    ...
    /**
     * here, all the nodes are listed, which have a connection to this potential area. Since a
     * single component can have several terminals, the following list only contains the node
     * indices, which are really connected to this potential area.
     */
    ...
    public ElementNodes(
        final AbstractBlockInterface element, final TerminalInterface connectedTerminal) {
      _element = element;
