...
public final class WorksheetSize {
  ...
  public ModelMVC<Point> _worksheetDimension =
      new ModelMVC<Point>(new Point(900, 900), "worksheet size");
  private static final int MIN_SIZE = 10;
  private final CircuitSheet _parent;
  private static final int BORDER_OFFSET = 1;
  /**
   * don't use this stuff in new code. This is just for backwards compatibility with the crappy old
   * .ipes file format.
   */
  ...
  WorksheetSize(final CircuitSheet parent) {
    _parent = parent;
    _worksheetDimension.addModelListener(
        new ActionListener() {
          @Override
    ...
    try {
      if (isComponentOutsideDrawingArea(sizeX, sizeY)) {
        ...
        if (_parent instanceof SubCircuitSheet) {
          ...
          final Collection<SubCircuitTerminable> allSheetTerminals =
              subSheet._subBlock._myTerminals;
          if (allSheetTerminals != null)
            for (SubCircuitTerminable term : allSheetTerminals) {
              if (term.getTerminalLocation() == EnumTerminalLocation.RIGHT) {
                setCheckedTerminalSheetPosition(
                    term, sizeX - 1, term.getSheetPosition().y, allSheetTerminals);
              }

              if (term.getTerminalLocation() == EnumTerminalLocation.BOTTOM) {
                setCheckedTerminalSheetPosition(
                    term, term.getSheetPosition().x, sizeY - 1, allSheetTerminals);
              }
  ...
  private void setCheckedTerminalSheetPosition(
      SubCircuitTerminable terminable,
      int posX,
      int posY,
      ...
      if (terminable.getSheetPosition().x != posX) {
        if (term.getSheetPosition().x >= posX) {
          throw new RuntimeException(
              "Error: cannot move terminal " + terminable.getStringID() + " to new position!");
        }
      }
  ...
  /**
   * Wenn Elemente bzw. Verbindungen ausserhalb des Worksheet sind, also unsichtbar, dann ist das
   * eine potentialle Fehlerquelle --> sollte also dem User nicht ermoeglicht werden --> wird hier
   * grundsaetzlich geprueft
   */
  private boolean isComponentOutsideDrawingArea(
      final int worksheetSizeX, final int worksheetSizeY) {
  ...
  public String toString() {
    return "Subcircuit "
        + hashCode()
        + " "
        + _worksheetDimension.getValue().x
        + " "
        + _worksheetDimension.getValue().y;
  }
}
