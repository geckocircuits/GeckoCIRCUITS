...
public class VoltageSourceDCMachineCalculator extends VoltageSourceCalculator
    implements BStampable, PostProcessable {
  ...
  public VoltageSourceDCMachineCalculator(
      TimeFunctionConstant timeFunction,
      InductorCouplingCalculator le,
      InductorCouplingCalculator la,
      final AbstractVoltageSource parent) {
  ...
  public void doPostProcess(double dt, double time) {
    ...
    // if (t==0) System.out.println(t+"   "+dt+"   "+ia+"   "+ie+"   "+momentLast+"   "+phi+"
    // "+momentElektr+"   "+omega+"   "+drehzahl+"   "+emk+"   omegaALT="+omegaALT+"   J="+J+"
    // Fr="+Fr);
    omegaALT = omega;
  }
}
