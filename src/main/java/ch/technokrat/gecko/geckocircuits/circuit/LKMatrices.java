...
public class LKMatrices {
  ...
  public double[] bVector,
      ...
      pALT,
      pALTALT,
      pALTALTALT; // b ... Stoervektor, p und pALT ... Knotenpotentiale PLUS SpgQuellen-Stroeme
  public double[] iALT,
      iALTALT,
      iALTALTALT; // Stroeme in allen Bauteilen (inklusive SpgQuellen-Stroeme)
  ...
  public void initMatrizen(
    ...
    matrixSize =
        netzliste.knotenMAX
            + netzliste.spgQuelleMAX
            + 1; // 'plus Eins' weil Null-Potential (Bezug fuer alle Knoten) vorhanden
    // System.out.println("netzliste.knotenMAX= "+netzliste.knotenMAX+"\tnetzliste.spgQuelleMAX=
    // "+netzliste.spgQuelleMAX);
    ...
    // ------------------------------------
    try {
      this.setzeAnfangsbedingungen(
          getAnfangsbedVomDialogfenster,
          solverType); // pALT und iALT werden gesetzt, zB. u(0) bei C oder i(0) bei L
    } catch (java.lang.RuntimeException ex) {
      // Calculate difference in days
      ex.printStackTrace();
      throw new RuntimeException(
          "The following error occured at the circuit initialization:\n"
              + ex.getMessage()
  ...
  private void initMatrizen(NetListLK netzliste) {
    ...
    this.elementANZAHL = netzliste.getElementANZAHLinklusiveSubcircuit();
    // ------------------------------------
    // netzliste.knotenMAX ... Zahl der Knoten minus Einer (eben der 'Ground'-Knoten) --> daher '+1'
    // in der Bestimmungsgleichung fuer k -->
    // Ordnung der Matrix a[][] ist  gesamtknotenzahl minus ground plus alleSpgQuellen plus
    // alleLKOP2elemente -->
    matrixSize =
        netzliste.knotenMAX
            + netzliste.spgQuelleMAX
  ...
  public void schreibeMatrix_A(double dt, double time, boolean capError) {
    ...
    for (int i1 = 0; i1 < elementANZAHL; i1++) {
      ...
      switch (netzliste.typ[i1]) {
        ...
        case LK_LKOP2:
          ...
          if ((zuLKOP2gehoerigeM_spgQnr != null) && (zuLKOP2gehoerigeM_spgQnr[i1] != null)) {
            ...
            for (int i7 = 0; i7 < anzahlKoppelPartner; i7++) {
              if (_solverType == SolverType.SOLVER_BE) {
                a[z][netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]] +=
                    (-zuLKOP2gehoerigeM_kWerte[i1][i7] / dt); //  +M/dt
              } else if (_solverType == SolverType.SOLVER_TRZ) {
                a[z][netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]] +=
                    (-2 * zuLKOP2gehoerigeM_kWerte[i1][i7] / dt); //  +2M/dt
              } else if (_solverType == SolverType.SOLVER_GS) {
                a[z][netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]] +=
                    (-1.5 * zuLKOP2gehoerigeM_kWerte[i1][i7] / dt); // +1.5M/dt
              }
            }
          }
          break;
        ...
        case TH_TEMP: //  +1
          a[x][z] += (+1.0);
          a[y][z] += (-1.0);
          if (!((int) netzliste.parameter[i1][0]
                  == SourceType.QUELLE_VOLTAGECONTROLLED_TRANSFORMER_NEW)
              && !((int) netzliste.parameter[i1][0]
            ...
            switch ((int) netzliste.parameter[i1][0]) {
              ...
              case SourceType.QUELLE_VOLTAGECONTROLLED_DIRECTLY:
                if (netzliste.parameter[i1][14] == 0) {
                  // voltage is function of node-potentials and, therefore, calculated here in
                  // matrix A
                  double gain = netzliste.parameter[i1][11];
                  int[][] nodePairDVC = netzliste.nodePairDirVoltContSrc;
                  int x1 = nodePairDVC[i1][0], y1 = nodePairDVC[i1][1];
          ...
          } // at initialization nodePairDirectVoltageControlledSource[][] is not defined --> all
          // according values set to zero
          break;
        case LK_M:
    ...
    ////                for (int i = 0; i < k; i++) {
    ////                    for (int j = 0; j < k; j++) {
    ////                        hash += Math.abs(1 + i + 3 * j +
    // Double.doubleToRawLongBits(a[i][j]));
    ////                    }
    ////                }
    ...
    for (int i1 = 0; i1 < elementANZAHL; i1++) {
      ...
      switch (netzliste.typ[i1]) {
        ...
        case LK_LKOP2:
          ...
          if ((zuLKOP2gehoerigeM_spgQnr != null) && (zuLKOP2gehoerigeM_spgQnr[i1] != null)) {
            ...
            if (_solverType == SolverType.SOLVER_BE) {
              for (int i7 = 0; i7 < anzahlKoppelPartner; i7++) {
                bVector[z] +=
                    (-p[netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]]
                        * (zuLKOP2gehoerigeM_kWerte[i1][i7] / dt)); //  +iALT*(M)/dt
                // weil bei Typ.LK_LKOP2 --> i[i1]= p[netzliste.knotenMAX
                // +netzliste.spgQuelleNr[i1]];  (siehe in berechneBauteilStroeme()!)
              }
            } else if (_solverType == SolverType.SOLVER_TRZ) {
              for (int i7 = 0; i7 < anzahlKoppelPartner; i7++) {
                bVector[z] +=
                    (-p[netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]]
                        * (2 * zuLKOP2gehoerigeM_kWerte[i1][i7] / dt)); //  +iALT*(2*M)/dt
              }
            } else if (_solverType == SolverType.SOLVER_GS) {
              for (int i7 = 0; i7 < anzahlKoppelPartner; i7++) {
                bVector[z] +=
                    ((-2.0 * p[netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]]
                            + 0.5
                                * pALTALT[
                                    netzliste.knotenMAX + (int) zuLKOP2gehoerigeM_spgQnr[i1][i7]])
                        * (zuLKOP2gehoerigeM_kWerte[i1][i7]
                            / dt)); //  (+iALT*2 - iALTALT*0.5)*(M/dt)
              }
            }
          }
          break;
        ...
        case LK_C:
          ...
          // bW = (2 * netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y]) + iALT[i1] + fac *
          // netzliste.parameter[i1][10];
          if (_solverType == SolverType.SOLVER_BE) {
            bW =
                (netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y])
                    + fac * netzliste.parameter[i1][10];
          } else if (_solverType == SolverType.SOLVER_TRZ) {
            bW =
                (2 * netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y])
                    + iALT[i1]
                    + fac * netzliste.parameter[i1][10];
          } else if (_solverType == SolverType.SOLVER_GS) {
            // bW = (1.5 * netzliste.parameter[i1][6] / dt) * ((4/3)*(pALT[x] - pALT[y]) -
            // (1/3)*(pALTALT[x] - pALTALT[y])) + fac * netzliste.parameter[i1][10]; simplifies to:
            bW =
                (netzliste.parameter[i1][6] / dt)
                        * (2 * (pALT[x] - pALT[y]) - 0.5 * (pALTALT[x] - pALTALT[y]))
                    + fac * netzliste.parameter[i1][10];
          }
        ...
        case TH_FLOW:
          switch ((int) netzliste.parameter[i1][0]) {
            ...
            case SourceType.QUELLE_SIN:
              bW =
                  -netzliste.parameter[i1][20]
                          * Math.sin(
                              2 * Math.PI * netzliste.parameter[i1][2] * t
                                  - Math.toRadians(netzliste.parameter[i1][4]))
                      + netzliste.parameter[i1][3];
              break;
            case SourceType.QUELLE_VOLTAGECONTROLLED_DIRECTLY_NEW:
            case SourceType.QUELLE_VOLTAGECONTROLLED_DIRECTLY:
              bW = 0;
            ...
            default:
              System.out.println(
                  "Fehler: Strom-Quelle nicht spezifiziert serzw45 w46 "
                      + (int) netzliste.parameter[i1][0]);
              break;
          }
        ...
        case TH_TEMP:
          switch ((int) netzliste.parameter[i1][0]) {
            ...
            case SourceType.QUELLE_CURRENTCONTROLLED_DIRECTLY_NEW:
            case SourceType.QUELLE_CURRENTCONTROLLED_DIRECTLY:
              // does not add a voltage --> voltage is function of node-potentials and, therefore,
              // calculated in matrix A
  ...
  public boolean berechneBauteilStroeme(
      double stoergroesse, double dt, double t, boolean isNewIteration, int errorCounter) {
    ...
    for (int i1 = 0; i1 < elementANZAHL; i1++) {
      ...
      switch (netzliste.typ[i1]) {
        ...
        case LK_L:
        case NONLIN_REL:
          AbstractNonLinearCircuitComponent element =
              (AbstractNonLinearCircuitComponent) netzliste.eLKneu[i1];
          if (netzliste.parameter[i1][0] < FAST_NULL_L) {
            if (_solverType == SolverType.SOLVER_BE) {
              netzliste.eLKneu[i1]._currentInAmps = +iALT[i1] + dt / FAST_NULL_L * (p[x] - p[y]);
            } else if (_solverType == SolverType.SOLVER_TRZ) {
              netzliste.eLKneu[i1]._currentInAmps =
                  +iALT[i1] + dt / (2 * FAST_NULL_L) * ((p[x] - p[y]) + (pALT[x] - pALT[y]));
            } else if (_solverType == SolverType.SOLVER_GS) {
              netzliste.eLKneu[i1]._currentInAmps =
                  (2.0 / 3.0) * dt / FAST_NULL_L * (p[x] - p[y])
                      + (4.0 / 3.0) * iALT[i1]
                      - (1.0 / 3.0) * iALTALT[i1];
            }
          } else if (_solverType == SolverType.SOLVER_BE) {
            netzliste.eLKneu[i1]._currentInAmps =
                +iALT[i1] + dt / netzliste.parameter[i1][0] * (p[x] - p[y]);
          } else if (_solverType == SolverType.SOLVER_TRZ) {
            netzliste.eLKneu[i1]._currentInAmps =
                +iALT[i1]
                    + dt / (2 * netzliste.parameter[i1][0]) * ((p[x] - p[y]) + (pALT[x] - pALT[y]));
          } else if (_solverType == SolverType.SOLVER_GS) {
            netzliste.eLKneu[i1]._currentInAmps =
                (2.0 / 3.0) * dt / netzliste.parameter[i1][0] * (p[x] - p[y])
                    + (4.0 / 3.0) * iALT[i1]
                    - (1.0 / 3.0) * iALTALT[i1];
          }
          ...
          if (!isNewIteration && element._isNonlinearForCalculationUsage) {
            ...
            double errorEstimator =
                Math.abs(
                    (correctionInductance - inductanceInAMatrix)
                        / (inductanceInAMatrix + correctionInductance));
            ...
            if (errorEstimator > 0.025) {
              ...
              a[y][x] += oldAW;
              double newAW =
                  getAWForInductance(netzliste.parameter[i1][0], netzliste.parameter[i1], dt);
              a[x][x] += newAW;
              a[y][y] += newAW;
              a[x][y] -= newAW;
              a[y][x] -= newAW;
              // AAAA System.out.println("new inductance value " + netzliste.parameter[i1][10] + " "
              // + dt / oldAW + " " + dt/newAW);
              netzliste.parameter[i1][12] = netzliste.parameter[i1][11];
              netzliste.parameter[i1][11] = newAW;

              //                            double correctionCurrent = (netzliste.parameter[i1][10]
              // - inductanceInAMatrix) / netzliste.parameter[i1][10] * (bVector[x] - bVector[y]) /
              // 2;
              //                            bVector[x] -= correctionCurrent;
              //                            bVector[y] += correctionCurrent;
              //
              //                            //netzliste.eLKneu[i1]._currentLosses *=
              // correctionFactor;
              //                            System.out.println("compare " + correctionCurrent + " "
              // + iALT[i1]);
              //                            netzliste.eLKneu[i1]._currentLosses +=
              // correctionCurrent;
        ...
        case LK_C:
          ...
          if (isNewIteration) {
            if (_solverType == SolverType.SOLVER_BE) {
              netzliste.eLKneu[i1]._currentInAmps =
                  netzliste.parameter[i1][6] / dt * ((p[x] - p[y]) - (pALT[x] - pALT[y]));
            } else if (_solverType == SolverType.SOLVER_TRZ) {
              netzliste.eLKneu[i1]._currentInAmps =
                  2 * netzliste.parameter[i1][6] / dt * ((p[x] - p[y]) - (pALT[x] - pALT[y]))
                      - iALT[i1];
            } else if (_solverType == SolverType.SOLVER_GS) {
              netzliste.eLKneu[i1]._currentInAmps =
                  netzliste.parameter[i1][6]
                      / dt
                      * (1.5 * (p[x] - p[y])
                          - 2 * (pALT[x] - pALT[y])
                          + 0.5 * (pALTALT[x] - pALTALT[y]));
            }
            ...
            if (capCorrection) {
              ...
              if (_solverType == SolverType.SOLVER_BE) {
                bWOld =
                    (netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y])
                        + facOld * netzliste.parameter[i1][10];
              } else if (_solverType == SolverType.SOLVER_TRZ) {
                bWOld =
                    (2 * netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y])
                        + iALT[i1]
              ...
              // double bWOld = (netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y]) + facOld *
              // netzliste.parameter[i1][10];
              ...
              double bWNew = 0;
              // double bWNew = (netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y]) + facNew *
              // netzliste.parameter[i1][10];
              if (_solverType == SolverType.SOLVER_BE) {
                bWNew =
                    (netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y])
                        + facNew * netzliste.parameter[i1][10];
              } else if (_solverType == SolverType.SOLVER_TRZ) {
                bWNew =
                    (2 * netzliste.parameter[i1][6] / dt) * (pALT[x] - pALT[y])
                        + iALT[i1]
            ...
            netzliste.parameter[i1][10] = netzliste.eLKneu[i1]._currentInAmps;
            netzliste.eLKneu[i1]._currentInAmps += nonLinearCorrectionCurrent;
            // System.out.println("" + t + " " + netzliste.parameter[i1][6] + " " +
            // netzliste.parameter[i1][7]);
          }
          break;
        case LK_D:
          ...
          if (diode._diodeChar != null) {
            ...
            if (dc.testIfWrongSegment(t, diodeVoltage, stoergroesse, acceptanceThreshold)) {
              ...
              // System.out.println("Diode t " + errorCounter + " " +
              // netzliste.eLKneu[i1].getIDStringDialog() + " " + netzliste.parameter[i1][0] + " " +
              //  netzliste.parameter[i1][2] + "  "+ netzliste.parameter[i1][3] + " " +
              // netzliste.parameter[i1][4]);
              // Korrektur Matrix a:
              a[x][x] += (-aALT + aNEU);
              a[y][y] += (-aALT + aNEU);
              a[x][y] += (+aALT - aNEU);
            ...
            if (((diodeVoltage /*
                                * + i[i1] * rD
                                */)
                    < (stoergroesse * uf) + acceptanceThreshold)
                && (rD < 10000 /*
                                 * Typ.rDoffDEFAULT
                                 */)) { // (uD < uf) und Diode "ON"
              double aALT = 1.0 / netzliste.parameter[i1][0]; // (1/rD)
              ...
              //                                if(((MOSFET) block).getAntiParallelDiode() ==
              // netzliste.eLKneu[i1]) {
              //                                    System.out.println("mosfet " +
              // block.getStringID());
              //                                }
              //                            }
              //                        }
            ...
            if ((((diodeVoltage) > (stoergroesse * uf - acceptanceThreshold))
                && (rD > 10000 /*
                                 * == Typ.rDoffDEFAULT
                                 */))) { // (uD > uf) und Diode "OFF"
          ...
          //                    if (((p[x] - p[y] /*
          //                             * + i[i1] * rD
          //                             */) < (stoergroesse * uf) + acceptanceThreshold) && (rD <
          // 10000/*
          //                             * Typ.rDoffDEFAULT
          //                             */)) {  // (uD < uf) und Diode "ON"
          //                        double aALT = 1.0 / netzliste.parameter[i1][0];  // (1/rD)
          //                        double bALT = netzliste.parameter[i1][1] /
          // netzliste.parameter[i1][0];  // (Uf/rD)
          //                        netzliste.parameter[i1][0] = netzliste.parameter[i1][3];  //
          // Diode auf "OFF" setzen
          //                        double aNEU = 1.0 / netzliste.parameter[i1][0];
          //                        double bNEU = netzliste.parameter[i1][1] /
          // netzliste.parameter[i1][0];
          //                        einSchrittZurueck = true;
          ////                        for(AbstractCircuitBlockInterface block : netzliste.eLKneu) {
          ////                            if(block instanceof MOSFET) {
          ////                                if(((MOSFET) block).getAntiParallelDiode() ==
          ...
          //                        bVector[y] += (+bALT - bNEU);
          //                    }
          //                    if ((((p[x] - p[y]) > (stoergroesse * uf - acceptanceThreshold)) &&
          // (rD > 10000/*
          //                             * == Typ.rDoffDEFAULT
          //                             */))) {  // (uD > uf) und Diode "OFF"
          //
          ...
          //                        double bALT = netzliste.parameter[i1][1] /
          // netzliste.parameter[i1][0];
          //                        netzliste.parameter[i1][0] = netzliste.parameter[i1][2];  //
          // Diode auf "ON" setzen
          //                        double aNEU = 1.0 / netzliste.parameter[i1][0];
          //                        double bNEU = netzliste.parameter[i1][1] /
          // netzliste.parameter[i1][0];
        ...
        case LK_THYR:
          ...
          if (((p[x] - p[y]) < (stoergroesse * uf + acceptanceThreshold))
              && (rD < 0.5 * netzliste.parameter[i1][3])) { // (uD < uf) und Thyristor "ON"
            double aALT = 1.0 / netzliste.parameter[i1][0]; // (1/rD)
            double bALT = netzliste.parameter[i1][1] / netzliste.parameter[i1][0]; // (Uf/rD)
          ...
          // in 'parameter[8]' wird beim Thyristor das aktuelle Gate-Signal hineingeschrieben (siehe
          // 'Simulationskern.runSimulation()')
          if ((netzliste.parameter[i1][8] == 1)
              && (((p[x] - p[y]) > (stoergroesse * uf - acceptanceThreshold))
                  && (rD
                      == AbstractSwitch
                          .RD_OFF_DEFAULT))) { // gate==1  und  (uD > uf) und Thyristor "OFF"
            double aALT = 1.0 / netzliste.parameter[i1][0];
            double bALT = netzliste.parameter[i1][1] / netzliste.parameter[i1][0];
        ...
        case LK_IGBT:
          ...
          // rD(t) - uf - rON - rOFF - i(t) - u(t) - xxx - xxx - gateStatusOnOff   --> aehnlich wie
          // THYR
          // Diode seriell zu Schalter kann nur aktiv werden, wenn das Schalter-Gate 'ON' ist (dh.
          // gateStatusOnOff==1)
          ...
          if ((((p[x] - p[y] /*
                             ...
                             * Typ.rDoffDEFAULT
                             */))
              && (netzliste.parameter[i1][8]
                  == 1)) { // (uD < uf) und IGBT-Serien-Diode "ON" und gateStatusOnOff==1
            double aALT = 1.0 / netzliste.parameter[i1][0]; // (1/rD)
            double bALT = netzliste.parameter[i1][1] / netzliste.parameter[i1][0]; // (Uf/rD)
          ...
          // in 'parameter[8]' wird beim IGBT das aktuelle Gate-Signal hineingeschrieben (siehe
          // 'Simulationskern.runSimulation()')
          if ((netzliste.parameter[i1][8] == 1)
              && (((p[x] - p[y]) > (stoergroesse * uf - acceptanceThreshold))
                  && (rD > 10000 /*
          ...
          if ((netzliste.parameter[i1][8] == 0)
              && (netzliste.parameter[i1][0]
                  == netzliste
                      .parameter[i1][
                      2])) { // bis jetzt 'ON', aber das Gate wurde soeben auf 'OFF' gesetzt
            // rD(t) - uf - rON - rOFF - i(t) - u(t) - xxx - xxx - gateStatusOnOff   --> aehnlich
            // wie THYR
            double aALT = 1.0 / netzliste.parameter[i1][0];
            double bALT = netzliste.parameter[i1][1] / netzliste.parameter[i1][0];
            netzliste.parameter[i1][0] = netzliste.parameter[i1][3]; // --> IGBT auf "OFF" setzen
        ...
        case LK_U:
          if (netzliste.parameter[i1][0] == SourceType.QUELLE_VOLTAGECONTROLLED_DIRECTLY_NEW
              || netzliste.parameter[i1][0] == SourceType.QUELLE_VOLTAGECONTROLLED_DIRECTLY
                  && t > 0) {
            double gain = netzliste.parameter[i1][11];
            int[][] nodePairDVC = netzliste.nodePairDirVoltContSrc;
            if (nodePairDVC == null) {
          ...
          netzliste.eLKneu[i1]._currentInAmps =
              p[
                  netzliste.knotenMAX
                      + netzliste
                          .spgQuelleNr[
                          i1]]; // SpgQuellen-Stroeme stehen als Unbekannte ausnahmsweise auch im
          // Knotenpotetial-Vektor
          break;
        case LK_M:
          // wird in LK_LKOP2 abgehandelt
          // in diesem 'Element' fliessen keine Stroeme, di/dt-Berechung erfolgt in
          // aktualisiereKnotenpotentiale()
          break;
        case LK_TERMINAL:
        case TH_TERMINAL:
        case REL_TERMINAL:
  ...
  public void aktualisiereKnotenpotentiale(double dt, double time) {
    ...
    // ------------------------------
    // aktuelle Strom- und Spg.-Werte in 'parameter[]' ablegen zwecks einfachem und direktem
    // Zugriff:
    for (int i1 = 0; i1 < elementANZAHL; i1++) {
      int x = netzliste.knotenX[i1];
      int y = netzliste.knotenY[i1];
      ...
      switch (netzliste.typ[i1]) {
        ...
        case LK_S: // verhaelt sich exakt wie ein hoch- bzw. niederohmiger Widerstand
          ...
          // ACHTUNG: bei Diodenschaltfehlern wird parameter[][3] und parameter[4] mit den
          // aktualisierten korrekten Werten
          // ueberschrieben --> die Abfrage von parameter[][3] und parameter[][4] zur
          // Schaltverlustberechnung darf daher erst
          // NACH Abschluss der Diodenzustands-Aktualisierung in der Simulation-Schleife in
          // 'SimulationsKern' erfolgen
          break;
        case LK_L:
        case NONLIN_REL:
        case LK_LKOP2:
          ...
          netzliste.parameter[i1][4] =
              (netzliste.eLKneu[i1]._currentInAmps - netzliste.parameter[i1][2]) / dt;
      ...
      txt[i1] =
          new String(
              "(id) "
                  + i1
                  + "\t"
                  + netzliste.eLKneu[i1].getStringID()
                  + "\t("
                  + netzliste.labelListe[netzliste.knotenX[i1]]
                  + " ["
                  + nf.format(p[x])
                  + "]  -  "
                  + netzliste.labelListe[netzliste.knotenY[i1]]
                  + " ["
                  + nf.format(p[y])
                  + "]");
    }
  ...
  // dadurch kann 'setzeAnfangsbedingungen()' sowohl fuer die DialogEingabewerte bei t0==0 als auch
  // bei CONTINUE nach PAUSE eingesetzt werden -->
  //
  private void setzeAnfangsbedingungen(
      boolean getAnfangsbedVomDialogfenster, final SolverType solverType) {
    //
    //        for (int i1=0;  i1<pALT.length;  i1++)  pALT[i1]= 0;  // default --> alle
    // Knotenpotentiale auf Null
    // =======================================
    ...
    if ((getAnfangsbedVomDialogfenster) && (matrixSize > 1)) {
      ...
      // damit stellen sich die richtigen Knotenpotentiale ein, wenn Anfangsbedingungen uC(0)
      // gesetzt sind
      // ---------
      // initialisieren:
    ...
    // CONTINUE -->
    //
    // ACHTUNG: Funktioniert nur, wenn die Schaltungsstruktur dh. die Knotenanzahl nicht veraendert
    // wird!!!!
    //
      ...
      switch (netzliste.typ[i1]) {
        ...
        case LK_LKOP2:
          ...
          if ((zuLKOP2gehoerigeM_spgQnr != null) && (zuLKOP2gehoerigeM_spgQnr[i1] != null)) {
            ...
            for (int i7 = 0; i7 < anzahlKoppelPartner; i7++) {
              p[netzliste.knotenMAX + netzliste.spgQuelleNr[i1]] =
                  iALT[i1]; // SpgQuellen-Stroeme stehen als Unbekannte ausnahmsweise auch im
              // Knotenpotetial-Vektor
            }
          }
          iALTALT[i1] = iALT[i1];
