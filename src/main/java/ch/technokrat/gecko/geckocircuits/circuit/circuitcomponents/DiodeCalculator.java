...
public final class DiodeCalculator extends CircuitComponent<Diode>
    implements AStampable, BStampable, CurrentCalculatable {
  ...
  public void stampVectorB(final double[] bVector, final double time, final double deltaT) {
    ...
    // System.out.println(_element._elementName.getValue() + " stamp B: " + bValue + "  VF: " +
    // _uForward + ", rDt: " + _rDt);
  ...
  public void calculateCurrent(final double[] pVector, final double deltaT, final double time) {
    ...
    if (_voltage + _rDt * _current <= disturbanceValue * _uForward) {
      if (_rDt < _rOff) {
        // System.out.println("voltage less than VF for " + _element._elementName.getValue() + "
        // when in ON state");
        // System.out.println("_voltage = " + _voltage + ", _current = " + _current + ", _rDt = " +
        // _rDt);
      ...
      if (_rDt > _rOn) {
        // System.out.println("voltage greater than VF for " + _element._elementName.getValue() + "
        // when in OFF state");
        // System.out.println("_voltage = " + _voltage + ", _current = " + _current + ", _rDt = " +
        // _rDt);
        diodeSwitchError = true;
    ...
    //        if (((p[matrixIndices[0]] - p[matrixIndices[1]]) < (Diode.stoerGroesse * _uForward))
    // && (_rD_t < _rOff)) {  // (uD < uf) und Diode "ON"
    //            _rD_t = _rOff;
    ...
    //
    //        if ((((p[matrixIndices[0]] - p[matrixIndices[1]]) > (Diode.stoerGroesse * _uForward))
    // && (_rD_t > _rOn))) {  // (uD > uf) und Diode "OFF"
    //            _rD_t = _rOn;
