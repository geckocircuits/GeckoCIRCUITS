...
abstract class AbstractMotorSM extends AbstractThreePhaseMotor {
  ...
  double Lls;
  final UserParameter<Double> statorLeakageInductance =
      UserParameter.Builder.<Double>start("statorLeakageInductance", 0.0010)
          ...
          .build();
  double psiT;
  final UserParameter<Double> fluxAtSaturationTransition =
      UserParameter.Builder.<Double>start("fluxAtSaturationTransition", 0.25)
          ...
          .build();
  double fT;
  final UserParameter<Double> tightnessOfSaturationTransition =
      UserParameter.Builder.<Double>start("tightnessOfSaturationTransition", 0.9)
          ...
          .build();
  double Rf;
  final UserParameter<Double> fieldResistance =
      UserParameter.Builder.<Double>start("fieldResistance", 0.1)
          ...
          .build();
  double Llf;
  final UserParameter<Double> fieldLeakageInductance =
      UserParameter.Builder.<Double>start("fieldLeakageInductance", 5.0E-4)
          ...
          .build();
  double Rkd;
  final UserParameter<Double> damperResistanceD =
      UserParameter.Builder.<Double>start("damperResistanceD", 0.1)
          .longName(I18nKeys.DAMPER_RESISTANCE)
          .shortName("Rdamp_d")
          .unit("ohm")
          .arrayIndex(this, 25)
          .build();
  final UserParameter<Double> initialStatorFluxD =
      UserParameter.Builder.<Double>start("initialStatorFluxD", 0.0)
          .longName(I18nKeys.INITIAL_STATOR_FLUX_D)
          .shortName("fluxS_d")
          .unit("A")
          .arrayIndex(this, getInitialStatorFluxIndexD())
          .build();
  final UserParameter<Double> initialStatorFluxQ =
      UserParameter.Builder.<Double>start("initialStatorFluxQ", 0.0)
          ...
          .build();
  double ifd, ifd0;
  final UserParameter<Double> initialFieldCurrent =
      UserParameter.Builder.<Double>start("initialFieldCurrent", 0.0)
  ...
  double Llkd;
  final UserParameter<Double> damperLeakageInductanceD =
      UserParameter.Builder.<Double>start("damperLeakageInductanceD", 0.0015)
          ...
          .build();
  double Llkq1;
  final UserParameter<Double> damperLeakageInductanceQ =
      UserParameter.Builder.<Double>start("damperLeakageInductanceQ", 0.0030)
          ...
          .build();
  double Rkq;
  public final UserParameter<Double> damperResistanceQ =
      UserParameter.Builder.<Double>start("damperResistanceQ", 0.2)
          .longName(I18nKeys.DAMPER_RESISTANCE)
          .shortName("Rdamp_q")
          .unit("ohm")
          .arrayIndex(this, 26)
          .build();
  double Lmd0;
  public final UserParameter<Double> unsaturatedMagnetizingInductanceD =
      UserParameter.Builder.<Double>start("unsaturatedMagnetizingInductance", 0.01)
          ...
          .build();
  double Lmdsat;
  public final UserParameter<Double> saturatedMagnetizingInductance =
      UserParameter.Builder.<Double>start("saturatedMagnetizingInductance", 0.0020)
  ...
  public void setzeParameterZustandswerteAufNULL() {
    ...
    isd =
        isd0 =
            2.0
                / 3.0
                * (isa0 * Math.cos(_thetaElectric)
                    + isb0 * Math.cos(_thetaElectric - 2.0 / 3 * Math.PI)
                    - (isa0 + isb0) * Math.cos(_thetaElectric + 2.0 / 3 * Math.PI));
    isq =
        isq0 =
            -2.0
                / 3.0
                * (isa0 * Math.sin(_thetaElectric)
                    + isb0 * Math.sin(_thetaElectric - 2.0 / 3.0 * Math.PI)
                    - (isa0 + isb0) * Math.sin(_thetaElectric + 2.0 / 3.0 * Math.PI));

    Mi =
        (1.0 / Lmd0 - 1.0 / Lmdsat * (0.5 - Math.atan(tauT * psiT) / Math.PI))
            / (0.5 + Math.atan(tauT * psiT) / Math.PI);
  ...
  void updateIsIfPsis(
      double deltaT, double ud, double uq, double uf, double ddt_psimd, double ddt_psimq) {
    double ddt_isd =
        (-_statorResistance * isd
                + Lls * _omegaElectric * isq
                + _omegaElectric * psimq
                + ud
                - ddt_psimd)
            / Lls;
    double ddt_isq =
        (-Lls * _omegaElectric * isd
                - _statorResistance * isq
                - psimd * _omegaElectric
                + uq
                - ddt_psimq)
            / Lls;
  ...
  public final List<String> getParameterStringIntern() {
    return Arrays.asList(
        "isa [A]", "isb [A]", "isc [A]", "omega", "n [rpm]", "theta [rad]", "Tel [Nm]");
  }

  double calculate_ud(final double uab, final double ubc) {
    return (2 * uab * Math.cos(_thetaElectric)
            + ubc * (Math.cos(_thetaElectric) + Math.sqrt(3) * Math.sin(_thetaElectric)))
        / 3;
  }

  double calculate_uq(final double uab, final double ubc) {
    return -(2 * uab * Math.sin(_thetaElectric)
            + ubc * (Math.sin(_thetaElectric) - Math.sqrt(3) * Math.cos(_thetaElectric)))
        / 3;
  }
}
