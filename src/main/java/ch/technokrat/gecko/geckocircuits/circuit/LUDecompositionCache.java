...
/**
 * Main-Function: getCachedLUDecomposition. For switched converters, one and the same LU
 * decomposition is typically re-computed very often. Therefore, this class provides a cache, which
 * speeds up the calculation for larger matrices.
 *
 * @author andy
 */
public class LUDecompositionCache {
  ...
  private final Map<Integer, AbstractCachedMatrix> _cachedMatrices =
      new HashMap<Integer, AbstractCachedMatrix>();
  ...
  private void printDebugMessages(final double time) {
    ...
    //    System.out.println("cache: " + mat + " " + (time - mat.getLatestAccessTime()) + " " +
    // mat.getAccessCounter());
    // }
    System.out.println(
        "cache hits: "
            + _cacheHitCounter
            ...
            + _cacheMissCounter
            + " "
            + (100.0 * _cacheHitCounter / (_cacheHitCounter + _cacheMissCounter))
            + "%");
