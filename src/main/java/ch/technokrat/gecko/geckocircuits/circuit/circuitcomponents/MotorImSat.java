...
public final class MotorImSat extends AbstractMotorIMCommon {
  public static final AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(
          MotorImSat.class, "IM-SAT", I18nKeys.IM_SAT, I18nKeys.SATURABLE_INDUCTION_MACHINE);

  final UserParameter<Double> _unsaturatedMagnetizingInductance =
      UserParameter.Builder.<Double>start("unsaturatedMagnetizingInductance", 0.05)
          .longName(I18nKeys.UNSATURATED_MAGNETIZING_INDUCTANCE)
          .shortName("Lm0")
          .unit("H")
          .arrayIndex(this, 21)
          .build();
  final UserParameter<Double> _saturatedMagnetizingInductance =
      UserParameter.Builder.<Double>start("saturatedMagnetizingInductance", 0.05)
          .longName(I18nKeys.SATURATED_MAGNETIZING_INDUCTANCE)
          .shortName("Lm_sat")
          .unit("H")
          .arrayIndex(this, 22)
          .build();
  final UserParameter<Double> _fluxSaturationTransition =
      UserParameter.Builder.<Double>start("fluxatSaturationTransition", 0.25)
          ...
          .arrayIndex(this, 23)
          .build();
  final UserParameter<Double> _tightnessOfSaturationTransition =
  ...
  public void setzeParameterZustandswerteAufNULL() {
    ...
    Mf = 1 / Lmsat;
    Mi =
        (1 / Lm0 - 1 / Lmsat * (0.5 - Math.atan(tauT * psiT) / Math.PI))
  ...
  void calculateMotorEquations(final double deltaT, double time) {
    ...
    double inv_Lm =
        (Mf - Mi)
                / Math.PI
                * ((psim - psiT) * Math.atan(tauT * (psim - psiT))
                    - psiT * Math.atan(tauT * psiT)
                    + 0.5
                        / tauT
                        * (Math.log(1 + (tauT * psiT) * (tauT * psiT))
                            - Math.log(1 + tauT * tauT * (psim - psiT) * (psim - psiT))))
                / psim
            + 0.5 * (Mf + Mi);
    // Block 'vrdq':
    double urd =
        2.0
            / 3.0
            * (urab * Math.cos(_thetaElectric) - urbc * Math.cos(_thetaElectric - 2 * Math.PI / 3));
    double urq =
        2.0
            / 3.0
            * (urab * Math.sin(_thetaElectric) - urbc * Math.sin(_thetaElectric - 2 * Math.PI / 3));
    // Block 'isdq,psimdq':
    double ddt_psimd =
        ((Rr / Llr - _statorResistance / Lls) * isd
                + isq * _omegaElectric
                - Rr * inv_Lm / Llr * psimd
                - _omegaElectric * psimq * (inv_Lm + 1 / Llr)
                + usd / Lls
                + urd / Llr)
            / (inv_Lm + 1 / Lls + 1 / Llr);
    double ddt_psimq =
        (-_omegaElectric * isd
                + (Rr / Llr - _statorResistance / Lls) * isq
                + _omegaElectric * (inv_Lm + 1 / Llr) * psimd
                - Rr * inv_Lm / Llr * psimq
                + usq / Lls
                + urq / Llr)
            / (inv_Lm + 1 / Lls + 1 / Llr);
    ...
    irb =
        ird * Math.cos(_thetaElectric + 2 * Math.PI / 3)
            + irq * Math.sin(_thetaElectric + 2 * Math.PI / 3);
  }
  ...
  protected void drawOnTop(final Graphics2D graphics) {
    // Saturation-Symbol:
    graphics.drawOval(
        (int) (-dpix * RADIUS_MOTOR_SYMBOL) + drMpix,
        (int) (-dpix * RADIUS_MOTOR_SYMBOL) + drMpix,
        (int) (dpix * 2 * RADIUS_MOTOR_SYMBOL) - 2 * drMpix,
        (int) (dpix * 2 * RADIUS_MOTOR_SYMBOL) - 2 * drMpix);
    double h1 = 0.1, h2 = 0.3, h3 = 0.4;
    graphics.drawLine(
        (int) (dpix * (-h1 - h2)), (int) (dpix * h3), (int) (-dpix * h1), (int) (dpix * h3));
    graphics.drawLine(
        (int) (dpix * (h1 + h2)), (int) (-dpix * h3), (int) (dpix * h1), (int) (-dpix * h3));
