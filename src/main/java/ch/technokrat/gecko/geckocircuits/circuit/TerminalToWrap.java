...
/**
 * I made this component to be wrapped from ReglerTerminal and LKreisterminal. Otherwise, I cannot
 * avoid code duplication. This is maybe not really "beautiful", however DRY: don't repeat yourself!
 *
 * @author andy
 */
public final class TerminalToWrap {
  ...
  public void reCalculateLocation(final Point moveToPoint) {
    ...
    final int checkedPointX =
        Math.min(moveToPoint.x + _parentComponent.getPositionVorVerschieben().x, wsSizeX - 1);
    final int checkedPointY =
        Math.min(moveToPoint.y + _parentComponent.getPositionVorVerschieben().y, wsSizeY - 1);
    ...
    if (_parentComponent.getParentCircuitSheet() instanceof SubCircuitSheet) {
      ((SubCircuitSheet) _parentComponent.getParentCircuitSheet())
          ._subBlock.recalculateTerminalPositions();
    }
  }
  ...
  public void absetzenElement() {
    ...
    if (_parentComponent.getParentCircuitSheet() instanceof SubCircuitSheet
        && !((SubCircuitSheet) _parentComponent.getParentCircuitSheet())
            ._subBlock.areTerminalPositionsOK()) {
      String problematicList = "";

      for (SubCircuitTerminable term :
          ((SubCircuitSheet) _parentComponent.getParentCircuitSheet())
              ._subBlock.getTerminalsWithWrongPosition()) {
        problematicList +=
            term.getStringID()
                + " position ("
                + term.getBlockTerminal().getRelativeX()
                + " "
                + term.getBlockTerminal().getRelativeY()
                + ")\n";
      }
  ...
  public void drawForeground(final Graphics2D graphics) {
    ...
    if (_parentComponent.getModus() == ComponentState.SELECTED
      ...
      final SubcircuitBlock subBlock =
          ((SubCircuitSheet) _parentComponent.getParentCircuitSheet())._subBlock;
      AffineTransform newtransform = new AffineTransform(oldTrans);
      graphics.setTransform(newtransform);
      graphics.translate(
          (-subBlock.getSheetPosition().x - getBlockTerminal().getRelativeX()) * dpix,
  ...
  public void importIndividual(final TokenMap tokenMap) {
    _terminalLocation =
        EnumTerminalLocation.getFromOrdinal(
            tokenMap.readDataLine("terminalLocation", _terminalLocation.ordinal()));
  }
  ...
  public static boolean sameBlockPosition(
      final SubCircuitTerminable terminable1, final SubCircuitTerminable terminable2) {
    final TerminalSubCircuitBlock termSub1 =
        (TerminalSubCircuitBlock) terminable1.getBlockTerminal();
    final TerminalSubCircuitBlock termSub2 =
        (TerminalSubCircuitBlock) terminable2.getBlockTerminal();
