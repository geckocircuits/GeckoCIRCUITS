...
public class NonLinearDialogPanel extends JPanel {
  ...
  public NonLinearDialogPanel(
      JDialog parentDialog,
      final AbstractNonLinearCircuitComponent elementLK,
  ...
  private JPanel baueGUIInput() {
    ...
    jbSaveAsFile.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent ae) {
            GeckoFileChooser fileChooser =
                GeckoFileChooser.createSimpleSaveFileChooser(_extension, _parentDialog);
            if (fileChooser.getUserResult() == GeckoFileChooser.FileChooserResult.CANCEL) {
              return;
            }
            try {
              savedExternalFile =
                  AbstractNonLinearCircuitComponent.writeNonLinearCharacteristicToFile(
                      data, fileChooser.getFileWithCheckedEnding());
            } catch (java.io.IOException e) {
              final String errorMessage =
                  "Error writing non-linear characteristic file "
                      + fileChooser.getFileWithCheckedEnding()
                      + "\n"
                      + e.getMessage();
              final String errorTitle = "ERROR - I/O exception";
              JOptionPane.showMessageDialog(
                  null, errorMessage, errorTitle, JOptionPane.ERROR_MESSAGE);
            }
          }
        });
    JButton jbLoadFromFile = GuiFabric.getJButton(I18nKeys.LOAD);
    jbLoadFromFile.setPreferredSize(BUTTON_DIMENSION);
    jbLoadFromFile.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent ae) {
            GeckoFileManagerWindow gfm =
                new GeckoFileManagerWindow(_nonlinearParent, _extension, _type, true);
        ...
        new ActionListener() {
          ...
          public void actionPerformed(final ActionEvent ae) {
            if (loadedFile != null) {
              ...
              // characteristic has been saved to an external file, and we should not attribute this
              // to the circuit element
              try {
                GeckoFile newFileFromExternal =
                    new GeckoFile(
                        savedExternalFile,
                ...
                if (existingNonLinFile.noOfUsers() > 1) {
                  switch (overwriteOption(existingNonLinFile.getName())) {
                    ...
                    case 1: // save as new file
                      String newInternalFileName =
                          getInternalFileName(_nonlinearParent.getStringID() + "new");
                  ...
                  if (originalDataIsChanged(data, _nonlinearParent.nonlinearData)) {
                    ...
                    switch (overwrite) {
                      case -1: // ESC key
                        return;
                      case 0:
                        _nonlinearParent.setNonlinearCharacteristic(
                            data); // overwrite existing file
                        break;
                      case 1:
                        String newInternalFileName =
                            getInternalFileName(_nonlinearParent.getStringID() + "new");
  ...
  private String getInternalFileName(String blockName) {
    String internalFileName =
        JOptionPane.showInputDialog(
            null,
            "Please select a non-linear characteristic identifier with extension *"
                + _extension
                + " (e.g. "
                + blockName.replace(".", "")
                + _extension
    ...
    int selected =
        JOptionPane.showOptionDialog(
            ...
            "Non-linear characteristic "
                + existingFileName
                + " is also used by other circuit elements.\n"
  ...
  private int overwriteExternal(String existingFileName) {
    ...
    int selected =
        JOptionPane.showOptionDialog(
            null,
            "This element uses the external file "
                + existingFileName
                + " for its non-linear characteristic.\n"
  ...
  public void setCharacteristicLoadedFromFile(final GeckoFile file) {
    ...
    if (_nonlinearParent.getNonLinearFile() == file) {
      ...
      for (int i = 0; i < data[0].length; i++) {
        System.out.println(
            "iii " + i + " " + data[0][i] + " " + data[1][i] + " " + (data[0][i] * data[1][i]));
      }
      table.setValues(data);
      loadedFile = file;
      updatePlot();
