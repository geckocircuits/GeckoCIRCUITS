...
public abstract class CircuitComponent<T extends AbstractTwoPortLKreisBlock> {
  ...
  public void assignTerminalIndices() {
    ...
    // System.out.println("element: " + _element._elementName.getValue() + " matrixIndices[0]=" +
    // matrixIndices[0] + " matrixIndices[1]=" + matrixIndices[1]);
  ...
  // to when going forward a timestep: copy the data of the previous timestep into the data from 2
  // previous timesteps ago
  ...
  public void stepBack() {
    if ((!stepped_back && (steps_reversed == 0))
        || (stepped_back && (steps_reversed < steps_saved))) {
      if (stepped_back) {
        historyBackward();
      }
      prev_time = var_history[0][0];
      // System.out.println("before: _potential 1 = " + _potential1 + " _potential 2 = " +
      // _potential2);
      _potential1 = var_history[0][1];
      _potential2 = var_history[0][2];
      // System.out.println("after: _potential 1 = " + _potential1 + " _potential 2 = " +
      // _potential2);
  ...
  public String toString() {
    return _parent.getStringID()
        + " calculator "; // To change body of generated methods, choose Tools | Templates.
  }
}
