...
public final class VerlustBerechnungDetailed
    implements GeckoFileable, AbstractLossCalculatorFabric {
  ...
  // Name der Datei (inkl. Pfad), die die Detail-Daten, d.h. Messkurven der Leit- und Schaltverluste
  // enthaelt
  ...
  public final List<LeitverlusteMesskurve> _messkurvePvCOND =
      new ArrayList<LeitverlusteMesskurve>();

  public VerlustBerechnungDetailed(
      final AbstractCircuitBlockInterface parent, final LossProperties lossParent) {
    _parent = parent;
  ...
  public List<LeitverlusteMesskurve> getCopyOfLeitverlusteMesskurvenArray() {
    ...
    // herausgegeben wird ein Kopie, damit man mit 'Cancel' im uebergeordneten Fenster die
    // Aenderungen nicht zwingend uebernehmen muss -->
    for (LeitverlusteMesskurve toCopy : _messkurvePvCOND) {
      ...
      double[][] dataCopy =
          new double[toCopy.getCurveData().length][toCopy.getCurveData()[0].length];
  ...
  // herausgegeben wird ein Kopie, damit man mit 'Cancel' im uebergeordneten Fenster die Aenderungen
  // nicht zwingend uebernehmen muss -->
  public List<SwitchingLossCurve> getCopyOfSchaltverlusteMesskurvenArray() {
    ...
    for (int i1 = 0; i1 < _messkurvePvSWITCH.size(); i1++) {
      SwitchingLossCurve curveCopy =
          new SwitchingLossCurve(
              _messkurvePvSWITCH.get(i1).tj.getValue(),
              _messkurvePvSWITCH.get(i1)._uBlock.getValue());
      returnValue.add(curveCopy);
      double[][] dataCopy =
          new double[_messkurvePvSWITCH.get(i1).getCurveData().length]
              [_messkurvePvSWITCH.get(i1).getCurveData()[0].length];
      for (int i2 = 0; i2 < dataCopy.length; i2++) {
        for (int i3 = 0; i3 < dataCopy[0].length; i3++) {
  ...
  public void importASCII(TokenMap tokenMap) {
    ...
    datnamGemesseneVerluste =
        tokenMap.readDataLine("datnamGemesseneVerluste", datnamGemesseneVerluste);
    ...
    try {
      ...
      if (!datnamGemesseneVerluste.equals(GlobalFilePathes.DATNAM_NOT_DEFINED)) {
        String aktualisierterPfad =
            DatenSpeicher.lokalisiereRelativenPfad(
                GlobalFilePathes.DATNAM, datnamGemesseneVerluste);
  ...
  public boolean leseDetailVerlusteVonDatei(final GeckoFile newLossFile) {
    ...
    try {
      ...
      try {
        ...
        try {
          BufferedReader in =
              newLossFile.getBufferedReader(); // new BufferedReader(new FileReader(fyomu));
          String z = null;
          while ((z = in.readLine()) != null) {
            datVec.add(z);
          }
    ...
    for (TokenMap lossBlock = tokenMap.getBlockTokenMap("<SchaltverlusteMesskurve>");
        lossBlock != null;
    ...
    for (TokenMap lossBlock = tokenMap.getBlockTokenMap("<LeitverlusteMesskurve>");
        lossBlock != null;
  ...
  public void initLossFile() {
    if (lossFileHashValue != 0) {
      try {
        ...
        // this means this is probably an old .ipes file without a valid hash key for the GeckoFile
        // (i.e. saved in an older version)
  ...
  public void readLossesFromFileAndSetDetailedLossType(final String fyomu) {
    ...
    try {
      file =
          new GeckoFile(new File(fyomu), GeckoFile.StorageType.INTERNAL, Fenster.getOpenFileName());
      ...
      _lossParent._lossType.setValueWithoutUndo(LossCalculationDetail.DETAILED);
        ...
        FileNotFoundException
            e) { // if not, see if it is a relative path to the .ipes file location
      ...
      if (!modelFilePath.equals("Untitled")) {
        ...
        try {
          file =
              new GeckoFile(
                  new File(newPath), GeckoFile.StorageType.INTERNAL, Fenster.getOpenFileName());
          _lossParent._lossType.setValueWithoutUndo(LossCalculationDetail.SIMPLE);
        } catch (FileNotFoundException e2) {
          System.err.println(
              "Loss file " + fyomu + " for component " + _parent.getStringID() + " not found!");
        }
      }
    }
  }

  public boolean schreibeDetailVerlusteAufDatei(
      final String fkaku,
      final List<SwitchingLossCurve> messkurvePvSWITCH,
      final List<LeitverlusteMesskurve> messkurvePvCOND,
      final GeckoFile.StorageType storageType) {
    ...
    try {
      ...
      BufferedWriter out =
          new BufferedWriter(new OutputStreamWriter(new FileOutputStream(lossesFile)));
      //
  ...
  /**
   * @return String to be displayed in circuit sheet (file name). When link could not be found,
   *     return null!
   */
  ...
  public void setzeNeueParameter(
      final List<SwitchingLossCurve> messkurvePvSWITCH,
      final List<LeitverlusteMesskurve> messkurvePvCOND) {
    _messkurvePvSWITCH.clear();
  ...
  public void removeLocalComponentFiles(final List<GeckoFile> filesToRemove) {
    assert false
        : "Remove files cannot be called here. Loss components"
            + "have a loss file as single selection!";
  }

  public final class LossCalculatorDetailed extends AbstractLossCalculatorSwitch {

    /** the curves reference voltage is rescaled to this voltage before the Lookup-table. */
