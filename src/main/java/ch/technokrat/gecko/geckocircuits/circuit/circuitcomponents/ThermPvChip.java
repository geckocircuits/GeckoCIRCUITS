...
public final class ThermPvChip extends AbstractCircuitBlockInterface
    implements ComponentCoupable,
        CurrentMeasurable,
        HiddenSubCircuitable,
        PostCalculatable,
        DirectVoltageMeasurable {

  public static final AbstractTypeInfo TYPE_INFO =
      new ThermalTypeInfo(ThermPvChip.class, "Pv", I18nKeys.THERMAL_LOSS_W);
  ...
  private final ComponentCoupling _componentCoupling =
      new ComponentCoupling(1, this, new int[] {0});
  ...
  private static final AbstractLossCalculator DUMMY_LOSS_CALC =
      new AbstractLossCalculator() {
  ...
  public void doInitialization() {
    ...
    if (_lossComponent instanceof LossCalculatable) {
      final AbstractLossCalculatorFabric verluste =
          ((LossCalculatable) _lossComponent).getVerlustBerechnung();
  ...
  public void doCalculation(final double deltaT, final double time) {
    // Junction-Temperatur --> Temperaturdifferenz an Rth, wobei Bezugspunkt das Null-Niveau ist
    // ('TH_NULLBEZUG_KNOTEN')
    ...
    _thFlow.parameter[1] =
        _currentInAmps; // Verluste repraesentieren Waermestrom [W/m2] in der gesteuerten
    // FLOW-Quelle
  }
  ...
  protected void drawConnectorLines(final Graphics2D graphics) {
    graphics.drawLine(
        (int) (dpix * (-WIDTH - DIAMETER_A)), 0, (int) (dpix * (WIDTH + DIAMETER_A)), 0);
    graphics.drawLine(
        (int) (dpix * (-WIDTH - DIAMETER_A)),
        0,
        (int) (dpix * (-WIDTH - DIAMETER_A)),
        (int) (dpix * HEIGHT));
  }
  ...
  protected void drawForeground(final Graphics2D graphics) {
    ...
    graphics.drawPolygon(
        ...
        POLYGON_POINTS);
    graphics.drawString(
        "Loss", (int) (-dpix * WIDTH + DIAMETER), (int) (graphics.getFont().getSize() / 2 - 2));
    // 'Erde'  -->

    graphics.fillRect(
        (int) (dpix * (-WIDTH - DIAMETER_A - EARTH_X)),
        (int) (dpix * HEIGHT),
        (int) (dpix * (2 * EARTH_X)),
        EARTH_Y);
  }
  ...
  public void checkComponentCompatibility(
    ...
    if (testObject instanceof AbstractResistor
        && ((AbstractResistor) testObject).getSimulationDomain() != ConnectorType.LK) {
      return;
    }
