...
public abstract class AbstractCircuitSource extends AbstractTwoPortLKreisBlock
    implements DirectVoltageMeasurable, ComponentCoupable, PotentialCoupable, CurrentMeasurable {

  static final double WIDTH = 0.9;
  static final double HEIGHT = 0.6;
  final UserParameter<Double> _dcValue =
      UserParameter.Builder.<Double>start("dcValue", 10.0)
          ...
          .arrayIndex(this, 1)
          .build();
  final UserParameter<CircuitSourceType> sourceType =
      UserParameter.Builder.<CircuitSourceType>start("sourceType", CircuitSourceType.QUELLE_DC)
          ...
          .arrayIndex(this, 0)
          .build();
  final UserParameter<Double> frequency =
      UserParameter.Builder.<Double>start("frequency", 50.0)
          .mapDomains(ConnectorType.LK, ConnectorType.RELUCTANCE, ConnectorType.THERMAL)
          ...
          .arrayIndex(this, 2)
          .build();
  final UserParameter<Double> phase =
      UserParameter.Builder.<Double>start("phaseDegrees", 0.0)
          .mapDomains(ConnectorType.LK, ConnectorType.RELUCTANCE, ConnectorType.THERMAL)
          ...
          .arrayIndex(this, 4)
          .build();
  final UserParameter<Double> directPotentialGain =
      UserParameter.Builder.<Double>start("directPotentialGain", 1.0)
          .mapDomains(ConnectorType.LK, ConnectorType.RELUCTANCE, ConnectorType.THERMAL)
          ...
          .arrayIndex(this, 11)
          .build();
  final UserParameter<Double> _didtInitialCurrent =
      UserParameter.Builder.<Double>start("didtInitialCurrent", 0.0)
          .longName(I18nKeys.NON_ACCESSIBLE)
          .shortName("non-accessibe")
          .arrayIndex(this, 15)
          .build();
  final UserParameter<Double> _offset =
      UserParameter.Builder.<Double>start("offset", 0.0)
          .mapDomains(getDomains())
          ...
          .arrayIndex(this, 3)
          .build();
  final UserParameter<Double> _amplitude =
      UserParameter.Builder.<Double>start("amplitude", getDefaultAmplitudeValue())
          .mapDomains(getDomains())
  ...
  private final PotentialCoupling _potCoupling =
      new PotentialCoupling(this, new int[] {0}, ConnectorType.CONTROL);

  @Override
  public final void checkComponentCompatibility(
    ...
    if (testObject instanceof DirectVoltageMeasurable
      ...
      insertList.addAll(
          Arrays.asList(measurable.getDirectVoltageMeasurementComponents(getSimulationDomain())));

      if (getSimulationDomain() == ConnectorType.LK) {
        insertList.addAll(
            Arrays.asList(
                measurable.getDirectVoltageMeasurementComponents(ConnectorType.RELUCTANCE)));
      }

      if (getSimulationDomain() == ConnectorType.RELUCTANCE) {
        insertList.addAll(
            Arrays.asList(measurable.getDirectVoltageMeasurementComponents(ConnectorType.LK)));
      }
    }
  }
  ...
  void addDirectOrSignalTextInfo() {
    ...
    switch (sourceType.getValue()) {
      case QUELLE_SIGNALGESTEUERT:
        ...
        break;
      case QUELLE_VOLTAGECONTROLLED_DIRECTLY:
        AbstractCircuitBlockInterface coupledComponent =
        ...
        if (coupledComponent != null) {
          if (directPotentialGain.getNameOpt().isEmpty()) {
            parStr =
                tcf.formatENG(directPotentialGain.getValue(), 2)
                    + " * v("
                    + coupledComponent.getStringID()
                    + ")";
          } else {
