...
public class TerminalControlBidirectional extends TerminalRelativePosition
    implements ControlTerminable {
  ...
  public void paintLabelString(Graphics2D graphics2D) {
    ...
    final int stringLength =
        (int) graphics2D.getFont().getStringBounds(_label.getLabelString(), frc).getWidth();
    int xShift = 0;
    if (!_hasDoubleLabel) {
      xShift = -stringLength;
    }
    if (!_label.getLabelString().isEmpty()) {
      graphics2D.drawString(
          ...
          (int) (_parentElement.dpix * getPosition().x) + xShift,
  ...
  public void paintComponent(Graphics graphics) {
    ...
    graphics.drawLine(
        dpix * getPosition().x,
        dpix * getPosition().y,
        _parentElement.getSheetPosition().x * dpix,
        dpix * getPosition().y);
