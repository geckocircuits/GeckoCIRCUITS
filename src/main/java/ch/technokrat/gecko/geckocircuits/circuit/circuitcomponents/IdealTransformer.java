...
public class IdealTransformer extends AbstractCircuitBlockInterface
    implements HiddenSubCircuitable, CurrentMeasurable, DirectVoltageMeasurable {

  private static final double HEIGHT = 0.8;
  public static AbstractTypeInfo TYPE_INFO =
      new CircuitTypeInfo(IdealTransformer.class, "Trans", I18nKeys.IDEAL_TRANSFORMER);
  final UserParameter<Double> _windingsRatio =
      UserParameter.Builder.<Double>start("windingsRatio", 5.0)
  ...
  final UserParameter<Double> _windings1 =
      UserParameter.Builder.<Double>start("windings1", 10.0)
          .longName(I18nKeys.WINDING_NUMBER_PRIMARY_SIDE)
          .shortName("n1")
          .unit("turns")
          .arrayIndex(this, 0)
          .build();
  final UserParameter<Double> _windings2 =
      UserParameter.Builder.<Double>start("windings2", 2.0)
          .longName(I18nKeys.WINDING_NUMBER_SECONDARY_SIDE)
          .shortName("n2")
          .unit("turns")
          .arrayIndex(this, 1)
          .build();
  final UserParameter<Double> _reversed =
      UserParameter.Builder.<Double>start("reversed", -1.0)
          .longName(I18nKeys.REVERSE_MAGNETIC_COUPLING)
          .shortName("reversed")
          .arrayIndex(this, 2)
          .build();

  private final AbstractVoltageSource primaryVoltageSource =
      (AbstractVoltageSource) AbstractTypeInfo.fabricHiddenSub(CircuitTyp.LK_U, this);
  private final AbstractVoltageSource secondaryVoltageSource =
      (AbstractVoltageSource) AbstractTypeInfo.fabricHiddenSub(CircuitTyp.LK_U, this);
  ...
  private ActionListener ratioListener =
      new ActionListener() {
        @Override
  ...
  public void initPar() {
    ...
    primaryVoltageSource.directPotentialGain.setValueWithoutUndo(
        -_reversed.getValue() * _windings1.getValue() / _windings2.getValue());
    secondaryVoltageSource.directPotentialGain.setValueWithoutUndo(
        -_reversed.getValue() * _windings1.getValue() / _windings2.getValue());
  ...
  @Override
  public AbstractBlockInterface[] getDirectVoltageMeasurementComponents(
  ...
  @Override
  public AbstractCircuitBlockInterface[] getCurrentMeasurementComponents(
      final ConnectorType connectorType) {
    setSubComponentsName();
  ...
  private void setzeSubcircuit() {
    ...
    primaryVoltageSource.sourceType.setValueWithoutUndo(
        CircuitSourceType.QUELLE_VOLTAGECONTROLLED_DIRECTLY);
    primaryVoltageSource.directPotentialGain.setValueWithoutUndo(
        -_reversed.getValue() * _windings1.getValue() / _windings2.getValue());
    secondaryVoltageSource.sourceType.setValueWithoutUndo(
        CircuitSourceType.QUELLE_VOLTAGECONTROLLED_TRANSFORMER);
    secondaryVoltageSource.directPotentialGain.setValueWithoutUndo(
        -_reversed.getValue() * _windings1.getValue() / _windings2.getValue());
  ...
  protected void drawConnectorLines(final Graphics2D graphics) {
    ...
    for (int yq = (int) (dpix * (0 - HEIGHT));
        yq <= (int) (dpix * (0 + HEIGHT));
        yq += (int) (2 * rq)) {
      g2.drawArc(
          (int) (dpix * (0 + 1) - 2.0 * rq),
          (int) (yq - rq),
    ...
    if (SchematischeEingabe2._lkDisplayMode.showParameter) {
      ...
      switch (getComponentDirection()) {
        case EAST_WEST:
          graphics.drawString(
              "" + _windings1.getValue().floatValue(),
              dpix * x
                  - graphics.getFontMetrics().stringWidth("" + _windings1.getValue().floatValue())
                      / 2,
              (int) (dpix * (y - 1.3)));
          graphics.drawString(
              "" + _windings2.getValue().floatValue(),
              dpix * x
                  - graphics.getFontMetrics().stringWidth("" + _windings1.getValue().floatValue())
                      / 2,
              (int) (dpix * (y + 1.3)) + graphics.getFont().getSize());
          break;
        case NORTH_SOUTH:
          graphics.drawString(
              "" + _windings1.getValue().floatValue(),
              (int) (dpix * (x - 1.5))
                  - graphics.getFontMetrics().stringWidth("" + _windings1.getValue().floatValue()),
              dpix * y + graphics.getFont().getSize() / 2 - 1);
          graphics.drawString(
              "" + _windings2.getValue().floatValue(),
              (int) (dpix * (x + 1.5)),
              dpix * y + graphics.getFont().getSize() / 2 - 1);
        ...
        case SOUTH_NORTH:
          graphics.drawString(
              "" + _windings2.getValue().floatValue(),
              (int) (dpix * (x - 1.5))
                  - graphics.getFontMetrics().stringWidth("" + _windings2.getValue().floatValue()),
              dpix * y + graphics.getFont().getSize() / 2 - 1);
          graphics.drawString(
              "" + _windings1.getValue().floatValue(),
              (int) (dpix * (x + 1.5)),
  ...
  protected void drawForeground(final Graphics2D graphics) {
    ...
    double pk1 = 1.0,
        pk2 = -0.48; // Punkt zur Markierung der eventuellen Kopplung mit anderen Spulen
    graphics.fillOval((int) (dpix * (+pk2) - pkd / 2), (int) (dpix * (-1.4) - pkd / 2), pkd, pkd);
    graphics.fillOval(
        (int) (dpix * (-pk2) - pkd / 2),
        (int) (dpix * (+_reversed.getValue() * 1.4) - pkd / 2),
        pkd,
        pkd);
  }
