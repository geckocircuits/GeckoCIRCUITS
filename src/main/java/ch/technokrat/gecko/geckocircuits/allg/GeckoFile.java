...
/**
 * @author anstupar This class is for handling all extra files that are used inside Gecko e.g. extra
 *     files for java block, loss model files, (in the future) nonlinear characteristic files
 */
public final class GeckoFile {
  ...
  /**
   * the pathname of this file relative to the path of the model files (.ipes) this file is
   * associated with
   */
  private String _relativePath;
  /**
   * the file separator on the OS used to add this file to the model; used for checking whether path
   * stored in file differs from current OS
   */
  private String _separator;
  /** byte array which contains the file contents if they have been read from ipes file */
  private byte[] _fileContents;
  /**
   * we need a unique identifer to save to ipes file when different elements point at a Gecko file
   */
  ...
  public GeckoFile(final File file, final StorageType storageType, final String modelFileName)
    ...
    if (!_file.exists()) {
      throw new FileNotFoundException(
          "GeckoFile constructor: Supplied File with path:"
    ...
    try {
      ...
      final String errorMessage =
          e.toString()
              + "GeckoFile constructor: could not get canonical "
   ...
   *
   * @param allComponents this parameter could probably be removed in the future. At the moment, I
   *     think this is the only possiblity to get the old GeckoFile stuff running in the new
   *     Software release.
   * @throws FileNotFoundException
   */
  public GeckoFile(final TokenMap tokenMap, final List<AbstractCircuitSheetComponent> allComponents)
      throws FileNotFoundException {
    _separator = tokenMap.readDataLine("fileSep", _separator);
    _hash = tokenMap.readDataLine("hashValue", -1);

    setStorageStrategy(
        StorageType.fromOrdinal(
            tokenMap.readDataLine("isExternal", StorageType.EXTERNAL.ordinal())));
    ...
    // Windows file names use backslashes - giving them "as is" to the replaceAll function causes an
    // exception becuase regex uses backslashes as an escape character
    ...
    final String absPathToUse =
        _absolutePath.replaceAll(separatorSequenceToReplace, separatorSequenceToInsert);
    final String relPathToUse =
        _relativePath.replaceAll(separatorSequenceToReplace, separatorSequenceToInsert);
    final int lastSeparatorIndex = GlobalFilePathes.DATNAM.lastIndexOf(File.separator);
    // now first try the relative path
    if (lastSeparatorIndex >= 0) {
      _file =
          new File(
              GlobalFilePathes.DATNAM.substring(0, lastSeparatorIndex)
                  + File.separator
                  + relPathToUse);
    } else {
    ...
    if (!_file.exists()) {
      ...
      if (_file.exists()) {
        ...
        // to keep things consistent (perhaps we changed file separator), set the stored
        // _absolutePath field to the used absolute path
    ...
    if (!(_file.exists()) && _storageStrategy.getStorageType() == StorageType.EXTERNAL) {
      throw new FileNotFoundException(
          "Error: Specified external file with relative path: "
              + relPathToUse
              + "\n and absolute path:"
              + absPathToUse
              + "\n NOT FOUND!");
  ...
  private String getRelativePath(final String absFilePath, final String givenAbsPath) {
    ...
    // if absModelPath is "Untitled" it is an unsaved file -> save with respect to current user
    // directory
    ...
    final String[] toSplit =
        absModelPath.split(
            regexCharacter); // relative file name is relative to the model (.ipes) path
    return findRelativePath(fromSplit, toSplit, absFilePath);
  }

  private static String findRelativePath(
      final String[] fromSplit, final String[] toSplit, final String absFilePath) {
    ...
    if ((fromSplit.length >= toSplit.length) && (common == toSplit.length)) {
      ...
      // if there is nothing common, there is no way to construct a relative path - the relative
      // path is then absolute
      if (common == 0) {
        return absFilePath;
      } else {
  ...
  private byte[] readFileIntoMemory() {
    ...
    try {
      ...
      final String errorMessage =
          "GeckoFile read in file contents: cannot find file. " + e.toString();
      Logger.getLogger(GeckoFile.class.getName()).log(Level.SEVERE, errorMessage);
    }
  ...
  /**
   * generate a unique identifier for this GeckoFile object which can be saved to file
   *
   * @return
   */
  private long generateHashCode() {
    ...
    code =
        _absolutePath.hashCode()
            + 2 * _relativePath.hashCode()
  ...
  /**
   * function which sets a particular model block as a user of this file CAREFUL - probably should
   * be called along with removeUser when any block is renamed
   *
   * @param blockID
   */
  ...
  /**
   * function which removes a particular model block from the user list of this file CAREFUL -
   * probably should be called along with removeUser when any block is renamed
   *
   * @param blockID
   */
  ...
  /**
   * get the file contents as a byte array CAREFUL - not a copy like above - editing return variable
   * edits the _fileContents field in this object (only applies to files stored internally)
   *
   * @return
   */
  ...
  /**
   * return an input stream for this file - pointing either to the actual file, or the internally
   * stored contents
   */
  ...
  /**
   * function to recompute relative path with respect to .ipes file - e.g. if file is "saved as" to
   * another location or if the file is saved for the first time
   *
   * @param absModelPath
   */
  ...
  /**
   * Get the last stored time stamp for file modification. This does not update the time stamp -
   * only returns the value from the last time it was updated.
   *
  ...
  private class ExternalStrategy extends AbstractStorageStrategy {
    ...
    void switchToNewType(final byte[] originalContents) throws FileNotFoundException {
      ...
      if (result == null
          || !(new File(result)).exists()) { // cancel pressed or external file is not existing!
