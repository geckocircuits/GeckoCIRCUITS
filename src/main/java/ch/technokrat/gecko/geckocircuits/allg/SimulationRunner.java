...
public final class SimulationRunner {
  ...
  public void startCalculation(boolean createNewSimThread, SolverSettings solverSettings)
      throws Exception {
    ...
    simKern.initialisiereCONTROLatSimulationStart(
        dtLoc); // wird nicht gemacht, wenn 'Continue' aktiviert wird
  ...
  void continueCalculation(final boolean createNewSimThread, final SolverSettings solverSettings)
      throws Exception {
    ...
    if (tAktuell >= tEND) { // naechstes Simulations-Fenster wird geoeffnet
      ...
      simKern.setScopeMenuesStartStop();
      simKern
          .setInitialConditionsFromContinue(); // LK_Matrix mit den letztberechneten Werten fuellen
  ...
  public void initSim(double dtLoc, double tEND) {
    ...
    simKern.initSimulation(
        ...
        tAktuell,
        tEND,
        _fenster._solverSettings._tPAUSE.getValue(),
        getAnfangsbedVomDialogfenster,
        nlContainer,
        false);
    ...
    for (AbstractBlockInterface block : _se.getElementCONTROL()) {
      if (block instanceof ReglerOSZI) {
        ((ReglerOSZI) block)
            .setSimulationTimeBoundaries(SimulationsKern.tSTART, SimulationsKern.tEND);
      }
    }
  }
  ...
  public void initSim() {
    this.initSim(
        _fenster._solverSettings.dt.getValue(), _fenster._solverSettings._tDURATION.getValue());
  }
      ...
      try {
        ...
        try {
          ...
          throw new OutOfMemoryError(
              "Could not allocate enough java RAM memory for the simulation!");
        } finally {
          if (!_fenster._solverSettings.inPreCalculationMode) {
            endRun();
          } else {
        ...
        GeckoSim._win._simRunner.simKern._simulationStatus =
            SimulationsKern.SimulationStatus.FINISHED;
        GeckoSim._win.jtfStatus.setText("Simulation aborted.");
        if (!_runWithoutThread) {
          error.printStackTrace();
          JOptionPane.showMessageDialog(
              null, error.getMessage(), "Severe error!", JOptionPane.ERROR_MESSAGE);
        } else {
          throw new RuntimeException(error);
        }
      }
  ...
  public void external_init(double tEnd) {
    ...
    nlContainer = NetListContainer.fabricStartSimulation(_se, simKern);
    simKern.initSimulation(
        _fenster._solverSettings.dt.getValue(),
        tSTART,
        tAktuell,
        tEnd,
        _fenster._solverSettings._tPAUSE.getValue(),
