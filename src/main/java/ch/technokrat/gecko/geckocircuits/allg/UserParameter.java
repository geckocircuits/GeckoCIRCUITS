...
public final class UserParameter<T> {
    ...
    try {
      ...
      System.err.println(
          "could not find domain "
              + " "
              + _parameterableObject.getSimulationDomain()
              + " "
              + _parameterableObject.getStringID()
              + " from component\n"
              + " "
  ...
  public T getValue() {
    if (!_nameOpt.isEmpty() && _value.getValue() instanceof Number) {
      Double returnValue =
          (Double) Fenster.optimizerParameterData.getNumberFromNameWOException(_nameOpt);
  ...
  public void readFromTokenMap(final TokenMap tokenMap) {
    ...
    if (tokenMap.containsToken(_identifier)) {
      ...
      if (oldValue instanceof ControlSourceType) {
        final ControlSourceType newValue =
            ControlSourceType.getFromID(
                tokenMap.readDataLine(
                    _identifier, ControlSourceType.QUELLE_RECHTECK.getOldGeckoID()));
        _value.setValueWithoutUndo((T) newValue);
        return;
      }

      if (oldValue instanceof SSAShape) {
        final SSAShape newValue =
            SSAShape.getFromOrdinal(
                tokenMap.readDataLine(_identifier, SSAShape.RECTANGLE.ordinal()));
        _value.setValueWithoutUndo((T) newValue);
        return;
      }

      if (oldValue instanceof CircuitSourceType) {
        final CircuitSourceType newValue =
            CircuitSourceType.getFromID(
                tokenMap.readDataLine(_identifier, SourceType.QUELLE_DC_NEW));
        _value.setValueWithoutUndo((T) newValue);
        return;
      }
      ...
      if (oldValue instanceof Color) {
        final Color newValue =
            new Color(tokenMap.readDataLine(_identifier, ((Color) _value.getValue()).getRGB()));
        _value.setValueWithoutUndo((T) newValue);
        return;
      }
  ...
  public static class Builder<T> {
    ...
    public UserParameter.Builder showWhenEnumValueIsSet(
        UserParameter<? extends Enum> enumParameter, Enum enumValue) {
    ...
    public UserParameter.Builder arrayIndex(
        final AbstractBlockInterface parameterBlock, final int index) {
      _index = index;
    ...
    /**
     * this method can be used for backwards-compatiblity with old names. Reason: Existing
     * GeckoSCRIPT-Code may rely on old "short names"
     *
     * @param alternativeName
     * @param additionalAlternativeNames
     * @return
     */
    public UserParameter.Builder addAlternativeShortName(
        final String... additionalAlternativeNames) {
