...
public class DialogSimParameter extends JDialog implements ActionListener {
  ...
  private JPanel getIntegrationPanel() {
    ...
    jtx.setText(
        "Backward-Euler: Very stable, with some numeric damping\n"
    ...
    jpTxt.setBorder(
        BorderFactory.createTitledBorder(
            BorderFactory.createEtchedBorder(), "Info", TitledBorder.LEFT, TitledBorder.TOP));
  ...
  private JPanel getSimParameterPanel() {
    ...
    jpTxt.setBorder(
        BorderFactory.createTitledBorder(
            BorderFactory.createEtchedBorder(), "t_BR", TitledBorder.LEFT, TitledBorder.TOP));
    //
    JTextArea jtx = new JTextArea();
    jtx.setForeground(GlobalColors.LAB_COLOR_DIALOG_1);
    jtx.setText(
  ...
  private JPanel getSteadyStatePanel() {
    ...
    // tf[5].setNumberToField(maxErrorNewton);       tf[5].setColumns(cols);
    ...
    jtx.setForeground(GlobalColors.LAB_COLOR_DIALOG_1);
    jtx.setText(
        "T_pre, dt_pre: The simulation is executed with the simulation time of T_pre and the stepwidth"
  ...
  public void actionPerformed(ActionEvent ae) {
    if (ae.getActionCommand().equals("Cancel")) {
      ...
      // try { maxIterationsNewton= (int)tf[4].getNumberFromField(); } catch (Exception nfe) {
      // allesOK=false; }
