...
public class DatenSpeicher implements Serializable {
  ...
  public final List<AbstractCircuitSheetComponent> _allSheetComponents =
  ...
  public int _fensterWidth = -1,
      _fensterHeight = -1; // Speicherung der individuellen Fenstergroesse
  // ------------------
  ...
  // NULL-Symbol ("") fuer Labels --> notwendig, weil " " als Separator bei der ASCII-Speicherung
  // genutzt wird, und "" schwer zum Wiederherstellen ist
  ...
  public DatenSpeicher(
      ...
      int uniqueFileId,
      SimulationAccess scripter,
      GeckoFileManager fileMgr,
      SchematischeEingabe2 se,
  ...
  public DatenSpeicher(
      String[] ascii, final boolean isBackupRead, OptimizerParameterData optimizer) {
    //        if(!isBackupRead) {
  ...
  public String exportASCII() {
    ...
    asc.append("\nANSICHT_SHOW_LK_PARAMETER ")
        .append(SchematischeEingabe2._lkDisplayMode.showParameter);
    asc.append("\nANSICHT_SHOW_LK_FLOWDIR ")
        .append(SchematischeEingabe2._lkDisplayMode.showFlowSymbol);
    asc.append("\nANSICHT_SHOW_LK_TEXTLINIE ")
        .append(SchematischeEingabe2._lkDisplayMode.showTextLine);
    asc.append("\nANSICHT_SHOW_THERM_NAME ")
        .append(SchematischeEingabe2._thermDisplayMode.showName);
    asc.append("\nANSICHT_SHOW_THERM_PARAMETER ")
        .append(SchematischeEingabe2._thermDisplayMode.showParameter);
    asc.append("\nANSICHT_SHOW_THERM_FLOWDIR ")
        .append(SchematischeEingabe2._thermDisplayMode.showFlowSymbol);
    asc.append("\nANSICHT_SHOW_THERM_TEXTLINIE ")
        .append(SchematischeEingabe2._thermDisplayMode.showTextLine);
    asc.append("\nANSICHT_SHOW_CONTROL_NAME ")
        .append(SchematischeEingabe2._controlDisplayMode.showName);
    asc.append("\nANSICHT_SHOW_CONTROL_PARAMETER ")
        .append(SchematischeEingabe2._controlDisplayMode.showParameter);
    asc.append("\nANSICHT_SHOW_CONTROL_TEXTLINIE ")
        .append(SchematischeEingabe2._controlDisplayMode.showTextLine);
  ...
  private int importASCII(String[] ascii, final boolean isBackupRead) {
    ...
    if (tokenMap.containsToken("tDURATION")) {
      ...
      GlobalFilePathes.datnamAbsLoadIPES =
          nameString.substring(
              (new String("path ").length())); // wichtig, weil Pfadname Leerzeichen enthalten kann
      fontSize = tokenMap.readDataLine("fontSize", fontSize);
      final String fontString = tokenMap.getLineString("fontTyp", "fontTyp ");
      _fontTyp =
          fontString.substring(
              (new String("fontTyp ")).length()); // wichtig, falls FontName Leerzeichen enthaelt!
    ...
    if (tokenMap.containsToken("ANSICHT_SHOW_LK_NAME")) {
      SchematischeEingabe2._lkDisplayMode.showName =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_LK_NAME", SchematischeEingabe2._lkDisplayMode.showName);

      SchematischeEingabe2._lkDisplayMode.showParameter =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_LK_PARAMETER", SchematischeEingabe2._lkDisplayMode.showParameter);

      SchematischeEingabe2._lkDisplayMode.showFlowSymbol =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_LK_FLOWDIR", SchematischeEingabe2._lkDisplayMode.showFlowSymbol);

      SchematischeEingabe2._lkDisplayMode.showTextLine =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_LK_TEXTLINIE", SchematischeEingabe2._lkDisplayMode.showTextLine);

      SchematischeEingabe2._thermDisplayMode.showName =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_THERM_NAME", SchematischeEingabe2._thermDisplayMode.showName);

      SchematischeEingabe2._thermDisplayMode.showParameter =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_THERM_PARAMETER", SchematischeEingabe2._thermDisplayMode.showParameter);

      SchematischeEingabe2._thermDisplayMode.showFlowSymbol =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_THERM_FLOWDIR", SchematischeEingabe2._thermDisplayMode.showFlowSymbol);

      SchematischeEingabe2._thermDisplayMode.showTextLine =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_THERM_TEXTLINIE", SchematischeEingabe2._thermDisplayMode.showTextLine);
      SchematischeEingabe2._controlDisplayMode.showName =
          tokenMap.readDataLine(
              "ANSICHT_SHOW_CONTROL_NAME", SchematischeEingabe2._controlDisplayMode.showName);
    ...
    GeckoSim._win.vItemShowParCONTROL.setSelected(
        SchematischeEingabe2._controlDisplayMode.showParameter);
    GeckoSim._win.vItemShowTextLineCONTROL.setSelected(
        SchematischeEingabe2._controlDisplayMode.showTextLine);
    GeckoSim._win.vItemShowNameCONTROL.setSelected(
        SchematischeEingabe2._controlDisplayMode.showName);

    GeckoSim._win.vItemShowParTHERM.setSelected(
        SchematischeEingabe2._thermDisplayMode.showParameter);
    GeckoSim._win.vItemShowFlowTHERM.setSelected(
        SchematischeEingabe2._thermDisplayMode.showFlowSymbol);
    GeckoSim._win.vItemShowTextLineTHERM.setSelected(
        SchematischeEingabe2._thermDisplayMode.showTextLine);
    ...
    if (tokenMap.containsToken("<scripterCode>")) {
      ...
      _scripterDeclarations =
          tokenMap.createSubBlock("<scripterDeclarations>", "<\\scripterDeclarations>");
      if (tokenMap.containsToken("<extraScriptSourceFiles>")) {
        _scripterExtraFiles =
            tokenMap.createSubBlock("<extraScriptSourceFiles>", "<\\extraScriptSourceFiles>");
      }
    }
    ...
    for (TokenMap verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungLK");
        verbindungMap != null;
        verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungLK")) {
    ...
    for (TokenMap verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungCONTROL");
        verbindungMap != null;
        verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungCONTROL")) {
    ...
    for (TokenMap verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungTHERM");
        verbindungMap != null;
        verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungTHERM")) {
    ...
    for (TokenMap elementLKMap = tokenMap.getSpecialBlockTokenMap("e");
        elementLKMap != null;
        elementLKMap = tokenMap.getSpecialBlockTokenMap("e")) {
      int typ = elementLKMap.readDataLine("typ", -1);
      AbstractBlockInterface newBlock =
          AbstractTypeInfo.fabricFromFile(CircuitTyp.getFromIntNumber(typ), elementLKMap);
      _allSheetComponents.add(newBlock);
    }

    for (TokenMap thermTokenMap = tokenMap.getSpecialBlockTokenMap("eTH");
        thermTokenMap != null;
        thermTokenMap = tokenMap.getSpecialBlockTokenMap("eTH")) {
      int typ = thermTokenMap.readDataLine("typ", -1);
      _allSheetComponents.add(
          AbstractTypeInfo.fabricFromFile(CircuitTyp.getFromIntNumber(typ), thermTokenMap));
    }

    for (TokenMap controlTokenMap = tokenMap.getSpecialBlockTokenMap("c");
        controlTokenMap != null;
        controlTokenMap = tokenMap.getSpecialBlockTokenMap("c")) {
      int typ = controlTokenMap.readDataLine("typ", -1);
      try {
        if (typ
            == SpecialTyp.TEXTFIELD
                .getTypeNumber()) { // this is for backwards-compatiblity before v 1.62
          _allSheetComponents.add(
              AbstractTypeInfo.fabricFromFile(SpecialTyp.getFromIntNumber(typ), controlTokenMap));
        } else {
          _allSheetComponents.add(
              AbstractTypeInfo.fabricFromFile(ControlTyp.getFromIntNumber(typ), controlTokenMap));
        }
    ...
    for (TokenMap specialTokenMap = tokenMap.getSpecialBlockTokenMap("sp");
        specialTokenMap != null;
        specialTokenMap = tokenMap.getSpecialBlockTokenMap("sp")) {
      int typ = specialTokenMap.readDataLine("typ", -1);
      try {
        AbstractBlockInterface newBlock =
            AbstractTypeInfo.fabricFromFile(SpecialTyp.getFromIntNumber(typ), specialTokenMap);
        _allSheetComponents.add(newBlock);
    ...
    if (fileBlock != null) {
      ...
      for (TokenMap fileSubBlock = fileBlock.getBlockTokenMap("<GeckoFile>");
          fileSubBlock != null;
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              null, e.getMessage(), "ERROR: File not found", JOptionPane.ERROR_MESSAGE);
        }
      }
    }
  ...
  // datnamAbsolutIPES   ... neuer aktueller absoluter pfad+name der *.ipes-Datei, die die
  // Topologie/Schaltungssimulation enthaelt
  // datnamAbsLoadIPES   ... gespeicherter pfad+name der *.ipes-Datei, die die
  // Topologie/Schaltungssimulation enthaelt
  // datnamAbsLoadDETAIL ... gespeicherter pfad+name einer Datei, die Zusatzinfo enhaelt, zB.
  // Halbleiter-Info oder Ersatzmodelle wie TH_MODUL
  // return-String       ... neuer aktueller absoluter pfad+name der Datei mit den Zusatzinfos
  //
  @Deprecated
  public static String lokalisiereRelativenPfad(
      String datnamAbsolutIPES, String datnamAbsLoadDETAIL) {

    String datnamAbsLoadIPES = GlobalFilePathes.datnamAbsLoadIPES;
    // -------------------------
    // (1) Ist die Pfadstruktur unveraendert? Kann man den alten (gespeicherten) absoluten Pfad der
    // Zusatzdatei verwenden?
  ...
  /**
   * if the same model is imported several times, the unique identifiers are not unique, anymore.
   * Therefore, we shift all identifiers by a constant, as well as component references.
   */
