 ...
 *     <p>WARNING: Somebody told me, that it would also be possible to get the jarpath via: final
 *     String path = GetJarPath.class.getProtectionDomain().getCodeSource().getLocation().getPath();
 *     final String decodedPath = URLDecoder.decode(path, "UTF-8"); However, this does not work,
 *     when using GeckoCIRCUITS as applet, since the security manager does not allow this approach.
 *     Therefore, the way I do this is a bit more complicated.
 */
public class GetJarPath {
  ...
  /**
   * we moved the class to another package - therefore the whole getJarPath-Procedure needs to have
   * a class from the calling package as initialization. therefore private constructor.
   *
   * @param clazz
   */
  ...
  /**
   * This bit of code seems to occur in a lot of places, so I factor it out here. The commented out
   * try/catch seems unnecessary, but put it back in if it's needed.
   *
  ...
  */
  /**
   * i made this function private... it is only used from getJarPath, and returns a mixture between
   * URL and filepath... nobody outside should see and use this here!
   *
   * @return
   */
  private static String getJarPathInsideJAR() {
    ...
    try {
      path =
          URLDecoder.decode(
              _refToCallingPackage
                  .getResource(_refToCallingPackage.getSimpleName() + ".class")
                  .toString(),
              "UTF-8");
   ...
   *
   * @return path to the DIRECTORY of our own jar-class. If we run the program from the IDE, the
   *     corresponding path to the build-directory is returned.
   */
  ...
  /**
   * If initialized, returns _JARFilePath - this is the path to the GeckkoCIRCUITS.jar, including
   * the jar-File name. If started from within netbeans, an empty string is returned.
   *
   * @return path to the FILE of our own jar-class. If we run the program from the IDE, an empty
   *     string is returned.
   */
  ...
  public static URL getPathPICS() {
    ...
    try {
      pfadURL =
          GetJarPath.class.getResource(GetJarPath.class.getSimpleName() + ".class").toURI().toURL();
    } catch (Exception e) {
      e.printStackTrace();
    }
    return pfadURL;
