...
public final class Fenster extends JFrame
  ...
  private static final String UNTITLED =
      "Untitled"; // wird in der Fenster-Leiste angezeigt, wenn eine neue Datei gestartet wird
  public static String aktuellerDateiName =
      UNTITLED; // ohne Pfadangabe! - nur zur Anzeige in der Fensterleiste (so wie bei Windows
  // ueblich)
  private String dateiEndung = "ipes";
  // --------------------------------------
  //
  private JSplitPane split;
  public static int seaBREITE =
      110; // Breite der 'SchematischeEingabeAuswahl2()'-Komponente am rechten Rand
  ...
  public JCheckBoxMenuItem vItemShowParTHERM,
      vItemShowFlowTHERM,
      vItemShowNameTHERM,
  ...
  // private JMenuItem mItemGoSteadyState, mItemSaveState, mItemLoadState, mItemResetState,
  // mItemClearStates;
  // --------------------------------------
  // --------------------------------------
  final GeckoStatusBar jtfStatus =
      new GeckoStatusBar("Ready ...", this); // Status-Anzeige der Schaltungs-Simulation
  ...
  public Fenster() {
    ...
    this.setDefaultCloseOperation(
        JFrame.DO_NOTHING_ON_CLOSE); // damit man den 'QuitWithoutSaving'-Dialog hochfahren kann ...
    ...
    if (!IS_BRANDED && !Fenster.IS_APPLET) {
      if (jarPath != null) {
        ...
        GlobalFilePathes.DATNAM =
            jarFile.getParent() + System.getProperty("file.separator") + "unsavedFile.ipes";
      } else {
        GlobalFilePathes.DATNAM =
  ...
  private void baueGUI() {
    ...
    mItemRECENT_1 =
        new JMenuItem(getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_1, RECENT_FILE_SPACE));
    mItemRECENT_1.addActionListener(this);
    mItemRECENT_1.setActionCommand("RECENT_1");
    mItemRECENT_2 =
        new JMenuItem(getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_2, RECENT_FILE_SPACE));
    mItemRECENT_2.addActionListener(this);
    mItemRECENT_2.setActionCommand("RECENT_2");
    mItemRECENT_3 =
        new JMenuItem(getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_3, RECENT_FILE_SPACE));
    mItemRECENT_3.addActionListener(this);
    mItemRECENT_3.setActionCommand("RECENT_3");
    mItemRECENT_4 =
        new JMenuItem(getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_4, RECENT_FILE_SPACE));
    ...
    editMenu.add(
        mItemDelete); // mItemDelete.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0));
    ...
    menueGroesse.addActionListener(
        new ActionListener() {
  ...
  /**
   * Verarbeitung derKeyEvents fuer DELETE, weil das scheinbar nicht mit den Menu-Accelerators
   * funktionert --> (Java-Bug??)
   */
  private void processKeyEvents(KeyEvent ke) {
    ...
    if ((ke.getKeyChar() == KeyEvent.VK_DELETE)
        ...
        || (ke.getKeyChar() == 4)) // '4' entspricht CONTROL_D
    {
  ...
  private void aktualisierePropertiesRECENT(String datnam) {
    ...
    if (!datnam.equals("LANGUAGE_UPDATE")) {
      ...
      // --------------
      mItemRECENT_1.setText(
          getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_1, RECENT_FILE_SPACE));
      mItemRECENT_2.setText(
          getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_2, RECENT_FILE_SPACE));
      mItemRECENT_3.setText(
          getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_3, RECENT_FILE_SPACE));
      mItemRECENT_4.setText(
          getTextMenuItemRECENT(GlobalFilePathes.RECENT_CIRCUITS_4, RECENT_FILE_SPACE));
      // --------------
    }
  ...
  public void saveFileAs() {
    ...
    GeckoFileChooser dialog =
        GeckoFileChooser.createSaveFileChooser(
            ".ipes", "Circuit Simulation Files (*.ipes)", this, new File(GlobalFilePathes.DATNAM));
    ...
    speicherVorgangLaeuft = false;
    _se.resetModelModified(); // damit wird verhindert, dass 'QuitWithoutSaving'-Dialog aufgerufen
    // wird, obwohl Datei bereits gespeichert
  ...
  public void rawSaveFile(File file) {
    ...
    try {
      ...
      //            DeflaterOutputStream out1= new DeflaterOutputStream(new FileOutputStream(new
      // File(GlobalFilePathes.DATNAM)));
  ...
  public void saveFile() {
    ...
    if (aktuellerDateiName.equals(UNTITLED)) {
      GeckoFileChooser fileChooser =
          GeckoFileChooser.createSaveFileChooser(
              ".ipes", "Circuit Simulation Files (*.ipes)", this, null);
    ...
    try {
      ...
      // ------------
      speicherVorgangLaeuft = false;
      _se.resetModelModified(); // damit wird verhindert, dass 'QuitWithoutSaving'-Dialog
      // aufgerufen wird, obwohl Datei bereits gespeichert
      //
    } catch (Exception e) {
      speicherVorgangLaeuft = false;
  ...
  public void openFileDialog() {
    ...
    GeckoFileChooser fileChooser =
        GeckoFileChooser.createOpenFileChooser(
            ".ipes", "Circuit Simulation Files (*.ipes)", this, currentDirectory);
    if (fileChooser.getUserResult() == GeckoFileChooser.FileChooserResult.CANCEL) {
      return;
    }
    try {
  ...
  public void openFile(BufferedReader in) throws IOException {
    ...
    _se.resetCircuitSheetsForNewFile();
    optimizerParameterData.clearAndInitializeWithoutUndo(
        daten._optimizerNames, daten._optimizerData);
    ...
    _se.initAdditionalFiles(
        _se._circuitSheet.getAllElements().getClassFromContainer(AbstractBlockInterface.class));
  }
  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    try {
      ...
      if (befehl.equals("New")) {
        ...
        _se.setConnectorTestMode(false);
        if (Fenster.IS_APPLET) {
          DialogAppletExamples dialogAppletExamples =
        ...
        GeckoFileChooser fileChooser =
            GeckoFileChooser.createSaveFileChooser(
                ".jar", ".jar Simulation Applet (*.jar)", this, currentDirectory);
      ...
      } else if (befehl.equals("vItemShowNameLK")) {
        SchematischeEingabe2._lkDisplayMode.showName = vItemShowNameLK.getState();
        if ((!SchematischeEingabe2._lkDisplayMode.showName)
        ...
        _se._visibleCircuitSheet.repaint();
      } else if (befehl.equals("aliasingCommand")) {
        _se.setAntialiasing(aliasingCONTROL.isSelected());
        jtfStatus.setAliasing(aliasingCONTROL.isSelected());
      } else if (befehl.equals("vItemShowParLK")) {
        ...
        _se._visibleCircuitSheet.repaint();
      } else if (befehl.equals("vItemShowTextLineLK")) {
        SchematischeEingabe2._lkDisplayMode.showTextLine = vItemShowTextLineLK.getState();
        _se._visibleCircuitSheet.repaint();
      } else if (befehl.equals("vItemShowFlowLK")) {
        SchematischeEingabe2._lkDisplayMode.showFlowSymbol = vItemShowFlowLK.getState();
        _se._visibleCircuitSheet.repaint();
        // ---------------------------------------------------------
      } else if (befehl.equals("vItemShowNameCONTROL")) {
        SchematischeEingabe2._controlDisplayMode.showName = vItemShowNameCONTROL.getState();
        if ((!SchematischeEingabe2._controlDisplayMode.showName)
          ...
          SchematischeEingabe2._controlDisplayMode.showTextLine =
              vItemShowTextLineCONTROL.getState();
        }
        _se._circuitSheet.repaint();
      } else if (befehl.equals("vItemShowParCONTROL")) {
        ...
        _se._circuitSheet.repaint();
      } else if (befehl.equals("vItemShowParTHERM")) {
        ...
        _se._circuitSheet.repaint();
      } else if (befehl.equals("vItemShowTextLineTHERM")) {
        SchematischeEingabe2._thermDisplayMode.showTextLine = vItemShowTextLineTHERM.getState();
        _se._circuitSheet.repaint();
      } else if (befehl.equals("vItemShowFlowTHERM")) {
        SchematischeEingabe2._thermDisplayMode.showFlowSymbol = vItemShowFlowTHERM.getState();
        _se._circuitSheet.repaint();
      } else if (befehl.equals("setParameters")) {
        new DialogOptimizerParameterSettings(
            optimizerParameterData, _se.getBlockInterfaceComponents());
      } else if (befehl.equals("setOrder")) {
        NetzlisteAllg nlA =
            NetzlisteAllg.fabricNetzlistComplete(
                _se.getConnection(ConnectorType.CONTROL), _se.getElementCONTROL());
        NetzlisteCONTROL.getOptimizedList(nlA);
        new DialogControlOrderN(this, true, NetzlisteCONTROL.getOptimizedList(nlA));
      } else if (befehl.equals("mItemConnectorTest")) {
        boolean checkConnectors = mItemConnectorTest.getState();
        _se.setConnectorTestMode(checkConnectors);
      } else if (befehl.equals("mItemFind")) {
        new DialogFindInModel(this, false, _se);
      } else if (befehl.equals("mItemCheckModel")) {
        NetzlisteAllg nlA =
            NetzlisteAllg.fabricNetzlistComplete(
                _se.getConnection(ConnectorType.CONTROL), _se.getElementCONTROL());
        NetListLK nlL = _se.getNetzliste(ConnectorType.LK_AND_RELUCTANCE);
        new DialogControlCheck(this, true, NetzlisteCONTROL.FabricRunSimulation(nlA))
            .setVisible(true);
        ...
        du.setVisible(true);
      } else if (befehl.equals("remoteSettings")) {
        DialogRemotePort drp = new DialogRemotePort(this, false);
        drp.setVisible(true);
      } else if (befehl.equals("3Dtherm")) {
      } else if (befehl.equals("geckoScript")) {
        _scripter.makeVisible();
      } else if (befehl.equals("magnet")) {
      } else if (befehl.equals("3Delmag")) {
        System.out.println("Nicht implementiert");
      } else if (befehl.equals("optimize")) {
        System.out.println("Nicht implementiert");
      } else if (befehl.equals("About")) {
        doAboutDialog();
      } else if (befehl.equals("Feedback")) {
        new DialogFeedback(this);
      } else if (befehl.equals("Licensing")) {
        new DialogLicensing();
      } else if (befehl.equals("Update")) {
  ...
  /**
   * Set up the simulation menu. this gets called both at initial gui generation, and at a provide
   * license event. We look up the simulation menu item, and substitute it with a new JMenuItem
   * created here.
   */
  ...
  void copyFile(final File zipFile, final File newFile) {
    ...
    final ProgressMonitor progressMonitor =
        new ProgressMonitor(Fenster.this, "Saving applet...", "Please wait.", 0, 100);
    ...
    if (size < 200) {
      ...
      JOptionPane.showMessageDialog(
          ...
          "GeckoCIRCUITS had a problem to save the model file:"
              ...
              + "File size on disk is "
              + size
              + " Bytes. Please check your\n"
    ...
    if (openFileId == daten._uniqueFileId) {
      ...
      if (dateTimeTestOrig.lastModified() < autoBackupTimeTest.lastModified()) {
        ...
        int n =
            JOptionPane.showConfirmDialog(
                this,
                "The automatic backup file has a later modification date: "
                    + new Date(autoBackupTimeTest.lastModified())
                    + "\nthan the current model file, which was last modified on: "
                    + new Date(autoBackupTimeTest.lastModified())
                    + "\n"
                    + "Do you like to load the backup file "
                    + dateiName
                    + "?",
                "Warning",
                JOptionPane.YES_NO_OPTION);
  ...
  public static void importComponentsFromFile(
      final String fileName, final String importIntoSubCircuitName) throws FileNotFoundException {
  ...
  public static DatenSpeicher loadDatenSpeicherFromFile(
      String dateiName, boolean isAutoBackupFile, OptimizerParameterData optimizer)
  ...
  public static String getTextMenuItemRECENT(String txt, int txtSpace) {
    ...
    try {
      ...
      String erg =
          txt.substring(0, space) + " .. " + System.getProperty("file.separator") + " " + dat;
      return erg;
    } catch (Exception e) {
      return txt;
    }
  ...
  private static String[] getLinesArrayFromIpesFile(String dateiName) {
    ...
    try {
      ...
      // neue Version 'gezipt' -->
      try {
        InflaterInputStream in1 =
            new InflaterInputStream(new FileInputStream(GlobalFilePathes.DATNAM));
        BufferedReader in = new BufferedReader(new InputStreamReader(in1));
        Vector datVec = new Vector();
        String z = null;
        while ((z = in.readLine()) != null) {
