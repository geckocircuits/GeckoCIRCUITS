...
public class TechFormat implements Serializable {
  ...
  public static final String FORMAT_AUTO =
      "AUTO"; // automatische Anpassung an die Groesse der zu formatierenden Zahl
  //
  ...
  // ------------------------------
  private String[] abk =
      new String[] {"p", "n", "u", "m", "k", "M"}; // moegliche technische "Zahlen"eingaben
  ...
  // 2k --> 2000;  2k2 --> 2200;  2m --> 0.002;  2m2 --> 2.2e-3;  2u --> 2e-6;  2u4 --> 2.4e-6;  7n
  // --> 7e-9;  usw.
  //
  public double parseT(String s) {
    try {
      ...
      for (int i1 = 0; i1 < sc.length; i1++) {
        if ((sc[i1] != '0')
          ...
          throw new NumberFormatException(
              "Ungueltiges Zahlenformat in 'TechFormat'  --> unzulaessiges Zeichen");
        }
        if (((sc[i1] == '+') || (sc[i1] == '-')) && (i1 != 0)) {
          // '+' und '-' stehen nicht am Beginn --> stehen sie vor dem Exponenten? (waere OK) -->
          if ((sc[i1 - 1] != 'e') && (sc[i1 - 1] != 'E')) {
            throw new NumberFormatException(
                "Ungueltiges Zahlenformat in 'TechFormat'  --> '+' bzw '-' an unzulaessiger Stelle");
          }
        }
        if ((sc[i1] == 'e')
            || (sc[i1] == 'E')
      ...
      if ((sc[sc.length - 1] == 'e') || (sc[sc.length - 1] == 'E')) {
        throw new NumberFormatException(
            "Ungueltiges Zahlenformat in 'TechFormat'  --> kein Exponent definiert");
      }
      if (anzTechZeichen > 1) {
        throw new NumberFormatException(
            "Ungueltiges Zahlenformat in 'TechFormat'  --> mehr als ein 'Sonder'zeichen");
      }
      if (anzKomma > 1) {
        throw new NumberFormatException(
            "Ungueltiges Zahlenformat in 'TechFormat'  --> mehr als ein Komma");
      }
      //
      // Annahme: alle Falscheingaben wurden eliminiert - ab jetzt keine Exception mehr moeglich
      // -----------------------
      // (3) Zeichen sind OK, wir parsen entsprechend den technischen Eingaben definiert in abk[]
      // zB. -->
      // 2k --> 2000;  2k2 --> 2200;  2m --> 0.002;  2m2 --> 2.2e-3;  2u --> 2e-6;  2u4 --> 2.4e-6;
      // 7n --> 7e-9;  usw.
      ...
      throw new NumberFormatException(
          "Ungueltiges Zahlenformat in 'TechFormat'  --> unbekanntes Problem [9324ubf902]");
    }
  }
  ...
  public void testAusgabe2() {
    ...
    for (int i1 = 0; i1 < 999999; i1++) {
      ...
      if (ratio < 0.9999) {
        System.out.println(
            data + "\t\t" + this.formatT(data, TechFormat.FORMAT_AUTO) + "\t\tratio= " + ratio);
      }
    }
    // ----------------------------
  }
