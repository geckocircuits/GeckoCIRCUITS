...
public final class ReglerLimit extends RegelBlock implements ControlInputTwoTerminalStateable {
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerLimit.class, "LIMIT", I18nKeys.LIMITER);
  ...
  final UserParameter<Double> _minLimit =
      UserParameter.Builder.<Double>start("minLimit", -1.0)
          .longName(I18nKeys.LOWER_LIMIT)
          .shortName(MIN)
          .showInTextInfo(TextInfoType.SHOW_WHEN_NON_EXTERNAL)
          .arrayIndex(this, 0)
          .build();
  final UserParameter<Double> _maxLimit =
      UserParameter.Builder.<Double>start("maxLimit", 1.0)
  ...
  final UserParameter<Boolean> _isExternalSet =
    ...
    final ActionListener minMaxAction =
        new ActionListener() {
          ...
          public void actionPerformed(final ActionEvent event) {
            ...
            assert _calculator == null || _calculator instanceof LimitCalculatorExternal
                : "Calculator not known!";
          }
        };
  ...
  public void drawBlockRectangle(final Graphics2D graphics) {
    ...
    if (_isExternalSet.getValue()) {
      graphics.drawLine(
          xPos * dpix, (int) ((yPos + 1 / 2.0) * dpix), xPos * dpix, (yPos + Y_DRAW_OFFSET) * dpix);
      graphics.setColor(GlobalColors.farbeInBearbeitungCONTROL);
      graphics.drawString(
          MIN, (int) (dpix * xPos) + X_DRAW_OFFSET, (int) (dpix * (yPos + Y_STRING_DRAW_OFFSET)));
      graphics.drawString(
          MAX,
          (int) (dpix * xPos) + X_DRAW_OFFSET,
          (int) (dpix * (yPos + Y_STRING_DRAW_OFFSET + 1)));
