...
/**
 * This class solves a dynamic system, where the transfer function is given via a numerator and
 * denominator polynom. The implementation uses a trapezoidal integration rule. The numerator degree
 * is allowed to be larger by the degree 2, w.r.t. the denominator degree.
 *
 * @author andy
 */
public final class StateSpaceCalculator {
  ...
  /**
   * the "real" polynomial part, after doing the polynomial division, i.e. the leading polynomial
   * without any fraction (denominator)
   */
  ...
  static class StateVariables {
    ...
    /**
     * these three history values are used for the derivative calculations, when a numerator degree
     * is bigger than the denominator degree
     */
    double _xOLDOLD;
    ...
    Matrix _result;
    /**
     * parentHash is used to determine if this state is appropriate for the parent
     * stateSpaceCalcualtor.
     */
    ...
    StateVariables(final double initialXIn, final int degree, int parentHash) {
      _xNEW = initialXIn;
      _xOLD =
          _xNEW; // at the first step, I set the history values identical, since otherwise we will
  ...
  StateSpaceCalculator(
      final double deltaT, final double[] numeratorPolynom, final double[] denomPolynom) {
  ...
  public void calculateTimeStep(
      final double[][] xIN, final double deltaT, final double[][] outputSignal, final double time) {
    ...
    switch (_leadingPolynom.length) {
      case MAX_DEGREE_DIFF:
        resultValue +=
            _leadingPolynom[2]
                * (_stateVariables._xNEW - 2 * _stateVariables._xOLD + _stateVariables._xOLDOLD)
                / deltaT; // second derivative
      case 2:
        double toAdd =
            _leadingPolynom[1]
                * (_stateVariables._xNEW - _stateVariables._xOLD)
                / deltaT; // first derivative
        resultValue += toAdd;
      case 1:
