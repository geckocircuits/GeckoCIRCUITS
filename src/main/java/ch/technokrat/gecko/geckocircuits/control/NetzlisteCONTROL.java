...
public final class NetzlisteCONTROL {
  ...
  private List<Integer>
      potIndex; // contains the potential indices for potentials connected with a scope
  ...
  private String[]
      labelListeReglerKnoten; // index ist Knotennummer und String-Eintrag ist zugehoeriger Label
  public RegelBlock[] _orderedControlBlocks;
  public final Map<Integer, IndexConnection> _scopePotentialMap =
      new HashMap<Integer, IndexConnection>();
  public final Map<CircuitSheet, HashMap<String, IndexConnection>> _labelNodeMap =
      new HashMap<CircuitSheet, HashMap<String, IndexConnection>>();
  ...
  public final List<AbstractTerminal> _outPortsWithoutConnection =
      new ArrayList<AbstractTerminal>();
  ...
  private void connectPotentialLabels(final NetzlisteAllg nlA) {
    ...
    for (AbstractBlockInterface elem : nlA.getElemente()) {
      if (elem != null
          && elem.isCircuitEnabled() != Enabled.DISABLED
          && !(elem instanceof ReglerTERMINAL)) {
    ...
    for (int i1 = 0; i1 < elementsControl.size(); i1++) {
      ...
      // Endknoten:
      List<AbstractTerminal> endNodes =
          elementsControl.get(i1).YOUT; // x-Werte (ScematicEntry) der Endknoten
      ...
      for (int i = 0; i < reg.YOUT.size(); i++) {
        ...
        for (RegelBlock reg2 : elementsControl) {
          for (int j = 0; j < reg2.XIN.size(); j++) {
            if (((ControlTerminable) reg.YOUT.get(i)).getNodeNumber()
                == ((ControlTerminable) reg2.XIN.get(j)).getNodeNumber()) {
        ...
        if (!connectionDone) {
          ...
          _outPortsStringWithoutConnection.add(
              reg.getStringID() + " Port no. " + i + " label: " + labelString);
    ...
    for (RegelBlock reg : elementsControl) {
      for (int i = 0; i < reg.XIN.size(); i++) {
        if (reg.checkInputWithoutConnectionAndFillInput(i)) {
          _inPortsStringWithoutConnection.add(
              reg.getStringID()
                  + " Port no. "
                  + i
                  + " label: "
                  + reg.XIN.get(i).getLabelObject().getLabelString());
  ...
  /**
   * mux and demuxes could lead to control loops, which are not real loops, since the mux/demux just
   * forwards the signals. This block removes all muxes and demuxes, and corrects the input/output
   * indices of the remaining blocks.
   */
  private void removeMuxAndDemux() {
    Map<Integer, ArrayList<ReglerDemux>> demuxes =
        new LinkedHashMap<Integer, ArrayList<ReglerDemux>>();
            ...
            for (RegelBlock regelblock : elementsControl) {
              for (AbstractTerminal term : regelblock.XIN) {
                ...
                if (cTerm.getNodeNumber() == demuxOutputIndex) {
                  ...
                  ((ControlTerminable) demux.YOUT.get(i))
                      .setNodeNumber(10000 + randomIndex.nextInt());
                }
              }
            }
          }
  ...
  public void berechneZeitschritt(final double deltaT, final double time) {
    ...
    ////                    Logger.getLogger(NetzlisteCONTROL.class.getName()).log(Level.SEVERE,
    // null, ex);
    ////                }
    ////        }
    ////        try {
    ...
    // _sortedReglerWONonCalculatables[k].getStringID() + " " + i + " " + j);
    //                            }
    //                        }
    //                    }
    //
  ...
  // Wird erst in 'SimulationsKern' aufgerufen, und zwar dort, wo die Simulation wirklich gestartet
  // wird
  // ... damit wird vermieden, dass beim Aufbau der Regelung im SchematicEntry jedesmal, wenn ein
  // Element oder ein Parameter
  // geaendert wird, und die NetzlisteCONTROL neu gebaut wird, auch die (bei grossen
  // Regelstrukturen) sehr zeitintensive
  // Berechnung der Bauelement-Optimierung durchgefuehrt wird und das SchematicEntry entsprechend
  // traege auf den User reagiert
  //
  public void optimiereAbarbeitungsListe() {
    ...
    // _orderedControlBlocks = blockOrderOtimizerNew.getOptimierteAbarbeitungsListe().toArray(new
    // RegelBlock[0]);

    BlockOrderOptimizer3 blockOrderOtimizer3 = new BlockOrderOptimizer3(elementsControl);
    _orderedControlBlocks =
        blockOrderOtimizer3.getOptimierteAbarbeitungsListe().toArray(new RegelBlock[0]);
  ...
  public void createControlCalculators() {
    ...
    final List<AbstractControlCalculatable> _sortedControlBlocksTmp =
        new ArrayList<AbstractControlCalculatable>();
    ...
    for (int i = 0; i < _orderedControlBlocks.length; i++) {
      try {
        AbstractControlCalculatable calc =
            _orderedControlBlocks[i].getInternalControlCalculatableForSimulationStart();
        ...
        throw new IllegalArgumentException(
            "Control block in " + _orderedControlBlocks[i].getStringID() + ":\n" + ex.getMessage());
      }
    }
  ...
  private void setGlobalDatacontainer() {
    ...
    for (int scopeInd = 0; scopeInd < inputCoords.size(); scopeInd++) {
        ...
        for (int potGebInd = 0; potGebInd < potLab.length; potGebInd++) {
          ...
          if (potentialTerminals.contains(scopeTerminal)) {
            if (!potIndex.contains(potGebInd)) {
              _scopePotentialMap.put(
                  potGebInd, new IndexConnection(scopeIndices.get(scopeInd), inputInd));
      ...
      ((ReglerOSZI) (_orderedControlBlocks[scopeIndices.get(scopeInd)]))
          .setDataContainerIndices(dataIndex);
    }
  ...
  /**
   * write the selected scope data to the global data container. Be careful, we use an
   * "array-cache", so that we don't have to re-create the arrays, which is a problem for the
   * garbage-collection.
   *
   * @param time
   */
  private void writeData(final double time) {
    try {
      ...
      for (int i = 0; i < potIndexArray.length; i++) {
        scopeData[i] =
            (float)
                _allSortedCalculatables[potIndexArray[i][0]]._inputSignal[potIndexArray[i][1]][0];
      }
  ...
  /** Gives every Controlblock the chance to free its resources if the simulation is paused */
