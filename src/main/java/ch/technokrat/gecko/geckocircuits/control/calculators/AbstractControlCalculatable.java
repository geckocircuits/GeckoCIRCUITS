...
public abstract class AbstractControlCalculatable {
  ...
  public void setInputSignal(
      final int inputIndex, final AbstractControlCalculatable output, final int outputIndex)
      throws Exception {
  ...
  /**
   * check if input port has no connection. If this is the case, fill the input port variable with a
   * dummy double[].
   *
   * @param inputIndex
  ...
  /**
   * TearDownOnPause will by called if the Simulation is paused or finished. Intended to be
   * overwritten by subclasses to free resources if necessary.
   */
