...
/**
 * Contains only the graphic representation of the ControlBlock. Everything else should be delegated
 * to the JavaBlock-Class
 *
 * @author andreas
 */
public final class ReglerJavaFunction extends RegelBlock
    implements VariableTerminalNumber, SpecialNameVisible, GeckoFileable, Operationable {

  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerJavaFunction.class, "JAVA", I18nKeys.JAVA_FUNCTION);
  ...
  final UserParameter<Integer> _inputTerminalNumber =
      UserParameter.Builder.<Integer>start("anzXIN", 3)
  ...
  final UserParameter<Integer> _outputTerminalNumber =
      UserParameter.Builder.<Integer>start("anzYOUT", 2)
          .longName(I18nKeys.NO_OUTPUT_TERMINALS)
          .shortName("numberOutputTerminals")
          .arrayIndex(this, -1)
          .build();

  final UserParameter<Boolean> _showName =
      UserParameter.Builder.<Boolean>start("showName", true)
  ...
  */
  ...
  public ReglerJavaFunction() {
    ...
    _variableBusWidth._useMatrix.addActionListener(
        new ActionListener() {
    ...
    _inputTerminalNumber.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
  ...
  public List<OperationInterface> getOperationEnumInterfaces() {
    ...
    OperationInterface codeOperation =
        new OperationInterface("setSourceCode", I18nKeys.SOURCE_CODE) {
          ...
          public Object doOperation(Object parameterValue) {
            ...
            _javaBlock._javaBlockSource =
                new JavaBlockSource.Builder()
                    .sourceCode(code)
                    .importsCode(_javaBlock._javaBlockSource._importsCode)
                    .initCode(_javaBlock._javaBlockSource._initCode)
                    .variablesCode(_javaBlock._javaBlockSource._variablesCode)
                    .build();
            if (_codeWindow != null) {
    ...
    OperationInterface importsOperation =
        new OperationInterface("setImportCode", I18nKeys.IMPORTS_EXAMPLE) {
          ...
          public Object doOperation(Object parameterValue) {
            ...
            _javaBlock._javaBlockSource =
                new JavaBlockSource.Builder()
                    .sourceCode(_javaBlock._javaBlockSource._sourceCode)
                    .importsCode(code)
                    .initCode(_javaBlock._javaBlockSource._initCode)
                    .variablesCode(_javaBlock._javaBlockSource._variablesCode)
                    .build();
    ...
    OperationInterface initOperation =
        new OperationInterface("setInitCode", I18nKeys.INIT_CODE) {
    ...
    OperationInterface variablesOperation =
        new OperationInterface("setVariablesCode", I18nKeys.VARIABLES_CODE) {
  ...
  private class JavaBlockCalculator extends AbstractControlCalculatable
    ...
    if (_populateFileList) {
      ...
      if (fileMissing) {
        final String errorMessage =
            filesMissing + " additional source files missing in Java block " + getStringID();
  ...
  public void drawBlockRectangle(final Graphics2D graphics) {
    ...
    graphics.fillRect(xKlickMin, yKlickMin, xKlickMax - xKlickMin, yKlickMax - yKlickMin);

    graphics.setColor(origColor);

    graphics.drawRect(xKlickMin, yKlickMin, xKlickMax - xKlickMin, yKlickMax - yKlickMin);

    // Rote Dreiecke zum Klicken --> Aenderung der Terminal-Anzahl:
    graphics.setColor(Color.red);
    final int delta =
        THREE; // Abstand vom roten Dreieck vom SCOPE-Block (nach oben bzw. nach unten)
    final int xd0 = (dpix * xPos) - dpix,
        xd1 = (dpix * (xPos) + DIAMETER) - dpix,
        xd2 = (dpix * (xPos)) - DIAMETER - dpix;
    final int yp0 = (int) (dpix * (yPos - WIDTH - HEIGHT) - delta),
        yp1 = (int) (dpix * (yPos - WIDTH) - delta);
    ...
    final int xdOUT0 = (dpix * xPos) + dpix,
        xdOUT1 = (dpix * (xPos) + DIAMETER) + dpix,
        xdOUT2 = (dpix * (xPos)) - DIAMETER + dpix;
    final int ypOUT0 = (int) (dpix * (yPos - WIDTH - HEIGHT) - delta),
        ypOUT1 = (int) (dpix * (yPos - WIDTH) - delta);
    ...
    graphics.fillPolygon(
        new int[] {xdOUT0, xdOUT1, xdOUT2}, new int[] {ypOUT0, ypOUT1, ypOUT1}, THREE);
    graphics.fillPolygon(
        new int[] {xdOUT0, xdOUT1, xdOUT2}, new int[] {ymOUT0, ymOUT1, ymOUT1}, THREE);
    ...
    _inputTri._yKlickMinTerminalSUB =
        yp0; // oberes Dreieck  --> SUB / Reduktion der Terminal-Anzahl
    _inputTri._yKlickMaxTerminalSUB = yp1;
    _inputTri._yKlickMinTerminalADD =
        ym1; // unteres Dreieck --> ADD / Erhoehung der Terminal-Anzahl
    ...
    _outputTri._yKlickMinTerminalSUB =
        ypOUT0; // oberes Dreieck  --> SUB / Reduktion der Terminal-Anzahl
    _outputTri._yKlickMaxTerminalSUB = ypOUT1;
    _outputTri._yKlickMinTerminalADD =
        ymOUT1; // unteres Dreieck --> ADD / Erhoehung der Terminal-Anzahl
  ...
  public void copyAdditionalParameters(final AbstractBlockInterface originalBlock) {
    ...
    this._javaBlock._javaBlockSource =
        new JavaBlockSource.Builder()
            .sourceCode(other._javaBlock._javaBlockSource._sourceCode)
            .importsCode(other._javaBlock._javaBlockSource._importsCode)
            .initCode(other._javaBlock._javaBlockSource._initCode)
            .variablesCode(other._javaBlock._javaBlockSource._variablesCode)
            .build();
  }
  ...
  protected final Window openDialogWindow() {
    if (GeckoSim.compiler_toolsjar_missing) {
      // tools.jar ist nicht vorhanden --> der Compiler, der fuer den JAVA-Block notwendig ist,
      // fehlt -->
      // es wurde daher ein 'Dummy'-Block hochgefahren, und hier wird ein Warnungs-Dialog gezeigt
      // -->
      JOptionPane.showMessageDialog(
          null, "No tools.jar library found!", "Error", JOptionPane.ERROR_MESSAGE);
      return null;
    } else {
