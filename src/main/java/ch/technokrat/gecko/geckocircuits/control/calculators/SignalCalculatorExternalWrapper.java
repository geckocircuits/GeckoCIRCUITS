  ...
  public void berechneYOUT(final double deltaT) {
    ...
    if (_inputSignal[FREQUENCY_INDEX][0] < 0) {
      throw new RuntimeException(
          "Frequency value: " + _inputSignal[FREQUENCY_INDEX][0] + " is not valid!");
    }
    _wrapped.setFrequency(_inputSignal[1][0]);

    if (_time
        == deltaT) { // when external parameters are used, the initialization is done 1 timestep
      // later.
      // the reason is that, for e.g. for setting a proper phaseshift value, we first have to
      // calculate a
      // proper input signal.
