...
/**
 * CompileObject takes sourcecode and compiles it into a map, where the map gives a connection
 * between the java file name and the compiled JavaFileObject. CompileObject is similar to
 * "immutable", everything is done in the constructor, output messages and so on can therefore be
 * final.
 *
 * @author andreas
 */
public final class CompileObject extends AbstractCompileObject {
  ...
  private final Map<String, RamJavaFileObject> _nameFileMap =
      new HashMap<String, RamJavaFileObject>();
  ...
  private final DiagnosticCollector<JavaFileObject> _diagnostics =
  ...
  public CompileObject(
      ...
      final List<GeckoFile> additionalSourceFiles) {
    super();
      ...
      if (task.call()) {
        ...
        for (Entry<String, RamJavaFileObject> entry : _nameFileMap.entrySet()) {
          ...
          if (fileName.startsWith(_className)) {
            _classMap.put(
                fileName,
                new CompiledClassContainer(entry.getValue().getByteArray(), _sourceString));
          } else {
            for (GeckoFile externalSource : additionalSourceFiles) {
              if (externalSource.getName().equals(fileName + ".java")) {
                _classMap.put(
                    fileName,
                    new CompiledClassContainer(
                        entry.getValue().getByteArray(), externalSource.getContentsString()));
              }
            }
          }
        }
  ...
  public static JavaFileObject generateJavaSourceCode(
      final String sourceText, final String className) {
    return new SimpleJavaFileObjectImpl(
        RamJavaFileObject.toURI(className), JavaFileObject.Kind.SOURCE, sourceText);
  }
  ...
  private JavaCompiler findCompiler() {
    ...
    if (compiler == null) { // this fixes the java 1.7 compilation problem
      ...
      if (compiler == null) {
        ...
        JOptionPane.showMessageDialog(null, _compilerMessage, "Error!", JOptionPane.ERROR_MESSAGE);
  ...
  private List<String> setCompilerOptions() {
    ...
    // add classpath to running program to be able to access all gecko classes from the newly
    // created class
  ...
  private CompilationTask createCompilationTask(final List<GeckoFile> additionalSourceFiles) {
    ...
    final StandardJavaFileManager standardFileManager =
        compiler.getStandardFileManager(_diagnostics, null, null);
    ...
    return compiler.getTask(
        _compilerWriter, jfm, _diagnostics, _compilerOptions, null, filesToCompile);
  }
