...
public final class DivCalculator extends AbstractTwoInputsOneOutputCalculator {
  ...
  public void berechneYOUT(final double deltaT) {
    ...
    if (Double.isNaN(
        result)) { // here, we assume 0/0 = 0, since possibly no input is selected to this block!
      assert _inputSignal[0][0] == 0
          : "illegal division! " + _inputSignal[0][0] + "/" + _inputSignal[1][0];
      assert _inputSignal[1][0] == 0
          : "illegal division! " + _inputSignal[0][0] + "/" + _inputSignal[1][0];
      _outputSignal[0][0] = 0;
    }
