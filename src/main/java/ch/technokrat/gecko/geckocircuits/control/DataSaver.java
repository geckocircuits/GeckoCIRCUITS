...
public final class DataSaver extends Observable implements Observer {
  ...
  public void doManualSaveBlocking() {
    if (_data.getContainerStatus() == ContainerStatus.RUNNING) {
      throw new RuntimeException(
          "Error: blocking save can only be initiated when simulation has stopped.");
    }
  ...
  private String findFreeFile(final String origFile) {
    ...
    for (int counter = 0; counter < MAX_FILE_COUNTER; counter++) {
      final String newFileName =
          origFile.substring(0, underscoreIndex) + "_" + counter + origFile.substring(dotIndex);
  ...
  private void initSave(final AbstractDataContainer data) throws SignalMissingException {
    ...
    if (_regler._fileOverwrite.equals(ReglerSaveData.FileOverwrite.DO_NUMBERING)
        && new File(_regler._file.getValue()).exists()) {
  ...
  abstract class AbstractLinePrinter {
    ...
    public AbstractLinePrinter(
        final File file, final AbstractDataContainer data, final ReglerSaveData settings)
    ...
    /**
     * Sometimes, when renaming signals, the "connection" to the right index is lost. Here, we
     * search for a signal with the right name. If it is not found, we output a warning signal and
     * continue with the old index.
     *
     * @param _settings
     * @param _data
     */
      ...
      for (int i = 0; i < originalNames.size(); i++) {
        ...
        if (!origName.equals(dataSignalName)) {
          for (int j = 0; j < _data.getRowLength(); j++) {
            ...
            if (realName.equals(
                origName)) { // if the original name is found at another position, just
        ...
        if (newSignalIndex >= _data.getRowLength()
          ...
          throw new SignalMissingException(
              "The signal \""
                  + origName
                  + "\" is not available as scope input in the\n"
                  + "simulation model. The signal \""
                  + origName
                  + "\" was removed from\n"
                  + "the selection.");
        }
  ...
  private class TxtLinePrinter extends AbstractLinePrinter {
    ...
    TxtLinePrinter(final File file, final AbstractDataContainer data, final ReglerSaveData settings)
        throws SignalMissingException {
