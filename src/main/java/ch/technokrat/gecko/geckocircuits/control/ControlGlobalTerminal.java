...
public final class ControlGlobalTerminal extends RegelBlock implements GlobalTerminable {
  ...
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(
          ControlGlobalTerminal.class, "GLOBAL_CONTROL", I18nKeys.GLOBAL_CONTROL_TERMINAL);
  ...
  public int istAngeklickt(final int mouseX, final int mouseY) {
    if (((getSheetPosition().x * dpix - dpix / 2 <= mouseX)
        && mouseX <= (getSheetPosition().x * dpix + dpix / 2)
        && (getSheetPosition().y * dpix - dpix / 2 <= mouseY)
        && (mouseY <= getSheetPosition().y * dpix + dpix / 2))) {
      return 1;
    } else {
      return 0;
    }
  ...
  protected void paintIndividualComponent(final Graphics2D graphics) {
    ...
    graphics.fillRect(
        dpix * getSheetPosition().x - diameter / 2,
        dpix * getSheetPosition().y - diameter / 2,
        diameter + 1,
        diameter + 1);
    ...
    graphics.fillOval(
        -diameter / 2 + dpix * getSheetPosition().x,
        -diameter / 2 + dpix * getSheetPosition().y,
        diameter + 1,
        diameter + 1);
  }
