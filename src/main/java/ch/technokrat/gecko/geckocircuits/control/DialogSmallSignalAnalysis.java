...
public class DialogSmallSignalAnalysis extends DialogElementCONTROL<ReglerSmallSignalAnalysis> {
  ...
  public void baueGUI() {
    ...
    _jComboSignal.addActionListener(
        new ActionListener() {
          @Override
  ...
  public JPanel createParameterPanel(UserParameter<? extends Number>... parameters) {
    JPanel returnValue =
        super.createParameterPanel(
            parameters); // To change body of generated methods, choose Tools | Templates.
  ...
  private void updatePlot() {
    ...
    for (int i = 0; i < NN; i++) {
      dcs1.insertValuesAtEnd(
          new float[] {(float) ssv[cIndex], (float) measuredValues[cIndex]}, time);
      time += dt;
      cIndex++;
      if (cIndex == NN) {
        cIndex = 0;
    ...
    //            dcs1.insertValuesAtEnd(new float[]{(float) data[1][i], (float) data[2][i]},
    // data[0][i]);
    //        }
  ...
  void baueGuiIndividual() {
    ...
    jCheckBoxAddOutput.addActionListener(
        new ActionListener() {
          @Override
