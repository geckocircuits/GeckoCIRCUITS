...
public class QuasiPeakCalculator {
  ...
  public double quasiPeakDetector(
      final float[] inputSignalTD, final double startValue, final double dt) {
   ...
   * @param fourIndex
   * @param fn
   * @param fourierTransform careful: parameter values are overwritten / changed
   * @param baseFreq
   * @return
   */
  private float[] calculateBandSpectrum(
      final int fourIndex, final float[] fourierTransform, final double baseFreq) {
  ...
  static void calculateMinMaxEstimation(
      final float[] min, final float[] max, boolean useBlackman, final Cispr16Fft fft) {
