...
public abstract class RegelBlock extends AbstractBlockInterface implements Serializable {
  ...
  public static final double[] EMPTY_OUTPUT = new double[] {};
  ...
  protected int[] xFl = new int[3],
      yFl = new int[3]; // Symbol-Pfeil, der optional die Stromflussrichtung im Element anzeigt
  protected int xKlickMin,
      xKlickMax,
      yKlickMin,
      yKlickMax; // Bereich zum Anklicken, Werte werden Reglelblock-spezifisch definiert
      ...
      if (outputNames[i].equals(outputName)) {
        try {
          ...
          throw new NullPointerException(
              "Error getting output: " + getStringID() + " " + outputName);
        }
      }
    }
    throw new IllegalAccessException(
        "output " + outputName + " in block " + getStringID() + " does not exist");
  }
  ...
  /**
   * is overwritten in some components for additional functionality, e.g. the scope or the
   * java-block for changing the terminal numbers.
   *
   * @param mousePixelX
   * @param mousePixelY
   * @return
  ...
  public int istAngeklickt(final int mousePixelX, final int mousePixelY) {
    if (((xKlickMin <= mousePixelX)
        && (mousePixelX <= xKlickMax)
        && (yKlickMin <= mousePixelY)
        && (mousePixelY <= yKlickMax))) {
      return 1;
    } else {
      return 0;
    }
  ...
  void setInputSignal(final int inputIndex, final RegelBlock outputBlock, final int outputIndex) {
    try {
      ...
      throw new RuntimeException(
          "Error in control netlist: The output signal of control block\n"
              + outputBlock.getStringID()
              + ", no. "
              + outputIndex
              + " "
