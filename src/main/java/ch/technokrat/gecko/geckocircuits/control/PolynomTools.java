...
public final class PolynomTools {
   ...
   */
  public static List<Double> evaluateFactorizedExpression(
      final List<NComplex> coefficients, final double factor) {
    ...
    for (int i = 1; i < coefficients.size(); i++) {
      ...
      for (int k = tmpReal.length - 2; k >= 0; k--) {
        final double real =
            -tmpReal[k] * coefficients.get(i).getRe() + tmpImag[k] * coefficients.get(i).getIm();
        final double imag =
            -tmpImag[k] * coefficients.get(i).getRe() - tmpReal[k] * coefficients.get(i).getIm();
        polynomReal[k] = real;
        polynomImag[k] = imag;
   ...
   *
   * @param numerator [1,2,3]-> 1 + 2x + 3 x^2. The maximum degree value (3x^2) is not allowed to be
   *     zero
   * @param denominator [1,2,3]-> 1 + 2x + 3 x^2. The maximum degree value (3x^2) is not allowed to
   *     be zero
   * @param leading Polynomial part as RETURN value (given as argument, since the degree of the
   *     leading polynom is not a priori known
   * @return the remainder of the division
   */
  public static double[] polynomialDivision(
      final double[] numerator, final double[] denominator, final double[] leading) {
