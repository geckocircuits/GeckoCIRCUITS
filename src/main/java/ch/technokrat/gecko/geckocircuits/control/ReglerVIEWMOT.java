...
public final class ReglerVIEWMOT extends ReglerWithSingleReference {
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerVIEWMOT.class, "VIEWMOT", I18nKeys.MACHINE_INTERNAL);
  ...
  protected void importIndividual(final TokenMap tokenMap) {
    ...
    if (!parameterString[0].isEmpty()) {
      if (parameterString[1].isEmpty()) {
        parameterString[1] =
            parameterString[0].substring(0, parameterString[0].lastIndexOf(".")); // --> "M-DC.4"
      }

      if (parameterString[2].isEmpty() || parameterString[2].equals("0")) {
        parameterString[2] =
            parameterString[0].substring(parameterString[0].lastIndexOf(".") + 1); // --> "M-DC.4"
      }
    }
  }
