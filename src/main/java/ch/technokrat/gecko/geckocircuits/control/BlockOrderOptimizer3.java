...
public final class BlockOrderOptimizer3 {
  ...
  private void doOrdering() {
    ...
    // System.out.println("number of loops " + countNumberOfOutOfOrderLoops(_optimizedList) + " of "
    // + _optimizedList.size());
  ...
  /**
   * the number of "out-of-order" control blocks is the optimization criteria. The algorithm
   * searches for each control block (node) all successors. If any of the successors is an input of
   * the node, the loop counter is increased. Please note that a loop is not meant here as a real
   * control loop, but a loop means that a block is executed (in the ordered list) out of order,
   * which infact is a cyclic dependency.
   */
        ...
        if (elements.get(j).isDirectInputOfElement(elements.get(i))) {
          System.out.println(
              "out of order: "
                  + j
                  + " "
                  + elements.get(j).getElementControl().getStringID()
                  + " "
                  + elements.get(j).getPriority()
                  + " "
                  + i
                  + " "
                  + elements.get(i).getElementControl().getStringID()
                  + " "
                  + elements.get(i).getPriority());
          loop++;
        }
      }
    }
  ...
  private static void doConsistencyChecks(
      final List<RegelBlock> outList, final List<RegelBlock> allControlsInput) {
    if (outList.size() != allControlsInput.size()) {
      System.err.println(
          "unequal length of lists: input "
              + allControlsInput.size()
              + " output: "
              + outList.size());
      assert false;
    }
  ...
  private void iterateIntoNegativeDirection(
      Set<ControlOrderNode> nextLevelComponents, final int level) throws LoopDetectionException {
