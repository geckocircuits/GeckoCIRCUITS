...
public abstract class AbstractPotentialMeasurement extends RegelBlock
    implements ch.technokrat.gecko.geckocircuits.circuit.PotentialCoupable,
        ch.technokrat.gecko.geckocircuits.circuit.ComponentCoupable {
  ...
  protected final void addTextInfoParameters() {
    ...
    if (SchematischeEingabe2._controlDisplayMode.showParameter) {
      ...
      if ((label1.isEmpty() || label2.isEmpty()) && (coupledElement == null)) {
        parStr = "not defined";
  ...
  public final void checkComponentCompatibility(
      Object testObject, List<AbstractBlockInterface> insertList) {
    if (testObject instanceof DirectVoltageMeasurable) {
      DirectVoltageMeasurable voltMeas = (DirectVoltageMeasurable) testObject;
      for (AbstractBlockInterface comp :
          voltMeas.getDirectVoltageMeasurementComponents(_measureConnectorType)) {
        insertList.add(comp);
      }
    }
  }
