...
public class SmallSignalCalculator extends AbstractControlCalculatable
  ...
  public SmallSignalCalculator(
      final double amplitude,
      final double freqLow,
      final double freqHigh,
      final SSAShape signalShape,
      final int noInputs,
      final int noOutput,
      boolean addOutput) {
    ...
    switch (_signalType) {
      ...
      case EXTERNAL:
        break;
      case RECTANGLE:
        _signalTypeCalculator =
            new SignalCalculatorRectangle(1, 2 * _amplitude, _freqStart, 0, -_amplitude, 0.5);
  ...
  private void calculateFourier() {
    ...
    for (int n = 0; n <= _nMax; n++) {
      ...
      if (n == 0) { // DC-Gleichanteil
        ...
        magnitudeValues[n] =
            Math.sqrt(data_aVals[n] * data_aVals[n] + data_bVals[n] * data_bVals[n]);
      }
    }
  }
  ...
  private void calculateBode() {
    ...
    for (int p = 0; p < _bode[0].length; p++) {
      ...
      float magnitude =
          (float)
              Math.sqrt(
                  ss_bVals[harmonic] * ss_bVals[harmonic]
