...
public class PreviewDialogSine extends PreviewDialog {
  ...
  JComponent createComponent() {
    ...
    final int[] kordX =
        new int[] {
          x0, x0, x0 - p2, x0 + p2, x0, x0, x2, x1 + rand, x1 - p1 + rand, x1 - p1 + rand, x1 + rand
        };
    ...
    for (int i1 = 0; i1 < sinX.length; i1++) {
      ...
      sinY[i1] =
          y0
              - (offset
                  + (int)
                      (ac * Math.sin((i1 * 2 * Math.PI / sinX.length) - phase * Math.PI / 180.0)));
    }
        ...
        new JComponent() {
          public void paint(Graphics g) {
            ...
            g.drawLine(
                ...
                y0 - offset - 10,
                x0 + xPh,
                y0 + 10); // Markierungsline fuer Phasenverschiebung
            g.drawLine(
                x1, y0 - offset / 2, x1, y0 - offset - ac / 2); // senkrechte Linie bei (2*PI)
            g.drawPolyline(
                new int[] {x0 + xPh, x0 + xPh, x0 + xPh - p2, x0 + xPh},
                new int[] {y0 - offset - 2 * p1, y0 - offset, y0 - offset - p1, y0 - offset - p1},
                4); // Pfeilspitze senkrecht
            g.drawPolyline(
                new int[] {x0 + xPh, x0 + xPh, x0 + xPh - p2, x0 + xPh},
                new int[] {y0 + 2 * p1, y0, y0 + p1, y0 + p1},
                4); // Pfeilspitze senkrecht
            g.drawPolyline(
                new int[] {x0 + xMx, x0 + xMx + p2, x0 + xMx, x0 + xMx, x0 + xMx + p2, x0 + xMx},
                new int[] {
                  y0 - offset - ac + p1,
                  y0 - offset - ac + p1,
                  y0 - offset - ac,
                ...
                6); // AC-max-Linie (senkrecht) plus Pfeilspitzen
            g.setColor(Color.white);
            ...
            g.drawPolyline(
                ...
                7); // Phasenverschiebung: Pfeil waagrecht
