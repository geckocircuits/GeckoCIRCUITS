...
public final class ReglerSlidingDFT extends AbstractReglerVariableInputs {
  ...
  public static final ControlTypeInfo T_INFO =
      new ControlTypeInfo(ReglerSlidingDFT.class, "SDFT", I18nKeys.SDFT);

  final UserParameter<Double> _averageSpan =
      UserParameter.Builder.<Double>start("windowSpan", DEFAULT_WIN_SIZE)
  ...
  private boolean isUndoRequired(
      final List<FrequencyData> oldList, final List<FrequencyData> newList) {
  ...
  public final class FrequencyData {
    ...
    public FrequencyData(final double frequency, final OutputData outputData) {
      ...
      _frequency =
          UserParameter.Builder.<Double>start("freq_" + stepNumber, frequency)
    ...
    public boolean equals(final Object obj) {
      ...
      if (Double.doubleToLongBits(this._frequency.getValue())
          != Double.doubleToLongBits(other._frequency.getValue())) {
        return false;
      }
  ...
  protected void importIndividual(final TokenMap tokenMap) {
    ...
    final double[] outputFrequencies =
        tokenMap.readDataLine("outputFrequencies[]", new double[] {DEFAULT_FREQENCY});
    ...
    for (int i = 0; i < outputFrequencies.length; i++) {
      FrequencyData data =
          new FrequencyData(outputFrequencies[i], OutputData.getFromIntCode(outputTypes[i]));
  ...
  private class FrequencyDataUndoableEdit implements UndoableEdit {
    ...
    public FrequencyDataUndoableEdit(
        final List<FrequencyData> oldList, final List<FrequencyData> newList) {
      _oldList = oldList;
      _newList = newList;
    }
