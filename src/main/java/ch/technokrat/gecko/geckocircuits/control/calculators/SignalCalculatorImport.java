...
public final class SignalCalculatorImport extends AbstractSignalCalculator
    implements InitializableAtSimulationStart {
  // Periode des Signals --> wichtig, wenn Repeat==ON / tSigStart ... lokale Zeit innerhalb der
  // importierten Signal-Periode
  ...
  public void berechneYOUT(final double deltaT) {
    ...
    if (timePointer == 0) {
      ...
      _outputSignal[0][0] =
          value1 + (value2 - value1) * (_time - _tSigStart - time1) / (time2 - time1);
    } else {
    ...
    for (int i = 1; i < _xy[0].length; i++) {
      ...
      if (newTime <= previousTime) {
        throw new IllegalArgumentException(
            "The data table in the signal data file has an error:\n"
  ...
  private void calculateSigStartTimeEstimation() {
    while (_tSigStart + _signalDuration < _time) {
      _tSigStart += _signalDuration; // tLokal 'zeigt' immer auf den Zeitpunkt des Beginns einer
      // Signal-Periode
    }
  }
