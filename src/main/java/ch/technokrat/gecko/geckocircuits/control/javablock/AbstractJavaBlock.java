...
public abstract class AbstractJavaBlock {
  ...
  public void doCompilationIfRequired() throws IOException {
    ...
    String sourceString =
        SourceFileGenerator.createSourceCode(
            _javaBlockSource,
            className,
            _reglerJavaBlock.YOUT.size(),
  ...
  private boolean checkIfCompilationRequired() {
    ...
    final String newSourceString =
        SourceFileGenerator.createSourceCode(
            _javaBlockSource,
            _compileObject.getClassName(),
            _reglerJavaBlock.YOUT.size(),
            _reglerJavaBlock._variableBusWidth);
  ...
  void initialize(final double[][] inputSignals, final double[][] outputSignals) throws Exception {
  ...
  abstract void calculateYOUT(
      final double time,
      final double deltaT,
      final double[][] inputSignals,
  ...
  void exportIndividualCONTROL(final StringBuffer ascii) {
    try {
      ...
      Logger.getLogger(ReglerJavaFunction.class.getName())
          .log(Level.SEVERE, "could not find class.", ex);
    }
  ...
  void importIndividualCONTROL(final TokenMap tokenMap) {
    ...
    try {
      ...
      final Map<String, CompiledClassContainer> classMap =
          (Map<String, CompiledClassContainer>) oInStream.readObject();
  ...
  final void createNewJavaSourceCopy(AbstractJavaBlock returnValue) {
    returnValue._javaBlockSource =
        new JavaBlockSource.Builder()
            .sourceCode(this._javaBlockSource._sourceCode)
            .importsCode(this._javaBlockSource._importsCode)
            .initCode(this._javaBlockSource._initCode)
            .variablesCode(this._javaBlockSource._variablesCode)
            .build();
  }
}
