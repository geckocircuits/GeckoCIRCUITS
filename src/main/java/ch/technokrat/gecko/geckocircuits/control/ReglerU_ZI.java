...
public final class ReglerU_ZI extends RegelBlock {
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerU_ZI.class, "UZI", I18nKeys.U_ZI_DIAGRAM);
  ...
  public void setTerminalKnotenLabel(String q, int knotenIndex) {
    // ... wird von 'setLabelAnfangsKnoten()' in ElementCONTROL aufgerufen, immer wenn die Netzliste
    // aktualisiert wird
  ...
  public void drawBlockRectangle(final Graphics2D g) {
    ...
    int ds1 = 3, ds2 = 3;
    g.drawRect(
        (int) (dpix * (x - 0.4)), (int) (dpix * (y)) - ds2, ds1, (int) (dpix * (1)) + 2 * ds2);
    g.drawRect(
        (int) (dpix * (x - 0.4)), (int) (dpix * (y + 2)) - ds2, ds1, (int) (dpix * (1)) + 2 * ds2);
  }
  ...
  protected void importIndividual(final TokenMap tokenMap) {
    _uziDisplay.jSpinnerLength1.setValue(
        new Float(tokenMap.readDataLine("scale1", (Float) _uziDisplay.jSpinnerLength1.getValue())));
    _uziDisplay.jSpinnerLength2.setValue(
        new Float(tokenMap.readDataLine("scale2", (Float) _uziDisplay.jSpinnerLength2.getValue())));
    _uziDisplay.jSpinnerZ1.setValue(
        new Float(tokenMap.readDataLine("scale3", (Float) _uziDisplay.jSpinnerZ1.getValue())));
    _uziDisplay.jSpinnerAverage1.setValue(
        new Float(
            tokenMap.readDataLine("average1", (Float) _uziDisplay.jSpinnerAverage1.getValue())));
    _uziDisplay.jSpinnerAverage2.setValue(
        new Float(
            tokenMap.readDataLine("average2", (Float) _uziDisplay.jSpinnerAverage2.getValue())));
    _uziDisplay.jSpinnerZ2.setValue(
        new Float(tokenMap.readDataLine("average3", (Float) _uziDisplay.jSpinnerZ2.getValue())));
  }
}
