 ...
 * @author andreas
 */
public final class SparseMatrixCalculator extends AbstractControlCalculatable
  ...
  private double Tp0 = 1 / 25e3,
      Tp =
          Tp0; // initiale Annahme fuer die Schaltfrequenz (muss erst ermittelt werden und kann sich
  // aendern)
  ...
  public void berechneYOUT(final double deltaT) {
    double ur = _inputSignal[1][0],
        us = _inputSignal[2][0],
        ut = _inputSignal[3][0]; // Eingangs- bzw. Netzseitig
    double uNmax = _inputSignal[4][0],
        uOUTmax = _inputSignal[5][0],
        fOUT = _inputSignal[6][0]; // Amplituden und Ausgangsfrequenz
    double fDR = _inputSignal[0][0]; // Taktfrequenz fuer die Pulsperiode
    double phi2 =
        _inputSignal[7][
            0]; // output-side angle for creating uaOUT*, ubOUT*, ucOUT* for PMSM-control; reliable
    // alternative to fOUT
    ...
    if (neuePulsperiodeBeginnt) {
      ...
      calculateSwitchingTimes(
          ur, us, ut, uNmax, uOUTmax, fOUT,
          phi2); // Einschaltdauern dOUT=[d1..d5] und dIN=[da,db] werden berechnet
    ...
    setPulseWidths(
        ...
        dIN[0],
        dIN[1],
        (1.0 / Tp)); // alle 9 Schaltsignale werden generiert
    tLokal += deltaT;
  ...
  public void setPulseWidths(
      double d1, double d2, double d3, double d4, double d5, double da, double db, double fDR) {
  ...
  private void calculateSwitchingTimes(
      double ur, double us, double ut, double uNmax, double uOUTmax, double fOUT, double phi2) {
    switch (seIN) {
      case 1:
        dIN[0] = -ut / ur;
        break;
    ...
    double k =
        1
            / Math.sqrt(3)
            * uOUTmax
            / (uNmax * uNmax); // Ann.: Ideales 3-ph. Spannungsnetz am Eingang
    double phiOUT =
        2 * Math.PI * fOUT * _time - Math.PI / 2; // old version, does not work for PMSM-control
    if (fOUT <= 0) {
