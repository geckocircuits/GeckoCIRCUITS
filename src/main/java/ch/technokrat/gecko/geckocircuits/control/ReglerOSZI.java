 ...
 * @author andreas
 */
public final class ReglerOSZI extends RegelBlock
    implements VariableTerminalNumber, SpecialNameVisible {

  private static final int TERM_POS_X = -2;
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(
          ReglerOSZI.class, "SCOPE", I18nKeys.SCOPE, I18nKeys.COMPONENT_FOR_DATA_VISUALIZATION);

  final UserParameter<Integer> _inputTerminalNumber =
      UserParameter.Builder.<Integer>start("tn", 0)
   ...
   */
  private int _xKlickMinTerm,
      _xKlickMaxTerm,
      _yKlickMinTermADD,
      _yKlickMaxTermADD,
      _yKlickMinTermSUB,
      _yKlickMaxTermSUB;
  // fuer Zugriff auf SCOPE und die Moeglichkeit zum Update der Labels wenn Terminal-Anzahl
  // geaendert wird
  ...
  private static final int DELTA =
      3; // Abstand vom roten Dreieck vom SCOPE-Block (nach oben bzw. nach unten)
  ...
  private final ScopeSettings _scopeSettings =
      new ScopeSettings(); // initiale ScopeSettings definieren   ;
  ...
  ExternalSignal getSin(final double length, final double period, final double weight) {
    ...
    returnValue =
        new ExternalSignal("Sig." + this._testcounter, times, values, getSubCircuitPath());
    this._testcounter++;
    return returnValue;
  }
  ...
  ExternalSignal getCos(final double length, final double period, final double weight) {
    ...
    returnValue =
        new ExternalSignal("TestSignal" + this._testcounter, times, values, getSubCircuitPath());
    this._testcounter++;
    return returnValue;
  }
  // */
  ...
  public void initScope() {
    ...
    final DataContainerNullData nullData =
        new DataContainerNullData(_grafer.getManager().getAllScopeSignals());
  ...
  // wird von ReglerOSZI ueberschrieben, um als Zusatzfunktionalitaet die Terminal-Anzahl direkt
  // aendern zu koennen
  @Override
  public int istAngeklickt(final int mouseX, final int mouseY) {

    final boolean symbolClicked =
        xKlickMin <= mouseX && mouseX <= xKlickMax && yKlickMin <= mouseY && mouseY <= yKlickMax;
    final boolean upperTriClicked =
        _xKlickMinTerm <= mouseX
            && mouseX <= _xKlickMaxTerm
            && _yKlickMinTermSUB <= mouseY
            && mouseY <= _yKlickMaxTermSUB;
    final boolean lowerTriClicked =
        _xKlickMinTerm <= mouseX
            && mouseX <= _xKlickMaxTerm
            && _yKlickMinTermADD <= mouseY
            && mouseY <= _yKlickMaxTermADD;
    if (symbolClicked) {
  ...
  /**
   * the scope has many, many settings, Therefore, we just use the export function/save to file to
  ...
  public class ReglerOSZICalculator extends AbstractControlCalculatable
      implements NotCalculateableMarker, MemoryInitializable {
  ...
  public void drawBlockRectangle(final Graphics2D graphics) {
    ...
    graphics.fillRect(
        ...
        (int) (dpix * (y - WIDTH)),
        (int) (dpix * (2 * WIDTH)),
        (int) (dpix * (1.0 * _inputTerminalNumber.getValue())));
    ...
    graphics.drawRect(
        (int) (dpix * (x - WIDTH)) + INSIDE_RECT,
        (int) (dpix * (y - WIDTH)) + 2 * INSIDE_RECT,
        (int) (dpix * (2 * WIDTH)) - 2 * INSIDE_RECT,
        (int) (dpix * (1.0 * _inputTerminalNumber.getValue())) - 4 * INSIDE_RECT);

    // Rote Dreiecke zum Klicken --> Aenderung der Terminal-Anzahl:
    graphics.setColor(Color.red);

    final int[] triXCoords =
        new int[] {(int) (dpix * x), (int) (dpix * (x) + DIAMETER), (int) (dpix * (x)) - DIAMETER};
    final int yp0 = (int) (dpix * (y - WIDTH - HEIGHT) - DELTA),
        yp1 = (int) (dpix * (y - WIDTH) - DELTA);
    ...
    final int[] triYCoords =
        new int[] {
          (int) (dpix * (y - WIDTH - HEIGHT) - DELTA),
          (int) (dpix * (y - WIDTH) - DELTA),
          (int) (dpix * (y - WIDTH) - DELTA)
        };
  ...
  public double[] getChannelCharacteristics(final int channel, final double start, final double end)
      throws GeckoInvalidArgumentException {

    if (_waveformChar == null
        || !_waveformChar.isValid()
        || (start != _charStart)
        || (end != _charEnd)) {
  ...
  // for use with GeckoSCRIPT - Fourier analysis
  public double[][] doFourierAnalysis(
      final int channel, final double start, final double end, final int harmonics)
      throws Exception {

    if ((_fourier == null)
        || (start != _fourStart)
        || (end != _fourEnd)
        || (harmonics > _fourier[0][0].length + 1)) {
  ...
  public void doInitialCalculation() {
    ...
    scopeWrapper =
        new DataContainerScopeWrapper(
            NetzlisteCONTROL.globalData,
