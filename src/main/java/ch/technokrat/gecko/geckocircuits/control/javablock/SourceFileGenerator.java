...
/**
 * Utility class (only static methods) which constructs the javablock sourcecode, when import code,
 * class source code, ... is given. createSourceCode returns the String for the .java-file!
 *
 * @author andreas
 */
public final class SourceFileGenerator {
  ...
  private static int estimateTotalStringSize(final JavaBlockSource javaBlockSource) {
    final int tmpSize =
        javaBlockSource._importsCode.length()
            + javaBlockSource._sourceCode.length()
            + javaBlockSource._initCode.length()
            + javaBlockSource._variablesCode.length();
  ...
  public static String createSourceCode(
      final JavaBlockSource source,
      final String className,
      final int outTerminalNumber,
      final VariableBusWidth variableBusWidth) {
    try {

      final StringBuilder sourceStringBuilder = new StringBuilder(estimateTotalStringSize(source));

      appendClassImportsAndHeader(
          sourceStringBuilder,
          source._importsCode,
          className,
          variableBusWidth._useMatrix.getValue());
      appendVariablesCode(
          sourceStringBuilder, source._variablesCode, outTerminalNumber, variableBusWidth);
      appendInitCode(sourceStringBuilder, source._initCode);
      appendFunctionSourceCode(
          sourceStringBuilder, source._sourceCode, variableBusWidth._useMatrix.getValue());
  ...
  private static void appendClassImportsAndHeader(
      final StringBuilder sourceStringBuilder,
      final String importsCode,
      final String className,
      final boolean isMatrix)
      throws IOException {
    ...
    if (isMatrix) {
      sourceStringBuilder.append(
          " implements java.io.Serializable, ch.technokrat.gecko.ControlCalculatableMatrix { \n");
    } else {
      sourceStringBuilder.append(
          " implements java.io.Serializable, ch.technokrat.gecko.ControlCalculatable { \n");
    }
  }

  private static void appendVariablesCode(
      final StringBuilder sourceStringBuilder,
      final String variablesCode,
      final int outTerminalNumber,
      final VariableBusWidth variableBusWidth)
      throws IOException {
    ...
    if (variableBusWidth._useMatrix.getValue()) {
      if (variableBusWidth._fixedOutputBusEnabled.getValue()) {
        ...
        sourceStringBuilder.append(
            "][" + variableBusWidth._fixedOutputBusWidth.getValue() + "];\n");
  ...
  private static void appendInitCode(final StringBuilder sourceStringBuilder, final String initCode)
      throws IOException {
    sourceStringBuilder.append("\n\t@Override\n");
  ...
  private static void appendFunctionSourceCode(
      final StringBuilder sourceStringBuilder, final String sourceCode, final boolean isMatrix)
      throws IOException {
    sourceStringBuilder.append("}\n\n    @Override\n");
