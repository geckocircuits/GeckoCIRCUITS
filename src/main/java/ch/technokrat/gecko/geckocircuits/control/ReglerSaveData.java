...
public final class ReglerSaveData extends RegelBlock implements Operationable {
  ...
  final UserParameter<Boolean> _printHeader =
      UserParameter.Builder.<Boolean>start("printHeader", true)
          .longName(I18nKeys.IF_TRUE_PRINT_HEADER)
          .shortName("printHeader")
          .arrayIndex(this, -1)
          .build();

  final UserParameter<Boolean> _transposeData =
      UserParameter.Builder.<Boolean>start("transposeData", false)
          .longName(I18nKeys.IF_TRUE_TRANSPOSE_DATA)
          .shortName("transposeData")
          .arrayIndex(this, -1)
          .build();

  final UserParameter<Integer> _skipDataPoints =
      UserParameter.Builder.<Integer>start("skipDataPoints", 1)
          .longName(I18nKeys.SKIP_DATA_POINTS)
          .shortName("skipDataPoints")
          .arrayIndex(this, -1)
          .build();

  final UserParameter<Integer> _significDigits =
      UserParameter.Builder.<Integer>start("significantDigits", DEFAULT_DIGITS)
          .longName(I18nKeys.SIGNIFICANT_DIGITS)
          .shortName("significantDigits")
          .arrayIndex(this, -1)
          .build();

  final UserParameter<String> _file =
      UserParameter.Builder.<String>start("filename", findInitialFile())
  ...
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerSaveData.class, "DataExport", I18nKeys.DATA_EXPORT_TO_FILE);
  ...
  protected void exportAsciiIndividual(final StringBuffer ascii) {
    DatenSpeicher.appendAsString(
        ascii.append("\nselectedSignalNames"),
        _selectedSignalNames.toArray(new String[_selectedSignalNames.size()]));
  ...
  protected void importIndividual(final TokenMap tokenMap) {
    _itemSeparator =
        TextSeparator.getFromOrdinal(
            tokenMap.readDataLine("itemSeparator", _itemSeparator.ordinal()));
    _headerSymbol =
        HeaderSymbol.getFromOrdinal(tokenMap.readDataLine("headerSymbol", _headerSymbol.ordinal()));
    _selectedSignalNames.clear();
    _selectedSignalNames.addAll(
        Arrays.asList(tokenMap.readDataLine("selectedSignalNames[]", new String[0])));
    ...
    _fileOverwrite =
        FileOverwrite.getFromOrdinal(
            tokenMap.readDataLine("fileOverwrite", _fileOverwrite.ordinal()));
  }
  ...
  public List<OperationInterface> getOperationEnumInterfaces() {
    ...
    returnValue.add(
        new OperationInterface("doSaveFile", I18nKeys.SAVE_DATA_DOC) {
          @Override
    ...
    returnValue.add(
        new OperationInterface("doSaveFileBlocking", I18nKeys.SAVE_DATA_DOC_BLOCKING) {
          @Override
          public Object doOperation(final Object parameterValue) {
    ...
    returnValue.add(
        new OperationInterface("setOutputFileName", I18nKeys.SAVE_DATA_DOC_BLOCKING) {
          @Override
          public Object doOperation(final Object parameterValue) {
