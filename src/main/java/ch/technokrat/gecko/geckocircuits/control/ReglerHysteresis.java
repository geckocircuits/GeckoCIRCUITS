...
public final class ReglerHysteresis extends RegelBlock implements ControlInputTwoTerminalStateable {
  ...
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerHysteresis.class, "HYS", I18nKeys.HYSTERESIS);

  public final UserParameter<Double> _hysteresisThreshold =
      UserParameter.Builder.<Double>start("h", DEF_HYS_THRES)
          .longName(I18nKeys.HYSTERESIS_THRESHOLD)
          .shortName("h")
          .showInTextInfo(TextInfoType.SHOW_WHEN_NON_EXTERNAL)
          .arrayIndex(this, 0)
          .build();
  public final UserParameter<Boolean> _useExternal =
  ...
  public ReglerHysteresis() {
    ...
    _hysteresisThreshold.addActionListener(
        new ActionListener() {

          @Override
          public void actionPerformed(final ActionEvent event) {
            if (_calculator instanceof HysteresisCalculatorInternal) {
              ((HysteresisCalculatorInternal) _calculator)
                  .setHValue(_hysteresisThreshold.getValue());
              return;
            }
            assert _calculator == null || _calculator instanceof HysteresisCalculatorExternal
                : "Calculator not know!";
          }
        });
  }
  ...
  public void drawBlockRectangle(final Graphics2D graphics) {
    ...
    if (_useExternal.getValue()) {
      graphics.drawLine(
          xPos * dpix, (int) ((yPos + 1 / 2.0) * dpix), xPos * dpix, (yPos + 2) * dpix);
      // int dy = (int) (SchematischeEingabe2.foCONTROL.getStringBounds("xxx", frc).getHeight() *
      // 0.25);
