...
public final class ReglerFromEXTERNAL extends RegelBlockSimulink implements VariableTerminalNumber {
  ...
  public static final ControlTypeInfo tinfo =
      new ControlTypeInfo(ReglerFromEXTERNAL.class, "FromEXT", I18nKeys.IMPORT_DATA_FROM_SIMULINK);
    ...
    if (StartupWindow.testDialogOpenSourceVersion("Simulink interface")) {
      ...
      return new AbstractControlCalculatable(0, _terminalNumber) {
        ...
        public void berechneYOUT(final double deltaT) {
          // 'parameter[]' muss vom externen Programm mit den aktuellen Signal-Werten beschrieben
          // werden
  ...
  public void drawBlockRectangle(final Graphics2D graphics) {
    ...
    graphics.drawPolygon(
        new int[] {
          (int) (dpix * (xPos - 0.4)) - DPFX,
          (int) (dpix * (xPos - 0.4)) - DPFX,
          (int) (dpix * (xPos - 0.4))
        },
        new int[] {(int) (dpix * pfym) - DPFY, (int) (dpix * pfym) + DPFY, (int) (dpix * pfym)},
        3);
    graphics.drawString(
        "From",
        (int) (dpix * (xPos - pf)),
        (int) (dpix * pfym - 1 * graphics.getFont().getSize() / 2));
    graphics.drawString(
        "EXTERN",
        (int) (dpix * (xPos - pf)),
    ...
    for (int i1 = 0; i1 < _terminalNumber; i1++) {
      ...
      graphics.drawString(
          Integer.valueOf(i1 + 1).toString(), xi1 + 8, yi1 + graphics.getFont().getSize() / 2);
      graphics.setColor(origColor);
    }
