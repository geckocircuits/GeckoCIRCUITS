...
public enum GeckoSymbol {
  ...
  void drawSymbol(final Graphics2D g2d, final float xPix, final float yPix) {
    ...
    switch (this) {
      ...
      case RECT_FILLED:
        g2d.fillRect(
            round(xPix - A_RECT / 2), round(yPix - A_RECT / 2), round(A_RECT), round(A_RECT));
        break;
      case TRIANG:
        int[] xPoints =
            new int[] {round(xPix - A_TRIANG / 2), round(xPix + A_TRIANG / 2), round(xPix)};
        int[] yPoints =
            new int[] {
              round(yPix + (int) (TRIANGLE_SIZE * A_TRIANG)),
              round(yPix + (int) (TRIANGLE_SIZE * A_TRIANG)),
              round(yPix - (int) (2 * TRIANGLE_SIZE * A_TRIANG))
            };
