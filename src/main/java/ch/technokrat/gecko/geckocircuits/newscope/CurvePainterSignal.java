...
final class CurvePainterSignal extends AbstractCurvePainter {
  ...
  public void reLoadData(final int minimumPixel, final int maximumPixel) {
    synchronized (
        this) { // if we don't do this synchronized, we will get concurrent modifications of the
      // curve painters.
      _allDrawables.clear();
      _allPoints.clear();
  ...
  public void loadDataRange(final int startPixel, final int stopPixel) {
    for (int i = startPixel; i < stopPixel; i++) {
      final Object value =
          _ramData.getDataValueInInterval(
              _xAxis.getValueFromPixel(i),
              _xAxis.getValueFromPixel(i + 1),
              _curve.getValueDataIndex());
      if (value == null) {
        continue;
      }
  ...
  private void appendDataPixel(
      final boolean lowPixelValue, final boolean hiPixelValue, final int xPixelValue) {
    synchronized (this) {
      if (_allDrawables.empty()) {
        _allDrawables.push(
            SignalStateDrawer.fabric(null, lowPixelValue, hiPixelValue, xPixelValue));
      } else {
        final SignalStateDrawer lastValue = _allDrawables.pop();
        final SignalStateDrawer newValue =
            SignalStateDrawer.fabric(lastValue, lowPixelValue, hiPixelValue, xPixelValue);
