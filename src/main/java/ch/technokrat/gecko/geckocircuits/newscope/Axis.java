...
public final class Axis {
  ...
  /** this constant is used so that also tick values at the interval border get drawn. */
   ...
   */
  public Axis(
      final Direction direction, final boolean invertTickDirection, final AbstractDiagram diagram) {
  ...
  abstract class AbstractAxisScale {
    ...
    protected void drawMinorTicks(
        final Graphics2D g2D, final Axis otherAxis, final List<Tick> majorTicks) {
      ...
      for (int i = 0; i < majorTicks.size() - 1; i++) {
        for (int j = 1; j < _axisTickSettings.getAnzTicksMinor(); j++) {
          final double value =
              majorTicks.get(i)._wert
                  + j
                      * (majorTicks.get(i + 1)._wert - majorTicks.get(i)._wert)
                      / _axisTickSettings.getAnzTicksMinor();
          final Tick minorTick = new Tick(value, _axisTickSettings.getTickLengthMin(), false);
          minorTick.drawTick(
              g2D,
              TECH_FORMAT.formatENG(minorTick._wert, DIGITS_TO_SHOW),
              _axisTickSettings.isShowLabelsMin());
  ...
  private class AxisLog extends AbstractAxisScale {
    ...
    protected double getValueFromPixel(final int xPix) {
      return getLimits()._yLo
          * Math.pow(
              LOG_AXIS_BASE, getSignDirection() * (xPix - getDirectionOrigin()) / getScaleFactor());
    }
    ...
    protected void drawMinorTicks(Graphics2D g2d, Axis otherAxis, List<Tick> majorTicks) {
      if (majorTicks.size() > 2) {
        ...
        for (int i = 0; i < noTicks; i++) {
          ...
          newTick.drawTick(g2d, TECH_FORMAT.formatT(newTick._wert, "#.#E0"), true);
        }
      }
    }
    ...
    protected List<Tick> drawMajorTicks(final Graphics2D g2d, final Axis otherAxis) {
      ...
      int anzTicks =
          (int) Math.round(Math.log10(getLimits()._yHi / getLimits()._yLo)) + MIN_LOG_TICKS;
      for (int i2 = 0; i2 < anzTicks; i2++) {
        ...
        if ((getLimits()._yLo <= wert) && (wert <= getLimits()._yHi)) {
          ...
          newTick.drawTick(
              g2d, TECH_FORMAT.formatT(newTick._wert, "#.E0"), _axisTickSettings.isShowLabelsMaj());
  ...
  private class AxisLin extends AbstractAxisScale {
    ...
    protected double getPixelFromValue(final double value) {
      ...
      if (!limits.equals(newLimits) || scaleFactor != scaleFactor2) {
        ...
        // System.out.println("correktur. " + scaleFactor + " " + scaleFactor2 + " " + limits + " "
        // + newLimits);
    ...
    protected List<Tick> drawMajorTicks(final Graphics2D g2D, final Axis otherAxis) {
      final HiLoData axisMinMax = _axisMinMax.getLimits();
      int anzTicks =
          (int) (axisMinMax._yHi / _tickSpacing) - (int) (axisMinMax._yLo / _tickSpacing) + 2;
      // int anzTicks = (int) (getLimits()._yHi / _tickSpacing) - (int) (getLimits()._yLo /
      // _tickSpacing) + 2;
      List<Tick> majorTicks = new ArrayList<Tick>();
      for (int i2 = 0; i2 < anzTicks; i2++) {
        final double wert =
            _tickSpacing * (int) (axisMinMax._yLo / _tickSpacing) + i2 * _tickSpacing;
        final int tick = (int) getPixelFromValue(wert);
        if ((axisMinMax._yLo <= wert) && (wert <= axisMinMax._yHi)) {
          ...
          newTick.drawTick(
              g2D,
              TECH_FORMAT.formatENG(newTick._wert, DIGITS_TO_SHOW),
  ...
  public long getAxisHash() {
    ...
    final HiLoData axisMinMax = _axisMinMax.getLimits();
    returnValue +=
        Double.doubleToLongBits(axisMinMax._yLo)
  ...
  private final class Tick {
    ...
    public Tick(
        final double newValue, final int newPixel, final int tickLength, final boolean isMajor) {
      _wert = newValue;
      ...
      if (zeigeLabels) {
        if (_direction == Direction.Y) {
          ...
          if (_invertTickDir) {
            g2d.drawString(
                label,
                _axisOriginPixel.x + (_axisTickSettings.getTickLengthMaj() + THREE),
                _pixelValue + FONT_TICK_LABEL.getSize() / 2 - 2);
          } else {
            g2d.drawString(
                label,
                _axisOriginPixel.x
                    - (_axisTickSettings.getTickLengthMaj() + THREE)
                    - (int) g2d.getFontMetrics().getStringBounds(label, g2d).getWidth(),
                _pixelValue + FONT_TICK_LABEL.getSize() / 2 - 2);
          }
        } else { // direction X
          final int centerOffset =
              (int) g2d.getFontMetrics().getStringBounds(label, g2d).getWidth() / 2;
          g2d.drawString(
              label,
              _pixelValue - centerOffset,
              _axisOriginPixel.y
                  + _axisTickSettings.getTickLengthMaj()
                  + THREE
                  + FONT_TICK_LABEL.getSize());
        }
      }
    }
  ...
  void drawAxisLabel(final Graphics2D g2d) {
    switch (_direction) {
      case X:
        g2d.drawString(
            _axisSettings.getAchseBeschriftung(),
            _axisOriginPixel.x + _axisLengthPix / 2,
            _axisOriginPixel.y + POS_TICK_LABELS);
        break;
      case Y:
        g2d.drawString(
            _axisSettings.getAchseBeschriftung(),
            _axisOriginPixel.x - POS_TICK_LABELS,
  ...
  void doPanning(final int mousePoint, final int panningStartPoint) {
    ...
    final HiLoData newLimits =
        HiLoData.hiLoDataFabric(
            originLimits._yLo - (float) shiftValue, originLimits._yHi - (float) shiftValue);
