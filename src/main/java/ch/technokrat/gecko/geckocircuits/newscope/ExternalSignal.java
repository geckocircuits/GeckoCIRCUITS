...
public class ExternalSignal extends AbstractScopeSignal {
  ...
  /**
   * Constructor to initialize a signal with its time values and its data values.
   *
  ...
  /**
   * Constructor to initialize a signal with its name, its time values and its data values.
   *
   ...
   */
  public ExternalSignal(
      final String name, final double[] times, final double[] values, String subCircuitPath) {
    this.setName(name);
    this.setData(times, values);
  ...
  /**
   * Sets the time values and the data values for an external signal. Sets the values _min and _max
   * of this signal.
   *
   * @param times
   * @param values
  ...
  /**
   * Returns the value of the external signal within the interval [startTime,stopTime]
   *
   * @param startTime The start time of the interval.
   * @param stopTime The stop time of the interval.
   * @return The value of the externalSignal within the interval as a float or HiLoData Object
   */
  public final Object getValueInInterval(final double startTime, final double stopTime) {
    ...
    if (this.size() > 0) {
      ...
      for (int i = firstIndex;
          start <= this.getTime(i) && this.getTime(i) <= stop && i < (this.size() - 1);
          i++) {
        nrOfValues++;
      }
      if (nrOfValues == 0) {
        ...
        ret2 =
            HiLoData.hiLoDataFabric(
                (float) this.getValue(firstIndex), (float) this.getValue(firstIndex));
  ...
  public final HiLoData getMinMax() {
    return HiLoData.hiLoDataFabric(
        (float) (this._min + this._offsetY), (float) (_max + this._offsetY));
  }
  ...
  /**
   * Implementation of getSignalName in AbstractScopeSignal returning the name of the signal.
   *
   * @return Returns the name of the external signal.
   */
  @Override
