...
public class CompressedData {
  ...
  private void compressData(byte[] compData) {
    ...
    for (int i = 0; i < _blockLength; i++) {
      // If not all of needed bits of the current byte have been added, add overhead bits to the
      // next byte
      if (offset != 0) {
  ...
  public void printCompressInfo() {
    ...
    System.out.println(
        "\tLast byte:\t"
            + Integer.toBinaryString(_compressedData[_compressedData.length - 1] & 0xFF));
    System.out.println(
        "\tLast Difference:\t"
            + Integer.toBinaryString(trimByte(_differences[_differences.length - 1], _bits)));
