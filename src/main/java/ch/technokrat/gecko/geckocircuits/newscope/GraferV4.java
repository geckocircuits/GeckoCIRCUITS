...
public final class GraferV4 extends JPanel {
  ...
  public Set<AbstractDiagram> getDiagramsForXUpdate(
      final MouseEvent event, final AbstractDiagram clickedDiagram) {
  ...
  public void createInitialAndSingleDiagram(
      final boolean xAxisLog, final boolean yAxisLog, final int numberCurves) {
  ...
  public void createInitialDiagramCISPR16(
      final boolean xAxisLog, final boolean yAxisLog, final int numberCurves) {
    createInitialDiagram();
    _manager.updateCurveNumber(numberCurves);
    AbstractDiagram diag = _manager.getDiagram(0);
  ...
  private class UpdateRunnable implements Runnable {
    ...
    /**
     * Please note: I want to enshure, that the update of the curves is done from one (this) and
     * only this thread.
     */
    ...
    public void run() {
      while (true) {
        try {
          ...
          if (!_isVisible) { // if not visible, don't redraw, but enshure that when component made
            // visible, we redraw.
            _loadAndDraw = true;
          ...
          if (dataContainer.getContainerStatus() == ContainerStatus.PAUSED
              && dataContainer != _lastDrawPausedContainer) {
            ...
            for (int i = 1; i < dataContainer.getRowLength(); i++) {
              endTime =
                  Math.max(
                      dataContainer.getTimeValue(dataContainer.getMaximumTimeIndex(i), i), endTime);
            }
            NiceScale xNiceScale =
                new NiceScale(HiLoData.hiLoDataFabric((float) startTime, (float) endTime), true);
            HiLoData niceLimits = xNiceScale.getNiceLimits();

            if (Math.abs(SimulationsKern.tEND - endTime) / Math.abs(SimulationsKern.tEND + endTime)
                > 0.01) {
    ...
    private void reloadDataAndPaint(final AbstractDataContainer container)
  ...
  public void importIndividualCONTROL(final TokenMap settingsMap) {
    ...
    for (TokenMap diagramMap = settingsMap.getBlockTokenMap("<Diagram>");
        diagramMap != null;
    ...
    switch (mausModus) {
      ...
      case SLIDER:
        ...
        if (!_sliderContainer.isSliderActive()) {
          ...
          // x-Schieber wird an den Anfang gesetzt: gleich fuer alle Diagramme, in GraferV3
          // definiert
          this.setAxisPositions();
          this.repaint();
        }
