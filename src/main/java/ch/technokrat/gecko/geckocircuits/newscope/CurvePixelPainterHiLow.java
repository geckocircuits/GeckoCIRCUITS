...
final class CurvePixelPainterHiLow extends AbstractCurvePixelPainter {
  ...
  public CurvePixelPainterHiLow(
      final float xPixel,
      final HiLoData value,
      final Axis yAxis,
  ...
  private static float calculateOverlap(
      final CurvePixelPainterHiLow dat1, final CurvePixelPainterHiLow dat2) {
    float returnValue;
    returnValue =
        Math.min(dat1._hiPixelValue, dat2._hiPixelValue)
            - Math.max(dat1._lowPixelValue, dat2._lowPixelValue);
    if (returnValue < 0) {
      returnValue = 0;
    }
    return returnValue;
