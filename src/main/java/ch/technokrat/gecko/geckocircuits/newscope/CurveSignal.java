...
class CurveSignal extends AbstractCurve {
  ...
  public void drawCurve(final Graphics2D g2d, final SliderContainer slider) {
    final int y0Kurve =
        _yAxis._axisOriginPixel.y
            - (_diagram.getHeight() - (DY_IN_OBEN + DY_IN_UNTEN))
            + _xAxis.getRequiredAxisSpace()
            - 2;

    final int positionSIGNAL = ((DiagramSignal) _diagram).getSignalPosition(this);
    _yTranslation =
        y0Kurve
            + _diagram._ySpaceUpper
            + SGN_HEIGHT
            - SGN_DIST
            + positionSIGNAL * (SGN_DIST + SGN_HEIGHT);

    g2d.translate(
        0, _yTranslation); // translate graphics object, so that the pixels are in the axis origin
