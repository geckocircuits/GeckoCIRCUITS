...
public final class CharacteristicsCalculator {
  ...
  private static boolean isAlreadyCalculated(
      AbstractDataContainer worksheet, int[] rows, double rng1, double rng2) {
    if (_valueCache == null) {
      return false;
    } else {
  ...
  private CharacteristicsCalculator(
  ...
  private double[] checkCalculationBounds(
      final AbstractDataContainer worksheet, final double start, final double end) {
    ...
    if (Math.abs(end - worksheet.getTimeValue(endIndex, 0)) > TIMETHRESHOLD) {
      ...
      System.out.println(
          "Difference range: " + Math.abs(bounderies[1] - worksheet.getTimeValue(endIndex, 0)));
    }
  ...
  private void calculate(
    ...
    for (int i1 = startIndex;
        i1 < worksheet.getMaximumTimeIndex(0)
            && worksheet.getTimeValue(i1 + 1, 0) > worksheet.getTimeValue(i1, 0)
            && worksheet.getTimeValue(i1, 0) < rng2;
        i1++) {
    ...
    for (int i2 = 0; i2 < dataRowLength; i2++) {
      ...
      _gsA1[i2] /= totalT / 2.0;
      _gsB1[i2] /= totalT / 2.0;
      final double sqrtHighHarm =
          Math.sqrt(
              _rms2[i2] * _rms2[i2]
                  - (_gsA1[i2] * _gsA1[i2] + _gsB1[i2] * _gsB1[i2]) / 2.0
                  - (_avg[i2] * _avg[i2]));
   ...
   * @throws Exception
   */
  public static CharacteristicsCalculator calculateFabric(
      final AbstractDataContainer worksheet, final double rng1, final double rng2) {
