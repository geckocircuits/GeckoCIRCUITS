...
class Slider {
  ...
  public void setXSliderPix(
      final int pixelValue, final Axis xAxis, final GraferV4 repaintComponent) {
    int checkedPixelValue = pixelValue;
    // do a range check:
    checkedPixelValue = Math.max(checkedPixelValue, xAxis._axisOriginPixel.x);
    checkedPixelValue =
        Math.min(checkedPixelValue, xAxis._axisOriginPixel.x + xAxis.getAxisLengthPixel());
    _xSliderPix = checkedPixelValue;
    setXSliderValue(xAxis.getValueFromPixel(_xSliderPix));

    // this is a graphics optimization, since drawing the whole plot during sliding is too time
    // consuming!
