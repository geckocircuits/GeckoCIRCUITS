...
class CurvePainterRegular extends AbstractCurvePainter {
  private final Stack<AbstractCurvePixelPainter> _allPainters =
      new Stack<AbstractCurvePixelPainter>();
  ...
  public void reLoadData(final int minimumPixel, final int maximumPixel) {
    synchronized (
        this) { // do this synchronized, otherwise we will get concurrent modificationss of
      // _allPainters.
      _allPainters.clear();
      _allPoints.clear();
  ...
  public void loadDataRange(final int startPixel, final int stopPixel) {
    ...
    for (int xPixel = startPixel; xPixel <= stopPixel; xPixel++) {
      ...
      final Object value =
          _ramData.getDataValueInInterval(x1Value, x2Value, _curve.getValueDataIndex());
      ...
      switch (_curve.getLineType()) {
        case CONNECT_NEIGHBOURS:
          pixelPainter =
              AbstractCurvePixelPainter.curvePixelPainterFabric(
                  xPixel, value, _curve._yAxis, previousValue);
          break;
        case BAR:
