...
final class CurveSettings {
  ...
  void importASCII(final TokenMap curveMap) {
    ...
    _curveLineStyle =
        GeckoLineStyle.getFromCode(curveMap.readDataLine("curveLineStyle", _curveLineStyle.code()));
    _curveShowPtSymbols = curveMap.readDataLine("showCurveSymbols", _curveShowPtSymbols);
    _crvSymbFarbe =
        GeckoColor.getFromCode(curveMap.readDataLine("symbolColor", _crvSymbFarbe.code()));
    _crvSymbShape =
        GeckoSymbol.getFromCode(curveMap.readDataLine("symbolShape", _crvSymbShape.code()));
    _crvTransparency = curveMap.readDataLine("crvTransparency", _crvTransparency);
    _crvFillingDigColor =
        GeckoColor.getFromCode(
            curveMap.readDataLine("fillDigitalColor", _crvFillingDigColor.code()));
