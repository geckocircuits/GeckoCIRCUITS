...
abstract class AbstractCurvePixelPainter {
  ...
  protected AbstractCurvePixelPainter(
      final float xPixel, final Axis yAxis, final AbstractCurvePixelPainter previousValue) {
    _xPixel = xPixel;
    _yAxis = yAxis;
  ...
  static AbstractCurvePixelPainter barPixelPainter(final int xPixel, Object value, Axis yAxis) {
    if (value instanceof HiLoData) {
      ...
      if (hiLow._yLo > 0 && hiLow._yHi > 0) {
        ...
        return new CurvePixelPainterHiLow(
            xPixel, barPixelRepresentation, yAxis, (AbstractCurvePixelPainter) null);
  ...
  public static AbstractCurvePixelPainter curvePixelPainterFabric(
      final float xPixel,
      final Object value,
      final Axis yAxis,
    ...
    if (value instanceof HiLoData) {
      ...
      if (Math.abs(lowPixelValue - hiPixelValue) > HI_LOW_THRESHOLD) { // real range is given
        ...
        if (previousValue == null || previousValue._lastValue == null) {
          ...
          return new CurvePixelPainterPointsLine(
              xPixel, (hiPixelValue + lowPixelValue) / 2, yAxis, previousValue);
        }
      }
      ...
      if (value instanceof Float) {
        return new CurvePixelPainterPointsLine(
            xPixel, (float) yAxis.getPixelFromValue((Float) value), yAxis, previousValue);
      }
    }
