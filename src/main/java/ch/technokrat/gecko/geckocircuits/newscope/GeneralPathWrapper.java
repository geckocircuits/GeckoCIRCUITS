...
/**
 * This class "wraps" a GeneralPath for drawing. This is done since GeneralPath is quite inefficient
 * when drawing. the "lineTo"-Method first checks, if the inserted point is "far away" from the
 * previous line. If not, the last drawn point is just moved to the newly inserted point.
 *
 * @author andy
 */
final class GeneralPathWrapper {
  ...
  private boolean detectSimplification(final float xPixel, final float yPixel) {
    ...
    final double lastSlope =
        Math.tan(
            (_lastPathpoint.getY() - _lastInsertPoint.getY())
                / (_lastPathpoint.getX() - _lastInsertPoint.getX()));
    final double slope =
        Math.tan((_lastInsertPoint.getY() - yPixel) / (_lastInsertPoint.getX() - xPixel));
   ...
   */
  private double calculateInvCircleRadius(
      final Point2D point1, final Point2D point2, final Point2D point3) {
