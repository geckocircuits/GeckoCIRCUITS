...
public abstract class AbstractDiagram extends JPanel {
  ...
  private List<AbstractCurve> _curves =
      Collections.unmodifiableList(new ArrayList<AbstractCurve>());
  ...
  public void populateCurveProperties(final TokenMap diagramMap, final GraferV4 grafer) {
    int counter = 0;
    for (TokenMap curveMap = diagramMap.getBlockTokenMap("<CurveDiagram>");
        curveMap != null;
  ...
  private class LabelPanel extends JPanel {
    ...
    protected void paintComponent(final Graphics graphics) {
      ...
      if (_grafer._manager.getDiagrams().get(_grafer._manager.getDiagrams().size() - 1)
          == AbstractDiagram.this) {
        graphics.drawString(
            _grafer.xAxisLabel,
            0,
            this.getHeight() - graphics.getFontMetrics().getHeight() / 2 + 1);
      }
    }
  };
  ...
  private void drawBorder(final Graphics2D graphics) {
    ...
    graphics.drawRect(
        _xAxis._axisOriginPixel.x,
        _xAxis._axisOriginPixel.y - _yAxis1.getAxisLengthPixel(),
        _xAxis.getAxisLengthPixel(),
        _yAxis1.getAxisLengthPixel());
  }

  /**
   * @param worksheetDaten
   * @param niceScale
   * @return value can be used to detect if a redraw has to be performed or not, e.g. when the axis
   *     limits where not changed, the axis does not have to be redrawn.
   */
  public boolean fitYRangesFromGlobalData(
      final AbstractDataContainer worksheetDaten, final boolean niceScale) {
  ...
  public void setAxisPositions(final boolean anyY2Axis) {
    ...
    if (_diagramSettings.getNameDiagram().startsWith("GRF")
        || _diagramSettings.getNameDiagram().isEmpty()) {
  ...
  private HiLoData calculateAutoScaleYMinMax(
      final AxisConnection axisConnection, final AbstractDataContainer worksheetDaten) {
  ...
  private void calculateAutoScaleYBothYAxis(final AbstractDataContainer worksheetDaten) {
    final HiLoData minMaxValue1 =
        calculateAutoScaleYMinMax(AxisConnection.ZUORDNUNG_Y, worksheetDaten);
    final HiLoData minMaxValue2 =
        calculateAutoScaleYMinMax(AxisConnection.ZUORDNUNG_Y2, worksheetDaten);
  ...
  public void loadDataFromContainer(final AbstractDataContainer container) {
    ...
    for (int i = 0; i < this._curves.size(); i++) {
      ...
      if (curve.getAxisConnection() != AxisConnection.ZUORDNUNG_NIX) {
        forceLoad =
            (this._grafer._manager.getAllScopeSignals().get(curve.getValueDataIndex())
                instanceof ExternalSignal);
        try {
  ...
  private void drawDiagramName(final Graphics2D g2d) {
    if (isDrawTitle()) {
      ...
      final int yTitle =
          _yAxis1._axisOriginPixel.y
              - _yAxis1.getAxisLengthPixel()
              - g2d.getFont().getSize() / THREE;
      g2d.drawString(
          _diagramSettings.getNameDiagram(),
          (getWidth() - stringWidth - _dxInRight / 2) / 2,
          yTitle);
    }
  }
  ...
  /**
   * this method creates new curves of corresponding type, when an old set of curves (probably of
   * other type) is given.
   *
   * @param oldCurves
   * @return
