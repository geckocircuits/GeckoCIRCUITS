...
/**
 * this is a helper class, to draw the signal curve efficiently. The constructor is private, since
 * the values should be obtained over the fabric. If subsequent pixel states are the same, the last
 * element ist returned with extended interval range.
 *
 * @author andy
 */
final class SignalStateDrawer {
  ...
  public static SignalStateDrawer fabric(
      final SignalStateDrawer lastValue,
      final boolean lowPixelValue,
      final boolean hiPixelValue,
  ...
  private static SignalStateDrawer findNewPixelDrawer(
      final SignalStateDrawer lastValue, final DrawState newState, final int xPixelValue) {
    SignalStateDrawer returnValue;
  ...
  private static boolean detectTransitionLine(
      final DrawState myDrawState, final DrawState oldDrawState) {
    ...
    switch (_drawState) {
      ...
      case ON_OFF:
        ...
        final Color shadedColor =
            new Color(
                findModifiedColor(oldColor.getRed()),
                findModifiedColor(oldColor.getGreen()),
