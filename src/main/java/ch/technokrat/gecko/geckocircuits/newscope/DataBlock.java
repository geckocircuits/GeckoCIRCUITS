...
public class DataBlock {
  ...
  /**
   * A value fits in a block either if the block has one element, or value = start + n*dt, with a
   * tolerance respective to dt
   *
   * @param value
  ...
  private boolean checkValueFit(final double value) {
    return _size == 1
        || (_startValue + _size * (1 - TOLERANCE) * _dt <= value
