...
/**
 * this class detects, which data values in the CurvePaintabel have to be loaded. Usually, data is
 * just added to the datacontainer, therefore do not do a full load.
 *
 * @author andy
 */
public final class DataLoader {
  ...
  void loadRequiredData(final AbstractDataContainer newData, boolean forceLoad) {
    ...
    if (_curvePaintable.getRamData() == null || !newData.equals(_curvePaintable.getRamData())) {
      ...
      final int maxExtendedPixel =
          getMaximumExtendedPixel(
              newData.getMaximumTimeIndex(_curvePaintable._curve.getValueDataIndex()));
    ...
    final int newMaximumIndex =
        _curvePaintable
            .getRamData()
            .getMaximumTimeIndex(_curvePaintable._curve.getValueDataIndex());
    ...
    if (_maxDataIndex != newMaximumIndex && _maxDataIndex >= 0
      ...
      _curvePaintable.loadDataRange(
          Math.max(_maxLoadedPixel, xAxis._axisOriginPixel.x), maxExtendedPixel);
      _maxLoadedPixel = maxExtendedPixel;
    }
  }

  /**
   * calculate the maximum pixel value (in the scope) for which the data should be loaded. Careful:
   * we are searching for the Next data point pixel, since, when we zoom into the scope, the drawn
   * signal lines should reach to the end of the scope window. Extended means, we search the pixel
   * value for the next bigger data point outside the scope.
   *
  ...
  private int getMaximumExtendedPixel(final int maximumDataContainerIndex) {
    ...
    final double maxTimeValue =
        _curvePaintable
            .getRamData()
            .getTimeValue(maximumDataContainerIndex, _curvePaintable._curve.getValueDataIndex());
    ...
    final AbstractTimeSerie timeSerie =
        _curvePaintable.getRamData().getTimeSeries(_curvePaintable._curve.getValueDataIndex());
    ...
    if (maxAxisIndex + 1 < maximumDataContainerIndex) {
      return Math.min(
          maxDataPixel, (int) xAxis.getPixelFromValue(timeSerie.getValue(maxAxisIndex + 2)));
    } else {
      return Math.min(maxDataPixel + 1, maxAxisPixel);
    }
  }

  /** @return the x-Axis pixel value of the data point, that is just outside (left) the diagram. */
