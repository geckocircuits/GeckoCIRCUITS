...
public final class PowerCalculator {
  ...
  private PowerCalculator(
    ...
    calculate(
        ...
        voltageIndices,
        currentIndices,
        selectedIndices.startTime,
        selectedIndices.stopTime);
  }
  ...
  private void calculate(
      final AbstractDataContainer worksheet,
      final int[] voltageIndices,
      final int[] currentIndices,
      final double rng1,
      final double rng2) {
    ...
    double deltaT =
        worksheet.getTimeValue(startIndex + 1, 0) - worksheet.getTimeValue(startIndex, 0);
    ...
    for (int row = 0; row < _noOfPowerAnals; row++) {
      ...
      double[] cnU = new double[CALC_NUM_HARM],
          cnI = new double[CALC_NUM_HARM],
          dphiUI = new double[CALC_NUM_HARM];
      ...
      _powerD[row] =
          Math.sqrt(
              _powerS[row] * _powerS[row]
                  - (_powerP[row] * _powerP[row] + _powerQ[row] * _powerQ[row]));
      _lambda[row] = _powerP[row] / _powerS[row];
      _cosPhi[row] = Math.cos(dphiUI[1]);
    }
  }

  public static PowerCalculator calculatorFabric(
      final AbstractDataContainer worksheet,
      final PowerAnalysisPanel.PowerCalculatorSelectionIndex selectedIndices) {
