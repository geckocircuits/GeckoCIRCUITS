...
public final class AxisGridSettings {
  ...
  /**
   * which gridlines should be drawn. This is not set by the user, but can be disabled when
   * gridline-distance is too small.
   */
  ...
  /**
   * wenn die Diagramme in einem sehr kleinen Fenster gezeichnet werden, dann muessen eventuell die
   * Grid-Linien ausgeblendet werden, um eine gewisse Uebersichtlichkeit zu wahren -->
   */
  ...
  void importASCII(final TokenMap axisMap) {
    ...
    _linStilGridNormal =
        GeckoLineStyle.getFromCode(
            axisMap.readDataLine("linStilGridNormal", _linStilGridNormal.code()));
    _linStilGridMinor =
        GeckoLineStyle.getFromCode(
            axisMap.readDataLine("linStilGridMinor", _linStilGridMinor.code()));
    _farbeGridNormal =
        GeckoColor.getFromCode(axisMap.readDataLine("colorGridNormal", _farbeGridNormal.code()));
    _farbeGridNormalMinor =
        GeckoColor.getFromCode(
            axisMap.readDataLine("colorGridMinor", _farbeGridNormalMinor.code()));
  }
}
