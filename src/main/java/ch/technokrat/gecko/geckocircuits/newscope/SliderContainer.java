...
final class SliderContainer {
  ...
  private static final String JOPTION_PANE_MESSAGE =
      "No curve selected! Please select\n" + "a curve by clicking on its label name.";
  ...
  public SliderValues getSliderValues(final int curveID) {
    //        System.out.println("xxyxyx. " + _ySliderValues.size() +" " +
    // _grafer.getManager().getNumberInputSignals());
  ...
  public void drawSlider(final Graphics graphics) {
    ...
    final AbstractDiagram bottomDiagram =
        _grafer.getManager().getDiagram(_grafer.getManager().getNumberDiagrams() - 1);
    final int stopPaintY =
        bottomDiagram.getY() + bottomDiagram.getHeight() - bottomDiagram.getBottomOffset();
  ...
  protected void goToZeroOrEqual(
      final IterationDirection direction, final boolean isZeroTransition, final GraferV4 grafer)
    ...
    final double newXValue =
        SliderUtils.nextZeroOrEqual(
            oldSliderXValue, selectedCurve, direction, data, transitionValue);
  ...
  protected void goToExtrema(
      final IterationDirection direction, final ExtremumType maxMin, final GraferV4 grafer)
      throws NoCurveSelectedException {
    ...
    final double oldSliderXValue = getActiveSliderXValue();
    final double newXValue =
        SliderUtils.nextExtrema(oldSliderXValue, curveSelected, direction, maxMin, data);
  ...
  protected void goToSignalFlank(
      final IterationDirection direction, final FlankType flankType, final GraferV4 grafer)
      throws NoCurveSelectedException {
    ...
    final double oldSliderXValue = getActiveSliderXValue();
    final double newXValue =
        SliderUtils.nextSIGNALFlank(oldSliderXValue, curveSelected, direction, flankType, data);
  ...
  private void checkCurveSelection(final AbstractCurve curve) throws NoCurveSelectedException {
    if (curve == null) {
      JOptionPane.showMessageDialog(
          GeckoSim._win,
          JOPTION_PANE_MESSAGE,
          JOPTION_PANE_MESSAGE_HEAD,
