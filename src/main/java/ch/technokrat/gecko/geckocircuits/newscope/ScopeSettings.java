...
/**
 * this class is probably the biggest bullshit you have ever seen! This was from Uwe's old Scope
 * implementation, for compatibility reasons, it is still here. It should be refactored or removed,
 * soon!
 */
...
public class ScopeSettings implements Serializable {
  ...
  private int[] linStilGridNormalX,
  ...
  private int[][] crvAchsenTyp; // wird ueber SET-Methode aktualisiert, damit die Matrix
  ...
  private static final int DIAGRAM_TYP_ZV = 91, DIAGRAM_TYP_SGN = 92;
  private boolean newFormat = false;
  private int noInputSignals;
  private boolean _isFormatBefore160;

  /**
   * Scope settings is not used anymore. The scope settings are saved internally in grapher. This
   * class is only here for backwards-compatibility, and will be removed, soon.
   *
   * @deprecated
   */
  @Deprecated
  ...
  public void loadSettings(GraferV4 impl) {
    if (_isFormatBefore160) {
      ...
      for (int i = 0; i < anzDiagram; i++) {
        ...
        xAxis._axisGridSettings.setLinStyleMin(
            GeckoLineStyle.getFromCode(linStilGridNormalXminor[i]));
        yAxis._axisGridSettings.setLinStyleMaj(GeckoLineStyle.getFromCode(linStilGridNormalY[i]));
        yAxis._axisGridSettings.setLinStyleMin(
            GeckoLineStyle.getFromCode(linStilGridNormalYminor[i]));
        ...
        if (newFormat) { // in the older verions, these properties were not saved at all (set to
          // false/zero)
        ...
        for (int jj = 1; jj < noInputSignals + 1; jj++) {
          AxisConnection axisConnection =
              AxisConnection.getFromCode(matrixZuordnungKurveDiagram[i][jj]);
        ...
        for (int j = 0; j < manager.getNumberInputSignals(); j++) {
          ...
          curveSettings._crvFillingDigColor =
              GeckoColor.getFromCode(crvFillingDigitalColor[i][j + 1]);
  ...
  public boolean importASCII(final TokenMap tokenMap) {
    ...
    linStilGridNormalXminor =
        tokenMap.readDataLine("linStilGridNormalXminor[]", linStilGridNormalXminor);
    linStilGridNormalY = tokenMap.readDataLine("linStilGridNormalY[]", linStilGridNormalY);
    linStilGridNormalYminor =
        tokenMap.readDataLine("linStilGridNormalYminor[]", linStilGridNormalYminor);
    ...
    if (tokenMap.containsToken("ORIGjtfXtickLengthMajNew[]")) {
      newFormat = true;
      ORIGjtfXtickLengthMaj =
          tokenMap.readDataLine("ORIGjtfXtickLengthMajNew[]", ORIGjtfXtickLengthMaj);
    }

    if (tokenMap.containsToken("ORIGjtfXtickLengthMinNew[]")) {
      newFormat = true;
      ORIGjtfXtickLengthMin =
          tokenMap.readDataLine("ORIGjtfXtickLengthMinNew[]", ORIGjtfXtickLengthMin);
    }

    if (tokenMap.containsToken("ORIGjtfYtickLengthMajNew[]")) {
      newFormat = true;
      ORIGjtfYtickLengthMaj =
          tokenMap.readDataLine("ORIGjtfYtickLengthMajNew[]", ORIGjtfYtickLengthMaj);
    }

    if (tokenMap.containsToken("ORIGjtfYtickLengthMinNew[]")) {
      ORIGjtfYtickLengthMin =
          tokenMap.readDataLine("ORIGjtfYtickLengthMinNew[]", ORIGjtfYtickLengthMin);
    }

    if (tokenMap.containsToken("ORIGjcbXShowLabelMajNew[]")) {
      ORIGjcbXShowLabelMaj =
          tokenMap.readDataLine("ORIGjcbXShowLabelMajNew[]", ORIGjcbXShowLabelMaj);
    }

    if (tokenMap.containsToken("ORIGjcbXShowLabelMinNew[]")) {
      ORIGjcbXShowLabelMin =
          tokenMap.readDataLine("ORIGjcbXShowLabelMinNew[]", ORIGjcbXShowLabelMin);
    }

    if (tokenMap.containsToken("ORIGjcbYShowLabelMajNew[]")) {
      ORIGjcbYShowLabelMaj =
          tokenMap.readDataLine("ORIGjcbYShowLabelMajNew[]", ORIGjcbYShowLabelMaj);
    }

    if (tokenMap.containsToken("ORIGjcbYShowLabelMinNew[]")) {
      ORIGjcbYShowLabelMin =
          tokenMap.readDataLine("ORIGjcbYShowLabelMinNew[]", ORIGjcbYShowLabelMin);
    }
    ...
    matrixZuordnungKurveDiagram =
        tokenMap.readDataLine("matrixZuordnungKurveDiagram[][]", matrixZuordnungKurveDiagram);
    ...
    crvFillingDigitalColor =
        tokenMap.readDataLine("crvFillingDigitalColor[][]", crvFillingDigitalColor);

    if (tokenMap.containsToken("powerAnalysisCurrents[]")) {
      powerAnalysisCurrentIndex =
          tokenMap.readDataLine("powerAnalysisCurrents[]", powerAnalysisCurrentIndex);
      powerAnalysisVoltageIndex =
          tokenMap.readDataLine("powerAnalysisVoltages[]", powerAnalysisVoltageIndex);
    }

    // Achtung: in alten Versionen sind diese beiden Parameter nicht gesetzt, daher hier
