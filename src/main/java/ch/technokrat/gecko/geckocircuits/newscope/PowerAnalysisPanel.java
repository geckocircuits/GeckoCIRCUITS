...
public final class PowerAnalysisPanel extends JPanel {
  ...
  public PowerAnalysisPanel(
      final AbstractDataContainer worksheet, final PowerAnalysisSettings powerAnalSettings) {
    super();
    ...
    for (int i = 0; i < pUIa.length; i++) {
      ...
      pUIa[i].setBorder(
          BorderFactory.createTitledBorder(
              BorderFactory.createEtchedBorder(), titles[i], TitledBorder.LEFT, TitledBorder.TOP));
    }
  ...
  public void calculate(
      final AbstractDataContainer _worksheet,
      final double startTimeValue,
      final double stopTimeValue) {
    final PowerCalculatorSelectionIndex pcsi =
        new PowerCalculatorSelectionIndex(startTimeValue, stopTimeValue);
  ...
  public class PowerCalculatorSelectionIndex {
    ...
    public void calculate(final AbstractDataContainer worksheet) {
      try {
        ...
        if (this._selCurrentInd.size()
            != 0) { // dh. keine Leistungsberechnung durchzufuehren weil deaktiviert
          ...
          for (int i : _selPowerAnal) {
            _pqTextFields[i][0].setText(
                _cf.formatT(ergP.getPowerP(counter), TechFormat.FORMAT_AUTO));
            _pqTextFields[i][1].setText(
                _cf.formatT(ergP.getPowerQ(counter), TechFormat.FORMAT_AUTO));
            _pqTextFields[i][2].setText(
                _cf.formatT(ergP.getPowerD(counter), TechFormat.FORMAT_AUTO));
            _pqTextFields[i][3].setText(
                _cf.formatT(ergP.getPowerS(counter), TechFormat.FORMAT_AUTO));
            _pqTextFields[i][4].setText(
                _cf.formatT(ergP.getLambda(counter), TechFormat.FORMAT_AUTO));
            _pqTextFields[i][5].setText(
                _cf.formatT(ergP.getCosPhi(counter), TechFormat.FORMAT_AUTO));
            counter++;
          }
        }
      } catch (Exception ex) {
