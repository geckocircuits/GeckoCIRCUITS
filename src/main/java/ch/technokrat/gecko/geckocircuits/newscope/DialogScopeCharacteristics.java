...
public class DialogScopeCharacteristics extends GeckoDialog {
  ...
  public DialogScopeCharacteristics(
      final JFrame parent,
      final AbstractDataContainer worksheet,
      final PowerAnalysisSettings powerAnalSettings,
        ...
        new JComponent() {
          ...
          public void paintComponent(final java.awt.Graphics graphics) {
            try {
              ...
              java.awt.Image equ1 =
                  new ImageIcon(new URL(GlobalFilePathes.PFAD_PICS_URL, "equ1.png")).getImage();
        ...
        new JComponent() {
          ...
          public void paintComponent(final java.awt.Graphics graphics) {
            try {
              ...
              graphics.fillRect(0, 0, RECT_SIZE, RECT_SIZE); // weisser Hintergrund
              java.awt.Image equ1 =
                  new ImageIcon(new URL(GlobalFilePathes.PFAD_PICS_URL, "equ2b.png")).getImage();
              graphics.drawImage(equ1, 0, 0, null);
  ...
  class CalculationThread extends Thread {
    ...
    public void run() {
      try {
        ...
        final CharacteristicsCalculator characteristics =
            CharacteristicsCalculator.calculateFabric(
                _worksheet,
                _panelDialRange.getStartTimeValue(),
                _panelDialRange.getStopTimeValue());

        for (int i1 = 0; i1 < _worksheet.getRowLength(); i1++) {
          _characErgPanel.setAvgText(
              _cf.formatT(characteristics.getAVGValue(i1), TechFormat.FORMAT_AUTO), i1);
          _characErgPanel.setRmsText(
              _cf.formatT(characteristics.getRMS2Value(i1), TechFormat.FORMAT_AUTO), i1);
          if (characteristics.getMinValue(i1) == LARGE_VAL) {
            _characErgPanel.setMinText(_cf.formatT(0, TechFormat.FORMAT_AUTO), i1);
          } else {
            _characErgPanel.setMinText(
                _cf.formatT(characteristics.getMinValue(i1), TechFormat.FORMAT_AUTO), i1);
          }

          if (characteristics.getMaxValue(i1) == -LARGE_VAL) {
            _characErgPanel.setMaxText(_cf.formatT(0, TechFormat.FORMAT_AUTO), i1);
          } else {
            _characErgPanel.setMaxText(
                _cf.formatT(characteristics.getMaxValue(i1), TechFormat.FORMAT_AUTO), i1);
          }

          _characErgPanel.setCRestText(
              _cf.formatT(characteristics.getKlirrValue(i1), TechFormat.FORMAT_AUTO), i1);
          _characErgPanel.setShapeText(
              _cf.formatT(characteristics.getShapeValue(i1), TechFormat.FORMAT_AUTO), i1);
          _characErgPanel.setThdText(
              _cf.formatT(characteristics.getTHDValue(i1), TechFormat.FORMAT_AUTO), i1);
          _characErgPanel.setRippleText(
              _cf.formatT(characteristics.getRippleValue(i1), TechFormat.FORMAT_AUTO), i1);
          _characErgPanel.setPeakPeakText(
              _cf.formatT(characteristics.getPeakToPeakValue(i1), TechFormat.FORMAT_AUTO), i1);
        }

        _powAnalPanel.calculate(
            _worksheet, _panelDialRange.getStartTimeValue(), _panelDialRange.getStopTimeValue());
