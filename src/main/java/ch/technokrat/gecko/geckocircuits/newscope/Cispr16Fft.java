...
public final class Cispr16Fft {
  ...
  private static final double COEFF_1 = 0.5;
  ...
  public Cispr16Fft(final DataContainerSimple data, final boolean useBlackman) {
    ...
    for (int i = 0; i < _zvResampled.length / 2 - 1; i++) {
      double abs =
          Math.sqrt(
              _zvResampled[2 * i] * _zvResampled[2 * i]
                  + _zvResampled[2 * i + 1] * _zvResampled[2 * i + 1]);
      if (abs > maxValue) {
        maxValue = abs;
      }
    }
  ...
  /**
   * this is not a "real" resampling. We need a 2^N number of samples, therefore we just continue
   * calculation with the last 2^N values, where N is as big as possible.
   *
   * @param data
   * @param reductor additional data reduction factor. If reductor = 0, no further reduction is
   *     done. If freductor = 1, the data size is halved...
   */
  ...
  public static void realft(final float[] data, final int isign) {
    ...
    try {
      ...
      System.err.println(
          "Error: could not find JTransforms class library!\n"
    ...
    if (nnn % 2 != 0) {
      throw new IllegalArgumentException(
          "Fast Fourier Transform (FFT) is only possible with n = 2^x  data size!");
    }
