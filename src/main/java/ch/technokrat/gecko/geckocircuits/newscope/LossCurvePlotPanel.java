...
public class LossCurvePlotPanel extends SimpleGraferPanel {
  ...
  public void highlightMessKurve(int index) {
    ...
    if (_moreDataFactor > 1) {
      diag.getCurves()
          .get(index + diag.getCurves().size() / 2)
          .setStroke(GeckoLineStyle.SOLID_FAT_2);
    }
    repaint();
  }

  public static LossCurvePlotPanel fabric(
      List<? extends LossCurve> lossCurvesWOTest, final LossCurve testCurve) {
    ...
    DataContainerManyTimeSeries dcs1 =
        new DataContainerManyTimeSeries(moreDataFactor * lossCurves.size(), 1000);
