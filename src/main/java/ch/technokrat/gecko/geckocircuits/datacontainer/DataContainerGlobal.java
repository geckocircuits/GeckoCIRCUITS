...
/**
 * We use a separate class containing the actual datacontainer, for easier memory management. The
 * _data-Field is wrapped by this class, so that no other class can reference to it. Otherwise,
 * garbage-collection would be difficult.
 */
public final class DataContainerGlobal extends AbstractDataContainer
    implements Observer, DataContainerValuesSettable, DataContainerIntegralCalculatable {
  ...
  public void init(final int rows, final String[] signalNames, final String xDataName) {
    ...
    final DataContainerCompressable data =
        new DataContainerCompressable(rows, new TimeSeriesConstantDt(), signalNames, xDataName);
    _settable = data;
    _data = data;
  ...
  public HiLoData getAbsoluteMinMaxValue(final int row) {
    try {
      ...
      return HiLoData.hiLoDataFabric(
          0, 1); // just return some value, so that the scope is not totally screwed up!
    }
  }
  ...
  public float getAVGValueInInterval(
    ...
    if (_data instanceof DataContainerIntegralCalculatable) {
      return ((DataContainerIntegralCalculatable) _data)
          .getAVGValueInInterval(intervalStart, intervalStop, columnIndex);
    } else {
      return 0f;
    }
  }
