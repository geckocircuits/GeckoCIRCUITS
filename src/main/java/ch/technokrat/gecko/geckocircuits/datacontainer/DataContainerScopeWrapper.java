...
// CHECKSTYLE:ON
/**
 * Be careful: in the scope, the number of input ports can be increased/decreased. Therefore, check
 * if we are out of bounds, and give zer0 as return value, if so.
 *
 * @author andy
 */
public final class DataContainerScopeWrapper extends AbstractDataContainer
    implements DataContainerIntegralCalculatable {
  ...
  public DataContainerScopeWrapper(
      final AbstractDataContainer globalContainer,
      final ScopeWrapperIndices indices,
      final DefinedMeanSignals meanSignals,
      final List<AbstractScopeSignal> signalsList) {
  ...
  public int getMaximumTimeIndex(final int row) {
    if (_meanWrapper == null) {
      ...
      return Math.min(
          _globalContainer.getMaximumTimeIndex(row), _meanWrapper.getMaximumTimeIndex(row));
    }
  }
  ...
  @Override
  public Object getDataValueInInterval(
      final double intervalStart, final double intervalStop, final int row) {
    ...
    if (row < this._scopeIndices.getTotalSignalNumber()) {
      ...
      if (container != null) {
        returnValue =
            container.getDataValueInInterval(
                intervalStart, intervalStop, _scopeIndices.getContainerRowIndex(row));
      }
    }
    return returnValue;
  }
  ...
  @Override
  public float getAVGValueInInterval(
