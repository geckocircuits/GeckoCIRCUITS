...
/**
 * Compresses the stored data with a effective difference compression algorithm & zip compression
 *
 * @author andy
 */
public final class DataContainerCompressable extends AbstractDataContainer
    implements DataContainerValuesSettable, DataContainerIntegralCalculatable {
  ...
  public DataContainerCompressable(
      final int rows,
  ...
  public void setValue(final float value, final int row, final int column) {
    ...
    // assert column / JUNK_SIZE == _data.size() - 1 : column + " " + column / JUNK_SIZE + " " +
    // _data.size();
  ...
  public boolean isInvalidNumbers(final int row) {
    try { // first enshure that all data ranges are read:
      ...
      Logger.getLogger(DataContainerCompressable.class.getName())
  ...
  private void checkContainerSize(final int column) {
    if (column >= _totalDataSize) {
      ...
      _data.add(
          new DataJunkCompressable(_memoryContainer, _totalDataSize, _rows, JUNK_SIZE, _timeSerie));
  ...
  private void calculateMinMax(int fromJunk, int toJunk, int maximumTimeIndex) {
    for (int calculateRow = 0; calculateRow < _rows; calculateRow++) {
      ...
      if (_totMinMaxValues[calculateRow]
          != null) { // if value was already calculated, insert this value to mergelist
