...
public final class DataContainerMeanWrapper extends AbstractDataContainer {
  ...
  public DataContainerMeanWrapper(
      final AbstractDataContainer wrapped, final ScopeWrapperIndices scopeIndices) {
    super();
    _wrapped = wrapped;
  ...
  public float getValue(final int columnIndex, final int row) {
    ...
    if (_wrapped instanceof DataContainerIntegralCalculatable) {
      ...
      return ((DataContainerIntegralCalculatable) _wrapped)
          .getAVGValueInInterval(
              centerTimeValue - timeInterval / 2,
              centerTimeValue + timeInterval / 2,
  ...
  public Object getDataValueInInterval(
    ...
    if (_wrapped instanceof DataContainerIntegralCalculatable) {
      ...
      // System.out.println("Test: " + (centerTimeValue - timeInterval / 2) + " " + (centerTimeValue
      // + timeInterval / 2) + "::" + (timeInterval) + "::" + (intervalStart) + " " +
      // (intervalStop));
      Object returnValue =
          ((DataContainerIntegralCalculatable) _wrapped)
              .getAVGValueInInterval(
                  centerTimeValue - timeInterval / 2,
                  centerTimeValue + timeInterval / 2,
                  _meanSignals.get(columnIndex).getConnectedScopeInputIndex());
  ...
  public void defineMeanSignals(final List<ScopeSignalMean> meanSignals) {
    ...
    for (ScopeSignalMean meanSignal : meanSignals) {
      ...
      _meanSignals.add(
          new ScopeSignalMean(
              new ScopeSignalRegular(containerRowIndex, null), meanSignal.getAveragingTime()));
      _scopeIndices.defineAdditionalSignal(
          this, meanSignal.getConnectedScopeInputIndex() + 1, _meanSignals.size() - 1);
    }
