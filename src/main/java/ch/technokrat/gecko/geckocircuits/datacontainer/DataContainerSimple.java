...
/** @author andy */
public class DataContainerSimple extends AbstractDataContainer
    implements DataContainerValuesSettable {
  ...
  public void insertValuesAtEnd(final float[] values, final double timeValue) {
    ...
    // YYY error! _abMinMaxValues[0] = HiLoData.mergeFromValue(_abMinMaxValues[0], (float)
    // timeValue);
  ...
  public final HiLoData getAbsoluteMinMaxValue(final int row) {
    try {
      ...
      System.out.println(
          "ArrayIndexOutOfBoundsException!! At index : "
              + row
              + ", _abMinMaxValues = "
              + _abMinMaxValues.length);
    }
  ...
  public final int getUsedRAMSizeInMB() {
    return (_data.length * _data[0].length * FLOAT_BYTES
            + _timeSerieArray.getMaximumIndex() * DOUBLE_BYTES)
        / MEGA_BYTE;
  }
  ...
  public Object getDataValueInInterval(double intervalStart, double intervalStop, int columnIndex) {
    ...
    //        System.out.println("Interval Start:" + intervalStart + "\tIntervallStop: " +
    // intervalStop + "\t columnIndex: " + columnIndex);
