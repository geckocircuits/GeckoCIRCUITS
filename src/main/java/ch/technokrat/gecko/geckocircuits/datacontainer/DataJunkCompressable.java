...
/**
 * @author andy Compresses the data employing a zip-compression algorithm. In order to achive a
 *     higher compression ratio, the difference compression algorithm of order 2 (MDIFFORDER) is
 *     used, see therefore the paper: "Lossless Compression of High-volume Numerical Data from
 *     Simulations, Engelson, Fritzson, Fritzson... For conveniece, the float data is directly
 *     stored inside a int array, therefore use the Float.floatBitsToInt and Float.intBitsToFloat to
 *     access the data values. Furhermore, the double values are rounded to less bits than float.
 */
public final class DataJunkCompressable implements DataJunk {
  ...
  public static void setMemoryPrecision() {
    ...
    if (lossyCompression
        == null) { // this is just for backward-compatibility (1.54-> 1.60, maybe remove in the
      // future!
  ...
  /**
   * this bit-field has 1111's at the beginning, whereas the last x bits are zero. And-Connection
   * with floatIntBits will round the float value. e.g. : 2 * Integer.MAX_VALUE + 1 - (1 + 2 + 4 + 8
  ...
  public DataJunkCompressable(
      final MemoryContainer container,
  ...
  /**
   * tells the data-junk to compress its cached data. This is performed within a new Thread, so that
   * the simulation is not slowed down.
   */
  ...
  private static float roundFloat(final float value) {
    ...
    // this is for testing - don't remove. Bitstring should start with 1111, and have trailing zeros
    // at the end.
   ...
   * @return the min-max value in the given row, between the columnStart and columnStop-Index
   *     (global indices)
   */
  @Override
  ...
  public AverageValue getAverageValue(
    ...
    for (int j = findCachedIndex(columnStart); j <= stopIndex && j < VALUE_CACHE_SIZE; j++) {
      ...
      if (returnValue == null) {
        ...
        if (tmp.getIntervalStop() < totalMaxTime) {
          ...
          returnValue.appendAverage(
              calculateRealAverageData(row, returnValue.getIntervalStop(), totalMaxTime));
        }
      }
    }
   ...
   * @return Min-Max values, that are really calculated from the original data. CAREFUL: this is
   *     computational expensive, since the data may have to be de-compressed!
   */
  ...
  private AverageValue calculateRealAverageData(
      final int row, final double startTime, final double stopTime) {
    double localMean = 0;
    double totalTime = 0;
    ...
    public void run() {
      synchronized (_parent) {
        ...
        try {
          ...
          // System.out.println("compressor:  " + _compressor.compressionRatio + " " +
          // _compressor.compressionTime);
          _memInBytes = _compressor.getCompressedMemory();

          //                compressionCounter++;
          //                compressionSum += 1.0 * byteLength / originalLength;
          //                System.out.println("compression ratio: " + compressionSum /
          // compressionCounter);
