...
public class GraferV3 extends JPanel {
  ...
  //
  public static final Stroke str_SOLID_PLAIN =
      new BasicStroke(1f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0.1f);
  public static final Stroke str_INVISIBLE =
      new BasicStroke(
          0.1f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0.1f); // eigentlich unsichtbar
  public static final Stroke str_SOLID_FAT_1 =
      new BasicStroke(2.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0.1f);
  public static final Stroke str_SOLID_FAT_2 =
      new BasicStroke(3.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0.1f);
  public static final Stroke str_DOTTED_PLAIN =
      new BasicStroke(
          1.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0.1f, new float[] {4, 4}, 0);
  public static final Stroke str_DOTTED_FAT =
      new BasicStroke(
          3.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0.1f, new float[] {4, 4}, 0);
  //
  ...
  protected Color[] farbeAchsenX,
      farbeAchsenY; // Farben der einzelnen Achsen, zB. unsichtbar --> weiss
  ...
  protected boolean[] autoAchseXmin,
      autoAchseXmax,
      autoAchseYmin,
      autoAchseYmax; // sind diese Werte auf AUTO gesetzt??
  protected String[] xAchseBeschriftung, yAchseBeschriftung;
  // -------------
  protected Color[] farbeGridNormalX,
      farbeGridNormalY,
      farbeGridNormalXminor,
      farbeGridNormalYminor;
  protected int[] linStilGridNormalX,
      linStilGridNormalY,
  ...
  protected int[][] showGridNormalXmajor,
      showGridNormalXminor,
      showGridNormalYmajor,
      showGridNormalYminor; // welche Grid-Linien sollen dargestellt werden?
  // --> zB. showGridNormalXmajor[i1]= {index_xAchse, index_yAchse}
  // -------------
  protected boolean[] xTickAutoSpacing,
      yTickAutoSpacing; // sollen die Tick-Abstaende automatisch bestimmt werden?
  protected double[] xTickSpacing, yTickSpacing; // Abstand zwischen 2 Ticks, ausgehend von Null
  protected int[] xAnzTicksMinor,
      yAnzTicksMinor; // Zahl der Minor-Ticks zwischen zwei regulaeren Ticks
  protected int[] xTickLaenge, yTickLaenge, xTickLaengeMinor, yTickLaengeMinor;
  protected boolean[] zeigeXticksUnten,
      zeigeYticksLinks; // Ticks auf der x-Achse koennen nach unten oder nach oben zeigen, analog
  // die der y-Achse
  //
  protected boolean[] zeigeLabelsXmaj,
      zeigeLabelsXmin,
      zeigeLabelsYmaj,
      zeigeLabelsYmin; // sollen die entsprechenden Labels bei den jeweiligen Ticks angezeigt
  // werden?
  protected int[] posXtickLabels,
      posYtickLabels; // wie weit sind die Tick-Beschriftungen von der jeweiligen Achse entfernt?
  protected Font[] foTickLabelX, foTickLabelY;
  // -------------
  protected int[][] tickX, tickY, tickXminor, tickYminor; // Pixelpunkt-Position der Ticks
  protected double[][] wertTickX,
      wertTickY,
      wertTickXminor,
      wertTickYminor; // Zahlenwerte der einzelnen Ticks
  protected double[] sfX, sfY; // Streckfaktoren fuer Umrechnung Pixel <--> Werte
  // -------------------------------------
  // (2) Kurven:
  protected DataContainer
      worksheetDaten; // hier stehen die Punkte aller Kurven drinnen  --> derzeit nur EIN Worksheet
  // implementiert
  // -------------
  protected int[] indexZurKurveGehoerigeXachse,
      indexZurKurveGehoerigeYachse; // Zuordnung Kurve <--> Achsen
  ...
  protected boolean[]
      kurvenPunktSymbolAnzeigen; // Sollen die Punkte der Kurve als Punkte dargestellt werden
  protected int[] crvSymbFrequ,
      crvSymbShape; // Details zum Zeichnen der Symbole auf den Kurven-Datenpunkten
  protected Color[] crvSymbFarbe; // Details zum Zeichnen der Symbole auf den Kurven-Datenpunkten
  protected double[] kurveClippling_xmin,
      kurveClippling_xmax,
      kurveClippling_ymin,
      kurveClippling_ymax; // definierter Zahlenwert fuers Clipping
  protected int[] clipXmin,
      clipXmax,
      clipYmin,
      clipYmax; // Art des Cilpping --> "AXIS", "NO CLIP", "VALUE"
  ...
  protected int[]
      kurvenTypZVvsSIGNAL; // wird direkt ohne 'get'-Fkt. von der abgeleiteten Klasse angeprochen
  // -------------------------------------
  ...
  public void setzeAchsenBegrenzungen(
      double[] xMin,
      double[] xMax,
      boolean[] autoScaleX,
      double[] yMin,
  ...
  public void setzeGridFarben(
      Color[] farbeGridNormalX,
      Color[] farbeGridNormalY,
      Color[] farbeGridNormalXminor,
      Color[] farbeGridNormalYminor) {
  ...
  public void setzeGridLinienStil(
      int[] linStilGridNormalX,
      int[] linStilGridNormalY,
      int[] linStilGridNormalXminor,
      int[] linStilGridNormalYminor) {
  ...
  public void showGridLines(
      int[][] showGridNormalXmajor,
      int[][] showGridNormalXminor,
      int[][] showGridNormalYmajor,
      int[][] showGridNormalYminor) {
  ...
  public void setzeTickLabelAnzeige(
      boolean[] xMaj, boolean[] yMaj, boolean[] xMin, boolean[] yMin) {
  ...
  public void setzeKurvePunktSymbolAnzeigen(
      boolean[] sym, int[] crvSymbFrequ, int[] crvSymbShape, Color[] crvSymbFarbe) {
  ...
  public void setzeKurveClipping(
      double[] xmin,
      double[] xmax,
      double[] ymin,
      double[] ymax,
  ...
  public void autoSettingsAnpassen() {
    // AUTO-Settings koennen erst abschliessend erfolgen, wenn alle relevanten Grafer-Daten
    // vorhanden sind
    // --> expliziter Aufruf
  ...
  protected void zeichneEinzelneKurve(Graphics2D g2, int i1, int anzKurvenpunkteImWorksheet) {
    ...
    int y0Kurve =
        _yAchseY[indexZurKurveGehoerigeYachse[i1]]; // zugehoerige y-Achse definiert y0 der Kurve
    ...
    for (int i2 = 0; i2 < anzKurvenpunkteImWorksheet; i2++) {
            ...
            (float)
                (x0Kurve
                    + (sfX[indexZurKurveGehoerigeXachse[i1]]
                        * (x - achseXmin[indexZurKurveGehoerigeXachse[i1]])));
      } else if ((xAchseTyp[indexZurKurveGehoerigeXachse[i1]] == ACHSE_LOG)) {
        xPix[i2] =
            (float)
                (x0Kurve
                    + (sfX[indexZurKurveGehoerigeXachse[i1]]
                        * this.lg10(x / achseXmin[indexZurKurveGehoerigeXachse[i1]])));
      }
      ...
      if (yAchseTyp[indexZurKurveGehoerigeYachse[i1]] == ACHSE_LIN) {
        yPix[i2] =
            (float)
                (y0Kurve
                    - (sfY[indexZurKurveGehoerigeYachse[i1]]
                        * (y - achseYmin[indexZurKurveGehoerigeYachse[i1]])));
        //                if(y == 20.15) {
        ...
        yPix[i2] =
            (float)
                (y0Kurve
                    - (sfY[indexZurKurveGehoerigeYachse[i1]]
                        * this.lg10(y / achseYmin[indexZurKurveGehoerigeYachse[i1]])));
      }
    }

    g2.setClip(
        x0Kurve + 1,
        y0Kurve - hoehePix[indexZurKurveGehoerigeYachse[i1]] - 1,
        breitePix[indexZurKurveGehoerigeYachse[i1]] + 2,
        hoehePix[indexZurKurveGehoerigeYachse[i1]] + 3);
    // --------------------------------
      ...
      for (int i2 = 0; i2 < anzKurvenpunkteImWorksheet; i2++) {
        ...
        if (i2 % crvSymbFrequ[i1] == 0) {
          switch (crvSymbShape[i1]) {
            case SYBM_CIRCLE:
              g2.drawOval(
                  Math.round(xPix[i2]) - dmCIRCLE / 2,
                  (int) Math.round(yPix[i2]) - dmCIRCLE / 2,
                  dmCIRCLE,
                  dmCIRCLE);
              break;
            case SYBM_CIRCLE_FILLED:
              g2.fillOval(
                  Math.round(xPix[i2]) - dmCIRCLE / 2,
                  Math.round(yPix[i2]) - dmCIRCLE / 2,
                  dmCIRCLE,
                  dmCIRCLE);
              break;
            case SYBM_CROSS:
              g2.drawLine(
                  Math.round(xPix[i2]) - hCROSS,
                  Math.round(yPix[i2]),
                  Math.round(xPix[i2]) + hCROSS,
                  Math.round(yPix[i2]));
              g2.drawLine(
                  Math.round(xPix[i2]),
                  Math.round(yPix[i2]) - hCROSS,
                  Math.round(xPix[i2]),
                  Math.round(yPix[i2]) + hCROSS);
              break;
            case SYBM_RECT:
              g2.drawRect(
                  Math.round(xPix[i2]) - aRECT / 2, Math.round(yPix[i2]) - aRECT / 2, aRECT, aRECT);
              break;
            ...
            case SYBM_TRIANG:
              g2.drawPolygon(
                  new int[] {
                    Math.round(xPix[i2]) - aTRIANG / 2,
                    Math.round(xPix[i2]) + aTRIANG / 2,
                    Math.round(xPix[i2])
                  },
                  new int[] {
                    Math.round(yPix[i2]) + (int) (0.29 * aTRIANG),
                    Math.round(yPix[i2]) + (int) (0.29 * aTRIANG),
                    Math.round(yPix[i2]) - (int) (0.58 * aTRIANG)
                  },
                  3);
              break;
  ...
  protected void zeichneEinzelneKoordinatenAchse_X(Graphics2D g2, int i1) {
    ...
    if (xAchseTyp[i1] == ACHSE_LIN) {
      ...
      int anzTicks =
          (int) (achseXmax[i1] / xTickSpacing[i1]) - (int) (achseXmin[i1] / xTickSpacing[i1]) + 1;
      double[] wertTickX_temp = new double[anzTicks];
      int[] tickX_temp = new int[anzTicks];
      int j = 0;
      for (int i2 = 0; i2 < anzTicks; i2++) {
        double wert =
            xTickSpacing[i1] * (int) (achseXmin[i1] / xTickSpacing[i1]) + i2 * xTickSpacing[i1];
      ...
      if (j != 0) {
        ...
        for (int i2 = 0; i2 < anzTicks; i2++) {
          ...
          g2.drawLine(
              ...
              _yAchseX[i1],
              tickX[i1][i2],
              _yAchseX[i1] - d); // Ticks auf der x-Achse

          if (zeigeLabelsXmaj[i1]) {
            g2.setFont(foTickLabelX[i1]);
            String label =
                tcf.formatENG(wertTickX[i1][i2], digitsX); // nf.format(wertTickX[i1][i2]);
            g2.drawString(
                label,
                -3 + tickX[i1][i2],
                _yAchseX[i1] + posXtickLabels[i1]); // Labels auf der x-Achse
          }
        }
        //
        double xTickSpacingMinor =
            xTickSpacing[i1] / xAnzTicksMinor[i1]; // Wert zwischen zwei Minor-Ticks auf der x-Achse
        ...
        for (int i2 = 0; i2 < xMinorTicksAnzahl; i2++) {
          ...
          g2.drawLine(
              ...
              _yAchseX[i1],
              tickXminor[i1][i2],
              _yAchseX[i1] - d); // Minor-Ticks auf der x-Achse
          if (zeigeLabelsXmin[i1]) {
            g2.setFont(foTickLabelX[i1]);
            String label =
                tcf.formatENG(
                    wertTickXminor[i1][i2], digitsX); // nf.format(wertTickXminor[i1][i2]);
            g2.drawString(
                label,
                tickXminor[i1][i2],
                _yAchseX[i1] + posXtickLabels[i1]); // Minor-Labels auf der x-Achse
          }
          // -----------------------------------------------------------------------------------------------------------------------
        }
      }
      ...
      // zwingend -->  xTickSpacing[i1]=AUTO  weil nur sinnvollerweise die Zehner-Dekaden mit Ticks
      // versehen werden
      ...
      if (j != 0) {
        ...
        for (int i2 = 0; i2 < anzTicks; i2++) {
          ...
          if (zeigeLabelsXmaj[i1]) {
            g2.setFont(foTickLabelX[i1]);
            String label =
                tcf.formatT(wertTickX[i1][i2], "#.E0"); // tcf.formatENG(wertTickX[i1][i2],digitsX);
            g2.drawString(
                label, tickX[i1][i2], _yAchseX[i1] + posXtickLabels[i1]); // Labels auf der x-Achse
          }
        }
        //
        ...
        for (int i2 = 0; i2 < xMinorTicksAnzahl; i2++) {
          ...
          if (zeigeLabelsXmin[i1]) {
            ...
            String label =
                tcf.formatT(
                    wertTickXminor[i1][i2],
                    "#.E0"); // tcf.formatENG(wertTickXminor[i1][i2],digitsX);
            g2.drawString(
                label,
                tickXminor[i1][i2],
                _yAchseX[i1] + posXtickLabels[i1]); // Minor-Labels auf der x-Achse
  ...
  protected void zeichneEinzelneKoordinatenAchse_Y(Graphics2D g2, int i1) {
    ...
    if (yAchseTyp[i1] == ACHSE_LIN) {
      ...
      int anzTicks =
          (int) (achseYmax[i1] / yTickSpacing[i1]) - (int) (achseYmin[i1] / yTickSpacing[i1]) + 1;
      ...
      for (int i2 = 0; i2 < anzTicks; i2++) {
        double wert =
            yTickSpacing[i1] * (int) (achseYmin[i1] / yTickSpacing[i1]) + i2 * yTickSpacing[i1];
      ...
      for (int i2 = 0; i2 < anzTicks; i2++) {
        g2.setColor(farbeAchsenY[i1]);
        g2.drawLine(
            _xAchseY[i1], tickY[i1][i2], _xAchseY[i1] + d, tickY[i1][i2]); // Ticks auf der y-Achse
        if (zeigeLabelsYmaj[i1]) {
          ...
          g2.drawString(
              ...
              _xAchseY[i1]
                  - posYtickLabels[i1]
                  - (int) g2.getFontMetrics().getStringBounds(label, g2).getWidth(),
              tickY[i1][i2] + foTickLabelY[i1].getSize() / 2 - 1); // Labels auf der y-Achse
        }
      }
      //
      double yTickSpacingMinor =
          yTickSpacing[i1] / yAnzTicksMinor[i1]; // Wert zwischen zwei Minor-Ticks auf der y-Achse
      ...
      for (int i2 = 0; i2 < yMinorTicksAnzahl; i2++) {
        ...
        g2.drawLine(
            ...
            tickYminor[i1][i2],
            _xAchseY[i1] + d,
            tickYminor[i1][i2]); // Minor-Ticks auf der y-Achse
        if (zeigeLabelsYmin[i1]) {
          g2.setFont(foTickLabelY[i1]);
          String label =
              tcf.formatENG(wertTickYminor[i1][i2], digitsY); // nf.format(wertTickYminor[i1][i2]);
          g2.drawString(
              label,
              _xAchseY[i1]
                  - posYtickLabels[i1]
                  - (int) g2.getFontMetrics().getStringBounds(label, g2).getWidth(),
              tickYminor[i1][i2] + foTickLabelY[i1].getSize() / 2); // Minor-Labels auf der y-Achse
        }
      }
      // ==================================
      ...
      // zwingend -->  yTickSpacing[i1]=AUTO  weil nur sinnvollerweise die Zehner-Dekaden mit Ticks
      // versehen werden
      ...
      if (j != 0) {
        ...
        for (int i2 = 0; i2 < anzTicks; i2++) {
          ...
          g2.drawLine(
              ...
              tickY[i1][i2],
              _xAchseY[i1] + d,
              tickY[i1][i2]); // Ticks auf der y-Achse
          if (zeigeLabelsYmaj[i1]) {
            g2.setFont(foTickLabelY[i1]);
            String label =
                tcf.formatT(wertTickY[i1][i2], "#.E0"); // tcf.formatENG(wertTickY[i1][i2],digitsY);
            g2.drawString(
                label,
                _xAchseY[i1]
                    - posYtickLabels[i1]
                    - (int) g2.getFontMetrics().getStringBounds(label, g2).getWidth(),
                tickY[i1][i2] + foTickLabelY[i1].getSize() / 2); // Labels auf der y-Achse
          }
        }
        //
        ...
        for (int i2 = 0; i2 < yMinorTicksAnzahl; i2++) {
          ...
          if (zeigeLabelsYmin[i1]) {
            ...
            String label =
                tcf.formatT(
                    wertTickYminor[i1][i2],
                    "#.E0"); // tcf.formatENG(wertTickYminor[i1][i2],digitsY);
            g2.drawString(
                label,
                _xAchseY[i1]
                    - posYtickLabels[i1]
                    - (int) g2.getFontMetrics().getStringBounds(label, g2).getWidth(),
                tickYminor[i1][i2]
                    + foTickLabelY[i1].getSize() / 2); // Minor-Labels auf der y-Achse
          }
        }
        // -----------------------------------------------------------------------------------------------------------------------
      }
    ...
    for (int i1 = 0; i1 < gridNormalX_zugeordneteXAchse.length; i1++) {
      ...
      if ((indexAchseX != -1) && (indexAchseY != -1)) {
        ...
        for (int i3 = 0; i3 < showGridNormalXminor.length; i3++) {
          if ((showGridNormalXminor[i3][0] == indexAchseX)
              && ((showGridNormalXminor[i3][1] == indexAchseY))
              && (tickXminor[indexAchseX] != null)) {
            for (int i2 = 0; i2 < tickXminor[indexAchseX].length; i2++) {
              ...
              grL.lineTo(
                  tickXminor[indexAchseX][i2], _yAchseY[indexAchseY] - hoehePix[indexAchseY]);
        ...
        for (int i3 = 0; i3 < showGridNormalXmajor.length; i3++) {
          if ((showGridNormalXmajor[i3][0] == indexAchseX)
              && ((showGridNormalXmajor[i3][1] == indexAchseY))
              && (tickX[indexAchseX] != null)) {
    ...
    for (int i1 = 0; i1 < gridNormalY_zugeordneteXAchse.length; i1++) {
      ...
      if ((indexAchseX != -1) && (indexAchseY != -1)) {
        ...
        for (int i3 = 0; i3 < showGridNormalYminor.length; i3++) {
          if ((showGridNormalYminor[i3][0] == indexAchseX)
              && ((showGridNormalYminor[i3][1] == indexAchseY))
              && (tickYminor[indexAchseY] != null)) {
            for (int i2 = 0; i2 < tickYminor[indexAchseY].length; i2++) {
              ...
              grL.lineTo(
                  _xAchseX[indexAchseX] + breitePix[indexAchseX], tickYminor[indexAchseY][i2]);
        ...
        for (int i3 = 0; i3 < showGridNormalYmajor.length; i3++) {
          if ((showGridNormalYmajor[i3][0] == indexAchseX)
              && ((showGridNormalYmajor[i3][1] == indexAchseY))
              && (tickY[indexAchseY] != null)) {
  ...
  protected void auto_BereichsgrenzenDerAchsen() {
    ...
    double[] minEmpfehlungLIN =
        new double[laenge]; // empfohlene Achsen-Min-Werte bei AUTO / ACHSE_LIN
    double[] maxEmpfehlungLIN =
        new double[laenge]; // empfohlene Achsen-Max-Werte bei AUTO / ACHSE_LIN
    double[] minEmpfehlungLOG =
        new double[laenge]; // empfohlene Achsen-Min-Werte bei AUTO / ACHSE_LOG
    double[] maxEmpfehlungLOG =
        new double[laenge]; // empfohlene Achsen-Max-Werte bei AUTO / ACHSE_LOG
    double[] min =
        new double[laenge]; // Min- u. Max-Werte der einzelnen Kolonnen von 'worksheetDaten'
    ...
    for (int achsenNr = 0; achsenNr < achseXmin.length; achsenNr++) {
      if (autoAchseXmin[achsenNr]) {
        // jetzt den Achsen, die von Kurven besetzt sind, die entsprechenden Minima und Maxima
        // zuteilen:
        ...
        for (int nrKurve = 0; nrKurve < anzahlKurven; nrKurve++) {
          if (achsenNr == indexZurKurveGehoerigeXachse[nrKurve]) {
            if ((xAchseTyp[achsenNr] == ACHSE_LIN)
                && (achseXmin[achsenNr]
                    > minEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][0]])) {
              achseXmin[achsenNr] = minEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][0]];
            } else if ((xAchseTyp[achsenNr] == ACHSE_LOG)
                && (achseXmin[achsenNr]
                    > minEmpfehlungLOG[kurve_index_worksheetKolonnen_XY[nrKurve][0]])) {
      ...
      if (autoAchseXmax[achsenNr]) {
        ...
        for (int nrKurve = 0; nrKurve < anzahlKurven; nrKurve++) {
          if (achsenNr == indexZurKurveGehoerigeXachse[nrKurve]) {
            if ((xAchseTyp[achsenNr] == ACHSE_LIN)
                && (achseXmax[achsenNr]
                    < maxEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][0]])) {
              achseXmax[achsenNr] = maxEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][0]];
            } else if ((xAchseTyp[achsenNr] == ACHSE_LOG)
                && (achseXmax[achsenNr]
                    < maxEmpfehlungLOG[kurve_index_worksheetKolonnen_XY[nrKurve][0]])) {
        ...
        for (int nrKurve = 0; nrKurve < anzahlKurven; nrKurve++) {
          if (achsenNr == indexZurKurveGehoerigeYachse[nrKurve]) {
            if ((yAchseTyp[achsenNr] == ACHSE_LIN)
                && (achseYmin[achsenNr]
                    > minEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][1]])) {
              achseYmin[achsenNr] = minEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][1]];
            } else if ((yAchseTyp[achsenNr] == ACHSE_LOG)
                && (achseYmin[achsenNr]
                    > minEmpfehlungLOG[kurve_index_worksheetKolonnen_XY[nrKurve][1]])) {
      ...
      if (autoAchseYmax[achsenNr]) {
        ...
        for (int nrKurve = 0; nrKurve < anzahlKurven; nrKurve++) {
          if (achsenNr == indexZurKurveGehoerigeYachse[nrKurve]) {
            if ((yAchseTyp[achsenNr] == ACHSE_LIN)
                && (achseYmax[achsenNr]
                    < maxEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][1]])) {
              achseYmax[achsenNr] = maxEmpfehlungLIN[kurve_index_worksheetKolonnen_XY[nrKurve][1]];
            } else if ((yAchseTyp[achsenNr] == ACHSE_LOG)
                && (achseYmax[achsenNr]
                    < maxEmpfehlungLOG[kurve_index_worksheetKolonnen_XY[nrKurve][1]])) {
  ...
  protected double[] auto_Achsenbegrenzung_Wertempfehlung(double z1, double z2) {
    // ----------------------
    // achse= [z1 .. z2] --> gesuchte Werte sollen 'rund'zahlig und eventuell ein bischen groesser
    // als z1, z2 sein
    ...
    if ((z1 == z2) || (Math.abs(z1 - z2) / (Math.abs(z1) + Math.abs(z2)) < 1e-12)) {
      ...
      // verschiedene Basis kann parallel untersucht werden zB. 2 und 5, und davon dann ein
      // endgueltige Basis ausgewaehlt werden
