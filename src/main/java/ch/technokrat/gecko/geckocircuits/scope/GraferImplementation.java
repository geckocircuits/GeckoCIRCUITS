...
@Deprecated
public final class GraferImplementation extends GraferV3
    implements MouseListener, MouseMotionListener {
  // Anzahl der Intervalle auf der x-Achse, in denen Hi- und Lo-Werte zwecks Datenkompression
  // ermittelt werden
  ...
  // gibt an, wieviel der Punkte im Worksheet als Kurve dargestellt werden sollen, siehe Fkt. weiter
  // unten
  ...
  public static final int ANZ_DIAGRAM_MAX =
      9; // Zahl der maximal moeglichen Diagramme in einem SCOPE
  public static int DX_IN_LINKS = 60,
      DX_IN_RECHTS = 70; // links- u. rechtsseitige x-Einrueckung der Achsen in Pixel
  public static int DY_IN_OBEN = 8,
      DY_IN_UNTEN =
          8; // y-Einrueckung der Achsen in Pixel von Oben bzw. Unten und y-Abstand zwischen 2
  // Diagrammen
  public static int ABSTAND_BESCHRIFTUNG_XACHSE =
      35; // soviel Abstand nach ganz unten gibt es zusaetzlich, damit die x-Achsen-Labels gesetzt
  // werden koennen
  ...
  public int[]
      notwendigeHoehePixGRF; // bei SIGNAL wird die Graph-Hoehe in Pix vorgegeben, eventuell wird
  // die Scope-Groesse angepasst (bei ZV --> '-1')
  public int[] diagramTyp; // ist das jeweilige Diagramm ein ZV-Typ oder ein Signal-Typ?
  public boolean[]
      jcbShowLegende; // sollen die Kurvennamen in Form einer Legende am linken Graph-Rand angezeigt
  // werden?
  ...
  public boolean[] autoScaleX,
      autoScaleY; // sollen die Achsenbegrenzungen automatisch an die Worksheetdaten angepasst
  // werden?
  ...
  public int[] linStilGridNormalX,
      linStilGridNormalXminor,
      linStilGridNormalY,
      linStilGridNormalYminor;
  ...
  //
  private boolean[]
      zeichneDiagrammUmrandung; // zu zeichnen, wenn der Grid wegen zu kleiner Darstellung (in
  // Pixelpunkten) abgeschaltet ist
  // -------------------------
  // Graph-Properties --> speziell fuer SIGNAL
  public int[]
      positionSIGNAL; // fuer SIGNAL --> enthaelt die y-Postiion (Reihenfolge) der Signal-Kurve
  // innerhalb des einzelnen Graphen
  ...
  public static final int ZUORDNUNG_X = 51,
      ZUORDNUNG_Y = 52,
      ZUORDNUNG_SIGNAL = 54,
      ZUORDNUNG_NIX = 55,
      ZUORDNUNG_MEAN = 56;
  ...
  // Kurven-Properties
  public int
      kurvenanzahl; // soviel verschiedene Kurven werden aktuell im SCOPE dargestellt --> entspricht
  // einer 'Kurven-ID'
  // zur Zuordnung der Kurvenindizes zur Zuordnungsmatrix:
  public int[] indexDerKurveInDerMatrix; // Abspeichern in folgendem Format: --> 1000*i1 +i2 wobei
  // (i1..Graphenanzahl / i2..Kurvenanzahl)
  public int[]
      indexDerKurveInDerMatrixALT; // Speicherung notwendig um Darstellung der SIGNAL-Ordnung
  // korrekt durchfuehren zu koennen
  //
  // jedem Eintrag in der Verknuepfungsmatrix entspricht eine potentielle Kurve -->
  public int[][] crvAchsenTyp; // wird ueber SET-Methode aktualisiert, damit die Matrix
  // 'matrixZuordnungKurveDiagram' nicht vergessen wird!
  ...
  public static final int MAUSMODUS_ZOOM_AUTOFIT =
      547; // Diagramm passt sich immer an die Datenwerte an
  public static final int MAUSMODUS_ZOOM_FENSTER =
      548; // man kann mit der Maus Zoom-Rechtecke markieren
  public static final int MAUSMODUS_ZEICHNE_LINIE = 550; // Linien zeichnen (als Objekte!)
  public static final int MAUSMODUS_WERTANZEIGE_SCHIEBER =
      554; // ein Schieber kann ueber alle Diagramme gelegt werden, die entsprechenden y-Werte alle
  // Kurven werden angezeigt
  //
  ...
  private int mausModusALT =
      MAUSMODUS_NIX; // damit man in den vorigen Modus zurueckkehren kann zB. nach Druecken von
  // AutoFit
  //
  private boolean simulationLaeuftGerade = false;
  private boolean nochNichtGeZoomt = true;
  private double[][] worksheetDatenTEMP =
      null; // vor dem Zoomen werden hier die Simulationsdaten abgelegt
  ...
  private double[] xSchieberWert =
      new double[] {-1, -1}; // einem einzelnen Pixelpunkt sind eventuell mehrere Werte zugeordnet
  private double[][]
      ySchieberWert; // pro Kurve gibt es zum xSchieberWert-Punktepaar ein entsprechendes
  // ySchieberWert-Punktepaar
  ...
  // zum Aufrufen externer Daten, die nicht mehr veraendert werden (im Gegensatz zum
  // kontinuierlichen Kurvenaufbau in der Schaltungssimulation) -->
  // bzw. beim Initialisieren/Aendern der Kurvenanzahl fuer die Simulation
  public final void setzeKurvenUndWorksheetDaten(
      final String[] header, final DataContainer workSheet) {
    ...
    // XXX _scopeSettings.update_ZVs(anzSignalePlusZeit, signalNamen);  // default-Initialisierung
    // der Kurven bzw. bei Aenderung der Kurven-Anzahl
    // XXX _scopeSettings.loadSettings(this);  // die in 'ScopeSettings' definierten Parameter
    // werden importiert
    // --------------------------
    if (this._usesExternalData) {
      // Daten werden von extern importiert -->
      this.definiereAchsenbegrenzungenImAutoZoom(
          workSheet); // minX[],maxX[],minY[],maxY[],minY2[],maxY2[] werden aus 'worksheetDaten'
      // berechnet
      this
          .initClipping(); // crvClipValXmin[][],crvClipValXmax[][],crvClipValYmin[][],crvClipValYmax[][] werden berechnet
      this.initAutotickSpacing(); // benoetigt 'minX[],maxX[],minY[],maxY[],...' zur Berechnung
    } else {
      // Daten kommen von der laufenden ZV-Simulation / hier werden SCOPEs initialisiert, bevor
      // Simulationsdaten da sind -->
      ...
      // (2) crvClipValXmin[][],crvClipValXmax[][],crvClipValYmin[][],crvClipValYmax[][] werden
      // berechnet
      this.initClipping();
    ...
    // -------------------------------------
    this.setzeAchsen(); // die default-Werte der Achsen werden definiert und richtig aufbereitet an
    // GraferV3 weitergegeben, die Tick-Parameter wurden in
    // 'initAutotickSpacing()' ermittelt
    this.setzeKurven(); // die default-Werte der Kurven (in 'setDefault_ZVs' definiert) werden
    // richtig aufbereitet an GraferV3 weitergegeben
    // --------------------------
    this.repaint();
  }
  ...
  public void akualisiereKurvenUndWorksheetDaten(final double t1, final double t2) {
    // --------------------------
    this.definiereAchsenbegrenzungenNumerischeSimulation(
        t1, t2); // minX[],maxX[],minY[],maxY[] werden aus 'worksheetDaten' berechnet
    // --------------------------
    this.setzeAchsen(); // die default-Werte der Achsen werden definiert und richtig aufbereitet an
    // GraferV3 weitergegeben, die Tick-Parameter wurden in
    // 'initAutotickSpacing()' ermittelt
  ...
  protected void zeichneKurven(final Graphics g) {
    ...
    for (int i1 = 0; i1 < anzahlKurven; i1++) {
      if (matrixZuordnungKurveDiagram[indexDerKurveInDerMatrix[i1] / 1000][
              indexDerKurveInDerMatrix[i1] % 1000]
          == this.ZUORDNUNG_SIGNAL) {
        try {
        ...
        // ----------
        if ((i1 > 0)
            && (_yAchseY[indexZurKurveGehoerigeYachse[i1]]
                != _yAchseY[indexZurKurveGehoerigeYachse[i1 - 1]])) {
          zd = 0;
        } else {
          if (i1 > 0) {
            zd++;
        ...
        this.beschrifteNamenDerEinzelnenZVKurve(
            g2, i1, zd); // ZV-Kurven-Beschriftung: wird zwecks Allgemeinheit nicht in 'GraferV3'
        // implementiert sondern gleich weiter unten
      }
    }
  }
  ...
  // i1 ... KurvenNummer
  private void beschrifteNamenDerEinzelnenZVKurve(
      final Graphics2D g2D, final int i1, final int zd) {
    // --------------------------------
    int yLinksObenKurve =
        _yAchseY[indexZurKurveGehoerigeYachse[i1]] - hoehePix[indexZurKurveGehoerigeYachse[i1]];
    String name = signalNamen[indexDerKurveInDerMatrix[i1] % 1000];
    cf.setMaximumDigits(4);
    String wert =
        cf.formatT(
            ySchieberWert[indexDerKurveInDerMatrix[i1] % 1000 - 1][0], TechFormat.FORMAT_AUTO);
    ...
    if (xSchieberAktiv) {
      ...
      g2D.drawString(
          ...
          yLinksObenKurve + g2D.getFont().getSize() + 2 * zd * delta);
      String labelString = "";
      if (inDiffMode) {
        labelString += "diff ";
      }
      ...
      g2D.drawString(
          labelString,
          this.getWidth() - DX_IN_RECHTS + TXT_DISTANCE_Y,
          yLinksObenKurve + g2D.getFont().getSize() + 2 * zd * delta + delta);
      g2D.setFont(oldFont);
    } else {
      g2D.drawString(
          name,
          this.getWidth() - DX_IN_RECHTS + TXT_DISTANCE_Y,
          yLinksObenKurve + g2D.getFont().getSize() + zd * delta);
    }
    // --------------------------------
  }
  ...
  protected void zeichneKoordinatenAchsen(final Graphics g) {
    ...
    // ===============================================
    // wenn die Grid-Linien ausgeblendet werden (automatisch, weil Diagramm zu klein in
    // Pixelpunkten),
    ...
    for (int i1 = 0; i1 < anzahlAchsenY; i1++) {
      if (diagramTyp[i1] == GraferImplementation.DIAGRAM_TYP_ZV) {
        zeichneEinzelneKoordinatenAchse_Y(
            g2, i1); // hier werden auch die Ticks fuer den Grid berechnet
      } else {
    ...
    // ===============================================
    // nachfolgend werden die X- und die Y-Achse noch einmal gezeichnet, damit sie nicht eventuell
    // vom Grid (der eine andere Farbe haben kann)
    // ueberdeckt werden
    // die Methoden 'this.zeichneGrid_NormalX(g)' und 'this.zeichneGrid_NormalY(g)' koennen nicht
    // vor die Schleife zum Zeichnen der Achsen
    // gestellt werden, weil in 'this.zeichneEinzelneKoordinatenAchse_X(g2,i1)' und
    // 'this.zeichneEinzelneKoordinatenAchse_Y(g2,i1)' zuerst einmal
    ...
    for (int i1 = 0; i1 < anzahlAchsenX; i1++) {
      ...
      if (linienStilAchsenX[i1] != GraferV3.INVISIBLE) {
        g2.draw(grL);
        g2.drawString(
            xAchseBeschriftung[i1],
            _xAchseX[i1] + breitePix[i1] / 2,
            _yAchseX[i1] + posXtickLabels[i1]);
      }
    ...
    for (int i1 = 0; i1 < anzahlAchsenY; i1++) {
      ...
      if (linienStilAchsenY[i1] != GraferV3.INVISIBLE) {
        g2.draw(grL);
        g2.drawString(
            yAchseBeschriftung[i1],
            _xAchseY[i1] - posYtickLabels[i1],
            _yAchseY[i1] - hoehePix[i1] / 2);
      }
      g2.setStroke(str_SOLID_PLAIN); // wieder auf 'default' setzen
    }
    // ==================================
  ...
  protected void zeichneEinzelneSIGNALKoordinatenAchse_Y(final Graphics2D g2, final int i1) {
    ...
    //
    // ==================================
    // es gibt einen y-Tick bei '0' und einen bei '1'; und zwar fuer jeden SIGNAL-Verlauf innerhalb
    // des entsprechenden Graphen
    int z = 0;
    ...
    wertTickY[i1] =
        new double[anzTicks]; // zum Tick gehoeriger y-Zahlenwert --> wird hier nicht verwendet
  ...
  private void reorderLine(final int[] positionSIGNAL, int z1, int z2) {
    ...
    // System.out.print("reorderLine() >>  ");  for (int i1=0;  i1<toBeOrdered.length;  i1++)
    // System.out.print(toBeOrdered[i1]+"  "); System.out.println();
    // -----------
  ...
  // daher muss der alte Wert von 'indexDerKurveInDerMatrix[]' gespeichert sein, und eine
  // entsprechende Umrechnung erfolgen
  private void setzeYPositionDerSIGNALverlaeufe() {
    // -------------------------------------
    if (positionSIGNAL != null) {
      // ------------------------
      // aktualisiert, weil sich die Kurvenanzahl geaendert haben koennte, zuwerst alles mit '-1'
      // markieren -->
      ...
      while (z2 < kurvenanzahl) {
        while ((z2 < kurvenanzahl)
            && (indexDerKurveInDerMatrix[z1] / 1000 == indexDerKurveInDerMatrix[z2] / 1000)) {
          z2++;
        }
    ...
    // for (int i1=0;  i1<positionSIGNAL.length;  i1++) System.out.println(i1+"  (1)
    // "+positionSIGNAL[i1]);  System.out.println("-----------");
    // System.out.println("******************************");
  }
  ...
  private void speichereALTeWerteFuerPosition() {
    ...
    // -------------------------------------
    // eine Kopie von 'indexDerKurveInDerMatrix' aufheben, damit Darstellungs-Abfolge von SIGNAL
    // korrekt erfolgen kann:
    indexDerKurveInDerMatrixALT = new int[indexDerKurveInDerMatrix.length];
    System.arraycopy(
        indexDerKurveInDerMatrix,
        0,
        indexDerKurveInDerMatrixALT,
        0,
        indexDerKurveInDerMatrix.length);
    // -------------------------------------
  }
  ...
  private void zeichneEinzelneSIGNALKurve(Graphics2D g2, int i1) {
    ...
    final int x0Kurve =
        _xAchseX[indexZurKurveGehoerigeXachse[i1]]; // zugehoerige x-Achse definiert x0 der Kurve
    final int y0Kurve =
        _yAchseY[indexZurKurveGehoerigeYachse[i1]]
            - (notwendigeHoehePixGRF[indexZurKurveGehoerigeXachse[i1]]
                - (DY_IN_OBEN + DY_IN_UNTEN)); // zugehoerige y-Achse definiert y0 der Kurve,
    // 'notwendigeHoehePixGRF[i1]' zur optischen Invertierung
    // (Kurve links ganz oben)
    final int delta =
        sgnDistance[indexZurKurveGehoerigeXachse[i1]] + sgnHeight[indexZurKurveGehoerigeXachse[i1]];
    ...
    for (int i2 = 0; i2 < _zvCounter; i2++) {
      ...
      if (xAchseTyp[indexZurKurveGehoerigeXachse[i1]] == ACHSE_LIN) {
        xPix[i2] =
            x0Kurve
                + (int)
                    (sfX[indexZurKurveGehoerigeXachse[i1]]
                        * (xValue - achseXmin[indexZurKurveGehoerigeXachse[i1]]));
      } else if ((xAchseTyp[indexZurKurveGehoerigeXachse[i1]] == ACHSE_LOG)) {
        xPix[i2] =
            x0Kurve
                + (int)
                    (sfX[indexZurKurveGehoerigeXachse[i1]]
                        * this.lg10(xValue / achseXmin[indexZurKurveGehoerigeXachse[i1]]));
      }
      // ------------------
    ...
    if (xSchieberAktiv) {
      g2.drawString(
          (ySchieberWert[indexDerKurveInDerMatrix[i1] % 1000 - 1][0]
                  < sgnSchwelle[indexZurKurveGehoerigeXachse[i1]]
              ? "off"
              : "on"),
          DX_IN_LINKS - 30,
          (y0Kurve + sgnDistance[indexZurKurveGehoerigeXachse[i1]])
              + (int) (positionSIGNAL[i1] * delta + sgnHeight[indexZurKurveGehoerigeXachse[i1]]));
    }
    g2.drawString(
        name,
        this.getWidth() - DX_IN_RECHTS + TXT_DISTANCE_Y,
    ...
    if (kurveLinienstil[i1] != GraferV3.INVISIBLE) {
      ...
      for (int i5 = 1; i5 < _zvCounter; i5++) {
        if (yPix[i5]
            != yPix[
                i5 - 1]) { // Umschaltvorgang wird in der Mitte zwischen 2 Datenpunkten realisiert
  ...
  private void definiereAchsenbegrenzungenImAutoZoom(DataContainer ws) {
    ...
    final double[] tickAbstandY = new double[ANZ_DIAGRAM_MAX],
        tickAbstandY2 = new double[ANZ_DIAGRAM_MAX];
    // --------------------------
    ...
    for (int i1 = 0; i1 < ws.getRowLength(); i1++) { // geht durch die Spalten
      for (int i2 = 0;
          i2 < _zvCounter;
          i2++) { // geht Zeile fuer Zeile durch die selektierte Spalte
    ...
    for (int i1 = 0; i1 < matrixZuordnungKurveDiagram.length; i1++) { // geht durch die Zeilen
      ...
      // wenn Y definiert ist, nicht aber Y2, dann gibt es noch keine Achsenbegrenzung fuer die
      // Y2-Achse (und umgekehrt)
    ...
    double[] yy1 = new double[anzGrfVisible],
        yy2 = new double[anzGrfVisible]; // Y-Achse  --> Muss noch individuell angepasst werden!!
    boolean[] scX = new boolean[anzGrfVisible],
        scY = new boolean[anzGrfVisible]; // ist Auto-Scaling eingeschaltet?
  ...
  private void definiereAchsenbegrenzungenNumerischeSimulation(double t1, double t2) {
    ...
    final double[] w1 = new double[worksheetDaten.getRowLength()],
        w2 = new double[worksheetDaten.getRowLength()];
    ...
    // --------------------------
    // CLIPPING:  Kann erst aufgerufen werden, wenn 'worksheet' und 'minX[],maxX[],...' definiert
    // sind -->
  ...
  public void setzeKurven() {
    ...
    for (int i1 = 0; i1 < this.getAnzahlDiagramme(); i1++) {
      for (int i2 = 0; i2 < matrixZuordnungKurveDiagram[0].length; i2++) {
        if ((matrixZuordnungKurveDiagram[i1][i2] == GraferImplementation.ZUORDNUNG_Y)
            || (matrixZuordnungKurveDiagram[i1][i2] == GraferImplementation.ZUORDNUNG_SIGNAL)) {
          kurvenanzahl++;
        }
      }
    }
    ...
    //
    indexDerKurveInDerMatrix =
        new int[kurvenanzahl]; // zur Zuordnung der Kurven-Indizes zur Zuordnungsmatrix
    //
    ...
    for (int kurvenIndex = 0; kurvenIndex < kurvenanzahl; kurvenIndex++) {
      for (int i1 = 0; i1 < this.getAnzahlDiagramme(); i1++) {
        ...
        for (int i2 = 0; i2 < matrixZuordnungKurveDiagram[0].length; i2++) {
          if ((matrixZuordnungKurveDiagram[i1][i2] == this.ZUORDNUNG_Y)
              || (matrixZuordnungKurveDiagram[i1][i2] == this.ZUORDNUNG_SIGNAL)) {
    ...
    // =====================================
    int[] crvAchsenTypLok =
        new int[kurvenanzahl]; // Fuer jeden Matrix-Eintrag gibt es einen eindeutigen Achsen-Typ (X
    // oder Y oder Y2)
    ...
    this.setzeKurvePunktSymbolAnzeigen(
        crvSymbShowLok, crvSymbFrequLok, crvSymbShapeLok, crvSymbFarbeLok);
    // =====================================
    // welche Art von Clipping (Achse, kein Clipping, Wert) ?  -->
    int[] crvClipXminLok = new int[kurvenanzahl],
        crvClipXmaxLok = new int[kurvenanzahl],
        crvClipYminLok = new int[kurvenanzahl],
        crvClipYmaxLok = new int[kurvenanzahl];
    // falls Clipping auf Wert, welcher konkrete Zahlenwert ?  -->
    double[] crvClipValXminLok = new double[kurvenanzahl],
        crvClipValXmaxLok = new double[kurvenanzahl];
    double[] crvClipValYminLok = new double[kurvenanzahl],
        crvClipValYmaxLok = new double[kurvenanzahl];
    for (int i1 = 0; i1 < kurvenanzahl; i1++) {
      final int im1 = (int) (indexDerKurveInDerMatrix[i1] / 1000);
      final int im2 = (int) (indexDerKurveInDerMatrix[i1] % 1000);
  ...
  public void setzeAchsen() {
    ...
    yGrfMIN[0] = 0;
    yGrfMAX[0] =
        (diagramTyp[0] == DIAGRAM_TYP_ZV)
            ? 0 + (int) (hoehe * (ySpacingDiagram[0] / ySpGes))
            : 0 + notwendigeHoehePixGRF[0];
    for (int i1 = 1; i1 < this.getAnzahlDiagramme(); i1++) {
      yGrfMIN[iyy] = yGrfMAX[iyy - 1];
      yGrfMAX[iyy] =
          (diagramTyp[i1] == DIAGRAM_TYP_ZV)
              ? yGrfMIN[iyy] + (int) (hoehe * (ySpacingDiagram[i1] / ySpGes))
              : yGrfMIN[iyy] + notwendigeHoehePixGRF[i1];
      iyy++;
    }
    // -----------
    int[] laenge_xAchse = new int[anzGrfVisible],
        posX_xAchse = new int[anzGrfVisible],
        posY_xAchse = new int[anzGrfVisible];
    int[] laenge_yAchse = new int[anzGrfVisible],
        posX_yAchse = new int[anzGrfVisible],
        posY_yAchse = new int[anzGrfVisible];
    ...
    for (int i1 = 0; i1 < this.getAnzahlDiagramme(); i1++) {
      ...
      if (ix == 0) {
        posY_xAchse[ix] =
            (diagramTyp[i1] == DIAGRAM_TYP_ZV)
                ? (int) (hoehe * (ySpacingDiagram[i1] / ySpGes) - DY_IN_UNTEN)
                : notwendigeHoehePixGRF[i1] - DY_IN_UNTEN;
      } else {
        posY_xAchse[ix] =
            (diagramTyp[i1] == DIAGRAM_TYP_ZV)
                ? posY_xAchse[ix - 1] + (int) (hoehe * (ySpacingDiagram[i1] / ySpGes))
                : posY_xAchse[ix - 1] + notwendigeHoehePixGRF[i1];
      }
      ix++;
      // Y-Achse:
      laenge_yAchse[iy] =
          (diagramTyp[i1] == DIAGRAM_TYP_ZV)
              ? (int) (hoehe * (ySpacingDiagram[i1] / ySpGes) - (DY_IN_OBEN + DY_IN_UNTEN))
              : notwendigeHoehePixGRF[i1] - (DY_IN_OBEN + DY_IN_UNTEN);
      posX_yAchse[iy] = posX_xAchse[ix - 1];
      posY_yAchse[iy] = posY_xAchse[ix - 1];
      iy++;
    }
    this.setzeAchsenBreiteHoeheX0Y0(
        laenge_xAchse, laenge_yAchse, posX_xAchse, posY_xAchse, posX_yAchse, posY_yAchse);
    // -----------
    double[] x1 = new double[anzGrfVisible], x2 = new double[anzGrfVisible]; // X-Achse
    double[] y1 = new double[anzGrfVisible],
        y2 = new double[anzGrfVisible]; // Y-Achse  --> Muss noch individuell angepasst werden!!
    boolean[] scX = new boolean[anzGrfVisible],
        scY = new boolean[anzGrfVisible]; // ist Auto-Scaling eingeschaltet?
    ...
    this.setzeGridFarben(
        farbeGridNormalXLok,
        farbeGridNormalYLok,
        farbeGridNormalXminorLok,
        farbeGridNormalYminorLok);
    // -----------
    ...
    this.setzeGridLinienStil(
        linStilGridNormalXLok,
        linStilGridNormalYLok,
        linStilGridNormalXminorLok,
        linStilGridNormalYminorLok);
    // -----------
    final int[][] showGridNormalXmajLok = new int[anzGrfVisible][2],
        showGridNormalXminLok = new int[anzGrfVisible][2];

    for (int i1 = 0; i1 < anzGrfVisible; i1++) {
      final int indexAchseX = gridNormalX_zugeordneteXAchse[i1];
      final int indexAchseY = gridNormalX_zugeordneteYAchse[i1];
      showGridNormalXmajLok[i1][0] =
          (xShowGridMaj[i1])
              ? indexAchseX
              : -1; // '-1' bedeutet: Grid-Linie fuer diese Achsenkombination nicht zeichnen
    ...
    this.showGridLines(
        showGridNormalXmajLok, showGridNormalXminLok, yShowGridMajor, yShowGridMinor);
    // -----------
    ...
    // -----------
    final boolean[] showLabelsXMaj = new boolean[anzGrfVisible],
        showLabelsXMin = new boolean[anzGrfVisible];
    final boolean[] showLabelsYMax = new boolean[anzGrfVisible],
        showLabelsYMin = new boolean[anzGrfVisible];
    ...
    // -----------
    boolean[] zeigeXticksUnten = new boolean[anzGrfVisible],
        zeigeYticksLinks = new boolean[anzGrfVisible];
  ...
  public void aktualisiereAchsenNachResizing() {
    ...
    try {
      ...
      Logger.getLogger(GraferImplementation.class.getName())
          .log(Level.SEVERE, "Nullpointer-Exception after resizing.");
    }
  }

  public void blendeEventuellGridLinienAus() {
    // -------------------------------------
    // wenn die Diagramme in einem sehr kleinen Fenster gezeichnet werden, dann muessen eventuell
    // die Grid-Linien ausgeblendet werden,
    // um eine gewisse Uebersichtlichkeit zu wahren -->
    //
    final double px1 = 230, px2 = 100, pxr = 2.5;
    final int[][] showGridXMax = new int[anzGrfVisible][2],
        showGridXMin = new int[anzGrfVisible][2];
    for (int i1 = 0; i1 < anzGrfVisible; i1++) {
      final int indexAchseX = gridNormalX_zugeordneteXAchse[i1];
      final int indexAchseY = gridNormalX_zugeordneteYAchse[i1];
      showGridXMax[i1][0] =
    ...
    final int[][] showGridNormalYmajLok = new int[anzGrfVisible][2],
        showGridNormalYminLok = new int[anzGrfVisible][2];
    for (int i1 = 0; i1 < anzGrfVisible; i1++) {
      final int indexAchseX = gridNormalY_zugeordneteXAchse[i1];
      final int indexAchseY = gridNormalY_zugeordneteYAchse[i1];
      showGridNormalYmajLok[i1][0] =
          (yShowGridMaj[i1])
              ? indexAchseX
              : -1; // '-1' bedeutet: Grid-Linie fuer diese Achsenkombination nicht zeichnen
    ...
    switch (mausModus) {
      ...
      case MAUSMODUS_WERTANZEIGE_SCHIEBER:
        if (!xSchieberAktiv) {
          xSchieberAktiv = true;
          xSchieberPix =
              _xAchseX[0]; // x-Schieber wird an den Anfang gesetzt: gleich fuer alle Diagramme, in
          // GraferV3 definiert
  ...
  public void mouseDragged(MouseEvent me) {
    ...
    if (mausModus == MAUSMODUS_NIX) ;
    else if (mausModus == MAUSMODUS_ZOOM_AUTOFIT) ;
    else if (mausModus == MAUSMODUS_ZOOM_FENSTER) {
  ...
  public void mausModus_ZOOM_FENSTER(
      int mx, int my, int mausAktion, boolean isControlDown, boolean isShiftDown) {
    ...
    switch (mausAktion) {
      ...
      case MOUSE_PRESSED:
        ...
        try {
          ...
          while (!((xGrfMIN[indexAngeklickterGraph] <= mx)
              && (mx <= xGrfMAX[indexAngeklickterGraph])
              && (yGrfMIN[indexAngeklickterGraph] <= my)
  ...
  private int[] getPixelFromValue(
      final double xWert, double yWert, final int index_xAchse, final int index_yAchse) {
    try {
      // -------------------
  ...
  protected void zeichne(final Graphics graphics) {
    ...
    switch (mausModus) {
      ...
      case MAUSMODUS_ZOOM_FENSTER:
        ...
        final int dx = Math.abs(x1 - x2), dy = Math.abs(y1 - y2);
      ...
      default:
        Logger.getLogger(GraferImplementation.class.getName())
            .log(Level.SEVERE, "Default in case statement reached.");
        break;
    }
    // -------------------
    if (xSchieberAktiv) {
      ...
      graphics.drawString(
          "t = " + cf.formatT((float) xSchieberWert[0], TechFormat.FORMAT_AUTO),
          dx,
          this.getHeight() - dy);
      ...
      graphics.drawString(
          "t = " + cf.formatT((float) xSchieberWert2[0], TechFormat.FORMAT_AUTO),
          dx + 130,
          this.getHeight() - dy);
      ...
      if (xSchieberWert2[0] >= 0) {
        ...
        graphics.drawString(
            "dt = "
                + cf.formatT(
                    (float) (xSchieberWert2[0] - xSchieberWert[0]), TechFormat.FORMAT_AUTO),
            dx + 260,
            this.getHeight() - dy);
      }
    }
  ...
  private void zoomRechteck(final boolean isUndoZoom) {
    ...
    //  der x-Bereich gilt auch fuer alle anderen Diagramm, der y-Bereich des entsprechenden
    // Diagramms entspricht dem Zoom-Rechteck
    //
    int indexAxis = -1;
    if (isUndoZoom) {
    ...
    //
    final double[] value1 = new double[worksheetDaten.getRowLength()],
        value2 = new double[worksheetDaten.getRowLength()];
  ...
  // In der SCOPE-Darstellung werden die Daten auf einige 1000 (Pixel)punkte in Hi-Lo-Darstellung
  // reduziert, um die Effizient
  // der grafischen Darstellung signifikant zu erhoehen. Wenn man beispielsweise hineinzoomt, dann
  // muessen die Daten mit
  // deutlich hoeherer Aufloesung nachgeladen und in Hi-Lo-Darstellung uebertragen werden, damit die
  // grafische Darstellung
  // nicht wichtige Info verliert (zB. 'ausgefranste' Rippelkurven, verschwundene Peaks, Aus
  // Rechtecken werden Dreiecke usw.)
  //
  private void getChangedDataResolution(double x1, double x2) {
    ...
    try {
      ...
      if (nochNichtGeZoomt) {
        ...
        worksheetDatenTEMP =
            new double[worksheetDaten.getRowLength()][worksheetDaten.getColumnLength()];
        for (int i1 = 0; i1 < worksheetDatenTEMP.length; i1++) {
          for (int i2 = 0; i2 < worksheetDatenTEMP[0].length; i2++) {
      ...
      if (x2RAM < x2) {
        for (;
            wsRAM.getValue(0, zeigerX2RAM) != 0 && x2RAM < x2;
            x2RAM = wsRAM.getValue(0, zeigerX2RAM)) {
          zeigerX2RAM++;
        }
      } else {
      ...
      if (dtSCOPE > wsRAM.getTimeIntervalResolution()) {
        ...
        for (int worksheetIndex = 0; worksheetIndex < INTERVALLE_ENTLANG_X + 2; worksheetIndex++) {
          ...
          while (higherIndex < maximumIndex
              && wsRAM.getEstimatedTimeValue(lowerIndex) < timeValue) {
            lowerIndex++;
          }
          ...
          while (higherIndex < maximumIndex
              && wsRAM.getEstimatedTimeValue(higherIndex) < timeValue + dtSCOPE) {
            higherIndex++;
          }
          ...
          for (int i1 = 0; i1 < lg1 - 1; i1++) {
            ...
            try {
              oldMeanValue =
                  0.5
                      * (worksheetDaten.getValue(i1 + 1, 2 * worksheetIndex - 1)
                          + worksheetDaten.getValue(i1 + 1, 2 * worksheetIndex - 2));
            } catch (Exception ex) {
  ...
  public double[] getXClipNo(final int im1, final int im2) {
    ...
    if (indexX == -1) {
      Logger.getLogger(GraferImplementation.class.getName())
          .log(Level.SEVERE, "Index error in plot.");
    }
