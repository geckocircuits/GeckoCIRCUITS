...
/**
 * This class is a wrapper for use in from external programs, e.g. MATLAB or other Java programs.
 * The communication is done via a memory-mapped file (MMF). This is the alternative, non-network
 * counterpart to GeckoRemoteObject.
 *
 * <p>It is implemented as a child class of GeckoRemoteObject to make inserting the alternative
...
public final class GeckoRemoteMMFObject extends GeckoRemoteObject {
  ...
  private static final String ERROR_STRING =
      "Error with calling remote method. See nested exception for details:\n";
  private static String _pathToJava = "";

  /**
   * Start GeckoCIRCUITS, set it up to use a given memory-mapped file. The memory-mapped file should
   * NOT exist in order to start a fresh new instance of GeckoCIRCUITS! If the file already exists,
   * this method will treat it as an existing GeckoCIRCUITS instance which is already running, and
   * will try to connect to it - which may fail if the file does not actually correspond to a
   * running GeckoCIRCUITS instance!
   *
   ...
   * @return a GeckoRemoteMFFObject connected to this new instance of GeckoCIRCUITS which can be
   *     used for controlling it remotely
   */
  ...
  /**
   * Start GeckoCIRCUITS, set it up to use a given memory-mapped file, initialized to the default
   * size. The memory-mapped file should NOT exist in order to start a fresh new instance of
   * GeckoCIRCUITS! If the file already exists, this method will treat it as an existing
   * GeckoCIRCUITS instance which is already running, and will try to connect to it - which may fail
   * if the file does not actually correspond to a running GeckoCIRCUITS instance!
   *
   * @param file the file which should memory-mapped to communicate with GeckoCIRCUITS
   * @return a GeckoRemoteObject connected to this new instance of GeckoCIRCUITS which can be used
   *     for controlling it remotely
   */
  ...
  /**
   * Connect to a running GeckoCIRCUITS instance via a memory-mapped file. For success, 1)
   * GeckoCIRCUITS must be running with MMF access enabled 2) The file must exist and given as the
   * access for MMF 3) GeckoCIRCUITS must not be occupied by another connection
   *
   * @param file the file to use for MMF access. If it does not exist, an error is thrown
   * @return a GeckoRemoteMFFObject connected to this instance of GeckoCIRCUITS which can be used
   *     for controlling it remotely
   */
  ...
  private void connectToExistingInstance(final GeckoMemoryMappedFile file) {
    ...
    if (file.isFree()) {
      ...
      if (newSessionID > 0) {
        ...
        System.out.println(
            "You are now connected to the GeckoCIRCUITS instance via file " + file.getFileName());
      } else if (newSessionID == 0) {
        throw new RuntimeException(
            "Connection to GeckoCIRCUITS via file "
                + file.getFileName()
                + " rejected by that instance of GeckoCIRCUITS.");
      } else if (newSessionID == -1) {
        throw new RuntimeException(
            "The GeckoCIRCUITS instance using file "
                + file.getFileName()
                + " is busy with another session. You cannot connect to it now.");
      } else { // -2
        throw new RuntimeException(
            "Connection attempt to GeckoCIRCUITS via "
                + file.getFileName()
                + " has timed out. Please check whether this file really belongs to a running GeckoCIRCUITS instance.");
      }
      // } catch (Exception e) {
      // throw new RuntimeException("Error connecting to existing GeckoCIRCUITS instance at file " +
      // file.getFileName(), e);
      // }
    } else {
      if (_mmf != null) {
        if (_mmf.getConnectionID() == sessionID) {
          System.out.println(
              "You are already connected to the GeckoCIRCUITS instance available through file "
                  + file.getFileName());
        } else {
          System.out.println(
              "The GeckoCIRCUITS instance at file "
                  + file.getFileName()
                  + " is busy with (an)other session(s). You cannot connect to it now.");
        }
      } else {
        throw new RuntimeException(
            "The GeckoCIRCUITS session at file "
                + file.getFileName()
    ...
    // } catch (Exception ex) {
    // throw new RuntimeException("Error connecting to existing GeckoCIRCUITS instance through file
    // " + file.getFileName() + ":\n" + ex.getMessage(), ex);
    // }
  ...
  /**
   * Set the path of the java executable to use (needed when starting GeckoCIRCUITS from a Netbeans
  ...
  public static void setJavaPath(final String pathToJavaExe) {
    if (new File(pathToJavaExe).exists()) {
      ...
      throw new RuntimeException(
          "Error: Path to Java executable: " + pathToJavaExe + " does not exist!");
    }
  }
  ...
  public void disconnectFromGecko() {
    try {
      if (_mmf == null) {
        ...
        System.out.println(
            "You have been disconnected from the GeckoCIRCUITS instance at file"
                + _mmf.getFileName());
        _mmf = null;
      }
      ...
      throw new RuntimeException(
          "Error disconnecting from GeckoCIRCUITS instance at file "
              + _mmf.getFileName()
  ...
  private boolean checkRemote() {
    if (_mmf == null) {
      System.err.println(
          "You are NOT connected to any instance of GeckoCIRCUITS! Use startNewRemoteInstance(file,size) or"
      ...
      if (_mmf.getConnectionID() == sessionID) {
        ...
        System.out.println(
            "Invalid session ID. Restart your connection by "
  ...
  /**
   * NOTE: Ideally, we would for all the remaining methods, like to get the method name by
   * reflection. This is ugly however in terms of compilation, see:
   * http://stackoverflow.com/questions/442747/getting-the-name-of-the-current-executing-method
   * Therefore, in each method, we construct the GeckoRemotePipeObject manually. Annoying, but gives
  ...
  public void simulateStep() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("simulateStep", new Object[0]);
  ...
  public void simulateSteps(final int steps) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("simulateSteps", new Object[] {steps});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getAccessibleParameters(final String componentName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getAccessibleParameters", new Object[] {componentName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
  ...
  public String[] getControlElements() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getControlElements", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getCircuitElements() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getCircuitElements", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getThermalElements() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getThermalElements", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getSpecialElements() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getSpecialElements", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getThyristors() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getThyristors", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getIdealSwitches() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getIdealSwitches", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getResistors() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getResistors", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getInductors() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getInductors", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String[] getCapacitors() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getCapacitors", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public Object doOperation(
      final String elementName, final String operationName, final Object parameterValue) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "doOperation", new Object[] {elementName, operationName, parameterValue});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
  ...
  public void setGlobalParameterValue(final String parameterName, final double value) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "setGlobalParameterValue", new Object[] {parameterName, value});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getGlobalParameterValue(final String parameterName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getGlobalParameterValue", new Object[] {parameterName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
  ...
  public void setParameter(
      final String elementName, final String parameterName, final double value) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "setParameter", new Object[] {elementName, parameterName, value});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void setParameters(
      final String elementName, final String[] parameterNames, final double[] values) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "setParameters", new Object[] {elementName, parameterNames, values});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getParameter(final String elementName, final String parameterName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getParameter", new Object[] {elementName, parameterName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getOutput(final String elementName, final String outputName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getOutput", new Object[] {elementName, outputName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getOutput(final String elementName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getOutput", new Object[] {elementName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public void runSimulation() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("runSimulation", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void initSimulation() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("initSimulation", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void initSimulation(final double deltaT, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("initSimulation", new Object[] {deltaT, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void continueSimulation() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("continueSimulation", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void simulateTime(final double time) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("simulateTime", new Object[] {time});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getSimulationTime() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getSimulationTime", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public void endSimulation() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("endSimulation", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void saveFileAs(final String fileName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("saveFileAs", new Object[] {fileName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void openFile(final String fileName) throws FileNotFoundException {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("openFile", new Object[] {fileName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void set_dt(final double value) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("set_dt", new Object[] {value});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void set_Tend(final double value) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("set_Tend", new Object[] {value});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double get_Tend_pre() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("get_Tend_pre", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public void set_dt_pre(final double value) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("set_dt_pre", new Object[] {value});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void set_Tend_pre(final double value) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("set_Tend_pre", new Object[] {value});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double[][] getFourier(
    ...
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getFourier", new Object[] {scopeName, scopePort, startTime, endTime, harmonics});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (double[][]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double[][] getFourier(
      final String scopeName, final double startTime, final double endTime, final int harmonics) {
  ...
  public void setPosition(final String elementName, final int xCoord, final int yCoord) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("setPosition", new Object[] {elementName, xCoord, yCoord});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public int[] getPosition(final String elementName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getPosition", new Object[] {elementName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
  ...
  public void deleteComponent(final String elementName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("deleteComponent", new Object[] {elementName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void deleteAllComponents(final String subcircuitName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("deleteAllComponents", new Object[] {subcircuitName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void createComponent(
      final String elementType, final String elementName, final int xCoord, final int yCoord) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "createComponent", new Object[] {elementType, elementName, xCoord, yCoord});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public void setOutputNodeName(
      final String elementName, final int nodeIndex, final String nodeName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "setOutputNodeName", new Object[] {elementName, nodeIndex, nodeName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public void setInputNodeName(
      final String elementName, final int nodeIndex, final String nodeName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "setInputNodeName", new Object[] {elementName, nodeIndex, nodeName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public String getOutputNodeName(final String elementName, final int nodeIndex) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getOutputNodeName", new Object[] {elementName, nodeIndex});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public String getInputNodeName(final String elementName, final int nodeIndex) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getInputNodeName", new Object[] {elementName, nodeIndex});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (String) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public double[] getTimeArray(
      final String signalName, final double tStart, final double tEnd, final int skipPoints) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getTimeArray", new Object[] {signalName, tStart, tEnd, skipPoints});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (double[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public float[] getSignalData(
    ...
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalData", new Object[] {signalName, tStart, tEnd, skipPoints});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
   ...
   * @param newName the new name that should be given. Throws an Exception, if name is already in
   *     use!
   * @throws Exception Setting the name was not possible, since another component uses this name
   *     already.
   */
  public void setComponentName(final String oldName, final String newName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("setComponentName", new Object[] {oldName, newName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void rotate(final String elementName) {
    ...
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("rotate", new Object[] {elementName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void setOrientation(final String elementName, final String direction) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("setOrientation", new Object[] {elementName, direction});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void importFromFile(final String fileName, final String subCircuitName) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("importFromFile", new Object[] {fileName, subCircuitName});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public float[][] getGlobalFloatMatrix() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getGlobalFloatMatrix", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (float[][]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public double[][] getGlobalDoubleMatrix() {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("getGlobalDoubleMatrix", new Object[0]);
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (double[][]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public void setGlobalFloatMatrix(final float[][] matrix) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("setGlobalFloatMatrix", new Object[] {matrix});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public void setGlobalDoubleMatrix(final double[][] matrix) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("setGlobalDoubleMatrix", new Object[] {matrix});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getSignalAvg(
      final String signalName, final double startTime, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalAvg", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double getSignalRMS(
      final String signalName, final double startTime, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalRMS", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double getSignalTHD(
      final String signalName, final double startTime, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalTHD", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getSignalMin(
    ...
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalMin", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public double getSignalMax(
    ...
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalMax", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double getSignalRipple(
      final String signalName, final double startTime, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalRipple", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double getSignalKlirr(
      final String signalName, final double startTime, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalKlirr", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double getSignalShape(
      final String signalName, final double startTime, final double endTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalShape", new Object[] {signalName, startTime, endTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (Double) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double[][] getSignalFourier(
      final String signalName, final double startTime, final double endTime, final int harmonics) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalFourier", new Object[] {signalName, startTime, endTime, harmonics});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (double[][]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public float[] floatFFT(final float[] timeDomainData) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("floatFFT", new Object[] {timeDomainData});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (float[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public void initSteadyStateDetection(
      String[] stateVariables, double frequency, double deltaT, double simulationTime) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "initSteadyStateDetection",
                new Object[] {stateVariables, frequency, deltaT, simulationTime});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isVoidReturn()) {
          return;
        } else if (returnValue.isErrorMessage()) {
  ...
  public double[] simulateToSteadyState(final boolean supressMessages) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject("simulateToSteadyState", new Object[] {supressMessages});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (double[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  @Override
  public double[] simulateToSteadyState(
      final boolean supressMessages,
      final double targetCorrelation,
      final double targetMeanPctDiff) {
    try {
      if (checkRemote()) {
        final GeckoRemotePipeObject method =
            new GeckoRemotePipeObject(
                "getSignalData",
                new Object[] {supressMessages, targetCorrelation, targetMeanPctDiff});
        final GeckoRemotePipeObject returnValue = _mmf.callMethod(sessionID, method);
        if (returnValue.isReturnValue()) {
          return (double[]) returnValue.getMethodReturnValue();
        } else if (returnValue.isErrorMessage()) {
  ...
  public void allowAdditionalClients(final int additionalClients) {
    throw new UnsupportedOperationException(
        "Not supported yet in GeckoRemoteMMFObject. For multiplce clients, use GeckoRemoteObject.");
  }
}
