...
public final class OutputWarningStream extends BufferedOutputStream {
  ...
  private static final int BUFFER_FRACTION =
      5; // this means, after cleaning the buffer, 1/5th of the original space is left
  ...
  private void maybeShowWarning() {
    if (!_verbosityWarnShown && !_ignoreFutureMessages) {
      ...
      final Object[] options = {"Ok", "Ignore further messages"};
  ...
  private void checkStringBufferSize() {
    if (_alternativeOutput.length() > MAX_STRING_BUFFER_SIZE) {
      _alternativeOutput.delete(
          0, MAX_STRING_BUFFER_SIZE - MAX_STRING_BUFFER_SIZE / BUFFER_FRACTION);
