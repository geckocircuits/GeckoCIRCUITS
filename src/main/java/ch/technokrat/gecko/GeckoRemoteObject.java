...
public class GeckoRemoteObject {
  ...
  /**
   * Allow additional clients to simultaneously connect to the connected-to instance of
   * GeckoCIRCUITS.
   *
   * @param additionalClients the number of additional (!) clients (beside the original one) that
   *     can be allowed to connect remotely to GeckoCIRCUITS if this number is set to 0 or less,
   *     only one client will be allowed to connect at a time. if this number is set to e.g. 2, 3
   *     clients in total will be allowed to connect a a time. NOTE: disabling extra client
   *     connections (e.g. passing 0) does NOT disconnect any currently connected clients; it only
   *     affects clients which attempt to connect in the future.
   */
