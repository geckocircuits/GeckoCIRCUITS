...
public class GeckoRemoteRegistry {
  ...
  public static String getIPAddress() {
    return _ipAddress;
    // add here the code for putting this in the properties file if necessary - although I wouldn't
  ...
  public static void enableRemotePort() throws Exception {
    ...
    try {
      ...
      GeckoRemoteInterface stub =
          (GeckoRemoteInterface) UnicastRemoteObject.exportObject(remoteAccess, 0);
      ...
      // old ones, open, too. This is ugly. there is a workaround,
      // http://stackoverflow.com/questions/1087163/closing-rmi-registry
      ...
      System.out.println(
          "GeckoCIRCUITS configured for remote access at port "
              + GeckoRemoteRegistry.getRemoteAccessPort()
              + ".");
  ...
  /**
   * Get IP numbers to choose from for REAL network access. Returns the local machine IP addresses
   * (of the machine's network interfaces) - not necessarily the "external" IP if the machine is
   * behind a NAT.
   *
   * @return an array of local IP addresses, excluding the localhost (127.0.0.1) and inactive
   *     interfaces at the time the method is called.
  ...
  public static String[] getMachineIPNumbers() throws SocketException {
    ...
    while (interfaces.hasMoreElements()) {
      ...
      // System.out.println("interface: " + interFace.getName() + " is loopback: " +
      // interFace.isLoopback() + " is up: " + interFace.isUp());
  ...
  /**
   * If the machine is behind a NAT of some sort (router, etc.) the above method does not return the
   * IP address via which the machine is reachable from the internet. This method queries a website
   * to get this address.
   *
