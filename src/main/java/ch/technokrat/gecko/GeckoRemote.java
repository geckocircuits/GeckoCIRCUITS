 ...
 * The communication is done via a network socket. Since the method count of this class is
 * ever-increasing, I wrote some reflection code which is doing the wrapper operations. PLEASE
 * (WARNING!) Whenever you modify something here, then execute the corresponding JUnit test. This
 * test ensures that nothing goes wrong with reflection and all the methods (method names, argument
 * types, ...).
 */
...
public class GeckoRemote {
  ...
  public static void connectToGecko(final String host, final int port) {
    try {
      ...
      final GeckoRemoteInterface geckoInstance =
          (GeckoRemoteInterface) registry.lookup(REGISTRY_NAME);
      ...
      if (geckoInstance instanceof CallbackServerInterface) {
        ...
        System.err.println(
            "Could not establish connection for callback. \nOutput messages from GeckoCIRCUITS"
      ...
      throw new RuntimeException(
          "There is no GeckoCIRCUITS instance at port "
              + port
              + " or it is not enabled for remote access.",
          ex);
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  /**
   * check whether this port is already used or if it's free and if it's used, is already a
   * GeckoCIRCUITS instance running or something else
   *
   * @param port
   * @return
   */
  ...
  private static boolean portFree(final int port) {
    ...
    try {
      ...
      portFree = false;
      // this means that port is used - maybe already by another GeckoCIRCUITS instance - check if
      // this is so
      ...
      try {
        ...
        existingInstance = (GeckoRemoteInterface) registry.lookup(REGISTRY_NAME);
          ...
          Exception
              originalException) { // this means there is no GeckoCIRCUITS already running there
        final String message =
            "Port "
                + port
                + " is already occupied by another process or is blocked. Please try another port number.";
        throw new RuntimeException(message, originalException);
      } finally { // this port is already occupied by a GeckoCIRCUITS instance - attempt to connect
        // to it
        if (existingInstance != null) {
          System.out.println(
              "Port "
                  + port
                  + " is already occupied by GeckoCIRCUITS. Attempting to connect to this instance:");
      ...
      if (socket != null) {
        try {
          ...
          portFree = false;
          throw new RuntimeException(
              "Error with using port " + port + ". Try another port number.", e);
        }
      }
    }
  ...
  private static void connectToExistingInstance(
      final GeckoRemoteInterface existingInstance, final int port) {
    try {
      if (existingInstance.isFree()
          && !existingInstance.checkSessionID(
              sessionID)) { // with multiple connections, GeckoCIRCUITS may be free AND this client
        ...
        try {
          ...
          _proxy =
              (GeckoRemoteIntWithoutExc)
                  Proxy.newProxyInstance(loader, interfaces, _invocationHandler);
          ...
          throw new RuntimeException(
              "Error connecting to existing GeckoCIRCUITS instance at port " + port, e);
        }
      } else {
        if (_wrapped != null) {
          if (_wrapped.checkSessionID(sessionID)) {
            System.out.println(
                "You are already connected to the GeckoCIRCUITS instance available at port "
                    + port);
          } else {
            System.out.println(
                "The GeckoCIRCUITS instance at port "
                    + port
                    + " is busy with (an)other session(s). You cannot connect to it now.");
          }
        } else {
          throw new RuntimeException(
              "The GeckoCIRCUITS session at port "
                  + port
      ...
      throw new RuntimeException(
          "Error connecting to existing GeckoCIRCUITS instance at port " + port, ex);
    }
  }
  ...
  public static void disconnectFromGecko() {
    try {
      if (_wrapped == null) {
        ...
        _wrapped = null;
        System.out.println(
            "You have been disconnected from the GeckoCIRCUITS instance at port " + portNumber);
      }

    } catch (RemoteException e) {
      throw new RuntimeException(
          "Error disconnecting from GeckoCIRCUITS instance at port "
              + portNumber
  ...
  private static void checkRemoteWithException() {
    if (_wrapped == null) {
      throw new RuntimeException(
          "You are NOT connected to any instance of GeckoCIRCUITS! Use startGui(port) or"
      ...
      try {
        if (!_wrapped.checkSessionID(sessionID)) {
          throw new RuntimeException(
              "Invalid session ID. Please restart your connection by "
  ...
  public static Object doOperation(
  ...
  @Deprecated
  public static double[] getSignalCharacteristics(
  ...
  @Deprecated
  public static double[][] getFourier(
  ...
  @Deprecated
  public static void initSteadyStateDetection(
  ...
  public static double[] simulateToSteadyState(
  ...
  @Deprecated
  public static void setLossFile(final String elementName, final String lossFileName)
      throws FileNotFoundException {
    _proxy.setLossFile(elementName, lossFileName);
  }

  @Deprecated
  public static void setNonLinear(final String elementName, final String characteristicFileName)
      throws FileNotFoundException {
  ...
  public static void setOutputNodeName(
  ...
  public static void setInputNodeName(
      final String elementName, final int nodeIndex, final String nodeName) {
  ...
  public static float[] getSignalData(
      final String signalName, final double tStart, final double tEnd, final int skipPoints) {
  ...
  public static void importFromFile(final String fileName, final String subCircuitName)
      throws FileNotFoundException {
  ...
  public static double getSignalAvg(
