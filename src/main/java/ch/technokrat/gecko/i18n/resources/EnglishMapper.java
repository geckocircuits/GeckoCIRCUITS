...
public class EnglishMapper {
  ...
  /**
   * Creates a new map with all single-line English key-value pairs and returns it
   *
  ...
  public static DoubleMap initEnglishMap_single() {
    ...
    //        dm.putPair("t1", "This Model file was created with an older version of
    // GeckoCIRCUITS.");
    //        dm.putPair("t2", "When you save model files, please consider that you cannot open
    // files");
    ...
    //        dm.putPair("WarningCheckAutoBackupDisbled", "Warning: Check for auto-backup file
    // disabled!");
    ...
    //        dm.putPair("DialogMemory.jLabel5.text", "(changes apply after restart of
    // GeckoCIRCUITS)");
    ...
    //        dm.putPair("DialogRemotePort.jPanelRemoteSettings.TabConstraints.tabTitle",
    // "Settings");
    ...
    //        dm.putPair("DialogUpdate.jButtonGetUpdate.text", "Download update from server
    // (.zip-file)   ");
    ...
    //        dm.putPair("GeckoFileManagerWindow.jButtonAddSelectedExistingFiles.text", "Add
    // Selected File(s) to local block");
    //        dm.putPair("GeckoFileManagerWindow.jPanel2.TabConstraints.tabTitle", "Files already
    // available in model (used by other elements):");
    ...
    //        dm.putPair("GeckoFileManagerWindow.jPanel1.TabConstraints.tabTitle", "Files in this
    // element:");
    //        dm.putPair("GeckoFileManagerWindow.jButtonRemove.text", "Remove Selection");
    //        dm.putPair("GeckoFileManagerWindow.title", "Gecko File Manager - add external files to
    // your model");
    ...
    //        dm.putPair("gearshichmantext", "Gear-Shichman: Behavior is in between Trapezoidal and
    ...
    //        dm.putPair("nonasciicharsnotresolved", "Probably non-ASCII-Characters are not resolved
    // properly. ");
    ...
    //        dm.putPair("couldnotallocatememorysim", "Could not allocate enough java RAM memory for
    // the simulation!");
    ...
    //        dm.putPair("t4", "This Model file was created with a newer version of
    // GeckoCIRCUITS.");
    ...
    //        dm.putPair("DialogFeedback.jLabel5.text", "<html>We want to improve our software! If
    // you would like to submit a bug-report, <br>give us suggestions about features that we should
    // implement in the software, <br>or you have other issues, then please visit our bugtracking
    // homepage:");
    //        dm.putPair("DialogFeedback.jLabelURL.text", "<html><a
    // href\"www.bugs.gecko-research.org\">www.bugs.gecko-research.org</a></html>");
    //        dm.putPair("DialogLicensing.jLabel2.text", "Licensing / Terms and Conditions for the
    // Software usage and IT Services of the Gecko-Simulations GmbH, CH-8006 Zurich, Switzerland");
    //        dm.putPair("DialogLicensing.jLabel1.text", "GeckoCIRCUITS uses the \"Batik SVG
    // Vector-Graphics libraries\" and \"JSyntaxPane\", which are both licensed under the Apache
    // License (Version 2.0):");
    //        dm.putPair("t10", "You attempted to close GeckoCIRCUITS while running in
    // Simulink-mode.");
    //        dm.putPair("t11", "Please note that only closing your MATLAB session will close
    // GeckoCIRCUITS.");
    ...
    //        dm.putPair("t7", "T_pre, dt_pre: The simulation is executed with the simulation time
    // of T_pre and the stepwidth");
    //        dm.putPair("t8", " of dt_pre in advance of the regular simulation. This enables an
    // easy finding of the");
    ...
    //        dm.putPair("proceedwithsimulation", "You can proceed with the simulation with the
    // current parameter set.");
    //        dm.putPair("parameterdefinitionmissing", "*** ERROR: Parameter Definition Missing
    // ***");
    //        dm.putPair("noerrors", "No Errors.");
    //        dm.putPair("simulationcannotbestarted", "The simulation cannot be started. Please set
    // the missing parameters first.");
    //        dm.putPair("interfacemethods", "Use the following Interface-methods to control
    // GeckoCIRCUITS:");
    //        dm.putPair("backwardeulertext", "Backward-Euler: Very stable, with some numeric
    // damping");
  ...
  /**
   * Creates a new map with all multiple-line English key-value pairs and returns it
   *
  ...
  public static DoubleMap initEnglishMap_multiple() {
    ...
    //        dm.putPair("DialogRemotePort.jTextAreaRemoteInfo.text", "Here you can allow the
    ...
    // machine are allowed; network\naccess will be implemented in a future\nversion.");
    //        dm.putPair("GeckoFileManagerWindow.jTextArea1.text", "\nFiles specified as
    // \"internal\" are read in by Gecko fully the first time they are added\nto a block and then
    // saved into the Gecko model file (*.ipes). \n\"External\" files are accessed every time a
    // block needs them, and must be distributed \nalong with the model file in order for the
    // simulation to work properly with them.\nYou can use the buttons on the right-hand side to
    // change an existing file's status.");
    //        dm.putPair("CautionOverwriteAutoBackupFile", "Caution: you try to overwrite your
    // auto-backup file. \nYou should rename your filename to prevent data-loss!\n Do you want to
    // continue anyway?");
    //        dm.putPair("filehaschangedwarning", "\n*** WARNING ***\n\nThe content of the file has
    // changed.\nDo you want to save the changes?\n");
    //        dm.putPair("modelerrortext", "\n*** MODEL ERROR ***\n\nThe simulation cannot be
    // performed.\n\nPlease check the circuit model for\n\n- Subcircuits without ground
    // connection\n- Parallel voltage / temperature sources\n- Parallel capacitors with initial
    // conditions set\n- Missing connections between elements\n\n");
    //        dm.putPair("MemoryWarning.jTextArea1.text", "Excessive memory usage detected! Your
    // simulation\nrequires more RAM memory than actually available.\nYou can try the following
    // steps before your simulation\nrun:\n- Increase RAM memory of GeckoCIRCUITS at program\n
    // startup (menu Tools -> Memory)\n- Decrease the number of scope input signals\n- Increase the
    // simulation stepwidth\n- Decrease the total simulation time\n\nWith the current settings, your
    // simulation will first\nslow down due to a reduced memory cache size.\nThen, it will abort
    // with an out-of-memory exception.");
    //        dm.putPair("warningproceedingsimulation", "\n*** WARNING ***\n\nThe node number of the
    // model has been changed.Proceeding with the simulation might give incorrect results.\n\n");
    //        dm.putPair("help1", "\nParameters (name-value pairs) must match the parameters defined
    // in the simulation model, \ne.g. $Rload. \n\nPairs of name-value in the input list must be
    // separated by space or tab. \n\nIf a name is defined twice, a warning is given, but the
    // simulation can be performed. If names are defined in the input list, that are not used in the
    // simulation model, a warning is given. \n\nIf parameter names in the simulation model are not
    // defined in the input list, an error is given. In this case the simulation cannot be
    // performed. \n");
    //        dm.putPair("numericalstabilitytext", "Error:\nNumerical instability of
    // switch.\nSimulation aborted at ");
    //        dm.putPair("DialogMemory.jTextArea1.text", "The \"Scope\" element needs to store the
    // simulation data for output/display.\nSmall simulation step widths and long simulation times
    ...
    // single-precision (float) resolution. This requires probably more\n  RAM memory that
    // available.\n\n- Lossy compression (low, medium, high): The number of significant digits\n  of
    // the simulation data is reduced. The amount of required memory is\n  therefore, also reduced,
    // significantly.\n  Disadvantage: The simulation data is rounded according to the setting:\n
    // Low: epsilon  1e-5\n  Medium: epsilon  5e-5\n  High: epsilon  2e-4\n  Please note that the
    // timestep-resolution as well as the simulation itself\n  are not affected by the lossy
    // compression. With the compression setting, \n  you can trad-off between  memory requirement
    // and data compression \n  ratio.");
    //        dm.putPair("t13", "The model has changed since last save to the model\nfile. Would you
    // like to save your model to the file:\n");
