...
public class UPbot {
  ...
  /**
   * Adds a single-line suggestion along with a comment to the designated Suggestions page for the
   * chosen language on the Wiki
   *
   * @param key Key of the translation
   * @param newTranslation New suggestion to upload
   * @param comment Comment to upload ("" for no comment)
   */
  public static void addTranslationSuggestion_single(
      I18nKeys key, String newTranslation, String comment) {
    try {
      ...
      SimpleArticle sa =
          b.readData(
              InitParameters.SINGLE_SUGGESTIONS_PAGE
                  + "_"
                  + InitParameters.getCurrentLanguageCode());
      progress = Math.min(progress + 6, 99); // update progress
      ...
      wikiCode =
          wikiCode.replace(
              ...
              newTranslation
                  + InitParameters.SEPARATOR
                  + title
                  + "\n"
                  + InitParameters.END_DELIMITER
                  + key);
      progress = Math.min(progress + 1, 99); // update progress
      sa.setText(wikiCode);
      progress = Math.min(progress + 4, 99); // update progress
      ...
      new TranslationDialog(
              InitParameters.DATABASE_ERROR_MESSAGEa, InitParameters.DATABASE_ERROR_MESSAGEc)
          .setVisible(true);
      connected = false;
    }
  }
  ...
  /**
   * Adds a multiple-line suggestion along with a comment to the designated Suggestions page for the
   * chosen language on the Wiki
   *
   * @param key Key of the translation
   * @param newTranslation New suggestion to upload
   * @param comment Comment to upload ("" for no comment)
   */
  public static void addTranslationSuggestion_multiple(
      I18nKeys key, String newTranslation, String comment) {
    try {
      // make the change on the Wiki database
      ...
      SimpleArticle sa =
          b.readData(
              InitParameters.MULTIPLE_SUGGESTIONS_PAGE
                  + "_"
                  + InitParameters.getCurrentLanguageCode());
      String wikiCode = sa.getText();
      wikiCode =
  ...
  /**
   * Method to get connection status - To be called after connection attempts
   *
  ...
  /**
   * Method to get upload progress - To be called while uploading from a separate thread
   *
  ...
  /** Method to reinitialize progress - To be called after completing upload instructions */
