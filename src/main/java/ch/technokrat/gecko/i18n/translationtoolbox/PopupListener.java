...
public final class PopupListener implements MouseListener {
  ...
  /**
   * Checks if pop-up triggering command is given and opens translation pop-up if the key is found
   * in one of the translated maps
   *
   * @param key Key of the pop-up triggering element text
  ...
  public void listenForPopup(final I18nKeys key, final MouseEvent evt) {
    if (evt.isControlDown()
        && evt.isShiftDown()
        && !LangInit.language.equals(SelectableLanguages.ENGLISH)) {
