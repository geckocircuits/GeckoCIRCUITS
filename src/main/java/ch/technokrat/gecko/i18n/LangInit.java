...
public class LangInit extends javax.swing.JDialog implements PropertyChangeListener {
  ...
  private class Task extends SwingWorker<Void, Void> {
    ...
    public Void doInBackground() {
      transMap_single =
          DLbot.getTranslations_single(); // get single-line translations from the Wiki
      connected = DLbot.getConnectionStatus(); // see if it worked
      if (connected) {
        transMap_multiple =
            DLbot.getTranslations_multiple(); // get multiple-line translations from the Wiki
      }
      return null;
    }
    ...
    jButton2.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            ...
            language =
                (SelectableLanguages) jComboBoxLanguageSelection.getSelectedItem(); // get language
            englishMap_single =
                EnglishMapper.initEnglishMap_single(); // initialize englishMap_single
            englishMap_multiple =
                EnglishMapper.initEnglishMap_multiple(); // initialize englishMap_multiple
            // download current translations if language != English
            if (!language.equals(SelectableLanguages.ENGLISH)) {
              // initialize progressMonitor
              progressMonitor =
                  new ProgressMonitor(LangInit.this, InitParameters.P_BAR_MESSAGE_DL, "", 0, 100);
