...
/**
 * A generic MVC framework. This seems to be very usefull when the MVC paradigma is used
 * extensively. The class does all the listener registration stuff, notification... Detailed
 * documentation, see http://www.onjava.com/pub/a/onjava/2004/07/07/genericmvc.html?page=1 Be aware:
 * weak references are used for the listeners. Therefore, after Observer deletion garbage collection
 * is able to remove the Observer object, even if a reference from the Listener-List still exists.
 */
public abstract class ModelMVCGeneric<T> implements Serializable {
  ...
  public transient WeakListModel listeners = new WeakListModel();
  private static final long serialVersionUID = 784638463745367L;

  /**
   * register Listener in list. Careful: the listener is not updated during registration, that means
   * the listener must aquire its initialisation data manually. But thats ok, since we have a
   * reference to the values, anyway. this saves some events.
   *
  ...
  /**
   * Send a notif event to a specifig Listener. Caution, the listener does not have to be in the
   * register list!
   *
  ...
  /**
   * value is the "model data" to be stored in the private class. To use the framework, create a
   * Member variable ot Property<T>, e.g. public final Property <Boolean> selected in the subclass.
   * Everytime the value is updated by the setValue() method, the attached listeners are informed.
   * Be careful, value is still mutable, if other access than setValue() is used (by reference for
   * instance).
   */
  protected T _value;

  /**
   * initialisation konstructor. Please consider that the write access to the value should be done
   * throug setValue() method.
   *
   * @param initialValue
   */
