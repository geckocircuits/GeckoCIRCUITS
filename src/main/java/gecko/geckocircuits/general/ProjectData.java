...
//        geckoOpt.exportASCII(asc);
    ...
    private int importASCII(String[] ascii, final boolean isBackupRead) {
        ...
        for (TokenMap verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungLK");
            ...
            _allSheetComponents.add(new Connection(verbindungMap, ConnectorType.LK));
        }
        ...
        for (TokenMap verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungCONTROL");
            ...
            _allSheetComponents.add(new Connection(verbindungMap, ConnectorType.CONTROL));
        }
        ...
        for (TokenMap verbindungMap = tokenMap.getSpecialBlockTokenMap("verbindungTHERM");
            ...
            _allSheetComponents.add(new Connection(verbindungMap, ConnectorType.THERMAL));
        }
        ...
        for (TokenMap controlTokenMap = tokenMap.getSpecialBlockTokenMap("c");
            ...
            try {
                if (typ == SpecialType.TEXTFIELD.getTypeNumber()) { // this is for backwards-compatiblity before v 1.62
                    _allSheetComponents.add(AbstractTypeInfo.fabricFromFile(SpecialType.getFromIntNumber(typ), controlTokenMap));
                } else {
        ...
        for (TokenMap specialTokenMap = tokenMap.getSpecialBlockTokenMap("sp");
            ...
            try {
                AbstractBlockInterface newBlock = AbstractTypeInfo.fabricFromFile(SpecialType.getFromIntNumber(typ), specialTokenMap);
                _allSheetComponents.add(newBlock);
