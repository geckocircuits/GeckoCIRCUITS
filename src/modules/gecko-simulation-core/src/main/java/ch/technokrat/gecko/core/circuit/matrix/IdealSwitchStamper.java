...
/**
 * Matrix stamper implementation for ideal switch components.
 *
 * The ideal switch is modeled as a variable resistor with two states:
 * - ON state: Low resistance (rOn) - conducting
 * - OFF state: High resistance (rOff) - blocking
 *
 * The switch is purely gate-controlled:
 * - Turns ON when gate signal = 1
 * - Turns OFF when gate signal = 0
 * - No voltage or current dependent switching (unlike diodes/thyristors)
 *
 * This is the simplest switching component - it has no forward voltage
 * drop and no current-zero turn-off requirements.
 *
...
public class IdealSwitchStamper implements IStatefulStamper {
    ...
    public static final int PARAM_R_ON = 1;

    /** Parameter index for OFF resistance */
    public static final int PARAM_R_OFF = 2;
    ...
    /**
     * Creates an ideal switch stamper with default parameters.
     */
    public IdealSwitchStamper() {
        this(DEFAULT_R_ON, DEFAULT_R_OFF);
    }

    /**
     * Creates an ideal switch stamper with specified parameters.
     *
     * @param rOn ON state resistance in Ohms
     * @param rOff OFF state resistance in Ohms
     */
    public IdealSwitchStamper(double rOn, double rOff) {
    ...
    public void stampVectorB(double[] b, int nodeX, int nodeY, int nodeZ,
        ...
        // Ideal switch has no forward voltage drop - no B vector contribution
    }
    ...
    public void updateState(double vx, double vy, double current, double time) {
        // Ideal switch state is controlled externally via gate signal
    ...
    /**
     * Updates the switch state based on gate signal.
     * This is the primary way to control the ideal switch.
     *
    ...
    /**
     * Creates parameter array for an ideal switch.
     *
    ...
    public String toString() {
        return "IdealSwitchStamper[" + (isOn ? "ON" : "OFF") +
               ", rOn=" + rOn +
               ", rOff=" + rOff + "]";
    }
}
