...
class CompressorIntMatrixTest {
    ...
    @Test
    void testCompressDecompress_SmallMatrix() {
        int[][] original = {
            {1, 2, 3},
            {4, 5, 6}
        };
        ...
        for (int i = 0; i < original.length; i++) {
            for (int j = 0; j < original[0].length; j++) {
                assertEquals(
                    original[i][j], decompressed[i][j], "Value mismatch at [" + i + "][" + j + "]");
            }
        }
    }

    @Test
    void testCompressDecompress_LargeMatrix() {
        int rows = 100;
        int cols = 50;
    ...
    @Test
    void testCompressDecompress_SingleRow() {
    ...
    @Test
    void testCompressDecompress_SingleColumn() {
        int[][] original = {
            {1}, {2}, {3}, {4}, {5}
        };
    ...
    @Test
    void testCompressDecompress_EmptyMatrix() {
    ...
    @Test
    void testCompressDecompress_ZeroValues() {
        int[][] original = {
            {0, 0, 0},
            {0, 0, 0}
        };
    ...
    @Test
    void testCompressDecompress_NegativeValues() {
        int[][] original = {
            {-100, -50, 0},
    ...
    @Test
    void testCompressDecompress_LargeIntValues() {
        int[][] original = {
    ...
    @Test
    void testCompressionRatio_Recorded() {
        ...
        assertTrue(compressor.compressionRatio > 0, "Compression ratio should be positive");
    }

    @Test
    void testCompressionTime_Recorded() {
        int[][] original = new int[50][50];

        CompressorIntMatrix compressor = new CompressorIntMatrix(original);

        // Time should be non-negative
        assertTrue(compressor.compressionTime >= 0, "Compression time should be >= 0");
    }

    @Test
    void testGetCompressedMemory() {
        int[][] original = new int[10][10];

        CompressorIntMatrix compressor = new CompressorIntMatrix(original);

        int compressedMemory = compressor.getCompressedMemory();
        assertTrue(compressedMemory > 0, "Compressed memory should be positive");
    }
    ...
    @Test
    void testDataIntegrity_RandomValues() {
        int rows = 20;
        int cols = 30;
        int[][] original = new int[rows][cols];
    ...
    @Test
    void testDataIntegrity_RepeatedPatterns() {
    ...
    @Test
    void testDataIntegrity_AlternatingValues() {
    ...
    @Test
    void testMultipleDecompressions() {
        int[][] original = {
            {1, 2, 3},
            {4, 5, 6}
        };
