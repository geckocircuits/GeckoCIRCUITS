 ...
 */
class CompressorIntMatrixExtendedTest {
    ...
    @Test
    void testDeltaEncodingEfficiency() {
    ...
    void testFloatBitsRoundTrip() {
        ...
        for (int i = 0; i < testFloats.length; i++) {
            ...
            assertEquals(testFloats[i], restored, 0.0f, "Float round-trip failed for " + testFloats[i]);
        }
    }
    ...
    void testNoisePattern() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 500; j++) {
                assertEquals(
                            noiseData[i][j], decompressed[i][j], "Mismatch at [" + i + "][" + j + "]");
            }
        }
    }

    @Test
    void testStepChangePattern() {
    ...
    @Test
    void testCompressionRatioRecorded() {
        ...
        assertTrue(
                  compressor.compressionRatio > 0, "Compression ratio should be positive");
    }

    @Test
    void testMultipleDecompressionsConsistent() {
